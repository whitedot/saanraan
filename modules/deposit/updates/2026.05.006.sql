CREATE TABLE IF NOT EXISTS sr_deposit_refund_requests (
    ...
    PRIMARY KEY (id),
    KEY idx_sr_deposit_refund_requests_account_status (account_id, status),
    KEY idx_sr_deposit_refund_requests_status_requested (status, requested_at),
    KEY idx_sr_deposit_refund_requests_transaction (transaction_id),
    KEY idx_sr_deposit_refund_requests_processed_by (processed_by_account_id)
);

INSERT IGNORE INTO sr_admin_account_permissions (account_id, menu_path, action_key, created_at)
SELECT account_id,
       '/admin/deposits/refund-requests',
       action_key,
       NOW()

...
function sr_deposit_notification_cases(): array
{
    return [
        ...
        'transaction_exchange_in' => [
            ...
            'label' => '환전 입금 알림',
            'description' => '환전으로 예치금이 입금되었을 때 보냅니다.',
            'default_enabled' => true,
        ],
        'transaction_use' => [
        ...
        'transaction_exchange_out' => [
            ...
            'label' => '환전 출금 알림',
            'description' => '환전으로 예치금이 출금되었을 때 보냅니다.',
            'default_enabled' => true,
        ],
        'transaction_exchange_fee' => [
            'event_key' => 'transaction.exchange_fee',
            'label' => '환전 수수료 알림',
            'description' => '환전 수수료로 예치금이 차감되었을 때 보냅니다.',
            'default_enabled' => true,
        ],
...
function sr_deposit_validate_admin_adjustment_limit(PDO $pdo, array $runtimeConfig, int $adminAccountId, string $permissionPath, int $amount, string $approvalIdentifier = '', string $approvalNote = ''): array
{
    ...
    if ($approvalAccountId <= 0 || sr_deposit_clean_text($approvalNote, 255) === '') {
        return ['error' => '대액 예치금 조정 기록을 만들 대상 회원과 사유가 필요합니다.', 'approval_account_id' => 0];
    }
...
function sr_deposit_transaction_type_label(string $transactionType): string
{
    $labels = [
        'adjustment' => '조정',
        'deposit' => '예치',
...
function sr_deposit_reference_type_label(string $referenceType): string
{
...
function sr_deposit_time_html(string $value): string
{
...
function sr_deposit_refund_request_count_for_account(PDO $pdo, int $accountId): int
{
    if ($accountId <= 0) {
        return 0;
    }

    $stmt = $pdo->prepare('SELECT COUNT(*) FROM sr_deposit_refund_requests WHERE account_id = :account_id');
    $stmt->execute(['account_id' => $accountId]);

    return (int) $stmt->fetchColumn();
}

function sr_deposit_refund_requests_for_account(PDO $pdo, int $accountId, int $limit = 20, int $offset = 0): array
{
    if ($accountId <= 0) {
        return [];
    }
...
function sr_deposit_transaction_notification_metadata(array $transaction, ?PDO $pdo = null): array
{
    ...
    return [
        ...
        'transaction_type' => $transactionType,
        'transaction_type_label' => sr_deposit_transaction_type_label($transactionType),
        'reason' => (string) ($transaction['reason'] ?? ''),
        'reference_type' => $referenceType,
        'reference_type_label' => sr_deposit_reference_type_label($referenceType),

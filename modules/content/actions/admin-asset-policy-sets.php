...
if ($policySetPage === 'edit') {
    ...
    if (!is_array($editPolicySet)) {
        sr_render_error(404, '콘텐츠 회원 그룹별 금액 조정 세트를 찾을 수 없습니다.');
    }
}
...
if (sr_request_method() === 'POST') {
    ...
    if ($isUpdate && !is_array($existing)) {
        $errors[] = '수정할 콘텐츠 회원 그룹별 금액 조정 세트를 찾을 수 없습니다.';
    }
    ...
    if (!sr_content_asset_policy_set_key_is_valid((string) $values['set_key'])) {
        $errors[] = '회원 그룹별 금액 조정 key는 영문 소문자로 시작하고 영문 소문자, 숫자, 밑줄만 사용할 수 있습니다.';
    } elseif (sr_content_asset_policy_set_key_exists($pdo, (string) $values['set_key'], $setId)) {
        $errors[] = '이미 사용 중인 회원 그룹별 금액 조정 key입니다.';
    }
    if ((string) $values['title'] === '') {
        $errors[] = '회원 그룹별 금액 조정 이름을 입력하세요.';
    }
    if (!in_array((string) $values['status'], sr_content_asset_policy_set_statuses(), true)) {
        $errors[] = '상태 값이 올바르지 않습니다.';
    }
    $errors = array_merge($errors, sr_admin_asset_group_policy_validation_errors($pdo, sr_content_asset_group_policies_from_value($policiesJson), '콘텐츠 회원 그룹별 금액 조정', true, $assetModuleOptions));
    ...
    $_SESSION['sr_content_asset_policy_set_notice'] = '콘텐츠 회원 그룹별 금액 조정을 저장했습니다.';
    sr_redirect('/admin/content/asset-policy-sets');
}

$policySetSort = sr_admin_sort_from_request(sr_content_asset_policy_set_sort_options(), sr_content_asset_policy_set_default_sort());
$policySets = sr_content_asset_policy_sets($pdo, false, $policySetSort);

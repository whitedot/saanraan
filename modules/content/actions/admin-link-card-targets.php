...
$searchTargets = $targets !== '' ? $targets : ($target !== '' ? $target : 'community_post,quiz_set,survey_form');
...
foreach ($items as $itemIndex => $item) {
    ...
    if ((string) ($items[$itemIndex]['admin_url'] ?? '') !== '') {
        $items[$itemIndex]['admin_url'] = $linkCardAbsoluteUrl((string) $items[$itemIndex]['admin_url']);
    }
}

...
function sr_content_clean_layout_extra_menu_area_key(string $value): string
{
    $value = strtolower(trim($value));
    return preg_match('/\A(?:[a-f0-9]{12}|[a-z][a-z0-9_]{0,59})\z/', $value) === 1 ? $value : '';
}

function sr_content_layout_extra_menu_label(string $value): string
{
    $value = trim(preg_replace('/\s+/', ' ', $value) ?? '');
    return function_exists('mb_substr') ? mb_substr($value, 0, 80) : substr($value, 0, 80);
}

function sr_content_layout_extra_menu_hash_key(array $usedKeys = [], string $seed = ''): string
{
    $used = array_fill_keys(array_map('strval', $usedKeys), true);
    if ($seed !== '') {
        for ($attempt = 0; $attempt < 20; $attempt++) {
            $key = substr(hash('sha256', 'content.layout_extra_menu|' . $seed . '|' . (string) $attempt), 0, 12);
            if (!isset($used[$key])) {
                return $key;
            }
        }
...
function sr_content_layout_extra_menu_items_from_value(mixed $value): array
{
    if (is_string($value)) {
    ...
    foreach (array_values($value) as $itemIndex => $item) {
        ...
        $menuKey = '';
        if (is_array($item)) {
            $areaKey = sr_content_clean_layout_extra_menu_area_key((string) ($item['area_key'] ?? ($item['key'] ?? ($item['slot_key'] ?? ''))));
            $label = sr_content_layout_extra_menu_label((string) ($item['label'] ?? ($item['name'] ?? '')));
            $menuKey = sr_content_clean_layout_menu_key((string) ($item['menu_key'] ?? ''));
        } else {
            $label = '';
            $menuKey = sr_content_clean_layout_menu_key((string) $item);
        }
        if ($menuKey === '') {
            continue;
        }
        if ($areaKey === '' || isset($items[$areaKey])) {
            $areaKey = sr_content_layout_extra_menu_hash_key(array_keys($items), (string) $itemIndex . '|' . $menuKey);
        }
        $items[$areaKey] = [
            'area_key' => $areaKey,
            'label' => $label,
...
function sr_content_layout_extra_menu_items_from_pair_values(mixed $areaKeys, mixed $labels, mixed $menuKeys): array
{
    ...
    foreach ($menuKeys as $index => $menuKeyValue) {
        $menuKey = sr_content_clean_layout_menu_key((string) $menuKeyValue);
        if ($menuKey === '') {
            continue;
        }
        $areaKey = sr_content_clean_layout_extra_menu_area_key((string) ($areaKeys[$index] ?? ''));
        if ($areaKey === '' || isset($items[$areaKey])) {
            $areaKey = sr_content_layout_extra_menu_hash_key(array_keys($items));
        }
        $label = sr_content_layout_extra_menu_label((string) ($labels[$index] ?? ''));
...
{
    return array_values(array_map(static fn (array $item): string => (string) $item['menu_key'], sr_content_layout_extra_menu_items_from_value($value)));
}

function sr_content_layout_extra_menu_items_from_settings(array $settings): array
{
    $items = [];
    foreach (sr_content_layout_extra_menu_items_from_value($settings['layout_extra_menu_keys_json'] ?? []) as $item) {
...
{
    return array_values(array_map(static fn (array $item): string => (string) $item['menu_key'], sr_content_layout_extra_menu_items_from_settings($settings)));
}

function sr_content_layout_extra_menu_keys_json(mixed $value): string
{
    $json = json_encode(sr_content_layout_extra_menu_items_from_value($value), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
...
function sr_content_settings(PDO $pdo): array
{
    ...
    $settings['multi_asset_payment_enabled'] = sr_content_bool_setting($settings['multi_asset_payment_enabled'] ?? true);
...
function sr_content_public_layout_context(array $settings, array $context = []): array
{
    ...
    $context['site_extra_menus'] = sr_content_layout_extra_menu_items_from_settings($settings);
...
function sr_content_save_settings(PDO $pdo, array $settings): void
{
    ...
    $rows = [
        ...
        ['multi_asset_payment_enabled', !empty($settings['multi_asset_payment_enabled']) ? '1' : '0', 'bool'],

...
return [
    'content_asset_access_pending_logs' => [
        ...
        'table_checks' => [
            'SELECT log_status FROM sr_content_asset_access_logs LIMIT 1',
        ],
    ],
    'content_asset_action_pending_logs' => [
        ...
        'delete_params' => ['cutoff' => 'sessions'],
        'table_checks' => [
            'SELECT log_status FROM sr_content_asset_action_logs LIMIT 1',
        ],
    ],
    'content_legal_asset_access_logs' => [
        ...
        'delete_sql' => "UPDATE sr_content_asset_access_logs
         SET account_id = 0,
        ...
        'delete_limited_sql' => "UPDATE sr_content_asset_access_logs
         SET account_id = 0,
             dedupe_key = CONCAT('anonymized:', id)
         WHERE id IN (
            SELECT id FROM (
    ...
    'content_legal_asset_action_logs' => [
        ...
        'count_sql' => "SELECT COUNT(*) AS count_value
         ...
         WHERE l.account_id > 0
           ...
           AND l.created_at < :cutoff",
        'count_params' => ['cutoff' => 'commerce_records'],
        'delete_sql' => "UPDATE sr_content_asset_action_logs
         SET account_id = 0,
             dedupe_key = CONCAT('anonymized:', id)
         WHERE id IN (
            SELECT id FROM (
        ...
        'delete_limited_sql' => "UPDATE sr_content_asset_action_logs
         SET account_id = 0,
             dedupe_key = CONCAT('anonymized:', id)
         WHERE id IN (
            SELECT id FROM (
    ...
    'content_legal_author_reward_logs' => [
        ...
        'cutoff_key' => 'commerce_records',
        'count_sql' => "SELECT COUNT(*) AS count_value
         FROM sr_content_author_reward_logs l
         INNER JOIN sr_member_accounts a ON a.id = l.author_account_id
         WHERE l.author_account_id > 0
           AND a.status IN ('withdrawn', 'anonymized')
           AND l.status <> 'pending'
           AND l.created_at < :cutoff",
        'count_params' => ['cutoff' => 'commerce_records'],
        'delete_sql' => "UPDATE sr_content_author_reward_logs
         SET author_account_id = 0,
        ...
        'delete_limited_sql' => "UPDATE sr_content_author_reward_logs
         SET author_account_id = 0,
             created_by_account_id = NULL,
             failure_reason = NULL,
             dedupe_key = CONCAT('anonymized:', id)
    ...
    'content_legal_view_payment_logs' => [
        ...
        'cutoff_key' => 'commerce_records',
        'count_sql' => "SELECT COUNT(*) AS count_value
         FROM sr_content_view_payment_logs l
        ...
        'count_params' => ['cutoff' => 'commerce_records'],
        'delete_sql' => "UPDATE sr_content_view_payment_logs
         SET account_id = NULL,
             coupon_dedupe_key = '',
        ...
        'delete_limited_sql' => "UPDATE sr_content_view_payment_logs
         SET account_id = NULL,
             coupon_dedupe_key = '',
             payment_dedupe_key = CONCAT('anonymized:', id),
             refund_note = '',
    ...
    'content_legal_file_download_logs' => [
        ...
        'delete_limited_sql' => "UPDATE sr_content_file_download_logs
         SET account_id = NULL,
             coupon_dedupe_key = '',
             refund_note = '',
             refunded_by_account_id = NULL

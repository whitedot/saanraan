...
- 에디터 본문 이미지의 권한 확인 기반 private 썸네일 표시
...
- 에디터 본문 이미지 출력은 `/content/body-file` 권한 검사를 통과한 뒤 `thumb=1` 요청에서 비공개 썸네일 cache를 만들고 PHP가 스트리밍한다. 이 cache 경로는 정적 공개 접근 대상으로 취급하지 않는다.

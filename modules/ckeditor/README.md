...
CKEditor 플러그인은 커뮤니티 게시글 작성 textarea에 CKEditor 5를 선택적으로 붙인다. 코어는 CKEditor를 알지 않고, 커뮤니티 모듈이 적용 대상과 HTML 저장/출력 정책을 소유한다.
...
CKEditor 초기화가 성공한 경우에만 form에 `body_format=html`이 추가된다. 에셋 로딩에 실패하거나 플러그인이 비활성화되면 기존 textarea가 그대로 제출되고 커뮤니티 게시글은 `plain` 형식으로 저장된다.

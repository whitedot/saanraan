...
<?php if ($detail !== null) { ?>
    ...
    $detailProviderKey = (string) ($detail['provider_key'] ?? '');
    $detailFields = [
    ...
    ?>
    <section class="card admin-list-card identity-verification-detail-card">
        <div class="card-header">
            <div>
                <h2 class="card-title">시도 #<?php echo sr_e((string) $detail['id']); ?></h2>
                <p class="form-help"><?php echo sr_e($identityProviderLabel($detailProviderKey)); ?> · <?php echo sr_e(sr_identity_verification_purpose_label((string) ($detail['purpose'] ?? ''))); ?></p>
            </div>
            <a class="btn btn-sm btn-outline-secondary" href="<?php echo sr_e(sr_url('/admin/identity-verifications')); ?>">목록</a>
        </div>
        <div class="table-wrapper">
            <table class="table table-list identity-verification-detail-table">
                <caption class="sr-only">본인확인 시도 상세</caption>
                <tbody>
                    <?php foreach ($detailFields as $field => $label) { ?>
                        <tr>
                            ...
                            <td>
                                <?php if ($field === 'status') { ?>
                                    <span class="admin-status <?php echo sr_e(sr_identity_verification_attempt_status_class($detailStatus)); ?>"><?php echo sr_e(sr_identity_verification_attempt_status_label($detailStatus)); ?></span>
                                <?php } elseif ($field === 'provider_key') { ?>
                                    <?php echo sr_e($identityProviderLabel($detailProviderKey)); ?>
                                    <small class="admin-summary-meta"><?php echo sr_e($detailProviderKey); ?></small>
                                <?php } elseif ($field === 'purpose') { ?>
                                    <?php echo sr_e(sr_identity_verification_purpose_label((string) ($detail[$field] ?? ''))); ?>
                                    <small class="admin-summary-meta"><?php echo sr_e((string) ($detail[$field] ?? '')); ?></small>
                                <?php } elseif ($field === 'account_id' && (int) ($detail['account_id'] ?? 0) > 0) { ?>
                                    <code><?php echo sr_e(sr_admin_member_public_hash(isset($config) && is_array($config) ? $config : sr_runtime_config(), (int) $detail['account_id'])); ?></code>
                                <?php } elseif ($field === 'failure_code') { ?>
                                    <?php if ((string) ($detail[$field] ?? '') !== '') { ?>
                                        <?php echo sr_e(sr_identity_verification_failure_code_label((string) $detail[$field])); ?>
                                        <small class="admin-summary-meta"><?php echo sr_e((string) $detail[$field]); ?></small>
                                    <?php } ?>
                                <?php } elseif (in_array($field, ['requested_at', 'completed_at', 'failed_at', 'expires_at'], true)) { ?>
                                    <?php echo sr_admin_time_html((string) ($detail[$field] ?? '')); ?>
                                <?php } else { ?>
                                    <?php echo sr_e((string) ($detail[$field] ?? '')); ?>
                                <?php } ?>
                            </td>
                        </tr>
                    <?php } ?>
    ...
    <?php if ($detailResult !== null) { ?>
        <section class="card admin-list-card identity-verification-detail-card">
            <div class="card-header">
                <div>
                    <h2 class="card-title">검증 결과 요약</h2>
                    <p class="form-help">원문 개인정보는 표시하지 않고 저장 여부와 최소 결과만 보여줍니다.</p>
                </div>
            </div>
            <div class="table-wrapper">
                <table class="table table-list identity-verification-detail-table">
                    <caption class="sr-only">본인확인 검증 결과 요약</caption>
                    <tbody>
                        <?php foreach (['id' => '결과 ID', 'provider_key' => '제공자', 'provider_transaction_id' => '제공자 거래 ID', 'birth_date' => '생년월일', 'gender' => '성별', 'nationality' => '국적', 'age_over_14' => '만 14세 이상', 'age_over_19' => '성인', 'verified_at' => '검증', 'expires_at' => '만료'] as $field => $label) { ?>
                            <tr>
                                <th><?php echo sr_e($label); ?></th>
                                <td>
                                    ...
                                    <?php } else { ?>
                                        <?php echo sr_e((string) ($detailResult[$field] ?? '')); ?>
                                    <?php } ?>
                                </td>
                            </tr>
                        <?php } ?>
        ...
        <table class="table table-list identity-verification-attempt-table">
            ...
            <tbody>
                ...
                <?php foreach ($attempts as $attempt) { ?>
                    ...
                    $attemptUrl = sr_url('/admin/identity-verifications/' . (int) ($attempt['id'] ?? 0));
                    ?>
                    <tr>
                        ...
                        <td class="admin-table-break">
                            ...
                            <small class="admin-summary-meta"><?php echo sr_e((string) ($attempt['purpose'] ?? '')); ?></small>
                            <?php if ((string) ($attempt['method'] ?? '') !== '') { ?>
                                <small class="admin-summary-meta"><?php echo sr_e((string) $attempt['method']); ?></small>
                            <?php } ?>
                        </td>
                        <td class="admin-table-break">
                        ...
                        <td class="admin-table-nowrap">
                            ...
                            <?php if ((string) ($attempt['failure_code'] ?? '') !== '') { ?>
                                <small class="admin-summary-meta"><?php echo sr_e(sr_identity_verification_failure_code_label((string) $attempt['failure_code'])); ?></small>
                                <small class="admin-summary-meta">코드 <?php echo sr_e((string) $attempt['failure_code']); ?></small>
                            <?php } ?>
                        </td>
                        <td class="admin-table-nowrap"><?php echo sr_admin_time_html((string) ($attempt['verified_at'] ?? '')); ?></td>
                        <td>
                            <a href="<?php echo sr_e($attemptUrl); ?>" class="btn btn-sm btn-icon btn-outline-secondary" aria-label="본인확인 상세" title="본인확인 상세"><?php echo sr_material_icon_html('visibility'); ?></a>
                        </td>
                    </tr>
                <?php } ?>
            </tbody>

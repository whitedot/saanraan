...
function sr_admin_module_upload_limit_bytes(): int
{
    ...
    foreach (['upload_max_filesize', 'post_max_size'] as $setting) {
        $bytes = sr_admin_parse_upload_size((string) ini_get($setting));
        if ($bytes > 0) {
            $limits[] = $bytes;
        }
    }
...
function sr_admin_module_sources_enabled(PDO $pdo, ?array $config = null): bool
{
    ...
    return !sr_admin_runtime_is_production($config);
}
...
function sr_admin_zip_upload_stats(ZipArchive $zip): array
{
    ...
    $uncompressedBytes = 0;
    $maxEntries = 1000;
    $maxUncompressedBytes = sr_admin_module_uncompressed_limit_bytes();
    ...
    for ($i = 0; $i < $entryCount; $i++) {
        ...
        if (!is_string($entry) || !sr_admin_zip_entry_is_safe($entry)) {
            throw new RuntimeException('zip 안에 안전하지 않은 경로가 있습니다.');
        }

        if (sr_admin_zip_entry_is_symlink($zip, $i)) {
        ...
        if ($uncompressedBytes > $maxUncompressedBytes) {
            throw new RuntimeException('압축 해제 후 모듈 크기는 ' . sr_admin_format_bytes($maxUncompressedBytes) . ' 이하여야 합니다.');
        }
    }
...
function sr_admin_validate_extracted_module_tree(string $extractDir): void
{
    ...
    foreach ($items as $item) {
        ...
        if (!sr_admin_path_is_inside($item->getPathname(), $extractDir)) {
...
function sr_admin_php_array_block(string $content, string $key): string
{
    foreach (['\'', '"'] as $quote) {
        ...
        $block = sr_admin_php_balanced_block($content, $openOffset, $token === '[' ? '[' : '(', $token === '[' ? ']' : ')');
        if ($block !== '') {
            return $block;
        }
    }
...
{
    $block = sr_admin_php_array_block($content, $key);
    if ($block === '') {
        return [];
    }
...
{
    $block = sr_admin_php_array_block($content, 'saanraan');
    if ($block === '') {
        return [];
    }
    ...
    foreach (['min_version', 'module_contract'] as $key) {
        $value = sr_admin_php_string_array_value($block, $key);
        if ($value !== '') {
            $saanraan[$key] = $value;
        }
    }

    $testedWith = sr_admin_php_string_list_array_value($block, 'tested_with');
    if ($testedWith !== []) {
...
function sr_admin_load_module_metadata_from_file(string $file): array
{
    ...
    foreach (['name', 'version', 'type', 'description'] as $key) {
        $value = sr_admin_php_string_array_value($content, $key);
        if ($value !== '') {
            $metadata[$key] = $value;
        }
    }

    $saanraan = sr_admin_php_saanraan_metadata($content);
    if ($saanraan !== []) {
...
function sr_admin_module_source_candidate(array $candidate): ?array
{
    ...
    $metadata = sr_admin_load_module_metadata_from_file($sourceDir . '/module.php');
    if ($metadata === []) {
        return null;
    }
...
{
    $inferredModuleKey = $requestedModuleKey !== '' ? $requestedModuleKey : sr_admin_infer_module_key_from_filename($filename);
    $candidates = [];
    ...
    foreach ($candidates as $candidate) {
        $source = sr_admin_module_source_candidate($candidate);
        if (is_array($source)) {
            return $source;
        }
    }
...
function sr_admin_validate_module_source(string $moduleKey, string $sourceDir, array $metadata): array
{
    ...
    foreach (sr_admin_module_metadata_errors($metadata) as $error) {
        $errors[] = $error;
    }
...
function sr_admin_extract_module_upload(array $file, string $requestedModuleKey): array
{
    ...
    $limit = sr_admin_module_upload_limit_bytes();
    if ($size <= 0 || $size > $limit) {
        throw new RuntimeException('업로드 파일 크기는 ' . sr_admin_format_bytes($limit) . ' 이하여야 합니다.');
    }
    ...
    $workRoot = sr_admin_module_work_dir('module-upload');
    $extractDir = $workRoot . '/upload-' . date('YmdHis') . '-' . sr_admin_random_suffix();
    ...
    try {
        $uploadStats = sr_admin_zip_upload_stats($zip);
    ...
    try {
        sr_admin_validate_extracted_module_tree($extractDir);

        $source = sr_admin_find_module_source($extractDir, $requestedModuleKey, $filename);
        ...
        if (!sr_admin_path_is_inside((string) $source['source_dir'], $extractDir)) {
            throw new RuntimeException('zip 안의 모듈 경로가 올바르지 않습니다.');
        }

        $errors = sr_admin_validate_module_source(
...
function sr_admin_install_module_source_files(string $moduleKey, string $sourceDir): array
{
    ...
    $modulesRoot = sr_admin_module_source_root();
    ...
    $backupDir = '';
    if (is_dir($targetDir)) {
        $backupRoot = sr_admin_module_work_dir('module-backups');
        $backupDir = $backupRoot . '/' . $moduleKey . '-' . date('YmdHis') . '-' . sr_admin_random_suffix();
    ...
    try {
        sr_admin_copy_directory($sourceDir, $targetDir);
    } catch (Throwable $exception) {
        if (is_dir($targetDir)) {
            sr_admin_remove_directory($targetDir);
        }

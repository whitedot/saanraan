...
$popupId = sr_request_method() === 'POST' ? (int) sr_post_string('popup_id', 20) : (int) sr_get_string('id', 20);
...
if (!is_array($sourcePopup)) {
    sr_render_error(404, '복사할 팝업레이어를 찾을 수 없습니다.');
}

$values = [
    'title' => sr_request_method() === 'POST' ? sr_post_string('title', 160) : sr_popup_layer_clean_single_line((string) $sourcePopup['title'] . ' 복사본', 160),
];
$errors = [];
...
if (sr_request_method() === 'POST') {
    ...
    if ($errors === []) {
        ...
        try {
            ...
            sr_redirect('/admin/popup-layers/edit?id=' . (string) $newPopupId);
...
include SR_ROOT . '/modules/popup_layer/views/admin-popup-layer-copy.php';

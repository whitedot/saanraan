...
function sr_survey_comment_body_format(PDO $pdo, ?array $settings = null): string
{
    return sr_editor_format_value($pdo, sr_survey_comment_editor_key($pdo, $settings));
}

function sr_survey_comment_body_plain_text(PDO $pdo, array $comment, ?array $settings = null): string
{
    $comment['body_format'] = sr_survey_comment_body_format($pdo, $settings);
    return sr_body_text_plain_text($comment, $pdo);
}

function sr_survey_comment_body_html(PDO $pdo, array $comment, ?array $settings = null): string
{
    $editorKey = sr_survey_comment_editor_key($pdo, $settings);
    ...
    return '<div class="sr-ckeditor" data-sr-editor-output data-sr-editor-body-theme="survey.' . sr_e($themeKey) . '">'
        . '<div class="ck-content" lang="' . sr_e(sr_locale()) . '" dir="ltr">' . $html . '</div></div>';
}

function sr_survey_comment_body_stylesheets(PDO $pdo, ?array $settings = null): array
{
    $editorKey = sr_survey_comment_editor_key($pdo, $settings);
...
function sr_survey_comment_input_values(?PDO $pdo = null, ?array $settings = null): array
{
    $parentCommentIdValue = sr_post_string('parent_comment_id', 20);
    $bodyFormat = $pdo instanceof PDO ? sr_survey_comment_body_format($pdo, $settings) : 'plain';
    $bodyText = sr_post_string_without_truncation('body_text', 4000);
...
{
    $errors = [];
    $bodyText = $values['body_text'] ?? null;
    if (!is_string($bodyText)) {
        return ['댓글은 4000자 이내로 입력하세요.'];
    }

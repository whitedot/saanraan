...
function sr_survey_group_by_key(PDO $pdo, string $groupKey, bool $enabledOnly = false): ?array
{
    if (!sr_survey_group_key_is_valid($groupKey) || !sr_survey_groups_table_exists($pdo)) {
        return null;
    }
    $sql = 'SELECT * FROM sr_survey_groups WHERE group_key = :group_key';
    if ($enabledOnly) {

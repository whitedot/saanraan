...
if (sr_request_method() === 'POST') {
    ...
    $_SESSION['sr_survey_admin_comment_notice'] = '댓글 상태를 변경했습니다.';
    $redirectQuery = (string) ($_SERVER['QUERY_STRING'] ?? '');
    sr_redirect('/admin/surveys/comments' . ($redirectQuery !== '' ? '?' . $redirectQuery : ''));
}

$commentFilters = sr_survey_admin_comment_filters_from_request();
$comments = sr_survey_admin_comments($pdo, $commentFilters, 200);
...
$commentNotice = (string) ($_SESSION['sr_survey_admin_comment_notice'] ?? '');
unset($_SESSION['sr_survey_admin_comment_notice']);
...
$adminPageSubtitle = '설문 댓글을 최신순으로 확인하고 필요한 조치를 진행합니다.';
                ...
                <?php foreach ($comments as $comment) { ?>
                    ...
                    <tr>
                        ...
                        <td class="admin-table-break">
                            ...
                            <a href="<?php echo sr_e(sr_url('/survey/' . rawurlencode((string) ($comment['survey_key'] ?? '')) . '?submitted=1#survey-comments')); ?>" target="_blank" rel="noopener noreferrer">관리용 키: <?php echo sr_e((string) ($comment['survey_key'] ?? '')); ?></a>
                        </td>
                        <td class="admin-table-break">

...
function sr_point_validate_admin_adjustment_limit(PDO $pdo, int $adminAccountId, int $amount): ?string
{
    $absoluteAmount = abs($amount);
    if ($absoluteAmount > sr_point_admin_adjustment_once_limit()) {
        return '포인트 관리자 조정 금액이 1회 상한을 초과했습니다.';
    }
    ...
    if ($usedAmount + $absoluteAmount > sr_point_admin_adjustment_daily_limit()) {
        return '포인트 관리자 조정 금액이 일일 상한을 초과했습니다.';
    }

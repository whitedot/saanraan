...
<form method="get" action="<?php echo sr_e(sr_url($pointAdminPage === 'transactions' ? '/admin/points/transactions' : '/admin/points/balances')); ?>" class="filtering-form filtering filtering-plain admin-asset-member-filter ui-form-theme">
    <div class="filtering-fields admin-asset-member-search-grid">
        <div class="filtering-field">
            <label for="point-member-search-field" class="filtering-label">검색조건</label>
            <select name="field" id="point-member-search-field" class="form-select filtering-input">
        ...
        <div class="filtering-field admin-asset-member-filter-keyword">
            <label for="point-member-search-keyword" class="filtering-label"><?php echo sr_e(sr_t('point::ui.search.bda397fc')); ?></label>
            <input type="text" id="point-member-search-keyword" name="q" value="<?php echo sr_e((string) ($accountLookupFilter['keyword'] ?? '')); ?>" class="form-input filtering-input" maxlength="120" placeholder="<?php echo sr_e(sr_t('point::ui.email.login.name.c26ba637')); ?>">
        </div>
        <button type="submit" class="btn btn-solid-primary filtering-submit"><?php echo sr_e(sr_t('point::ui.search.4b8d541e')); ?></button>
    </div>
</form>
...
<?php if ($pointAdminPage === 'transactions') { ?>
    <section class="card admin-list-card admin-list-form">
        ...
        <div class="admin-icon-button-legend" aria-label="아이콘 버튼 설명">
            <span class="admin-icon-button-legend-item"><?php echo sr_material_icon_html('edit'); ?> <?php echo sr_e(sr_t('point::ui.text.b9d9b240')); ?></span>
        </div>
	    </section>
    ...
    <section class="card admin-list-card admin-list-form">
        ...
        <table class="table table-list admin-asset-balance-table">
            ...
            <tbody>
                <?php if ($balances === []) { ?>
                    <tr>
                        <td colspan="5" class="admin-empty-state"><?php echo sr_e($pointDisplayName . ' 잔액이 없습니다.'); ?></td>
                    </tr>
                <?php } else { ?>
    ...
    <?php foreach ($pointAdjustModalAccounts as $pointAdjustModalAccount) { ?>
        ...
        <div id="<?php echo sr_e($pointAdjustModalId); ?>" class="modal-overlay modal-overlay-fade overlay hidden pointer-events-none opacity-0" role="dialog" tabindex="-1" aria-labelledby="<?php echo sr_e($pointAdjustFieldPrefix); ?>_title" aria-hidden="true" inert>
            <div class="modal-dialog">
                <form method="post" action="<?php echo sr_e(sr_url('/admin/points/balances' . ((string) $pointAdjustModalAccount['account_public_hash'] !== '' ? '?account_identifier=' . rawurlencode((string) $pointAdjustModalAccount['account_public_hash']) : ''))); ?>" class="modal-content ui-form-theme" data-sr-validate-form>
                    <div class="modal-header">
                        <h3 id="<?php echo sr_e($pointAdjustFieldPrefix); ?>_title" class="modal-title"><?php echo sr_e($pointDisplayName . ' 조정'); ?></h3>
                        <button type="button" class="btn btn-icon btn-ghost-light modal-close" aria-label="<?php echo sr_e(sr_t('point::ui.close.1e8c1020')); ?>" data-overlay="#<?php echo sr_e($pointAdjustModalId); ?>">
...
<?php if ($pointAdminPage === 'transactions' && $transactions !== []) { ?>
    <?php foreach ($transactions as $pointRefundTransaction) { ?>
        ...
        <div id="<?php echo sr_e($pointRefundModalId); ?>" class="modal-overlay modal-overlay-fade overlay hidden pointer-events-none opacity-0" role="dialog" tabindex="-1" aria-labelledby="<?php echo sr_e($pointRefundFieldPrefix); ?>_title" aria-hidden="true" inert>
            <div class="modal-dialog">
                <form method="post" action="<?php echo sr_e(sr_url('/admin/points/transactions?account_identifier=' . rawurlencode((string) $pointRefundTransaction['account_public_hash']))); ?>" class="modal-content ui-form-theme" data-sr-validate-form>
                    <div class="modal-header">
                        <h3 id="<?php echo sr_e($pointRefundFieldPrefix); ?>_title" class="modal-title"><?php echo sr_e($pointDisplayName . ' 환불 처리'); ?></h3>
                        <button type="button" class="btn btn-icon btn-ghost-light modal-close" aria-label="<?php echo sr_e(sr_t('point::ui.close.1e8c1020')); ?>" data-overlay="#<?php echo sr_e($pointRefundModalId); ?>">
                            <?php echo sr_material_icon_html('close'); ?>
                        </button>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-field">
                                ...
                                <p class="form-help"><?php echo sr_e(sr_t('point::ui.refund_expiration_policy.help')); ?></p>
                            </div>
                        </div>
                        <div class="form-row">

...
SET @schema_sql = IF(
    @schema_has_community_posts_home_feed_candidate = 0,
    'ALTER TABLE {{SR_TABLE_PREFIX}}community_posts ADD COLUMN home_feed_candidate TINYINT(1) NOT NULL DEFAULT 1 AFTER hidden_before_status',
    'DO 0'
);
...
SET @schema_sql = IF(
    ...
    'ALTER TABLE {{SR_TABLE_PREFIX}}community_posts ADD KEY idx_sr_community_posts_home_status_id (home_feed_candidate, status, id)',
    'DO 0'
);
PREPARE schema_stmt FROM @schema_sql;
EXECUTE schema_stmt;
...
SET @schema_sql = IF(
    ...
    'ALTER TABLE {{SR_TABLE_PREFIX}}community_posts ADD KEY idx_sr_community_posts_home_status_view_id (home_feed_candidate, status, view_count, id)',
    'DO 0'
);
PREPARE schema_stmt FROM @schema_sql;
EXECUTE schema_stmt;
...
LEFT JOIN {{SR_TABLE_PREFIX}}community_board_settings home_setting
 ...
 AND home_setting.setting_key = 'home_feed_enabled'
SET p.home_feed_candidate = IF(COALESCE(home_setting.setting_value, '1') IN ('0', 'false', 'no', 'off'), 0, 1);

SET @schema_has_community_attachment_storage_driver = (
    ...
    WHERE TABLE_SCHEMA = DATABASE()
      AND TABLE_NAME = 'toy_community_attachments'
      AND COLUMN_NAME = 'storage_driver'
);

SET @schema_sql = IF(
    @schema_has_community_attachment_storage_driver = 0,
    'ALTER TABLE toy_community_attachments ADD COLUMN storage_driver VARCHAR(20) NOT NULL DEFAULT ''local'' AFTER storage_path',
    'DO 0'
);
...
SET @schema_sql = IF(
    ...
    'ALTER TABLE toy_community_attachments ADD COLUMN storage_key VARCHAR(255) NOT NULL DEFAULT '''' AFTER storage_driver',
    'DO 0'
);
PREPARE schema_stmt FROM @schema_sql;
EXECUTE schema_stmt;
...
SET @schema_has_community_attachment_storage_index = (
    ...
    WHERE TABLE_SCHEMA = DATABASE()
      AND TABLE_NAME = 'toy_community_attachments'
      AND INDEX_NAME = 'idx_toy_community_attachments_storage'
);

SET @schema_sql = IF(
    @schema_has_community_attachment_storage_index = 0,
    'ALTER TABLE toy_community_attachments ADD KEY idx_toy_community_attachments_storage (storage_driver, storage_key)',
    'DO 0'
);
PREPARE schema_stmt FROM @schema_sql;
EXECUTE schema_stmt;

...
$adminPageTitle = '커뮤니티 멤버 그룹별 금액 조정';
...
<?php if ($policySetPage === 'list') { ?>
    <section class="admin-card admin-list-card card admin-list-form">
        <div class="card-header">
            <div>
                <h2 class="card-title">멤버 그룹별 금액 조정 목록</h2>
            </div>
        ...
        <div class="table-wrapper">
            <table class="table">
                ...
                <tbody>
                    <?php if (($policySets ?? []) === []) { ?>
                        <tr>
                            <td colspan="5" class="admin-empty-state">등록된 멤버 그룹별 금액 조정 규칙이 없습니다.</td>
                        </tr>
                    <?php } else { ?>
                            ...
                            <tr>
                                ...
                                <td class="admin-table-actions-cell">
                                    <div class="admin-row-actions">
                                        <a class="btn btn-sm btn-icon btn-outline-secondary" href="<?php echo sr_e(sr_url('/admin/community/asset-policy-sets?mode=edit&id=' . (string) (int) ($policySet['id'] ?? 0))); ?>" aria-label="멤버 그룹별 금액 조정 수정" title="수정"><?php echo sr_material_icon_html('edit'); ?></a>
                                    </div>
                                </td>
                            </tr>
                        <?php } ?>
...
<?php } else { ?>
    <form method="post" action="<?php echo sr_e(sr_url('/admin/community/asset-policy-sets')); ?>" class="admin-form ui-form-theme">
        ...
        <section class="admin-card card">
            <h2><?php echo $policySetPage === 'edit' ? '멤버 그룹별 금액 조정 수정' : '멤버 그룹별 금액 조정 생성'; ?></h2>
        ...
        <?php
        $assetGroupPolicySectionTitle = '멤버 그룹별 금액 조정 규칙';

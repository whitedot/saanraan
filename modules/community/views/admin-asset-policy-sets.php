...
$adminPageTitle = '커뮤니티 멤버 그룹별 설정';
$adminPageSubtitle = '커뮤니티 적립, 차감, 유료 열람, 첨부 다운로드에서 멤버 그룹별 최종 금액을 설정하는 규칙입니다.';
...
<?php if ($policySetPage === 'list') { ?>
    <section class="admin-card admin-list-card card admin-list-form">
        <div class="card-header">
            <div>
                <h2 class="card-title">멤버 그룹별 설정 목록</h2>
            </div>
        ...
        <div class="admin-list-summary-row">
            <?php if (empty($policySetSort['is_default'])) { ?>
                <a href="<?php echo sr_e(sr_admin_sort_url(sr_community_asset_policy_set_sort_options(), sr_community_asset_policy_set_default_sort())); ?>" class="btn btn-sm btn-icon btn-outline-danger admin-sort-reset" aria-label="커뮤니티 멤버 그룹별 설정 목록 기본 정렬로 초기화" title="기본 정렬로 초기화"><?php echo sr_material_icon_html('restart_alt'); ?></a>
            <?php } ?>
            <?php echo sr_admin_pagination_summary_html($policySetPagination); ?>
        </div>
        <div class="table-wrapper">
            <table class="table">
                <caption class="sr-only">커뮤니티 멤버 그룹별 설정 목록</caption>
                ...
                <tbody>
                    <?php if (($policySets ?? []) === []) { ?>
                        <tr>
                            <td colspan="5" class="admin-empty-state">등록된 멤버 그룹별 설정이 없습니다.</td>
                        </tr>
                    <?php } else { ?>
                            ...
                            <tr>
                                ...
                                <td class="admin-table-actions-cell">
                                    <div class="admin-row-actions">
                                        <a class="btn btn-sm btn-icon btn-outline-secondary" href="<?php echo sr_e(sr_url('/admin/community/asset-policy-sets?mode=edit&id=' . (string) (int) ($policySet['id'] ?? 0))); ?>" aria-label="멤버 그룹별 설정 수정" title="수정"><?php echo sr_material_icon_html('edit'); ?></a>
                                    </div>
                                </td>
                            </tr>
                        <?php } ?>
...
<?php } else { ?>
    <form method="post" action="<?php echo sr_e(sr_url('/admin/community/asset-policy-sets')); ?>" class="admin-form ui-form-theme">
        ...
        <section class="admin-card card">
            <h2><?php echo $policySetPage === 'edit' ? '멤버 그룹별 설정 수정' : '멤버 그룹별 설정 생성'; ?></h2>
        ...
        <?php
        $assetGroupPolicySectionTitle = '멤버 그룹별 설정 규칙';

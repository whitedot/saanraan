...
$adminContainerClass = 'admin-community-payments admin-ui-scope';
$paymentSortOptions = isset($paymentSortOptions) && is_array($paymentSortOptions) ? $paymentSortOptions : sr_community_admin_payment_history_sort_options();
$paymentDefaultSort = isset($paymentDefaultSort) && is_array($paymentDefaultSort) ? $paymentDefaultSort : sr_community_admin_payment_history_default_sort();
$paymentSort = isset($paymentSort) && is_array($paymentSort) ? $paymentSort : $paymentDefaultSort;
$canEditCommunityPayments = !empty($canEditCommunityPayments);
...
$kindLabels = ['community_post_read' => '게시글 열람', 'community_attachment_download' => '첨부 다운로드'];
...
$adminPageTitleUrl = sr_admin_page_title_reset_url(true, '/admin/community/payments');
...
$detailFilterOpen = (int) ($filters['board_id'] ?? 0) > 0
    || (int) ($filters['target_id'] ?? 0) > 0
    || (int) ($filters['account_id'] ?? 0) > 0
    || (string) ($filters['date_from'] ?? '') !== ''
    || (string) ($filters['date_to'] ?? '') !== '';
?>
<form method="get" action="<?php echo sr_e(sr_url('/admin/community/payments')); ?>" class="filtering-form admin-community-payment-filter ui-form-theme">
        ...
        <div class="filtering filtering-card<?php echo $detailFilterOpen ? ' filtering-open' : ''; ?>" data-filtering>
            <div class="filtering-fields filtering-fields-fit">
                <div class="filtering-field">
                    ...
                    <?php echo sr_admin_filter_radio_toggle_group_html('community_payment_filter_kind', 'kind', $kindLabels, $selectedKinds, '전체'); ?>
                </div>
                <div class="filtering-field">
                    <span class="filtering-label">결제 유형</span>
                    <?php echo sr_admin_filter_radio_toggle_group_html('community_payment_filter_payment_type', 'payment_type', $paymentTypeLabels, $selectedPaymentTypes, '전체'); ?>
                </div>
                <div class="filtering-field">
                    <span class="filtering-label">환불 상태</span>
                    <?php echo sr_admin_filter_toggle_group_html('community_payment_filter_refund_status', 'refund_status', $refundStatusLabels, $selectedRefundStatuses, '전체'); ?>
                </div>
                <label class="filtering-field-fill filtering-field" for="community_payment_filter_q">
                    <span class="filtering-label">검색</span>
                    <input id="community_payment_filter_q" type="text" name="q" value="<?php echo sr_e((string) ($filters['q'] ?? '')); ?>" class="form-input filtering-input" maxlength="120" placeholder="게시글, 첨부, 쿠폰 키, 결제 키">
                </label>
            </div>
            <div id="community_payment_detail_filters" class="filtering-body" data-filtering-body<?php echo $detailFilterOpen ? '' : ' hidden'; ?>>
                <label class="filtering-field" for="community_payment_filter_board">
                    <span class="filtering-label">게시판</span>
                    <select id="community_payment_filter_board" name="board_id" class="form-select filtering-input">
                ...
                <div class="filtering-field">
                    ...
                    <?php echo sr_admin_filter_radio_toggle_group_html('community_payment_filter_settlement_kind', 'settlement_kind', $settlementFilterLabels, $selectedSettlementKinds, '전체'); ?>
                </div>
                <div class="filtering-field">
                ...
                <label class="filtering-field" for="community_payment_filter_target_id">
                    ...
                    <input id="community_payment_filter_target_id" type="number" min="1" name="target_id" value="<?php echo (int) ($filters['target_id'] ?? 0) > 0 ? sr_e((string) (int) $filters['target_id']) : ''; ?>" class="form-input filtering-input">
                </label>
                <label class="filtering-field" for="community_payment_filter_account_id">
                    <span class="filtering-label">회원</span>
                    <input id="community_payment_filter_account_id" type="text" name="account_id" value="<?php echo (int) ($filters['account_id'] ?? 0) > 0 ? sr_e(sr_admin_member_public_hash(isset($config) && is_array($config) ? $config : sr_runtime_config(), (int) $filters['account_id'])) : ''; ?>" class="form-input filtering-input" maxlength="80" autocomplete="off">
                </label>
                <label class="filtering-field" for="community_payment_filter_date_from">
                    <span class="filtering-label">시작일</span>
                    <input id="community_payment_filter_date_from" type="date" name="date_from" value="<?php echo sr_e((string) ($filters['date_from'] ?? '')); ?>" class="form-input filtering-input">
                </label>
                <label class="filtering-field" for="community_payment_filter_date_to">
                    <span class="filtering-label">종료일</span>
                    <input id="community_payment_filter_date_to" type="date" name="date_to" value="<?php echo sr_e((string) ($filters['date_to'] ?? '')); ?>" class="form-input filtering-input">
                </label>
            </div>
            <div class="filtering-actions">
                <button type="button" class="btn btn-solid-light filtering-toggle" data-filtering-toggle aria-expanded="<?php echo $detailFilterOpen ? 'true' : 'false'; ?>" aria-controls="community_payment_detail_filters">상세검색</button>
...
<section class="card admin-list-card admin-list-form">
    <div class="card-header">
        <div>
            ...
            <p class="form-help">커뮤니티 유료 게시글 열람과 첨부 다운로드 결제 단위를 함께 조회합니다.</p>
        </div>
        <a href="<?php echo sr_e(sr_url('/admin/community/attachment-downloads')); ?>" class="btn btn-sm btn-outline-secondary">첨부 다운로드 내역</a>
    </div>
    ...
    <div class="table-wrapper">
        <table class="table table-list admin-community-payment-table">
            <caption class="sr-only">커뮤니티 유료 결제 내역</caption>
            <thead>
                <tr>
            ...
            <tbody>
                ...
                <?php foreach ($paymentLogs as $paymentLog) { ?>
                    ...
                    $canProcess = $canEditCommunityPayments && $accountId > 0 && $refundStatus === '' && $settlementKind !== $legacySettlementKind && $settlementKind !== '' && ($sourceKind === 'community_post_read' || $accessIds !== []);
                    $modalId = 'community-payment-refund-modal-' . $sourceKind . '-' . (int) ($paymentLog['source_id'] ?? 0);
                    ?>
                    <tr>
                        <td class="admin-table-nowrap"><?php echo sr_community_time_html((string) ($paymentLog['created_at'] ?? '')); ?></td>
                        <td class="admin-table-break">
                            ...
                            <small class="admin-summary-meta">게시글 #<?php echo sr_e((string) (int) ($paymentLog['post_id'] ?? 0)); ?><?php echo (int) ($paymentLog['attachment_id'] ?? 0) > 0 ? ' · 첨부 #' . sr_e((string) (int) ($paymentLog['attachment_id'] ?? 0)) : ''; ?></small>
                        </td>
                        <td class="admin-table-break">
                            <?php if ($accountId > 0) { ?>
                        ...
                        <td class="admin-table-break">
                            ...
                            <?php } elseif (!$canEditCommunityPayments) { ?>
    ...
    <?php echo sr_admin_status_description_list_html('community_payment_refund_status', $refundStatusLabels, [], '환불 처리 상태 설명'); ?>
</section>

<?php echo sr_admin_pagination_html($paymentPagination, '커뮤니티 결제 내역 페이지'); ?>
...
<?php foreach ($paymentLogs as $paymentLog) { ?>
    ...
    $accountId = (int) ($paymentLog['account_id'] ?? 0);
    $accessIds = $paymentLogAccessIds($paymentLog);
    if (!$canEditCommunityPayments || $accountId <= 0 || $refundStatus !== '' || $settlementKind === $legacySettlementKind || $settlementKind === '' || ($sourceKind !== 'community_post_read' && $accessIds === [])) {
        continue;
    }
    $modalId = 'community-payment-refund-modal-' . $sourceKind . '-' . (int) ($paymentLog['source_id'] ?? 0);
    $fieldPrefix = 'community_payment_refund_' . $sourceKind . '_' . (int) ($paymentLog['source_id'] ?? 0);
    ?>
    <div id="<?php echo sr_e($modalId); ?>" class="modal-overlay modal-overlay-fade overlay hidden pointer-events-none opacity-0" role="dialog" tabindex="-1" aria-labelledby="<?php echo sr_e($fieldPrefix); ?>_title" aria-hidden="true" inert>
        <div class="modal-dialog">
            <form method="post" action="<?php echo sr_e(sr_url('/admin/community/payments')); ?>" class="modal-content ui-form-theme">
                <div class="modal-header">
                    <h3 id="<?php echo sr_e($fieldPrefix); ?>_title" class="modal-title"><?php echo (int) ($paymentLog['settlement_amount'] ?? 0) > 0 ? '커뮤니티 결제 수동 환불' : '커뮤니티 접근권 회수'; ?></h3>
                ...
                <div class="modal-body">
                    ...
                    <input type="hidden" name="intent" value="<?php echo $sourceKind === 'community_post_read' ? 'refund_post_read_payment' : 'refund_attachment_download'; ?>">
                    <input type="hidden" name="<?php echo $sourceKind === 'community_post_read' ? 'payment_log_id' : 'download_log_id'; ?>" value="<?php echo sr_e((string) (int) ($paymentLog['source_id'] ?? 0)); ?>">

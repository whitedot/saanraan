...
if ($policySetPage === 'edit') {
    ...
    if (!is_array($editPolicySet)) {
        sr_render_error(404, '커뮤니티 회원 그룹별 금액 규칙 묶음을 찾을 수 없습니다.');
    }
}
...
if (sr_request_method() === 'POST') {
    ...
    if ($isUpdate && !is_array($existing)) {
        $errors[] = '수정할 커뮤니티 회원 그룹별 금액 규칙 묶음을 찾을 수 없습니다.';
    }
    ...
    $errors = array_merge($errors, sr_admin_asset_group_policy_validation_errors($pdo, sr_community_asset_group_policies_from_value($policiesJson), '커뮤니티 회원 그룹별 금액 규칙', true, $assetModuleOptions));
    ...
    $_SESSION['sr_community_asset_policy_set_notice'] = '커뮤니티 회원 그룹별 금액 규칙을 저장했습니다.';

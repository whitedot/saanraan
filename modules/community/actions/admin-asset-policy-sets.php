...
if ($policySetPage === 'edit') {
    ...
    if (!is_array($editPolicySet)) {
        sr_render_error(404, '커뮤니티 회원 그룹/레벨 혜택을 찾을 수 없습니다.');
    }
}
...
if (sr_request_method() === 'POST') {
    ...
    if ($isUpdate && !is_array($existing)) {
        $errors[] = '수정할 커뮤니티 회원 그룹/레벨 혜택을 찾을 수 없습니다.';
    }
    ...
    if (!sr_community_asset_policy_set_key_is_valid((string) $values['set_key'])) {
        $errors[] = '회원 그룹/레벨 혜택 key는 영문 소문자로 시작하고 영문 소문자, 숫자, 밑줄만 사용할 수 있습니다.';
    } elseif (sr_community_asset_policy_set_key_exists($pdo, (string) $values['set_key'], $setId)) {
        $errors[] = '이미 사용 중인 회원 그룹/레벨 혜택 key입니다.';
    }
    if ((string) $values['title'] === '') {
        $errors[] = '회원 그룹/레벨 혜택 이름을 입력하세요.';
    }
    if (!in_array((string) $values['status'], sr_community_asset_policy_set_statuses(), true)) {
        $errors[] = '상태 값이 올바르지 않습니다.';
    }
    $errors = array_merge($errors, sr_admin_asset_group_policy_validation_errors($pdo, sr_community_asset_group_policies_from_value($policiesJson), '커뮤니티 회원 그룹/레벨 혜택'));
    ...
    $_SESSION['sr_community_asset_policy_set_notice'] = '커뮤니티 회원 그룹/레벨 혜택을 저장했습니다.';

...
$sessionErrors = $_SESSION['sr_community_asset_policy_set_errors'] ?? [];
$sessionValues = $_SESSION['sr_community_asset_policy_set_values'] ?? [];
unset($_SESSION['sr_community_asset_policy_set_notice'], $_SESSION['sr_community_asset_policy_set_errors'], $_SESSION['sr_community_asset_policy_set_values']);
...
if ($policySetPage === 'edit') {
    ...
    if (!is_array($editPolicySet)) {
        sr_render_error(404, '커뮤니티 멤버 그룹별 설정 세트를 찾을 수 없습니다.');
    }
}
...
if (sr_request_method() === 'POST') {
    ...
    if ($isUpdate && !is_array($existing)) {
        $errors[] = '수정할 커뮤니티 멤버 그룹별 설정 세트를 찾을 수 없습니다.';
    }
    ...
    $values = [
        ...
        'description' => sr_post_string_without_truncation('description', 1000) ?? '',
        'status' => sr_post_string('status', 30),
        'policies_json' => $policiesJson,
    ];

    if (!sr_community_asset_policy_set_key_is_valid((string) $values['set_key'])) {
        $errors[] = '멤버 그룹별 설정 key는 영문 소문자로 시작하고 영문 소문자, 숫자, 밑줄만 사용할 수 있습니다.';
    } elseif (sr_community_asset_policy_set_key_exists($pdo, (string) $values['set_key'], $setId)) {
        $errors[] = '이미 사용 중인 멤버 그룹별 설정 key입니다.';
    }
    if ((string) $values['title'] === '') {
        $errors[] = '멤버 그룹별 설정 이름을 입력하세요.';
    }
    if (!in_array((string) $values['status'], sr_community_asset_policy_set_statuses(), true)) {
        $errors[] = '상태 값이 올바르지 않습니다.';
    }
    $errors = array_merge($errors, sr_admin_asset_group_policy_validation_errors($pdo, sr_community_asset_group_policies_from_value($policiesJson), '커뮤니티 멤버 그룹별 설정', true, $assetModuleOptions));

    if ($errors !== []) {
        $_SESSION['sr_community_asset_policy_set_errors'] = $errors;
        $_SESSION['sr_community_asset_policy_set_values'] = $values + ['id' => $setId];
        sr_redirect($isUpdate ? '/admin/community/asset-policy-sets?mode=edit&id=' . (string) $setId : '/admin/community/asset-policy-sets?mode=new');
    }

    $savedId = sr_community_save_asset_policy_set($pdo, $values, (int) $account['id'], $setId);
    sr_audit_log($pdo, [
        'actor_account_id' => (int) $account['id'],
        'actor_type' => 'admin',
        'event_type' => $isUpdate ? 'community.asset_policy_set.updated' : 'community.asset_policy_set.created',
        'target_type' => 'community_asset_policy_set',
        'target_id' => (string) $savedId,
        'result' => 'success',
        'message' => 'Community asset policy set saved.',
        'metadata' => $values,
    ]);

    $_SESSION['sr_community_asset_policy_set_notice'] = '커뮤니티 멤버 그룹별 설정을 저장했습니다.';
...
include SR_ROOT . '/modules/community/views/admin-asset-policy-sets.php';

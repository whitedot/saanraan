...
if ($policySetPage === 'edit') {
    ...
    if (!is_array($editPolicySet)) {
        sr_render_error(404, '커뮤니티 회원 그룹별 설정 세트를 찾을 수 없습니다.');
    }
}
...
if (sr_request_method() === 'POST') {
    ...
    if ($isUpdate && !is_array($existing)) {
        $errors[] = '수정할 커뮤니티 회원 그룹별 설정 세트를 찾을 수 없습니다.';
    }
    ...
    $errors = array_merge($errors, sr_admin_asset_group_policy_validation_errors($pdo, sr_community_asset_group_policies_from_value($policiesJson), '커뮤니티 회원 그룹별 설정', true, $assetModuleOptions));
    ...
    $_SESSION['sr_community_asset_policy_set_notice'] = '커뮤니티 회원 그룹별 설정을 저장했습니다.';

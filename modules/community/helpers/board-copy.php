...
function sr_community_board_copy_storage_warnings(array $counts): array
{
    ...
    if ($bytes > 0) {
        return [
            ...
            '공유 호스팅은 디스크 용량이 남아 있어도 DB 용량, 파일 개수 제한, inode 제한 때문에 중간에 멈출 수 있습니다. 큰 게시판은 호스팅 관리 화면에서 여유 용량을 먼저 확인한 뒤 실행하세요.',
        ];
    }

    return [
        '첨부파일이 없어도 게시글과 댓글을 새로 저장하므로 DB 용량이 부족하면 복사가 실패할 수 있습니다. 큰 게시판은 호스팅 관리 화면에서 DB 여유 용량을 먼저 확인하세요.',
    ];
}

...
function sr_quiz_comment_body_format(PDO $pdo, ?array $settings = null): string
{
    return sr_editor_format_value($pdo, sr_quiz_comment_editor_key($pdo, $settings));
}

function sr_quiz_comment_body_plain_text(PDO $pdo, array $comment, ?array $settings = null): string
{
    $comment['body_format'] = sr_quiz_comment_body_format($pdo, $settings);
    return sr_body_text_plain_text($comment, $pdo);
}

function sr_quiz_comment_body_html(PDO $pdo, array $comment, ?array $settings = null): string
{
    $editorKey = sr_quiz_comment_editor_key($pdo, $settings);
    ...
    return '<div class="sr-ckeditor" data-sr-editor-output data-sr-editor-body-theme="quiz.' . sr_e($themeKey) . '">'
        . '<div class="ck-content" lang="' . sr_e(sr_locale()) . '" dir="ltr">' . $html . '</div></div>';
}

function sr_quiz_comment_body_stylesheets(PDO $pdo, ?array $settings = null): array
{
    $editorKey = sr_quiz_comment_editor_key($pdo, $settings);
...
function sr_quiz_comment_input_values(?PDO $pdo = null, ?array $settings = null): array
{
    $parentCommentIdValue = sr_post_string('parent_comment_id', 20);
    $bodyFormat = $pdo instanceof PDO ? sr_quiz_comment_body_format($pdo, $settings) : 'plain';

...
sr_public_layout_begin($pdo ?? null, $site ?? null, $seo, sr_quiz_ui_kit_layout_context($quizLayoutSettings, [
    'include_installed_layout_options' => true,
]));
?>

    <main class="example-quiz-theme quiz-ui-kit" data-example-theme-view="quiz.ui-kit" data-theme-ui-kit-view="quiz.sample">
        ...
        <div class="ui-kit-sample-body quiz-ui-kit-samples ui-form-theme">
            <?php foreach ($uiKitSamples as $sampleKey => $sampleLabel) { ?>
                <section id="ui-kit-<?php echo sr_e($sampleKey); ?>" class="quiz-ui-kit-section ui-kit-space-before-base" aria-labelledby="ui-kit-title-<?php echo sr_e($sampleKey); ?>">
                    <h2 id="ui-kit-title-<?php echo sr_e($sampleKey); ?>" class="quiz-ui-kit-section-title"><?php echo sr_e($sampleLabel); ?></h2>
                    <?php
                    $sampleFile = SR_ROOT . '/modules/quiz/views/ui-kit-samples/' . $sampleKey . '.php';
...
<script>
(function () {
    var root = document.querySelector('.quiz-ui-kit-samples');
    if (!root) {
        return;
    }

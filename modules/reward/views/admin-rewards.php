...
<section>
    <h2>회원 조회</h2>
    <form method="get" action="<?php echo sr_e(sr_url($rewardAdminPage === 'transactions' ? '/admin/rewards/transactions' : ($rewardAdminPage === 'adjust' ? '/admin/rewards/adjust' : '/admin/rewards/balances'))); ?>" class="admin-filter-form ui-form-theme">
...
<?php if ($rewardAdminPage === 'adjust') { ?>
    <form method="post" action="<?php echo sr_e(sr_url('/admin/rewards/adjust' . ($accountIdentifierFilter !== '' ? '?account_identifier=' . rawurlencode($accountIdentifierFilter) : ''))); ?>" class="admin-form-layout ui-form-theme ui-form-showcase">
        <section class="card">
            <h2>적립금 조정</h2>

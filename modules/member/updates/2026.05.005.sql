SET @schema_has_member_accounts_login_id_hash = (
    ...
    WHERE TABLE_SCHEMA = DATABASE()
      AND TABLE_NAME = 'sr_member_accounts'
      AND COLUMN_NAME = 'login_id_hash'
);

SET @schema_sql = IF(
    @schema_has_member_accounts_login_id_hash = 0,
    'ALTER TABLE sr_member_accounts ADD COLUMN login_id_hash CHAR(64) NULL AFTER account_identifier_hash',
    'DO 0'
);
...
SET @schema_sql = IF(
    ...
    'ALTER TABLE sr_member_accounts ADD UNIQUE KEY uq_sr_member_login_id_hash (login_id_hash)',
    'DO 0'
);
PREPARE schema_stmt FROM @schema_sql;
EXECUTE schema_stmt;

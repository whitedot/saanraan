...
<section>
    ...
    <form method="post" action="<?php echo toy_e(toy_url('/admin/member-groups')); ?>">
        ...
        <?php if (is_array($editGroup)) { ?>
            <a href="<?php echo toy_e(toy_url('/admin/member-groups')); ?>">새 그룹</a>
        <?php } ?>
    </form>
</section>
        ...
        <tbody>
            ...
            <?php foreach ($groups as $group) { ?>
                <tr>
                    ...
                    <td><a href="<?php echo toy_e(toy_url('/admin/member-groups?edit_id=' . (string) $group['id'])); ?>">수정</a></td>
                </tr>
            <?php } ?>
        </tbody>
    </table>
...
<section>
    <h2><?php echo is_array($editRule) ? '자동 규칙 수정' : '자동 규칙'; ?></h2>
    <form method="post" action="<?php echo toy_e(toy_url('/admin/member-groups')); ?>">
        <?php echo toy_csrf_field(); ?>
        ...
        <?php if (is_array($editRule)) { ?>
            <a href="<?php echo toy_e(toy_url('/admin/member-groups')); ?>">새 자동 규칙</a>
        <?php } ?>
    </form>
        ...
        <tbody>
            ...
            <?php foreach ($groupRules as $rule) { ?>
                <tr>
                    ...
                    <td><a href="<?php echo toy_e(toy_url('/admin/member-groups?edit_rule_id=' . (string) $rule['id'])); ?>">수정</a></td>
                </tr>
            <?php } ?>
        </tbody>
    </table>
...
<section>
    <h2>자동 규칙 재평가</h2>
    <form method="post" action="<?php echo toy_e(toy_url('/admin/member-groups/recalculate')); ?>">
        <?php echo toy_csrf_field(); ?>
        <input type="hidden" name="intent" value="evaluate_account">
        <p>
            <label>회원 ID<br>
    ...
    <form method="post" action="<?php echo toy_e(toy_url('/admin/member-groups/recalculate')); ?>">
        <?php echo toy_csrf_field(); ?>
        <input type="hidden" name="intent" value="evaluate_batch">
        <p>
            <label>모듈 key<br>
                <input type="text" name="source_module_key" maxlength="60">
            </label>
...
<section>
    ...
    <form method="post" action="<?php echo toy_e(toy_url('/admin/member-groups')); ?>">
        <?php echo toy_csrf_field(); ?>
        <input type="hidden" name="intent" value="grant_manual">
        <p>
            <label>회원 ID<br>
                <input type="number" name="account_id" min="1" required>
            </label>
                ...
                <tr>
                    ...
                    <td>
                        <?php if ((string) $membership['assignment_type'] === 'manual' && (string) $membership['status'] === 'active') { ?>
                            <form method="post" action="<?php echo toy_e(toy_url('/admin/member-groups')); ?>">
                                <?php echo toy_csrf_field(); ?>
                                <input type="hidden" name="intent" value="revoke_manual">

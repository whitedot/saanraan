...
릴리스 전 보안 수정 검증 결과는 [릴리스 검증 기록 템플릿](release-verification-template.md)의 실패와 제한, 리스크별 릴리스 판정 연결 항목에 맞춰 기록한다.
...
HTML sanitizer, CKEditor, URL 임베드 변경은 rich text sanitizer fixture와 URL 임베드 계약 fixture를 직접 실행한다.

```bash
php .tools/bin/check-rich-text-sanitizer.php
php .tools/bin/check-url-embed-contracts.php
```
...
4. 1.0 전에는 저장소 `docs/` 기준 문서를 갱신한다. GitHub Wiki는 현재 운영 문서의 정본으로 사용하지 않는다.

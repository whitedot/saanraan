...
사이드바는 모듈의 `admin.category_label`을 라벨로만 표시하고, 실제 조작 가능한 메뉴는 `모듈 그룹 > 메뉴 항목`의 2단계 구조로 유지한다. 라벨은 시스템, 회원, 사이트, 커뮤니티, 운영처럼 메뉴를 구분하는 시각적 구획일 뿐이며 접기/펼치기 버튼이나 링크로 사용하지 않는다. 사이트 분류에는 public layout/theme 기본 홈페이지와 초기화면 선택을 관리하는 `홈페이지` 화면을 둔다.

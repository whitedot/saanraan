...
사이트 설정 화면은 사이트 이름, 시간대, locale, 운영 상태, 공개 화면 레이아웃, UI 색상 모드, 관리자 스킨처럼 운영자가 의미를 이해하고 직접 변경할 수 있는 전용 필드만 다룬다. 공개 화면과 관리자 화면 표시 옵션은 같은 `화면` 섹션에서 한 번에 저장한다. `sr_site_settings`는 내부 저장소로 유지하지만, 임의 key/value를 추가·삭제하는 범용 관리자 화면은 제공하지 않는다.

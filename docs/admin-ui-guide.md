...
- 활성 회원 그룹 중 여러 항목을 선택하는 설정은 그룹 key 직접 입력 대신 `sr_admin_member_group_key_select_html()` 다중 셀렉트를 사용한다. 저장 처리는 배열 POST를 우선 받되 기존 문자열 입력도 정규화해 과거 데이터나 수동 요청과 호환되게 둔다.

...
포인트, 적립금, 예치금처럼 잔액 목록에서 대상 회원이 이미 분명한 경우에는 조정 작업을 독립 화면으로 분리하지 않고 잔액 화면의 행 단위 모달에서 처리한다. 잔액 화면에는 기본 조정 모달 버튼을 항상 노출해 운영자가 회원 공개 해시를 직접 입력할 수도 있게 한다. 서버 검증과 원장 저장은 각 모듈 action이 계속 소유하고, 과거 `/admin/*/adjust` 직접 접근은 같은 회원의 잔액 화면으로 되돌린다.

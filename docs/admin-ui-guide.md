...
관리자 권한 화면은 필터 검색 목록이 아니라 이미 소유자 또는 메뉴 권한이 부여된 회원 목록을 기본으로 보여준다. 새 권한 부여는 `권한 추가` 모달에서 시작하고, 회원 검색은 별도 스택 모달로 열어 선택 후 권한 추가 모달로 돌아온다. 신규 권한은 1차 분류, 2차 메뉴, 조회/수정/삭제 권한을 선택한 현재 상태를 저장 시점에 적용한다. 1차 분류나 2차 메뉴에서 `전체`를 선택하면 해당 범위의 모든 메뉴에 같은 작업 권한을 적용한다. 목록의 소유자가 아닌 회원은 행 단위 `권한 회수` 버튼으로 메뉴 권한을 즉시 비울 수 있다.
...
- `required` 입력 항목은 view 마크업의 해당 label 또는 복합 행의 `span.form-label`에 `<span class="sr-required-label">(필수)</span>`를 직접 넣어 표시한다. 관리자 shell이나 공통 JavaScript로 필수 표기를 후처리하지 않는다.

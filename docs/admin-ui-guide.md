...
대시보드 섹션은 화면 구조를 바꾸는 관리 도구가 아니라 개인 작업 배치에 가깝다. 따라서 드래그 순서는 DB에 저장하지 않고, 모듈이 제공하는 기본 표시 순서는 `dashboard.php`의 `order` 값으로 유지한다. 섹션 이동은 카드 전체가 아니라 카드 헤더의 아이콘 핸들에서만 시작하며, 관리자 대시보드 본문은 최대 3열 그리드로 배치한다. 다른 섹션의 좌우에 놓을 때는 수직 고스트 라인을 표시하고 일반 폭으로 배치하며, 위아래에 놓을 때는 행 사이 슬롯 중앙에 수평 고스트 라인 하나만 표시하고 한 줄 전체를 차지하는 1열 섹션으로 배치할 수 있다. 고스트 라인은 드래그 중 카드 배치를 밀지 않는 overlay로만 표시한다.

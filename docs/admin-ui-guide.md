...
- 체크박스와 라디오처럼 컨트롤 옆 문구가 실제 레이블인 항목도 같은 `<label>` 안에 컨트롤과 문구를 함께 둔다. 공통 관리자 렌더링은 좌측 레이블 칸에는 전체 문구를 그대로 표시하고, 우측 필드 칸의 원래 레이블은 `허용`, `사용`, `확인`처럼 필요한 부분만 보이게 줄인다. 우측에서 생략된 맥락은 `.sr-only` 텍스트로 남겨 보조기기가 전체 의미를 읽을 수 있게 한다.

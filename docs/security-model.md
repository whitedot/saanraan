...
회원 로그인 세션은 member 모듈 설정 `session_lifetime_seconds`가 정하는 절대 상한을 따른다. 기본값은 86400초이고, 관리자 UI는 1800~2592000초 범위를 서버에서 검증한다. 이 값은 마지막 활동 기준 idle timeout이 아니라 로그인 또는 세션 회전 시점 기준이므로 `sr_member_sessions.expires_at`은 활동만으로 연장되지 않는다. 설정 단축은 기존 세션에도 읽기 시점 상한으로 적용하고 cleanup은 `created_at` 기준 상한을 지난 row를 삭제한다. 설정 연장은 저장된 `expires_at`을 넘겨 기존 세션을 되살리지 않는다. 설정 helper가 없거나 설정 조회만 실패하면 86400초로 후퇴하되, 세션 row 조회 실패는 로그인 세션을 무효로 처리한다.

...
콘텐츠 CKEditor 본문 이미지는 다운로드 파일과 분리된 콘텐츠 모듈 본문 파일 모델이 소유한다. 업로드 endpoint는 관리자 권한과 CSRF를 확인하고, 저장된 본문 HTML에서 정화 후 남은 프록시 URL만 서버가 파일 참조로 인정한다. 비공개, 유료 열람, 미발행 콘텐츠의 본문 이미지 프록시는 콘텐츠 접근 정책 또는 관리자 권한을 다시 확인해야 하며, 클라이언트 hidden 값이나 공개 URL 문자열만으로 접근을 허용하지 않는다.

...
커뮤니티 게시글의 CKEditor HTML 저장은 커뮤니티 모듈 서버 helper가 허용 태그/속성 기반으로 정화한다. 클라이언트 에디터가 보낸 HTML을 신뢰하지 않으며, `script`, event handler, inline style, `iframe`, `object`, `embed`, `form`, `javascript:` URL, data URL 이미지는 허용하지 않는다.

...
회원 대상 외부 push endpoint는 원문을 delivery나 개인정보 export에 복사하지 않는다. 알림 모듈은 endpoint 원문을 app key 기반 암호문과 fingerprint로 분리해 저장하고, delivery에는 `endpoint:{id}` 내부 참조만 남긴다. runner는 전송 직전에 endpoint 소유 계정, 활성 상태, 개인 수신처 타입, 알림 audience/account, provider 설정과 복호화 가능 여부를 다시 확인하며, 연결 해제나 복호화 실패처럼 provider 문제가 아닌 경우에는 재시도하지 않고 delivery를 취소한다. 외부 push payload는 사이트 알림 본문 전체를 복사하지 않고 제목, 짧은 안내, 내부 링크만 보낸다.

...
관리자 긴 폼의 수동 임시저장은 로그인 관리자 계정 ID, 허용된 폼 key와 대상 key 조합으로 분리하고 해당 화면의 수정 권한과 CSRF를 저장·삭제 요청마다 다시 확인한다. CSRF token, 확인 문구와 파괴 작업 확인값은 payload에 보관하지 않으며, payload는 깊이·항목 수·개별 값 크기·전체 1MB 제한을 적용한다. 임시저장본은 실제 설정이나 레코드가 아니므로 최종 저장 때 기존 도메인 서버 검증을 우회하지 않고, 최종 저장 성공 뒤에만 삭제한다. 90일이 지난 관리자 임시저장본은 새 임시저장 요청마다 최대 100건씩 정리한다. 관리자 임시저장 payload는 해당 관리자 계정의 개인정보 사본에 포함하고 계정 정리 시 함께 삭제한다.
...
마이페이지(`/mypage`, 호환 경로 `/account`)는 로그인 세션만으로 본문을 바로 보여주지 않는다. 로그인 세션에서 최초 진입할 때 `/mypage/verify`에서 현재 비밀번호를 다시 확인하고, 확인 상태를 현재 account ID와 `sr_member_sessions` token hash에 묶어 같은 로그인 세션의 마이페이지 이동에 재사용한다. 세션 token이 바뀌거나 다른 계정으로 전환되면 이전 확인 상태는 무효다. 비밀번호가 없는 계정은 활성 TOTP 또는 미사용 복구 코드로 재확인하고, 추가 재확인 수단도 없는 OAuth-only 계정은 이미 완료된 로그인 세션으로 계속한다는 확인 POST를 거친다. 회원 환경설정의 `identity_account_security_required`가 켜져 있으면 비밀번호·2차 인증 확인 다음에 `member.account_security` 목적 본인확인까지 완료해야 한다. 직접 GET/POST로 세부 경로에 접근해도 같은 gate를 통과하며, 비밀번호·2차 인증 확인 실패는 회원 재인증 rate limit과 인증 로그에 포함한다.
...
`sr_member_require_login()`은 비로그인 요청을 `/login`으로 보낼 때 현재 내부 경로와 쿼리스트링을 `next` 값으로 전달한다. public layout의 로그인 링크와 로그인이 필요한 기능 링크도 가능하면 현재 내부 경로와 쿼리스트링을 `next`로 명시해 Referer에만 의존하지 않는다. 로그인 action은 명시적인 안전한 내부 `next` 값이나 같은 호스트 referrer를 사용할 수 있으면 로그인 성공 후 그 위치로 보내고, 사용할 수 없거나 `/login`, `/login/mfa`, `/logout` 같은 인증 흐름 경로이면 `/`로 보낸다. `next`는 내부 상대 URL만 허용하고 encoded slash/backslash, 제어문자, dot segment를 포함한 path는 기본 `/`로 접는다.
...
이메일 인증을 로그인 필수 조건으로 사용하더라도 `admin` 모듈의 `owner` 역할인 매니저 계정은 인증 완료 전 로그인할 수 있다. 예외 판정은 `admin-account-role.php` 계약을 통해 수행하고 비밀번호 로그인, OAuth 기존 계정 로그인, MFA challenge 재진입, 기존 세션 재검증에 같은 기준을 적용한다. 일반 회원과 스탭은 이메일 인증 필수 정책을 그대로 적용받으며, 계정 상태가 `active`가 아니면 소유자도 로그인할 수 없다.

로그인 2차 인증 정책이 `disabled`이면 마이페이지 보안 화면에서도 2차 인증 섹션 전체를 노출하지 않는다. 준비·활성화 직접 POST는 기존 서버 정책대로 비활성 provider로 거부한다.

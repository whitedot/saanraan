...
기본 CSP는 자체 출처를 기준으로 한다. `script-src`와 `connect-src`는 자체 출처만 허용하며, 외부 스타일/폰트는 현재 관리자 폰트 로딩에 필요한 `https://cdn.jsdelivr.net`만 허용한다. 외부 스크립트 또는 연결 출처를 추가할 때는 UI 렌더링에 필요한지, 자체 호스팅으로 대체할 수 있는지, 운영 화면 전체에 주는 영향을 함께 검토한다.

...
기본 CSP는 자체 출처를 기준으로 한다. 예외적으로 관리자/공개 UI-KIT 조회 화면에서 아이콘 샘플을 렌더링하기 위해 `script-src`에 `https://code.iconify.design`, `connect-src`에 `https://api.iconify.design`을 허용한다. Iconify 런타임은 스크립트 로드 후 아이콘 데이터를 API에서 가져오므로 두 출처가 함께 필요하다. 외부 스크립트 또는 연결 출처를 추가할 때는 UI 렌더링에 필요한지, 자체 호스팅으로 대체할 수 있는지, 운영 화면 전체에 주는 영향을 함께 검토한다.

...
외부 스킨·테마 패키지는 보안 sandbox가 아니라 애플리케이션 권한으로 실행되는 특권 PHP 코드다. `sr-packages/` manifest 검증은 패키지 내부 상대 경로, view/asset 확장자, 버전과 계약 shape, 직접 HTTP asset 노출 경로를 제한하지만 PHP include의 의미를 안전하게 바꾸지 않는다. 따라서 패키지 배치는 임의 PHP 배포와 같은 승인 수준으로 다루고, 직접 웹 접근은 `.htaccess` 또는 nginx 규칙으로 막으며, 공개 asset은 `/sr-package-asset` front controller가 manifest에 선언된 id만 제공한다. 자세한 계약은 [스킨·테마 패키지 기준](skin-theme-packages.md)을 따른다.
...
-> GET /sr-package-asset이면 manifest에 선언된 패키지 asset만 응답

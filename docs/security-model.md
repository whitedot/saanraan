...
기본 CSP는 자체 출처를 기준으로 한다. 외부 스타일/폰트와 연결 출처는 현재 관리자 폰트 로딩과 브라우저 개발자 도구의 보조 요청에 필요한 `https://cdn.jsdelivr.net`을 허용한다. YouTube URL 임베드를 위해 frame 출처는 자체 출처와 `https://www.youtube-nocookie.com`, `https://www.youtube.com`만 허용한다. CKEditor 플러그인의 기본 운영값은 저장소에 포함된 `modules/ckeditor/vendor/ckeditor5/` 파일을 쓰는 직접 호스팅이다. 선택 CDN 모드는 공식 CDN의 스크립트와 스타일을 불러올 수 있도록 `https://cdn.ckeditor.com`을 허용한다. 외부 스크립트, 연결 출처, frame 출처를 추가할 때는 UI 렌더링에 필요한지, 자체 호스팅으로 대체할 수 있는지, 운영 화면 전체에 주는 영향을 함께 검토한다.
...
회원 자산 알림은 자산 거래나 쿠폰 상태 변경이 성공한 뒤에만 생성한다. 알림 모듈이 비활성화되어 있거나 알림 템플릿이 없으면 no-op으로 처리하고, 포인트/적립금/예치금 원장 처리나 쿠폰 지급/사용/수동 환불 처리를 실패시키지 않는다. 포인트/적립금 만료 소비 매핑처럼 특정 회원 계정 ID에 연결되는 자산 보조 기록은 해당 모듈의 개인정보 사본 제공에 포함한다. 외부 채널은 이벤트 템플릿 또는 포인트/적립금/예치금/쿠폰 환경설정의 케이스별 알림 채널에 따라 발송 작업 queue를 만들며, 환전 환경설정 화면에서 바꾸는 환전 알림도 각 자산 모듈의 케이스별 채널 설정으로 저장한다. 기본 자산 알림은 사이트 알림으로 둔다. 포인트 회원 알림 케이스의 기본값은 사용 안 함이고, 적립금/예치금/쿠폰 회원 알림 케이스는 기존 동작 보존을 위해 기본값을 사용으로 둔다.

회원 대상 외부 push endpoint는 원문을 delivery나 개인정보 export에 복사하지 않는다. 알림 모듈은 endpoint 원문을 app key 기반 암호문과 fingerprint로 분리해 저장하고, delivery에는 `endpoint:{id}` 내부 참조만 남긴다. 가역 비밀값 암호화와 fingerprint는 코어의 `sr_secret_at_rest_*()` helper를 사용하며, helper는 app key에 바인딩된 `sr2` envelope만 만들고 app key가 없으면 fallback key 없이 실패한다. 회원의 Telegram endpoint 연결과 해제는 `/account/notifications`에서 현재 비밀번호 재확인과 CSRF를 통과해야 하며, 성공 시 회원 인증 로그, 감사 로그, 사이트 보안 알림을 남긴다. 같은 provider/endpoint fingerprint를 다른 계정에 중복 연결할 수 없고, 계정별 provider 활성 endpoint는 5개를 넘길 수 없다. runner는 전송 직전에 endpoint 소유 계정, 활성 상태, 개인 수신처 타입, 알림 audience/account, provider 설정과 복호화 가능 여부를 다시 확인하며, 연결 해제나 복호화 실패처럼 provider 문제가 아닌 경우에는 재시도하지 않고 delivery를 취소한다. 외부 push payload는 사이트 알림 본문 전체를 복사하지 않고 제목, 짧은 안내, 내부 링크만 보낸다. 회원이 endpoint를 해제하거나 개인정보 cleanup이 실행되면 endpoint row는 보존 식별자만 남기고 `endpoint_ciphertext`를 비운 disabled 상태로 전환한다.
...
회원 2차 인증은 로그인 완료 지점 앞의 gate로 다룬다. 비밀번호 로그인, 가입 직후 자동 로그인, OAuth 기존 계정 로그인, OAuth completion 자동 로그인은 모두 `sr_member_login_or_start_mfa()`를 거친다. 활성 TOTP factor는 `sr_member_mfa_factors`에서 조회하고, pending challenge는 `sr_account_id`와 분리된 세션 값으로만 저장한다. `/mypage/security`는 비밀번호가 있는 계정에서 현재 비밀번호 재확인을 통과한 뒤 pending TOTP factor를 만들고, 수동 입력 secret과 `otpauth://` URI를 1회 flash로 보여준다. 사용자가 인증 앱의 첫 코드를 제출하면 pending factor를 active로 전환하고, 같은 time step 코드를 즉시 로그인에 재사용할 수 없도록 `last_used_step`을 함께 기록한다. 이 활성화 트랜잭션은 기존 미사용 백업 코드를 폐기하고 새 백업 코드 묶음을 `sr_hmac_hash()` 기반 hash로 저장하며, 원문 백업 코드는 session flash로 한 번만 보여준다. `/login/mfa` POST는 app key 기반 secret-at-rest 암호문을 복호화해 TOTP 코드를 검증하거나 미사용 백업 코드를 조건부 update로 소비한 뒤에만 `sr_member_login()`을 호출한다. `/mypage/security`의 백업 코드 재발급과 2차 인증 해제는 CSRF와 재인증을 요구한다. 비밀번호가 있는 계정은 현재 비밀번호를 확인하고, 비밀번호가 없는 OAuth-only 계정은 현재 TOTP 코드 또는 미사용 백업 코드로 재확인한다. 백업 코드 재발급은 기존 미사용 코드를 revoked로 전환하고 새 코드를 1회 flash로 보여주며, 2차 인증 해제는 활성/pending TOTP factor를 disabled로 전환하고 남은 미사용 백업 코드를 revoked로 전환한다. TOTP는 `last_used_step` 조건부 업데이트로 같은 time step 재사용을 막고, 백업 코드는 `status = 'unused'` 조건부 업데이트로 재사용을 막는다. MFA factor secret ciphertext, fingerprint, recovery code hash는 개인정보 사본 제공에 포함하지 않고, factor 상태/표시 메타데이터와 recovery code 상태별 개수만 제공한다. 탈퇴와 관리자 withdrawn/anonymized 전환은 다른 모듈 cleanup 계약을 실행하기 전에 회원 모듈 소유 MFA factor와 recovery code row를 직접 삭제한다.

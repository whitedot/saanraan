...
텍스트 색상은 기본 상속을 우선한다. `html`, `body`, `.admin-content`의 기본 글자색을 충분히 진하게 두고, view 또는 모듈 CSS는 일반 라벨, 표 셀, 제목에 옅은 회색을 개별 지정하지 않는다. 도움말, 힌트, 빈 상태, 보조 메타처럼 우선순위가 낮은 텍스트만 muted 계열 class나 전용 선택자를 사용한다.

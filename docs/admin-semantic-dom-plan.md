...
- `assets/common.css`: 사이트 공통 디자인 토큰, 공통 UI kit 원형, 재사용 유틸리티
...
- 공통 토큰, UI kit 원형, 공통 유틸은 `assets/common.css`에 둔다.
- `assets/common.css`는 프로젝트 공통 manifest로 유지한다.
- 관리자 런타임은 `assets/common.css`, `assets/admin-ui.css`, `modules/admin/assets/admin.css` 순서로 호출한다.

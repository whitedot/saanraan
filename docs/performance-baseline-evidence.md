...
| 캐시 경로 | 파일 캐시는 `storage/cache/` 아래에서만 허용하고, 현재 기본 허용 경로는 HTML Purifier 정의 캐시인 `storage/cache/htmlpurifier`와 공개 이미지 썸네일 캐시인 `storage/cache/thumbnails`다. 썸네일 캐시는 생성 파일명 패턴의 이미지 파일만 직접 접근을 허용한다. | `.tools/bin/check-performance-baseline.php`, `.tools/bin/check-dependency-policy.php`, `.tools/bin/check-storage-helpers.php` |
...
| 이미지 응답 헤더 | 공개 이미지, 관리자 아이콘, 아바타, 본문 이미지 proxy는 공통 helper로 public/private cache-control, optional content length, nosniff를 적용한다. SVG 로고는 추가 CSP도 helper allowlist를 통과한다. | `sr_send_file_headers()`, `.tools/bin/check-output-helpers.php`, `.tools/bin/check-performance-baseline.php` |

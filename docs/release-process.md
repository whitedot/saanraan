...
- 1.0 릴리스 후보라면 [1.0 범위 잠금 기준](1.0-scope.md)의 포함/제외 범위를 확인하고, 본인확인은 beta 구현과 provider 실인증 smoke 보강 기준으로, 회원 마이그레이션과 결제 플러그인은 계획 단계 기능으로 구분되어 있는지 확인한다.
...
SR_SMOKE_BASE_URL=http://127.0.0.1:PORT php .tools/bin/release-installed-gate-status.php --run-http-smoke
...
설치 DB가 필요한 검증은 preflight가 대체하지 않는다. 릴리스 후보에서는 `php .tools/bin/release-installed-gate-status.php`를 실행해 새 설치/업데이트, read-only CLI 게이트, 관리자 read-only 화면, 기본 HTTP smoke, 인증/퀴즈/자산 mutation smoke, 개인정보 export/cleanup, CKEditor browser smoke, 성능 수동 점검의 현재 상태와 미실행 사유를 기록한다. 사람이 읽는 릴리스 노트에는 기본 출력의 상태표, `gate-result-summary`, `unresolved-gates`를 남기고, 자동화/보관용 증거가 필요하면 `php .tools/bin/release-installed-gate-status.php --json` 출력의 `metadata`, `gates`, `result_counts`, `unresolved_gates`를 함께 보관한다. `--markdown-table`과 `--json`은 서로 배타적인 출력 형식이므로 함께 지정하지 않는다. 상태표 도구는 `SR_SMOKE_BASE_URL`과 `SR_BROWSER_QA_BASE_URL`의 URL userinfo를 metadata, gate 환경값, 실행 출력 요약에 남기기 전에 마스킹해야 한다. CI나 릴리스 스크립트에서 미해결 설치 DB 게이트를 실패로 다루려면 `--fail-on-unresolved`를 함께 지정한다. 기본 route, 보안 헤더, 보호 경로 HTTP smoke는 로컬/staging `SR_SMOKE_BASE_URL`을 지정한 뒤 `php .tools/bin/release-installed-gate-status.php --run-http-smoke`로 상태표 안에서 실행한다. 기존 설치본 업데이트 적용 smoke는 local/staging disposable DB에서만 `SR_SMOKE_ALLOW_MUTATION=1`, 관리자 계정, `--run-update-smoke`를 함께 지정해 실행한다. 현재 CLI 사용자가 공유호스팅 보안 권한 때문에 `config/config.php`를 읽지 못하면 권한을 넓히지 말고 웹 서버 사용자 또는 로컬/staging 전용 실행 사용자로 `php .tools/bin/release-installed-gate-status.php --run-readonly --fail-on-unresolved`를 다시 실행한다. 로컬/staging HTTP와 관리자 계정이 준비된 경우에는 `SR_SMOKE_BASE_URL`, `SR_SMOKE_ADMIN_IDENTIFIER`, `SR_SMOKE_ADMIN_PASSWORD`를 지정한 뒤 `php .tools/bin/release-installed-gate-status.php --json --fail-on-unresolved`로 구조화 증거를 남긴다.
...
- `release-installed-gate-status.php`의 필수 설치 DB 게이트 상태표, `gate-result-summary`, `unresolved-gates`, 필요 시 `--json` 구조화 증거
- 포함된 모듈 목록과 각 모듈 버전

...
| R-03 | 공유호스팅 queue/cron/배치 | `accepted-for-1.0` | 상시 worker 부재로 예약/발송/정리 지연, timeout, 중복 실행 | [운영 상태 점검 기준](operational-status.md), [성능과 캐시 기준](performance-policy.md), `.tools/bin/ops-status.php`, `.tools/bin/expire-points.php --dry-run`, `/admin/operations`, `.tools/bin/check-operational-status.php`, `.tools/bin/check-community-board-copy-job-lock.php`, [스모크 테스트 기준](smoke-test.md) | 설치 DB 또는 staging에서 실제 지연/실패 rows 기준 운영자 화면과 CLI 출력, 포인트 만료 dry-run preview 기록 |

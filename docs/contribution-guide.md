...
새 `check-*.php` 검증 도구를 추가할 때는 `.tools/bin/check.php` 통합 게이트에 연결한다. 오래 걸리는 deep QA처럼 의도적으로 독립 실행해야 하는 도구만 `.tools/bin/check-tool-gate-coverage.php`의 standalone 목록에 이유를 두고 남기며, 이 점검은 새 도구가 통합 게이트에서 빠지는 상태를 실패로 처리한다.

...
공통 helper는 계약 파일별 target 구조를 먼저 검증한다. `target_type`은 문자열이어야 하고, `target_key`는 앞뒤 공백이 없는 문자열이어야 하며, `target_id`는 정수 또는 앞뒤 공백과 leading zero가 없는 숫자 문자열이어야 한다. `coupon_definition`, `banner`, `popup_layer`, `member_group`은 1 이상의 숫자형 `target_id`가 필요하고, `site_setting`은 `target_id=0`만 허용한다. `member_group`과 `site_setting`은 빈 `target_key`를 허용하지 않는다.
...
같은 관리자 사이트 설정 화면에서 저장하는 `site.title_suffix`, `site.meta_description`은 별도 소비 모듈 계약이 아니라 저장 action에서 다룬다. 값이 이전 저장값 그대로이고 이전 사이트명을 포함하면 새 사이트명으로 함께 보정하고, 운영자가 별도로 수정한 값에 이전 사이트명이 남아 있으면 저장 action의 직접 검증으로 막는다.

관리자/공개 layout의 사이트명 fallback은 자동으로 새 설정을 읽는 동작이므로 자동 수정 대상이 아니다. 필요하면 낮은 위험 안내 row로만 표시한다.

사이트명 변경 저장 액션은 소비 모듈 설정값을 자동 update/delete/치환하지 않는다. POST에서는 최신 참조 현황을 다시 조회하고, 계약 오류가 있으면 저장을 중단한다. 정상 참조 row만 있으면 사이트명 변경은 저장하고, 이전 사이트명이 포함된 모듈 설정은 토스트와 참조 현황으로 후속 확인을 안내한다.
...
- `count_function` 함수 본문 전체가 대응 `rows_function($pdo, $target, $context)` row 수를 직접 세는 단일 반환문인지 확인
...
- 사이트명을 로고 alt text에 직접 포함한 뒤 사이트명 변경 화면에서 참조 현황과 관리자 이동 링크가 표시되고, 변경 저장 후 토스트로 후속 확인이 안내되는지 확인
- 사이트명을 사이트 title suffix, 사이트 기본 설명에 이전 저장값 그대로 포함한 뒤 사이트명 변경 저장이 새 사이트명으로 함께 보정되는지 확인

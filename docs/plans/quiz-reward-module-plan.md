...
평가 기준일은 2026-06-06이다. 현재 저장소에는 `quiz` 모듈 기반 파일, 설치/업데이트 스키마, `/admin/quiz` CRUD, `/admin/quiz/attempts` 시도/보상 조회, `/quiz/{quiz_key}` 풀이/제출/자동 채점, 공개 기간/회원 그룹/시도 제한, 콘텐츠와 커뮤니티 source 연결, 모달/page fallback CTA, 개인정보 계약, 자산 원장 조회 계약, 쿠폰 보상 참조 계약, 정합성 검사가 반영되어 있다. 이 문서는 마일스톤 2 범위의 계획 정합성과 구현 중 지켜야 할 결정을 기록한다.
...
- 개인정보 export/cleanup은 계정별 시도, 제출 답안 snapshot, 결과 snapshot, 보상 지급 로그를 포함하고, 탈퇴/익명화 시 account 연결과 직접 식별 가능한 네트워크 식별 hash를 비운다.
- HTTP smoke test 범위는 `/quiz`, 콘텐츠/커뮤니티 source 시작, 퀴즈 풀이, 완료 후 `return_to` 복귀, 중복 제출 방지, 보상 원장 또는 쿠폰 발급 생성을 포함한다.

...
평가 기준일은 2026-06-06이다. 현재 저장소에는 `quiz` 모듈 기반 파일, 설치/업데이트 스키마, `/admin/quiz` CRUD, `/admin/quiz/attempts` 시도/보상 조회, `/quiz/{quiz_key}` 풀이/제출/자동 채점, 공개 기간/회원 그룹/시도 제한, 콘텐츠 source 연결과 모달/page fallback CTA, 개인정보 계약, 자산 원장 조회 계약, 정합성 검사가 반영되어 있다. 이 문서는 마일스톤 2 범위의 계획 정합성과 구현 중 지켜야 할 결정을 기록한다.

...
퀴즈 보상 정책, 시도 제한, 정답 판정, 중복 보상 방지, 결과 로그는 `quiz` 모듈이 소유한다. MVP 보상은 자동 채점 통과 직후 `ledger_asset` provider로 1회 지급하는 범위로 닫고, 포인트/적립금/예치금 지급은 활성 자산 모듈의 `member-assets.php` 계약을 읽어 실행한다. 보상 지급은 원장 거래와 퀴즈 시도/완료 로그를 같은 DB transaction 안에서 묶고, MVP `per_quiz` dedupe는 `account_id + quiz_id` 기준과 보상 grant row 잠금으로 막아 보상 정책을 다시 저장하거나 동시에 제출해도 같은 퀴즈 보상이 중복 지급되지 않게 한다.

...
평가 기준일은 2026-06-06이다. 현재 저장소에는 `quiz` 모듈 기반 파일, 설치 스키마, 개인정보 계약, 자산 원장 조회 계약, 정합성 검사 초안이 반영되어 있다. 이 문서는 마일스톤 2 범위의 계획 정합성과 구현 중 지켜야 할 결정을 기록한다.
...
퀴즈 보상 정책, 시도 제한, 정답 판정, 중복 보상 방지, 결과 로그는 `quiz` 모듈이 소유한다. MVP 보상은 자동 채점 통과 직후 `ledger_asset` provider로 1회 지급하는 범위로 닫고, 포인트/적립금/예치금 지급은 활성 자산 모듈의 `member-assets.php` 계약을 읽어 실행한다. 보상 지급은 원장 거래와 퀴즈 시도/완료 로그를 같은 DB transaction 안에서 묶고, 중복 제출은 `account_id + quiz_id + reward_policy` 기준의 dedupe 키로 막는다.

...
평가 기준일은 2026-06-06이다. 현재 저장소에는 `quiz` 모듈 기반 파일, 설치 스키마, `/admin/quiz` 최소 CRUD, `/quiz/{quiz_key}` 풀이/제출/자동 채점, 콘텐츠 source 연결과 모달/page fallback CTA, 개인정보 계약, 자산 원장 조회 계약, 정합성 검사 초안이 반영되어 있다. 이 문서는 마일스톤 2 범위의 계획 정합성과 구현 중 지켜야 할 결정을 기록한다.
...
6. 퀴즈 완료 후 유효한 내부 복귀 위치가 있으면 원래 콘텐츠/커뮤니티 화면으로 돌아가고, 없으면 퀴즈 결과 또는 퀴즈 메인으로 이동한다.

## 보상 처리 방향

퀴즈 보상 정책, 시도 제한, 정답 판정, 중복 보상 방지, 결과 로그는 `quiz` 모듈이 소유한다. MVP 보상은 자동 채점 통과 직후 `ledger_asset` provider로 1회 지급하는 범위로 닫고, 포인트/적립금/예치금 지급은 활성 자산 모듈의 `member-assets.php` 계약을 읽어 실행한다. 보상 지급은 원장 거래와 퀴즈 시도/완료 로그를 같은 DB transaction 안에서 묶고, MVP `per_quiz` dedupe는 `account_id + quiz_id` 기준으로 막아 보상 정책을 다시 저장해도 같은 퀴즈 보상이 중복 지급되지 않게 한다.
...
위 값이 변조되거나 대상이 숨김/삭제/권한 없음 상태이면 완료 후 퀴즈 결과 페이지 또는 `/quiz`로 fallback한다.
...
- MVP 문제 유형은 `single_choice` 정답형으로 닫고, `multiple_choice`, OX, 단답은 후속으로 둔다.
- MVP source는 `content/content_item`으로 고정한다. 독립 `/quiz` 메인과 개별 풀이 페이지는 제공하지만, MVP의 source 연계와 완료 후 복귀 검증은 콘텐츠 1건 기준으로 닫는다. 커뮤니티 연계와 콘텐츠/커뮤니티 동시 검증은 v1 후속으로 둔다.
...
- HTTP smoke test 범위는 `/quiz`, 선택한 단일 source 시작, 퀴즈 풀이, 완료 후 `return_to` 복귀, 중복 제출 방지, 보상 원장 생성을 포함한다.

## 판정

현재 제안된 "모달 우선, 필요 시 퀴즈 페이지 이동, 퀴즈 자체 메인 페이지 보유, 완료 후 출발 화면 복귀" 방향은 프로젝트 경계 규칙과 잘 맞는다. 위 범위 결정까지 반영하면 마일스톤 2 MVP는 구현 착수 가능한 수준으로 정합적이다. 남은 확장은 `multiple_choice`, `total_score`, `category_score`, 콘텐츠/커뮤니티 양쪽 동시 연계, 쿠폰 provider를 후속 단계에서 순차적으로 연다.

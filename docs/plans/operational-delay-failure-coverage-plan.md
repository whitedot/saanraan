...
- `/admin/operations`는 원본 작업 row를 해결하거나 상태 전이하지 않고 현재 운영 신호만 `확인됨`으로 표시할 수 있다.
- 확인 상태는 `sr_site_settings.admin.operational_status.acknowledged`에 `label`, 상태, 건수, 가장 오래된 시각으로 만든 지문을 저장한다.
- 같은 지문이 유지되는 동안 관리자 화면에서만 `확인됨`으로 낮춰 보이고, `확인됨` 상태에서만 한 번 더 `정상 취급`으로 낮출 수 있다.
- 새 실패가 생기거나 대상 지문이 바뀌면 `확인됨`과 `정상 취급` 모두 다시 `확인 필요` 또는 `지연 초과`로 돌아온다.
- CLI와 릴리스 게이트용 read-only 점검은 확인 표시를 적용하지 않고 원본 지연/실패 신호를 유지한다.

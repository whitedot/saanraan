...
| 3 | 적립금 출금 신청 대기 | `sr_reward_withdrawal_requests.status = 'pending'` | 3영업일 또는 설정값 초과 시 확인 필요 | 회원 ID / 신청 ID | `/admin/rewards/withdrawal-requests` | 시스템 이상이라기보다 운영 업무 SLA 점검이다. 별도 그룹으로 표시하는 것이 좋다. |
| 3 | 예치금 환불 신청 대기 | `sr_deposit_refund_requests.status = 'pending'` | 3영업일 또는 설정값 초과 시 확인 필요 | 회원 ID / 신청 ID | `/admin/deposits/refund-requests` | 출금/환불 운영 대기 누락 방지용이다. |
...
- `운영 지연/실패 점검` 안에 넣는다면 상태 라벨을 `처리 대기`/`처리 지연`처럼 분리하거나, 대시보드 업무 카드로 빼는 방안을 우선 검토한다.

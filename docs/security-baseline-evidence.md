...
| 보안 헤더 | `nosniff`, `SAMEORIGIN`, `Referrer-Policy`, 기본 CSP, YouTube URL 임베드용 `frame-src`, no-store를 전송한다. HTTPS production에서는 HSTS를 전송한다. | `core/helpers/runtime.php::sr_send_security_headers()` | `.tools/bin/check-security-baseline.php`, `.tools/bin/smoke-http.php` |

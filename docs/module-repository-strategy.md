...
이 문서는 Toycore 모듈을 코어 리포지토리 밖에서 관리할 때의 장단점, 권장 구조, 전환 단계를 정리한다. 단, 별도 리포지토리는 모든 외부 제작자의 기본 출발점이 아니라 반복 배포, 공개 배포, 공식 모듈 관리를 위한 선택 경로다.

결론부터 말하면, Toycore 초기에는 `core + 기본 필수 모듈(member, admin)`은 같은 리포지토리에 두고, 공식 선택 모듈이나 릴리스 주기가 분리되는 도메인 모듈부터 별도 리포지토리로 관리한다. 일반 외부 제작자에게는 먼저 `모듈 폴더 -> zip 업로드` 흐름을 안내하고, 별도 리포지토리는 필요해졌을 때 선택하게 한다. 코어 실행과 운영 설치가 Composer, Git, CLI에 의존하면 저가형 웹호스팅 목표와 충돌하므로 배포 산출물은 항상 복사 가능한 PHP/SQL 디렉터리여야 한다.

## 0. 사용자에게 노출하는 단계

Toycore는 모듈 관리 수준을 다음처럼 나눈다.

```text
초보자 기본:
모듈 폴더 -> zip 생성 -> /admin/modules 업로드

반복 제작자:
모듈 프로젝트 폴더 -> AGENTS/README/package-module -> 로컬 점검 -> zip 생성

공개/팀 개발:
별도 Git 리포지토리 -> CHANGELOG/release zip -> 선택적 CI

공식 maintainer:
별도 리포지토리 -> checksum/registry -> minimal/standard/ops 배포 패키지 조립
```

따라서 문서와 도구는 신규 제작자에게 Git 리포지토리부터 요구하지 않는다. 별도 리포지토리는 코어 제작자와 공식 모듈 maintainer에게는 기본 운영 방식이지만, 외부 개발자에게는 선택지다.
...
- 일반 외부 제작자에게도 별도 Git 리포지토리를 기본 제작 경로로 안내하지 않는다.
- 기본 제작 안내는 모듈 폴더와 zip 업로드 흐름으로 둔다.
...
초보자에게 가장 먼저 보여줄 구조는 업로드 가능한 모듈 폴더다.
...
반복 제작이나 공개 배포가 필요하면 프로젝트 루트와 런타임 모듈 폴더를 분리한다.
...
대안으로 프로젝트 루트가 곧 모듈 루트인 구조도 가능하다.
...
외부 모듈의 `module.php`에는 호환 정보를 명시한다.
...
외부 모듈은 먼저 로컬에서 `check-external-module.php`로 점검한다. GitHub Actions가 필요한 제작자는 `docs/module-ci-template.yml`을 `.github/workflows/check.yml`로 복사해 같은 점검을 push 때 자동 실행할 수 있다.
...
- 외부 제작자는 모듈 폴더와 zip 업로드 흐름만으로도 시작할 수 있다.
- 반복 배포, 팀 개발, 공개 배포, 공식 모듈 관리는 별도 Git 리포지토리를 권장한다.
...
릴리스 패키징 명령의 기본 탐색 위치는 toycore.git과 같은 상위 디렉터리다.
...
다른 위치를 쓰는 릴리스 환경에서는 `TOYCORE_MODULE_REPO_ROOT`로 모듈 리포지토리 상위 디렉터리를 지정한다. 외부 모듈 점검은 이 기본 탐색 위치에 의존하지 않고, 점검에 사용할 Toycore 소스 경로를 명시해 실행한다.
...
공식 모듈이나 반복 배포 모듈로 관리해야 한다고 판단하면, 구현 전에 리포지토리 생성을 요청한다. 개인 사이트용 1회성 모듈은 모듈 폴더와 zip 업로드 흐름으로 먼저 시작해도 된다.

...
| 시스템 | `antispam` | `beta` | `check-antispam-runtime.php`, 산술 challenge runtime fixture, honeypot/min submit marker, provider action/hostname 검증 fixture, 활성 `antispam-providers.php` 계약 로딩 fixture, 회원가입과 비회원 커뮤니티 글/댓글 적용 marker, secret masking 점검 | 로컬/staging 설치 DB에서 회원가입, 비회원 게시글, 비회원 댓글 산술 challenge mutation smoke와 실제 provider staging key 또는 provider mock endpoint 기반 브라우저 수동 smoke |

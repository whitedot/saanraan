...
| 회원 | `member_oauth` | `beta` | 모듈 골격, `sr_member_oauth_accounts`/`sr_member_oauth_states` 스키마, state/nonce/PKCE helper, provider 계약 정규화, 개인정보 export·cleanup 계약 | mock provider callback, 기존 계정 로그인, 계정 연결/해제, OAuth completion 동의 gate HTTP smoke |
...
| 운영 | `policy_documents` | `beta` | 설치 SQL 스키마, 기본 문서 seed, published version 조회 helper, snapshot helper, 관리자 문서/버전 화면, 안내메일 job/delivery 개인정보 export·cleanup 계약, `check-privacy-contract-matrix.php` | 회원가입/커뮤니티/OAuth completion 연계 fixture, 안내메일 batch 발송 runtime fixture, 설치 DB 관리자 화면과 public 동의 렌더 smoke |

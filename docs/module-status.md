...
| 회원 | `coupon` | `beta` | `check-coupon-admin-validation.php`, `check-coupon-redemption-runtime.php`, `check-coupon-claim-campaign-runtime.php`, 유료 열람/다운로드 쿠폰 우선 적용 runtime fixture, 쿠폰 접근권 부분 실패 rollback fixture, 무료 발급 캠페인 dedupe/재고/회원당 제한 fixture, 읽기 참조 계약 점검 | `release-installed-gate-status.php` 자산/쿠폰/유료 접근권 mutation smoke, 설치 DB 쿠폰존 claim mutation smoke |

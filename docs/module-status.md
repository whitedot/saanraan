...
| 회원 | `identity_verification` | `beta` | `sr_identity_verification_attempts`/`sr_identity_verification_results`/`sr_identity_verification_links` 스키마, state/nonce HMAC 저장, provider 계약 로딩, 기본 제공자 필수 관리자 설정 UI, provider 설정 저장/secret 마스킹, return/callback 상태 전이, 계정 purpose 연결 helper, 개인정보 export·cleanup·retention·operational-status 계약 | KCP 테스트 상점과 KG이니시스 계약/샘플 기반 외부 provider smoke, #324 커뮤니티 소비 정책 fixture |

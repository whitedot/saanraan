...
| 회원 | `identity_verification` | `beta` | `sr_identity_verification_attempts`/`sr_identity_verification_results`/`sr_identity_verification_links`/`sr_identity_verification_identity_locks` 스키마, state/nonce HMAC 저장, CI HMAC 기반 중복 계정 연결 방지, provider 계약 로딩, 기본 제공자 필수 관리자 설정 UI, provider 설정 저장/secret 마스킹, return/callback 상태 전이, 팝업 시작/완료 후 원래 화면 갱신과 return token 회수, 가입 입력 자동 채움용 짧은 identity snapshot, 회원가입용 token-bound 결과 조회와 새로고침 초기화, 계정 purpose 연결 helper, 소비 모듈용 본인확인 요구 정책 helper, 세션 단위 결과 helper, 성인 결과 조회 helper, purpose별 우선 provider 선택, 개인정보 export·cleanup·retention·operational-status 계약 | KCP/KG이니시스 외부 provider staging smoke, 회원/커뮤니티/콘텐츠/자산 소비 정책 fixture |
| 회원 | `message` | `beta` | `check-message-policy.php`, `check-privacy-export-runtime.php`, `check-admin-action-security.php`, 쪽지 발신/수신 정책 runtime fixture, 개인정보 export runtime fixture | `release-installed-gate-status.php` 인증 smoke, 쪽지 수발신/삭제/신고/수신 설정 설치 DB mutation smoke |
...
| 회원 | `coupon` | `beta` | `check-coupon-admin-validation.php`, `check-coupon-redemption-runtime.php`, `check-coupon-claim-campaign-runtime.php`, 유료 열람/다운로드 쿠폰 우선 적용 runtime fixture, 쿠폰 사용처 가격 스냅샷 fixture, 쿠폰 접근권 부분 실패 rollback fixture, 무료 발급 캠페인 dedupe/재고/회원당 제한/관리자 수정 fixture, 읽기 참조 계약 점검 | `release-installed-gate-status.php` 자산/쿠폰/유료 접근권 mutation smoke, 설치 DB 쿠폰존 claim mutation smoke |
| 사이트 | `site_menu` | `stable-candidate` | `check-site-menu-seed-order.php`, seed order 점검, 메뉴 렌더 runtime fixture, 헤더 3단계 dropdown marker, 영속 메뉴 tree cache 재사용·payload 검증·무효화 fixture, URL 안전성 fixture | 신규 설치 seed HTTP smoke, 공개/콘텐츠/커뮤니티/퀴즈 헤더 dropdown 수동 smoke |
...
| 사이트 | `popup_layer` | `beta` | `check-popup-layer-targets.php`, `check-ckeditor-assets.php`, 대상 참조 점검, 기간/대상 조건 렌더 runtime fixture, 쿠폰 CTA 렌더 fixture, CKEditor 업로드와 임시 파일 cleanup fixture | 출력 슬롯, 본문 sanitizer, 브라우저 수동 smoke |
...
| 운영 | `notification` | `beta` | `check-mention-ux.php`, `check-notification-runtime.php`, 이벤트 템플릿 runtime fixture, delivery queue fixture, 상태 전이 fixture, claim/lock/backoff/dead-letter runner fixture, Slack/Discord/Telegram 운영 push provider fixture, 회원 Slack/Discord/Telegram push endpoint 암호화 저장/endpoint 참조 queue/발송 전 재검증/연결·해제 재인증/cleanup ciphertext 제거 fixture, `.tools/bin/run-notification-deliveries.php`, 관리자 수동 실행, 웹 종료 runner 설정, 운영 상태 기준 | 설치 DB 또는 staging에서 delivery queue, 수동/CLI runner, 실패 재시도와 dead-letter, 이벤트 템플릿 smoke, Slack/Discord/Telegram mock 또는 staging provider smoke, 회원 Slack/Discord/Telegram 연결/해제와 queued delivery 취소 smoke |
| 운영 | `policy_documents` | `beta` | 설치 SQL 스키마, 기본 문서 seed, 문서 생성 시 첫 version 저장, plain/Markdown/직접 HTML/CKEditor 본문 입력, published version 조회 helper, version key 제거와 운영자 비노출 UI, 사이트 설정 기반 표준 약관/방침 자동 채우기, 이전 version 기록 자동 렌더 옵션, snapshot helper, 관리자 문서/버전 화면, 안내메일 job/delivery 개인정보 export·cleanup 계약, 회원가입/커뮤니티/OAuth completion 연계 marker, `check-privacy-contract-matrix.php`, `check-policy-documents-runtime.php`, `check-member-oauth-runtime.php` | 안내메일 batch 발송 설치 DB smoke, 설치 DB 관리자 화면과 public 동의 렌더 smoke |
...
| 플러그인 | `ckeditor` | `beta` | `check-rich-text-sanitizer.php`, `check-htmlpurifier-runtime.php`, `check-ckeditor-assets.php`, `check-browser-qa.php`, `ckeditor-browser-smoke.spec.js`, `smoke-ckeditor-upload-save.php`, HTML Purifier 배치/캐시 경로, rich text sanitizer fixture, CKEditor 48.3.0 self-hosted asset hash, 브라우저 asset 로딩/fallback smoke와 upload adapter 4개 시나리오, disposable 설치 DB의 업로드·저장 sanitizer·공개/draft 본문 이미지 접근 smoke 실행 기록 | 실제 릴리스 후보에서 `release-installed-gate-status.php --run-ckeditor-upload-save-smoke` CKEditor upload/save browser smoke 재검증, 유료 본문 이미지 접근 browser smoke |
| 플러그인 | `markdown_editor` | `beta` | `check-markdown-editor-runtime.php`, renderer 계약 fixture, full/inline/plain 출력 fixture, style asset URL과 profile hash fixture, 항상 활성인 표·체크 목록·코드 블록 문법 fixture, 기본/커스텀 stylesheet 모드 fixture, 라이브 원문/렌더링·패널 헤더 펼치기·오른쪽 Figma형 속성 인스펙터·fullscreen CSS 모달·렌더링 헤더 색상 아이콘 버튼·사이트 글꼴 상속·공개 본문 색상 우선순위·`.markdown-editor-body` 범위 `--md-*` 선언·이전 `--sr-*` 저장값 변환·관리자/공개 CSS·토큰 값 일치·UI kit 참고 H1-H6 기본 크기·하단 스티키 저장·CSS 동기화 marker | 설치 DB에서 플러그인 활성/비활성 editor option, 관리자 속성 인스펙터/preview, 패널 펼치기/분할 복원, CSS 모달, 기본/커스텀 전환, 직접 CSS 저장·거부 경계, 콘텐츠/커뮤니티/팝업레이어/약관 문서 Markdown 저장·출력, 업데이트 자동 활성화 smoke |
...
| 플러그인 | `identity_kcp` | `beta` | `identity-provider.php` 계약, KCP V2 거래등록/결과조회 endpoint 설정, 테스트 환경 site_cd/ENC_KEY fallback, 내장 KCP V2 암복호화 fallback, provider 원문 비저장 mapping | KCP 테스트 provider 기반 성공/실패/취소 smoke |
...
| 계획 | 상태 | 기준 문서 |
| --- | --- | --- |
| 본인확인 provider 실인증 smoke | `planned` | [본인확인 플러그인 계획](plans/identity-verification-plugin-plan.md) |

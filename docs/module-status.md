...
| 회원 | `identity_verification` | `beta` | `sr_identity_verification_attempts`/`sr_identity_verification_results`/`sr_identity_verification_links`/`sr_identity_verification_identity_locks` 스키마, state/nonce HMAC 저장, CI HMAC 기반 중복 계정 연결 방지, provider 계약 로딩, 기본 제공자 필수 관리자 설정 UI, provider 설정 저장/secret 마스킹, return/callback 상태 전이, 계정 purpose 연결 helper, 소비 모듈용 본인확인 요구 정책 helper, 세션 단위 결과 helper, 성인 결과 조회 helper, purpose별 우선 provider 선택, 개인정보 export·cleanup·retention·operational-status 계약 | KCP/KG이니시스 외부 provider staging smoke, 회원/커뮤니티/콘텐츠/자산 소비 정책 fixture |

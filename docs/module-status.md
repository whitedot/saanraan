...
| 플러그인 | `markdown_editor` | `beta` | `check-markdown-editor-runtime.php`, renderer 계약 fixture, full/inline/plain 출력 fixture, style asset URL과 profile hash fixture, 항상 활성인 표·체크 목록·코드 블록 문법 fixture, 원본/변경 stylesheet 모드 fixture, 라이브 원문/렌더링·패널 헤더 펼치기·아이콘 컨텍스트 툴바·fullscreen CSS 모달·상단 색상 모드·하단 저장 영역·숨은 control template·CSS 동기화 marker | 설치 DB에서 플러그인 활성/비활성 editor option, 관리자 라이브 툴바 설정/preview, 패널 펼치기/분할 복원, CSS 모달, 원본/변경값 전환, 직접 CSS 저장·거부 경계, 콘텐츠/커뮤니티/팝업레이어/약관 문서 Markdown 저장·출력, 업데이트 자동 활성화 smoke |

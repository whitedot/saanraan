...
모듈은 저장소 밖에서 만들 수 있지만, Toycore 안으로 들어온 뒤에는 별도의 외부 모듈 계급이나 소스 관리 생명주기를 갖지 않는다. Toycore는 현재 배치된 모듈 폴더를 읽고 설치, 활성화, 업데이트 상태만 관리한다.
...
개인 사이트나 실험 기능은 Toycore 저장소 밖에서 폴더로 만들 수 있다. 이 경우에도 설치 기준은 같다.

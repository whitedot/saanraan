...
모듈 계약 버전은 외부 저장소나 원격 배포 생명주기를 추적하기 위한 값이 아니다. 현재 `modules/{module_key}`에 놓인 모듈 폴더가 현재 코어의 메타데이터, 계약 파일, 요청 흐름 기준과 호환되는지 확인하는 로컬 호환성 기준이다.

계약 파일은 기능별 진입점이다. 모든 모듈이 모든 계약 파일을 가질 필요는 없지만, 실제로 배치한 계약 파일은 `module.php`의 `contracts.provides`와 일치해야 한다.

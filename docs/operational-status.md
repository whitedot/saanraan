...
| `notification.deliveries.queued` | 이메일과 외부 채널 발송 작업이 대기 또는 처리 중 | 1시간 | `/admin/notification-deliveries` 발송 목록, 외부 발송 설정, 처리기 실행 상태 |
...
자동 보존 정리는 public 요청과 admin 요청 범위를 분리해 마지막 시도와 마지막 성공 시각을 별도로 저장한다. 성공 시각은 bounded 정리가 완료된 뒤에만 갱신하며, 정리 중 예외가 발생하면 마지막 실패 시각과 짧은 실패 메시지를 site setting에 남기고 예외 로그를 이어서 기록한다. 실패 뒤에는 매 사용자 요청에서 즉시 반복하지 않고 최대 5분의 짧은 재시도 간격을 적용하며, 정상 성공 뒤에는 운영자가 설정한 정리 간격을 적용한다. 운영자는 `/admin/retention`의 `자동 정리 실행 상태` 표에서 범위별 마지막 시도, 마지막 성공, 마지막 실패, 실패 메시지를 확인한 뒤 보관 기간 설정, DB 권한, 저장소 권한, 대상 모듈 cleanup 계약을 점검한다.

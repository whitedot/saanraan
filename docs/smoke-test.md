...
스킨·테마 패키지를 배치하거나 공개 레이아웃/스킨 설정을 바꾼 경우 `/admin/packages`에서 외부 테마와 스킨 manifest가 유효/무효 상태로 표시되고, 참조 요약과 레이아웃 fallback health가 오류 없이 표시되는지 확인한다. 사이트 설정의 공개 레이아웃과 콘텐츠/커뮤니티/퀴즈/설문 환경설정의 레이아웃 선택지는 유효한 외부 테마만 저장할 수 있어야 하며, 현재 route 도메인을 지원하지 않는 레이아웃은 공개 렌더링에서 `common.basic`으로 fallback해야 한다. 패키지 asset은 `/sr-package-asset?type=...&key=...&asset=...` 경로로만 응답하고 `/sr-packages/...` 직접 접근은 차단되어야 한다.

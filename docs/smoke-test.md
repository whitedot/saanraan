...
커뮤니티 업데이트 후 기존 `/admin/community/settings` 권한 보유 운영자에게 `/admin/community/asset-policy-sets`의 같은 액션 권한이 승계되는지 확인. 커뮤니티 멤버 그룹별 설정 목록에서 이름, Key, 상태, 수정일 헤더 정렬이 허용 목록 기반으로 동작하는지 확인

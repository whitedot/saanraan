...
커뮤니티 환경설정 저장은 레벨 점수, 쪽지 작성 정책/회원 그룹, 커뮤니티 홈 레이아웃, 게시글 에디터, 복합 자산 차감 선택과 금액을 함께 바꿔 `sr_module_settings` 값이 갱신되는지 확인한다. 저장 실패가 발생하면 화면 검증 메시지 또는 `storage/logs/error.log`에 원인이 남아야 한다

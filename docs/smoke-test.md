...
페이지 자산 기능을 활성화한 환경에서는 유료 열람, 유료 다운로드, 완료 액션이 비로그인 접근을 로그인으로 보내고, 로그인 회원은 잔액/중복 정책에 맞게 처리되는지 확인. 복합 자산 차감 설정은 포인트, 적립금, 예치금 순서로 잔액을 합산 차감하는지 확인
...
커뮤니티 자산 기능을 활성화한 환경에서는 글/댓글 적립, 글/댓글 작성 차감, 게시글 유료 열람, 첨부 다운로드 차감이 비로그인/잔액 부족/중복 처리 정책에 맞게 동작하는지 확인. 복합 자산 차감 설정은 포인트, 적립금, 예치금 순서로 잔액을 합산 차감하는지 확인. 첨부 URL 직접 접근도 게시글 유료 열람 정책을 우회하지 않는지 함께 확인
...
/admin/settings 화면 섹션에서 기본 홈페이지 / 접속, 커뮤니티 홈/공개 페이지 초기화면 선택과 fallback 확인
선택 모듈 관리자 메뉴 노출

...
같은 환경설정에서 댓글 에디터를 textarea, HTML, CKEditor, Markdown 중 활성 선택지로 바꿔 `comment_editor`가 저장되는지 확인한다. 게시판 관리에서 별도 댓글 에디터를 선택하면 댓글·답글 작성과 기존 댓글 출력에 게시판 값이 우선하고, HTML은 허용된 마크업만 남으며 Markdown은 활성 renderer와 출력 stylesheet를 사용하는지 확인한다. 비회원 댓글 폼은 기본 textarea를 유지해야 한다.

...
페이지 그룹을 만든 환경에서는 /pages/group?key=... 공개 목록이 200으로 열리고 비사용/보관 그룹은 공개 접근이 차단되는지 확인
페이지 모듈 설치 환경에서는 공용 배너/팝업레이어 직접 선택과 `page.view` 출력 위치 규칙이 공개 페이지에 반영되는지 확인
페이지 자산 기능을 활성화한 환경에서는 유료 열람, 유료 다운로드, 완료 버튼 자산 처리가 비로그인 접근을 로그인으로 보내고, 로그인 회원은 잔액/중복 정책에 맞게 처리되는지 확인. 완료 버튼 자산 처리는 페이지 조회만으로 실행되지 않아야 한다. 복합 자산 차감 설정의 차감 우선순위: 포인트 > 적립금 > 예치금
...
/admin/settings 화면 섹션에서 기본 홈페이지 / 접속, 커뮤니티 홈/공개 페이지/페이지 그룹 초기화면 선택과 fallback 확인
선택 모듈 관리자 메뉴 노출

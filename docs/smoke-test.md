...
`/admin/community/reports` 신고 목록은 `operation_key=community.report_set_status`, `selected_report_ids[]`, `target_status`를 검증하고 현재 페이지 선택, 전체 선택, 공통 검토 메모 적용, 조건부 상태 전이, 선택 없음, 100건 초과, 존재하지 않는 ID, 동시 상태 변경 충돌, 감사 로그 metadata를 확인한다. 일괄 신고 상태 변경은 대상 게시글/댓글/쪽지 조치까지 실행하지 않아야 하며, 대상 조치는 기존 개별 행 처리로만 적용해야 한다.

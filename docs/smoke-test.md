...
Markdown Editor 플러그인은 `php .tools/bin/check-markdown-editor-runtime.php`로 renderer 계약, full/inline/plain 렌더링, profile hash, style asset URL, `github-markdown-css` 기반 기본 stylesheet, 시각 컨트롤과 실제 CSS property의 `sr-control` 양방향 연결, scoped stylesheet 검증, Markdown 비활성 저장 차단 fixture를 확인한다. 설치 DB가 있는 로컬/staging에서는 플러그인 비활성 상태에서 새 Markdown 선택지가 숨겨지고 새 Markdown 저장이 거부되는지, 플러그인 활성 상태에서 콘텐츠/커뮤니티/팝업레이어/약관 문서의 Markdown 저장과 공개 출력이 같은 renderer stylesheet를 로드하는지 확인한다. `/admin/markdown-editor/settings`에서는 별도 편집/분할/미리보기 버튼 묶음이 없어야 한다. 원문 패널 헤더의 펼치기는 편집만 표시하고 렌더링 패널 헤더의 펼치기는 미리보기만 표시하며, 단독 보기에서 같은 버튼을 누르면 분할 보기로 돌아가고 icon/aria-label/title도 상태에 맞게 바뀌어야 한다. 렌더링된 제목·문단·링크·목록·인용·코드·표·구분선을 클릭할 때 상단 컨텍스트 툴바가 같은 대상으로 바뀌고, 속성 그룹이 텍스트가 아닌 아이콘 버튼과 aria-label/title로 표시되는지 확인한다. 속성 아이콘을 누르면 입력값이 렌더링 결과 위의 popover 서브메뉴로 열리고 미리보기 내용의 시작 위치와 높이가 변하지 않아야 한다. 같은 아이콘 재클릭, 바깥 클릭, Escape로 닫히고 Escape에서는 focus가 원래 아이콘으로 돌아가야 한다. 오른쪽 고정 검사기는 표시하지 않고 편집기는 전체 폭을 사용해야 한다. light/dark는 편집기 상단에서 바로 조정하고 별도 배율 control은 표시하지 않아야 한다. CSS 아이콘은 원본/변경값과 CSS 원문만 담은 전체화면 모달을 열어야 하며 전체 초기화와 닫기가 동작해야 한다. 표·체크 목록·코드 블록 문법은 선택 옵션 없이 항상 렌더링되어야 한다. 설정 저장 버튼과 참고 프로젝트 표기는 모달 밖 편집기 하단에 표시되어야 한다. 툴바 값을 변경하면 숨은 form control과 CSS 원문이 동기화되고 짧은 debounce 뒤 실제 렌더링 결과가 바뀌어야 한다. CSS 원문 직접 편집은 모달 안에서 반영 중 상태를 표시하고, 서버가 scoped CSS를 검증한 뒤 반환한 CSS를 미리보기 style에 적용해 완료 상태를 표시해야 하며 오류 CSS는 적용하지 않아야 한다. 모든 대상의 `Margin`, `Padding`, `Border`와 텍스트 대상의 `Text`를 확인하고, 재시작 아이콘은 현재 요소만 초기화해야 한다. `참조 원본`을 저장하면 사용자 CSS를 보존하면서 번들 원본이 공개 출력되고, 범위를 벗어난 selector, 외부 URL, at-rule, `</style` 종료 문자열은 거부되어야 한다.

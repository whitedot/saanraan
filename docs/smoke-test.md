...
CKEditor 플러그인을 활성화한 환경에서는 콘텐츠 본문, 커뮤니티 게시글, 관리자 본문 textarea가 설정에 따라 에디터로 강화되고, 초기화 성공 시에만 `body_format=html`로 저장되는지 확인. 직접 호스팅 모드는 `modules/ckeditor/vendor/ckeditor5/ckeditor5.umd.js`와 `ckeditor5.css`를 로드해야 한다. CKEditor asset 로딩을 실패시킨 경우에는 일반 textarea 제출이 유지되고 서버가 `plain` 저장으로 fallback해야 한다. 악성 HTML은 저장/출력 과정에서 허용 태그와 속성만 남아야 한다. 콘텐츠 본문 이미지 업로드는 관리자 권한, CSRF, 업로드 token을 요구하고, 저장 전 temporary 이미지는 업로드한 관리자에게 보이며, 저장 후 정화된 HTML에 남은 프록시 URL만 콘텐츠별 로컬 경로로 이동되어야 한다. 유료/비공개 콘텐츠의 본문 이미지 프록시는 콘텐츠 접근 정책을 우회하지 않아야 하며, 본문에서 제거되거나 콘텐츠가 영구 삭제된 이미지는 콘텐츠 모듈의 저장 경로 기준으로 정리되어야 한다.

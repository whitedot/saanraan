...
콘텐츠별 레이아웃을 `선택 안 함`으로 저장하면 공통 header/footer와 부가 영역 없이 제목·본문만 출력되고, 제목 숨김을 선택하면 화면의 제목만 사라지는지 확인한다. 레이아웃 사용 여부와 모든 콘텐츠 상세 theme에서 CKEditor로 입력한 제목·목록·표·인용·코드·이미지 정렬과 캡션은 같은 `.sr-ckeditor[data-sr-editor-output] .ck-content`, 번들 CKEditor stylesheet, 프로젝트 editor theme로 유지되어야 한다. 공개 CKEditor 본문 배경은 투명하고 `.ck-editor__main`, `.ck-editor__editable_inline`, `ck-read-only`, `aria-readonly`는 없어야 한다. Markdown renderer 본문도 전용 stylesheet가 유지되어야 한다. 유료 열람 콘텐츠에서는 일반 콘텐츠 화면과 같은 overlay modal과 선택된 콘텐츠 테마 스타일이 표시되고 차감, 환전 확인, 쿠폰 적용 POST가 일반 레이아웃과 같은 결과를 내는지 라이트·다크 모드에서 확인한다. 일반/무레이아웃 상세 모두에서 콘텐츠 수정 권한 관리자는 관리자 수정 화면, 회원 제출 콘텐츠 작성자는 본인 제출본 수정 화면으로 연결되고 다른 회원에게는 수정 링크가 없어야 한다. 승인된 제출본 수정은 기존 공개본을 유지한 채 수정안 상태로 저장되며 검수 중에는 공개 화면에 동시 수정 링크가 없어야 한다.

CKEditor 공개 본문에 좌우 padding이 없고 font-size·line-height와 첫·마지막 블록의 세로 여백은 입력 영역의 원본 token과 같은지 확인한다. 이를 위해 테두리나 최소 높이가 생기지 않는지도 함께 확인한다.
콘텐츠 그룹을 만든 환경에서는 /content/group?key=... 공개 목록이 200으로 열리고 비사용/보관 그룹은 공개 접근이 차단되는지 확인
콘텐츠 사이드바를 켠 환경에서는 콘텐츠 메인에는 사이드바가 없고 그룹·검색 목록, 상세 읽기, 회원 콘텐츠 작성/수정 화면에만 표시되는지 확인한다. 콘텐츠 그룹/사이트 메뉴/표시 안 함 선택, 인기 콘텐츠·최신 댓글 1~10건 설정이 저장값과 일치하고 사이트 메뉴 선택 시 값 누락이 서버에서 거부되는지 확인한다. 인기 콘텐츠와 최신 댓글은 비공개 콘텐츠, 비밀 댓글, 현재 상세 콘텐츠를 노출하지 않아야 하며 댓글 요약에는 HTML 태그가 문자로도 남지 않아야 한다. 라이트·다크 모드에서 본문, 보조 글자, 링크, 경계선이 테마 토큰을 따르고 좁은 화면에서는 사이드바가 본문 아래 한 열로 이동하는지 확인한다.
...
썸네일 파일 캐시와 각 모듈의 URL 임베드 캐시가 관리자 페이지당 표시 수보다 많으면 다음 페이지와 마지막 부분 페이지까지 이동할 수 있어야 한다. 필터의 전체 파일 수·용량·정리 대상 집계는 현재 페이지 행 수가 아니라 모든 일치 파일을 기준으로 유지되어야 한다.

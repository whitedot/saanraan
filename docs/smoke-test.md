...
/admin/banners 목록에서 현재 페이지 배너 선택 체크박스, 전체 선택, 선택 수 표시, 상태 일괄 변경이 동작하는지 확인한다. 서버는 `intent=batch_status`, `operation_key=banner.set_status`, `selected_banner_ids[]`, `target_status`를 다시 검증해야 하며, 다른 모듈에서 참조 중인 enabled 배너는 일괄 비활성화가 차단되어야 한다.

...
활성/설치 모듈 key 목록과 계약 파일 경로 목록은 같은 요청에서 라우팅, 레이아웃, 선택 모듈 연동이 반복 참조하므로 PDO 연결과 registry 변경 token을 cache key로 하는 요청 단위 메모리 캐시를 사용한다. 모듈 설치, 활성화, 비활성화, 설치 실패 상태 전환, 파일 전용 버전 동기화는 registry cache를 즉시 비워 같은 요청의 후속 판정이 이전 상태를 사용하지 않게 한다. 계약 파일의 반환값과 renderer 결과까지 이 cache에 넣지는 않는다.

...
1. 관리자 목록은 기본적으로 페이지네이션을 사용한다. 단, `/admin/modules`처럼 DB 성장형 업무 데이터가 아니라 배포된 모듈/플러그인 파일과 설치 행을 조율하는 bounded 관리 화면은 전체 표시를 허용하고 스티키 섹션 탭 같은 빠른 이동 수단을 둔다.

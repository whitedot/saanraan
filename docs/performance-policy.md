...
세 캐시는 모두 캐시 hit에서 사용자 요청 부담을 줄인다. cache miss, 갱신 필요, 코드 정책상 재계산 대상 상태에서는 공유호스팅 제약상 같은 사용자 요청 안에서 재계산이나 파일 생성이 일어날 수 있으므로, 느린 화면을 판단할 때는 hit 경로와 miss 경로를 나누어 측정한다.

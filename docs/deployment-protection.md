...
`ZipArchive`가 없으면 관리자 모듈 화면에서 zip 업로드를 처리할 수 없다. 이 경우 FTP나 호스팅 파일 관리자로 모듈 파일을 배치한 뒤 관리자 화면에서 설치와 DB 업데이트를 진행한다. `ZipArchive`가 있으면 소유자가 `/admin/modules`에서 모듈 파일 반영을 일시 허용한 뒤 zip 업로드 버튼을 사용할 수 있다. zip 업로드 요청은 소유자 비밀번호 재확인을 통과하고 모듈 소스 반영이 일시 허용된 경우에만 처리하며, 업로드가 끝나면 자동으로 다시 비활성화한다.

...
- `.tools/bin/smoke-http.php`는 동적 진입점 응답에서 `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, 기본 `Content-Security-Policy`와 YouTube URL 임베드용 `frame-src`가 실제로 전송되는지 확인한다.

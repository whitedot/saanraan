...
- `.tools/bin/check-content-layout-selection.php`는 콘텐츠별 레이아웃 선택이 공개 상세 화면에 적용되고, 명시적인 `content.none`만 무레이아웃으로 처리되며, 무레이아웃 제목 표시값이 콘텐츠·리비전·복사 경로에 보존되는 계약을 확인한다. 같은 검사는 시리즈 선택과 회차 입력이 독립 행으로 유지되고 무레이아웃 renderer가 공통 layout shell이나 시리즈 영역을 호출하지 않는지도 고정한다. 일반·basic·sample·무레이아웃 상세는 모두 `sr_content_public_body_html()`과 같은 CKEditor 원본·프로젝트 stylesheet를 사용한다. 공개 wrapper는 `data-sr-editor-output`으로 배경을 투명하게 유지하고 `.ck-editor__main`, `.ck-editor__editable_inline`, `ck-read-only`, `aria-readonly` 같은 입력 widget 구조와 축약 public reset은 호출하지 않는다. Markdown 본문은 renderer stylesheet 계약을 유지한다. 유료 열람 확인에서는 기존 콘텐츠 modal을 재사용하고 선택된 콘텐츠 테마의 common/module stylesheet와 overlay script를 직접 로드하며, 일반 modal과 동일한 차감 확인·환전 확인·요청 token·쿠폰 선택 필드를 제출하는지도 확인한다. 또한 관리자/회원 작성자의 수정 URL 분기, 모든 상세 테마와 무레이아웃의 공통 수정 링크, 승인·초안·수정 요청·반려 제출본만 작성자 수정 대상으로 찾고 검수 중 제출본은 제외하는 계약을 확인한다.

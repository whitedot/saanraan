...
- `.tools/bin/check-community-attachment-runtime.php`는 커뮤니티 첨부 다운로드 자산 로그 placeholder의 dedupe, 0원 완료 로그 settlement metadata, 첨부 다운로드 접근권 중복 방지, 유료 첨부 다운로드 수동 환불의 포인트 복원/접근권 회수/payment_ledger item reverse, anonymized entitlement 무시를 SQLite fixture로 확인한다.

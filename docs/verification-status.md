...
- 공통 `sr_ledger_create_transaction()` helper의 table pair allowlist는 현재 `deposit`에 한정한다. `point`와 `reward`는 만료/소진 필드인 `expires_at`, `expires_remaining`을 함께 다루므로 자체 insert helper 경로를 유지하되, reconciliation 대상에는 계속 포함한다.
...
- `.tools/bin/check-privacy-contract-matrix.php`는 매트릭스의 상태와 `module.php` 계약 선언, 실제 `privacy-export.php`/`privacy-cleanup.php` 파일 존재, 계약 파일 반환 형태와 callable signature, `install.sql`의 계정 연결 컬럼과 식별자성 컬럼 문서화, ROPA 처리활동 씨앗의 31개 번들 모듈 row와 필수 컬럼 marker, `operational_retained` 모듈의 보존 사유/운영자 접근 범위/1.0 전 검토 항목, `export_retained`/field-level cleanup 모듈의 보존 사유/고위험 필드/1.0 전 검토 항목을 대조한다.
...
- `.tools/bin/check-privacy-export-runtime.php`는 SQLite fixture로 `quiz`, `survey`, `content`, `community` export 계약과 `asset_exchange`, `coupon`, `deposit`, `notification`, `payment_ledger`, `point`, `reward` 보존형 export 계약을 실행한다. 대상 계정의 상세 답변, snapshot, 접근권, 자산 로그, 다운로드 로그, 작가 신청, 시리즈, 댓글/게시글, 신고/쪽지/스크랩/동의 증적이 포함되고 다른 계정 row가 제외되는지 확인한다. 보존형 fixture는 결제 record/item, 금액성 원장, 쿠폰 환불, 유료 쿠폰 발급본의 claim snapshot/asset reference, 환불/출금 계좌, 알림 delivery, 포인트/적립금 만료 소비 매핑 같은 운영 증빙 필드가 대상 계정 기준으로 포함되는지 확인한다.
- `.tools/bin/check-privacy-cleanup-runtime.php`는 SQLite fixture로 `quiz`, `survey`, `content`, `community`, `notification`, `payment_ledger`, `policy_documents`, `reward` cleanup 계약을 실행한다. 대상 계정의 응시/응답, 보상 grant, 접근권, 다운로드 로그, 작가 신청, 닉네임, 레벨, 동의 증적, 시리즈 메타데이터, 댓글/게시글 작성자 snapshot이 익명화되고 다른 계정 row가 유지되는지 확인한다. notification push endpoint ciphertext 제거, payment_ledger 결제 record 계정 연결 제거와 item account 참조 익명화, policy_documents 안내메일 delivery 계정 연결 제거, reward 출금 은행정보와 요청자/관리자 note 제거도 함께 확인한다.

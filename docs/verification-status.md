...
- `.tools/bin/check-module-foundation-lifecycle.php`는 SQLite fixture로 `asset_ledger`와 `payment_ledger` 기반 모듈 비활성화가 활성 의존 모듈이 있는 동안 차단되는지 확인한다. `asset_ledger`는 자산 모듈과 커뮤니티 회수 callback 의존성을 기준으로 검사하고, `payment_ledger`는 실제 결제 record를 생성하는 `content`, `community`의 활성 상태를 기준으로 검사한다. 또한 `module.php`의 `requires.modules` 기반 모듈 선언과 `sr_module_foundation_dependencies()`의 관리자 자동 준비/수명주기 기준이 서로 빠지지 않는지도 확인한다.

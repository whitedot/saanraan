...
- `.tools/bin/check-member-oauth-runtime.php`는 OAuth state/nonce/PKCE 원문 비저장, state 1회 사용, link state account binding, provider subject HMAC, 연결/해제 helper, 마지막 로그인 수단 해제 차단, callback과 completion의 policy document 동의 gate marker를 SQLite fixture와 정적 marker로 확인한다. 실제 외부 provider callback과 브라우저 완료 흐름은 설치 DB가 있는 로컬 또는 staging smoke에서 별도 기록한다.
- `.tools/bin/check-policy-documents-runtime.php`는 새 published version 게시 시 기존 published version이 보관 처리되는지, version 단위 안내메일 job이 idempotent한지, 안내메일 batch가 정지/탈퇴 등 비활성 계정 delivery를 `skipped`로 닫아 job이 영구 queued로 남지 않는지 SQLite fixture로 확인한다.

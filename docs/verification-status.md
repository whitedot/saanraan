...
- `.tools/bin/check-site-menu-seed-order.php`는 사이트 메뉴 seed order, 공개 렌더링, 현재 항목/상위 항목 표시, 섹션 루트 메뉴의 하위 경로 활성화, 3단계 제한, 외부 링크 보호, URL fail-closed를 확인한다. 하위 메뉴가 있는 항목의 `sr-site-menu-item-has-children`, `aria-haspopup`, `aria-expanded`, `aria-controls`와 레이아웃 소유 dropdown CSS/공통 JS marker도 확인한다. 동일 `menu_key`의 enabled item tree와 메뉴명이 요청 메모리 초기화 뒤에도 파일 캐시에서 재사용되고, `sr_site_menu_clear_cache()` 호출 후에는 다음 렌더링이 DB tree를 다시 조회하는지도 SQLite fixture로 확인한다.

...
- 공통 `sr_ledger_create_transaction()` helper의 table pair allowlist는 현재 `deposit`과 `reward`를 허용한다. `reward`는 `expires_at`, `expires_remaining`, `expired_at`을 allowlisted extra transaction column으로 넘기고, 만료/소진/회수 정책은 여전히 reward 모듈 helper가 소유한다. `point`는 환불 분할 경로가 남아 있어 자체 insert helper 경로를 유지하되 reconciliation 대상에는 계속 포함한다.
...
- `.tools/bin/check-privacy-export-runtime.php`는 SQLite fixture로 `message`, `quiz`, `survey`, `content`, `community` export 계약과 `asset_exchange`, `coupon`, `deposit`, `notification`, `payment_ledger`, `point`, `reward` 보존형 export 계약을 실행한다. 대상 계정의 상세 답변, snapshot, 접근권, 자산 로그, 다운로드 로그, 작가 신청, 시리즈, 쪽지, 댓글/게시글, 신고/스크랩/동의 증적이 포함되고 다른 계정 row가 제외되는지 확인한다. 보존형 fixture는 결제 record/item, 금액성 원장, 쿠폰 환불, 유료 쿠폰 발급본의 claim snapshot/asset reference, 환불/출금 계좌, 알림 delivery, 포인트/적립금 만료 소비 매핑 같은 운영 증빙 필드가 대상 계정 기준으로 포함되는지 확인한다.

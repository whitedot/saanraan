...
- `.tools/bin/check-community-account-guards.php`는 커뮤니티 계정 guard foundation의 모듈 소유 스키마, 기본 비활성 설정, 관리자 설정 저장/UI marker, 설치/업데이트 버전 marker, active guard unique index, helper 로더, privacy export/cleanup marker를 확인한다. SQLite fixture는 만료된 guard가 현재 active guard 조회에서 제외되는지, terminal 전이가 `active_guard_uid`를 비우고 reviewer/snapshot metadata를 남기는지 확인한다.

...
- `.tools/bin/check-output-helpers.php`는 공개 레이아웃 후보, 로고 위치 후보, output slot renderer metadata가 같은 요청 안에서 반복 contract 조회를 줄이는지 SQLite counting fixture로 확인한다. output slot cache는 HTML, callable, resource가 아니라 `module_key`와 `contract_file` metadata만 저장해야 한다.
...
- 같은 점검은 `slack_webhook` 외부 푸시 채널을 허용 채널과 회원 알림 생성 채널에서 분리하고, 관리자 운영 알림의 Slack delivery queue, 공통 claim/process dispatch, 실패 정책, Slack webhook 응답 판정, secret masking marker를 확인한다. 실제 Slack provider 호출과 브라우저 widget/설정 저장 smoke는 로컬 또는 staging에서 별도 기록한다.
...
- `.tools/bin/check-antispam-runtime.php`는 자동등록방지 모듈의 산술 challenge, honeypot, 최소 제출 시간, provider failure policy, 활성 `antispam-providers.php` 계약 로딩, `antispam_captcha_providers` 플러그인의 Turnstile/hCaptcha/reCAPTCHA 응답 fixture, reCAPTCHA 최소 점수, secret masking, 회원가입과 비회원 커뮤니티 글/댓글 적용 marker를 확인한다. 실제 provider key와 브라우저 widget 동작은 설치 DB가 있는 로컬 또는 staging smoke에서 별도 기록해야 하며, 운영 DB에서 mutation smoke를 실행하지 않는다.

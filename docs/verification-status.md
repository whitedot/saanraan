...
- `.tools/bin/check-privacy-contract-matrix.php`는 매트릭스의 상태와 `module.php` 계약 선언, 실제 `privacy-export.php`/`privacy-cleanup.php` 파일 존재, 계약 파일 반환 형태와 callable signature, `install.sql`의 계정 연결 컬럼과 식별자성 컬럼 문서화, ROPA 처리활동 씨앗의 30개 번들 모듈 row와 필수 컬럼 marker, `operational_retained` 모듈의 보존 사유/운영자 접근 범위/1.0 전 검토 항목, `export_retained`/field-level cleanup 모듈의 보존 사유/고위험 필드/1.0 전 검토 항목을 대조한다.

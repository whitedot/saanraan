...
- `.tools/bin/check-community-feed-cache-contract.php`는 #369 v1 feed cache 계약을 확인한다. everyone-discoverable 공개 게시판 baseline, viewer class 없는 context hash, fetch/display count 분리, `sr_community_feed_cache` DB 영속 저장소 schema, cache miss 시 사용자 홈 feed가 latest/popular snapshot row를 생성하는지, cache hit 시 snapshot의 post id로 현재 게시글 row를 hydrate하는지 fixture로 확인한다. `author_account_id` 기반 card snapshot, `published_comment_count` 보류, thumbnail source marker, author label/본문/CSRF/계정별 권한/유료 접근권/thumbnail URL 금지 필드가 snapshot에 들어가지 않는지도 확인한다. `/admin/community/feed-cache` 점검 화면의 route/menu/permission/view marker와 운영 화면이 같은 context helper를 쓰는지도 확인한다.
- `.tools/bin/measure-community-home-feed.php`는 커뮤니티 홈 최신글/인기글 통합 쿼리를 SQLite fixture, `SR_COMMUNITY_FEED_MEASURE_DSN` 명시 DB, 또는 `SR_COMMUNITY_FEED_MEASURE_CONFIG=1` config DB에서 실행해 #361 형식의 EXPLAIN, cold/warm 응답 시간, 반환 row 수를 출력하는 측정 하니스다. 실제 홈 feed와 같은 query helper를 사용한다. `SR_COMMUNITY_FEED_MEASURE_FIXTURE=1`은 임시 MySQL/MariaDB DB에 fixture table을 drop/create해 문법과 실행계획을 smoke 확인한다. `SR_COMMUNITY_FEED_MEASURE_COMPARE=1`은 인덱스/쿼리 후보 비교를 출력하고, candidate index 생성/삭제는 로컬/staging disposable DB에서 `SR_COMMUNITY_FEED_MEASURE_COMPARE_MUTATION=1`을 명시할 때만 수행한다. SQLite/MariaDB fixture 결과는 target MySQL/MariaDB 측정을 대체하지 않으며, 영속 feed cache table 착수 근거로 쓰려면 대표 로컬 또는 staging DB의 명시 DSN 또는 config DB 기록을 남긴다.

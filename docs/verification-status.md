...
- `.tools/bin/check-community-board-settings.php`는 게시판 그룹/게시판 운영 설정 key, 커뮤니티 환경설정의 게시글 본문 길이 기본값, 관리자 저장 action, 공개 목록 페이지당 글 수/기본 정렬/본문 요약 적용, 게시판 목록의 제목·내용·작성자 검색조건, 게시글·댓글 본문 길이 검증, 댓글 수 기반 게시글 수정·삭제 잠금 marker, 커뮤니티 전체검색 helper 계약을 확인한다. 런타임 fixture는 정렬 key normalization, HTML 본문 plain length 계산, 본문 요약 자르기 helper, 게시판 목록의 제목/본문/공개 작성자명 검색 범위, 전체검색의 제목/본문 검색 범위 및 비밀글 본문 제외가 기대값을 유지하는지 확인한다.

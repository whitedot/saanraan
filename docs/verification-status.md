...
- `.tools/bin/measure-community-home-feed.php`는 커뮤니티 홈 최신글/인기글 통합 쿼리를 SQLite fixture 또는 `SR_COMMUNITY_FEED_MEASURE_DSN` 명시 DB에서 실행해 #361 형식의 EXPLAIN, cold/warm 응답 시간, 반환 row 수를 출력하는 측정 하니스다. 실제 홈 feed와 같은 query helper를 사용한다. `SR_COMMUNITY_FEED_MEASURE_FIXTURE=1`은 임시 MySQL/MariaDB DB에 fixture table을 drop/create해 문법과 실행계획을 smoke 확인한다. SQLite/MariaDB fixture 결과는 target MySQL/MariaDB 측정을 대체하지 않으며, 영속 feed cache table 착수 근거로 쓰려면 대표 로컬 또는 staging DB의 명시 DSN 모드 기록을 남긴다.

...
- `.tools/bin/check-community-report-auto-actions.php`는 신고 임계치 자동 임시 조치의 모듈 소유 스키마, 기본 비활성 설정, 설치/업데이트 버전 marker, 활성 대상 unique index, 상태 helper, 신고 저장 후 평가 호출, 관리자 환경설정 저장/화면 marker를 확인한다. SQLite fixture는 terminal 상태 전이가 `active_target_uid`를 비우고 해제/검토 metadata를 남기는지, 임계치 도달 시 게시글과 첨부가 자동 숨김 처리되고 같은 대상의 중복 활성 조치가 막히는지 확인한다.

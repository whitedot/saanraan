...
- `.tools/bin/check-asset-deadlock-retry.php`, `.tools/bin/check-paid-download-delivery.php`, `.tools/bin/check-asset-exchange-logs.php`, `.tools/bin/check-asset-exchange-runtime.php`, `.tools/bin/check-payment-ledger-runtime.php`, `.tools/bin/check-coupon-admin-validation.php`, `.tools/bin/check-admin-form-validation.php`, `.tools/bin/check-admin-ux-writing.php`, `.tools/bin/check-skin-theme-ui.php`, `.tools/bin/check-admin-asset-limits.php`, `.tools/bin/check-reward-abuse-standards.php`, `.tools/bin/check-asset-settlement-contract.php` 같은 정적/정합성 점검은 `php .tools/bin/check.php` 통합 게이트에 포함된다. `check-payment-ledger-runtime.php`는 `sr_payment_records`와 `sr_payment_record_items`의 dedupe 흡수, record/item 연결, 취소 상태 전이를 SQLite fixture로 확인한다. `check-admin-form-validation.php`는 공통 관리자 opt-in 폼 검증 helper와 사이트 메뉴, 쿠폰, 배너 대표 폼의 `data-sr-validate-form`/`data-validation-message`/조건부 필수 marker를 확인한다. `check-coupon-admin-validation.php`는 유료 발급 캠페인 폼의 가격/통화/허용 자산 조건부 필수 UI와 client validation hook, 환급 가능 target-specific 쿠폰 저장 시 `revoke_access` capability를 요구하는 서버 검증도 확인한다. `check-admin-ux-writing.php`는 관리자 안내 문구 기준 문서화, `form-help` 표준, 모듈 간 비유 금지 문구, 관리자 토스트의 UI kit alert class 연결을 확인한다. `check-skin-theme-ui.php`는 관리자/공개 skin과 theme 설정 저장, view 파일 선언, render action 참조가 현재 코드 배치와 맞는지 확인하고, 배포판에 `basic`만 포함한다는 계약에 따라 제거된 sample theme과 퀴즈 alias skin 경로가 다시 생기지 않게 막는다. `check-reward-abuse-standards.php`는 적립금 회수/출금 신청 가능액뿐 아니라 적립금 만료 잔여량 소비, 소비 매핑, 기한 경과 지급분 차감, 중복 만료 방지도 SQLite fixture로 확인한다. settlement 계약 점검은 정확 충당/ceil overpay 금지/통화 불일치 실패 fixture와 함께 콘텐츠/커뮤니티 자산 입력이 사용자 입력 순서와 무관하게 결정적 차감 순서로 정규화되는지도 확인한다. 유료 다운로드 전달 점검은 파일 또는 signed URL 준비 실패 시 차감이 일어나지 않고, 차감 실패 시 전달하지 않으며, 성공 시 준비-차감-전달 순서를 유지하는 fixture를 포함한다. 환전 로그 점검은 완료 환전 묶음 정정 helper를 SQLite fixture로 실행해 수수료가 있는 정정의 원장 반전, 정정 로그, 중복 정정 차단을 확인하고, 관리자 로그 화면의 CSRF/edit 권한 기반 정정 action과 감사 로그 marker를 확인한다. 환전 runtime 점검은 실제 reward/deposit 자산 계약을 통해 성공 실행의 양쪽 원장/수수료/로그 연결, 입금 원장 저장 실패 시 출금 원장/로그가 남지 않는 rollback, 실패 로그가 잔액과 원장을 바꾸지 않고 요청 금액/실패 사유/처리자 증거를 남기는지 확인한다.
- `.tools/bin/check-install-update-schema-parity.php`는 `database/core/install.sql`, `modules/*/install.sql`, core/module update SQL의 table/column/index DDL target을 정적으로 대조해 신규 설치 스키마와 누적 update 최종 의도가 발산하지 않는지 확인한다. 설치 DB 권한이 있는 로컬/staging에서는 `--live-install --prefix=check_`로 현재 DB에 `check_` prefix 신규 설치 SQL을 실제 실행하고 종료 시 해당 prefix allowlist 테이블을 정리해, SQL 문법과 MySQL 실행 가능성도 확인할 수 있다.
- `.tools/bin/check-i18n-contract.php`는 정적 문자열 `sr_t('...')` 호출이 fallback locale `ko` 번역 파일에 선언되어 있는지, key가 기존 `module::dot.hierarchy` 또는 core dot hierarchy 규약을 따르는지, 요청 locale miss가 `sr_translation_fallback_events()`에 기록되는지 확인한다. 이 검사는 #374-a 문자열 선언 gate이며 기존 하드코딩 문구 전수 이관이나 두 번째 locale 내용 검증은 #374-b 범위로 남긴다.
- `.tools/bin/check-module-foundation-lifecycle.php`는 SQLite fixture로 `asset_ledger`와 `payment_ledger` 기반 모듈 비활성화가 활성 의존 모듈이 있는 동안 차단되는지 확인한다. `asset_ledger`는 자산 모듈과 커뮤니티 회수 callback 의존성을 기준으로 검사하고, `payment_ledger`는 실제 결제 record를 생성하는 `content`, `community`의 활성 상태를 기준으로 검사한다. 또한 일반 `requires.modules` 역방향 의존성도 검사해, provider/plugin처럼 활성 하위 모듈이 요구하는 부모 모듈은 먼저 하위 모듈을 비활성화하기 전까지 비활성화할 수 없는지 확인한다. `module.php`의 `requires.modules` 기반 모듈 선언과 `sr_module_foundation_dependencies()`의 관리자 자동 준비/수명주기 기준이 서로 빠지지 않는지도 확인한다.
...
- `.tools/bin/check-coupon-redemption-runtime.php`는 SQLite fixture로 쿠폰 사용 dedupe, max uses 상태 전이, 환불 시 dedupe key 격리, issue 재활성화, content/community 접근권 회수를 확인한다. 콘텐츠 유료 열람, 콘텐츠 유료 다운로드, 커뮤니티 유료 게시글 열람과 첨부 다운로드 경로는 선택된 `access` 쿠폰을 자산 차감보다 먼저 적용하고, 쿠폰 접근권 부여 시 자산 거래와 자산 접근 로그를 만들지 않는지 확인한다. 또한 `fixed_discount`와 `percent_discount`가 일부 금액만 덮을 때 쿠폰 사용 row와 남은 자산 차감이 같은 작업 단위로 확정되고, redemption snapshot에 할인액과 남은 결제액이 저장되는지도 확인한다. 같은 fixture는 전액 쿠폰과 쿠폰+자산 복합 결제에서 `payment_ledger` record가 결제 전 금액과 실제 settlement 금액을 분리해 저장하고, coupon redemption, asset transaction, 도메인 asset/access log, access entitlement item을 같은 결제 묶음으로 연결하는지도 확인한다. #391 콘텐츠/커뮤니티 열람 payment-unit fixture는 전액 쿠폰, 자산-only, 일부 쿠폰+자산 결제가 각각 `sr_content_view_payment_logs`와 `sr_community_post_read_payment_logs`에 1개 row로 기록되고 기존 접근권 재사용은 새 row를 만들지 않는지 확인한다. 같은 fixture는 콘텐츠/커뮤니티 열람 payment-unit 환불이 전액 쿠폰, 자산-only, 일부 쿠폰+자산에서 쿠폰 state-only 환불, 자산 refund, source-scope 접근권 회수, payment ledger item reversal, 결제 record refunded 전이를 한 번에 처리하고, 일부 쿠폰+자산 redemption의 쿠폰 단독 환불이 fail-closed되는지도 확인한다. `.tools/bin/check-payment-ledger-runtime.php`는 payment record dedupe 불변성, 중복 재호출의 item 묶음 불일치 거부, 부분/전체 환불의 item `reversed`와 record `refunded` 전이, 쿠폰 접근권 환불이 자산 item을 닫지 않는 부분 환불 경계, unknown subject contract 거부, 결제 대상 계약의 `subject_module` 제공자 소유 경계, 취소 재호출 시 reversed item 보존, 익명화된 record의 late replay 흡수와 계정 재연결 방지를 확인한다. 대상별 사용 가능 쿠폰 목록은 같은 target에 매칭되는 issue만 반환하고, 사용자가 선택한 `coupon_issue_id`가 다른 target이면 사용을 거부하는지도 같은 fixture에서 확인한다. 다운로드 파일용 `content_file`과 커뮤니티 첨부용 `community_attachment` 쿠폰 target contract의 검색, 상태 점검, 관리자 URL과 쿠폰 기반 유료 다운로드 접근권 회수도 같은 fixture에서 확인한다. content_file, community_post, community_attachment 쿠폰 사용 row에는 pricing resolver가 반환한 금액, 통화, 정책 요약, 가격 시각, whitelisted target snapshot이 저장되는지 확인한다. 이미 접근권이 있는 content_file/community_post에 다른 dedupe로 쿠폰 사용을 시도하면 `already_entitled` no-op으로 끝나고 쿠폰 사용 row와 `used_count`가 남지 않는지도 확인한다. 접근권 insert 실패를 trigger로 주입해 콘텐츠/커뮤니티 쿠폰 사용 row와 `used_count`가 함께 rollback되는 부분 실패 fixture와, 접근권 회수 실패 시 환불 상태 전이가 rollback되어 기존 `redeemed` row와 지급 건 사용 횟수가 유지되는 fixture도 포함한다. 환불 회수는 redemption의 `target_type` 계약으로만 실행되어, 같은 dedupe key가 다른 소비 모듈에 남아 있어도 해당 target 밖 접근권을 지우지 않는지도 확인한다. 환급 가능 사용 내역의 target에 `revoke_access_function` 계약이 없거나 계약이 0건을 회수하면 환불 전이가 rollback되는 fail-closed fixture도 포함한다. #391 state-only 쿠폰 환불 primitive fixture는 redemption/issue 상태만 전이하고 접근권 회수와 알림 발송은 외부 오케스트레이터에 남기는지 확인한다. `fixed_discount`와 `percent_discount` 정의 저장, 정액 할인 통화 생략 시 KRW 기본값, 할인값 보존, 환급 가능 할인 쿠폰 저장 거부, 아직 구현하지 않은 `stored_value` 모델의 저장 거부도 확인한다. 사용기간 정책 fixture는 미래 시작 쿠폰이 active 보유 목록에는 남지만 target 후보, 사용 SQL, usable count에서는 제외되고 만료된 발급 override가 issue row 생성 전에 거부되는지 확인한다. 회원 보유 목록은 300건 이후 마지막 페이지까지 조회되고, 더 최신인 무관 쿠폰 300건 뒤의 오래된 대상 쿠폰도 SQL 대상 필터로 찾는지 함께 확인한다. `.tools/bin/check-coupon-claim-campaign-runtime.php`는 공개 claim token이 account+campaign 세션 key에 묶이고 성공 후에만 회전되는지, claim helper가 campaign row `FOR UPDATE` 잠금 아래에서 한도 검증과 점유를 진행하는 정적 계약, active 유료 캠페인 저장 시 허용 자산의 purchase power 통화 호환성을 검증하는지, 유료 발급본 claim snapshot이 `settlement_kind=paid`, snapshot/rounding policy version, allocation별 purchase power metadata를 동결하고 settlement rounding metadata fallback을 하드코딩하지 않는지, 유료 발급본 환불이 claim snapshot의 원 자산 거래를 refund로 복원하고 claim log 점유를 반환해 새 nonce 재구매를 허용하는지도 확인한다. 유료 발급 POST가 캠페인 허용 목록 밖의 설치 자산을 선택 목록에 섞으면 허용 자산만 골라 fallback하지 않고 발급 전 실패로 끝나는지도 확인한다. 캠페인 fixture는 정의의 미래 시작일을 캠페인 발급본 `issued_at`으로 클램프하고 발급본 고정 만료와 상대 만료를 동시에 저장하지 못하게 막는지도 확인한다.
- 같은 쿠폰 runtime fixture는 #391 기준으로 콘텐츠 파일 다운로드 로그가 쿠폰 redemption 링크와 `content_file_download_refund_v1`을 저장하고, 다운로드 로그 source와 현재 접근권 source가 다르면 수동 회수가 fail-closed로 접근권을 남기는지도 확인한다.
- `.tools/bin/check-read-reference-contracts.php`는 `coupon-targets.php`의 전역 `target_type` 중복 금지와 쿠폰/배너/팝업레이어 소비 helper의 중복 덮어쓰기 방지도 확인한다.
- `.tools/bin/check-content-file-cleanup-runtime.php`는 콘텐츠 삭제 시 파일 row redaction, 다운로드 링크 숨김, 다운로드 로그 snapshot redaction, 링크 참조 제거, 시리즈 회차 제거, 임베드 참조 removed 전환, 저장소 cleanup 실패 미발생을 SQLite fixture로 확인한다. 같은 fixture는 50개를 넘는 공개 연결 파일을 모두 읽고, 관리자 기본 선택지 상한 밖의 현재 시리즈와 사용 중지 연결 파일을 선택지에 보존하는지도 확인한다. 로컬 저장소 쓰기 권한이 있는 환경에서는 고유 fixture 파일과 본문 이미지를 실제로 만들고 삭제 여부까지 확인한다.
- `.tools/bin/check-content-copy-runtime.php`는 콘텐츠 복사 시 HTML 본문 임베드 ref key 재작성, 새 owner ref 생성, 원본 ref 유지, 조회수 초기화, 명시적/legacy 첨부 링크 복사, 시리즈 사본과 회차 메타데이터 복사를 SQLite fixture로 확인한다.
- `.tools/bin/check-content-layout-selection.php`는 콘텐츠별 레이아웃 선택이 공개 상세 화면에 적용되고, 명시적인 `content.none`만 무레이아웃으로 처리되며, 무레이아웃 제목 표시값이 콘텐츠·리비전·복사 경로에 보존되는 계약을 확인한다. 같은 검사는 시리즈 선택과 회차 입력이 독립 행으로 유지되고 무레이아웃 renderer가 공통 layout shell이나 시리즈 영역을 호출하지 않는지도 고정한다. 일반·basic·무레이아웃 상세는 모두 `sr_content_public_body_html()`과 같은 CKEditor 원본·프로젝트 stylesheet를 사용한다. 공개 wrapper는 `data-sr-editor-output`으로 배경을 투명하게 유지하고 `.ck-editor__main`, `.ck-editor__editable_inline`, `ck-read-only`, `aria-readonly` 같은 입력 widget 구조와 축약 public reset은 호출하지 않는다. Markdown 본문은 renderer stylesheet 계약을 유지한다. 유료 열람 확인에서는 기존 콘텐츠 modal을 재사용하고 선택된 콘텐츠 테마의 common/module stylesheet와 overlay script를 직접 로드하며, 일반 modal과 동일한 차감 확인·환전 확인·요청 token·쿠폰 선택 필드를 제출하는지도 확인한다. 선택한 테마가 존재하지 않으면 basic asset으로 fallback하는지 확인하고, 관리자/회원 작성자의 수정 URL 분기, 모든 상세 테마와 무레이아웃의 공통 수정 링크, 승인·초안·수정 요청·반려 제출본만 작성자 수정 대상으로 찾고 검수 중 제출본은 제외하는 계약도 확인한다.
- 같은 검사는 입력 widget과 좌우 padding 없이 CKEditor 원본 font-size·line-height 및 첫·마지막 블록 spacing token을 유지하는 계약도 확인한다. `.tools/bin/check-ckeditor-assets.php`는 콘텐츠 상세와 커뮤니티·콘텐츠·퀴즈·설문 댓글이 같은 공식 `ckeditor5.css`, 프로젝트 토큰 stylesheet, `.sr-ckeditor[data-sr-editor-output] .ck-content` 구조를 사용하는지 고정한다.
- `.tools/bin/check-view-count-runtime.php`는 커뮤니티/콘텐츠/퀴즈/설문 조회수의 세션 중복 방지와 단건 증가 helper가 같은 세션에서 같은 대상을 두 번 세지 않고 유효한 대상만 1씩 증가시키는지 확인한다.
- `.tools/bin/check-community-attachment-runtime.php`는 커뮤니티 첨부 다운로드 자산 로그 placeholder의 dedupe, 0원 완료 로그 settlement metadata, 첨부 다운로드 접근권 중복 방지, 전액 쿠폰 다운로드 로그의 coupon redemption 링크와 `refund_policy_version` 기록, 유료 첨부 다운로드 수동 환불의 포인트 복원/접근권 회수/payment_ledger item reverse와 쿠폰 링크 보존, anonymized entitlement 무시를 SQLite fixture로 확인한다.
...
- 공통 `sr_ledger_create_transaction()` helper의 table pair allowlist는 현재 `deposit`, `point`, `reward`를 허용한다. `point`와 `reward`는 `expires_at`, `expires_remaining`, `expired_at`을 allowlisted extra transaction column으로 넘기고, 만료/소진/환불 분할/회수 정책은 여전히 각 자산 모듈 helper가 소유한다.
...
- `.tools/bin/ops-status.php`는 설치된 환경에서 주요 지연/실패 count, 허용 지연, 가장 오래된 시각과 status별 summary를 출력하고, `--help`로 사용법을 확인할 수 있다.
- `/admin/operations`는 같은 기준을 관리자 화면에서 read-only로 보여주며, 허용 지연을 넘긴 항목은 `지연 초과`로 구분한다.
- `.tools/bin/check-output-helpers.php`는 공개 레이아웃 후보, 로고 위치 후보, output slot renderer metadata가 같은 요청 안에서 반복 contract 조회를 줄이는지 SQLite counting fixture로 확인한다. output slot cache는 HTML, callable, resource가 아니라 `module_key`, `contract_name`, `contract_version`, `contract_file` metadata만 저장해야 한다. 같은 fixture는 output slot provider의 `stylesheets`/`scripts` asset metadata가 `sr_output_slot_asset_paths()`로 수집되는지도 확인한다.
- `.tools/bin/check-public-call-order.php`는 public route wildcard shadowing, public layout begin/end 균형, output slot을 렌더링하는 public view의 `output_slots` 선선언과 렌더 context 일치, 콘텐츠/커뮤니티 본문 embed stylesheet 선계산, public POST mutation의 CSRF 선행 marker를 확인한다.
- `.tools/bin/check-operational-status.php`는 지연 신호, 허용 지연 표기, `overdue` 상태 marker, CLI row/summary 출력 형식, 관리자 route/view 연결을 확인한다. 운영 상태 점검 정의의 `table`/`age_column` 안전 식별자 검사와 `where` 조건의 세미콜론, SQL 주석, DDL/DML 키워드 거부도 SQLite fixture로 확인하며, notification delivery, community board copy, point expiration 일부 번들 신호가 실제 count/overdue로 계산되는지도 fixture로 확인한다. 포인트 만료 수동 CLI 경로는 SQLite fixture에서 만료 대상 지급분 하나만 `expire` 음수 거래로 차감하고, 원 지급 row의 `expires_remaining`/`expired_at`을 닫으며, 두 번째 실행이 중복 차감하지 않는지도 확인한다. 같은 점검은 `ops-status.php`, `expire-points.php`, `run-notification-deliveries.php` help marker, 정책 문서 안내메일 claim/재대기/취소 marker, 게시판 복사 실패 map 표시 marker, retention 자동 정리 실패 표시 marker도 확인한다.
...
- 같은 점검은 `slack_webhook`, `discord_webhook`, `telegram_bot` 외부 푸시 채널을 일반 생성 채널과 분리하고, 관리자 운영 알림의 provider별 delivery queue, 회원 개인 endpoint 참조 queue, 공통 claim/process dispatch, 실패 정책, provider 응답 판정, secret masking marker를 확인한다. 실제 provider 호출과 브라우저 설정 저장 smoke는 로컬 또는 staging에서 별도 기록한다.
- `.tools/bin/check-site-menu-seed-order.php`는 사이트 메뉴 seed order, 공개 렌더링, 현재 항목/상위 항목 표시, 섹션 루트 메뉴의 하위 경로 활성화, 3단계 제한, 외부 링크 보호, URL fail-closed를 확인한다. 하위 메뉴가 있는 항목의 `sr-site-menu-item-has-children`, `aria-haspopup`, `aria-expanded`, `aria-controls`와 레이아웃 소유 dropdown CSS/공통 JS marker도 확인한다. 동일 `menu_key`의 enabled item tree와 메뉴명이 요청 메모리 초기화 뒤에도 파일 캐시에서 재사용되고, 손상 payload는 DB 값으로 복구되며, `sr_site_menu_clear_cache()` 호출 후에는 다음 렌더링이 DB tree를 다시 조회하는지도 SQLite fixture로 확인한다. 무효화 전 generation으로 뒤늦게 쓰는 payload가 거부되는지도 함께 검증한다. 공개 사이드바 fixture는 손상 generation marker에서 캐시 읽기·쓰기를 중단하고 marker 회전 실패 시 기존 payload와 요청 메모리를 최선 범위에서 정리하는지도 확인한다.
- `.tools/bin/check-community-board-copy-job-lock.php`는 게시판 복사 작업의 `lock_token` 검증 helper와 stage/map 처리 경로의 token 전달 marker를 확인하고, stale token과 종료 상태 job이 lock assertion을 통과하지 못하는지 SQLite fixture로 확인한다. 작업 상세 화면은 map 상태별 count와 대표 실패 map을 보여 주며, 실패 재시도는 실패 map을 대기 상태로 되돌리고 이미 복사된 항목은 유지한다는 확인 문구를 표시한다.
- 커뮤니티 게시판 삭제는 작은 게시판은 즉시 삭제하고 큰 게시판은 `sr_community_board_delete_jobs` 작업으로 전환한다. 설치/update parity, 운영 상태 점검, HTTP smoke는 삭제 작업 route, 테이블 drift, pending/failed 운영 신호를 확인한다.
...
- `.tools/bin/check-community-board-settings.php`는 게시판 그룹/게시판 운영 설정 key, 커뮤니티 환경설정의 게시글 본문 길이와 게시판 사이드 메뉴 기본값, 관리자 저장 action, 공개 목록 페이지당 글 수/기본 정렬/본문 요약 적용, 목록·읽기·쓰기 화면의 사이드 메뉴와 지연 인기글 교체 보존, 게시판 목록의 제목·내용·작성자 검색조건, 게시글·댓글 본문 길이 검증, 댓글 수 기반 게시글 수정·삭제 잠금 marker, 커뮤니티 전체검색 helper 계약을 확인한다. 런타임 fixture는 정렬 key와 사이드 메뉴 key normalization, HTML 본문 plain length 계산, 본문 요약 자르기 helper, 게시판 목록의 제목/본문/공개 작성자명 검색 범위, 전체검색의 제목/본문 검색 범위 및 비밀글 본문 제외가 기대값을 유지하는지 확인한다.
- `.tools/bin/check-content-search.php`는 콘텐츠 전체검색 route, 상단 검색 폼, 커뮤니티형 topbar/고정 메뉴 shell, 라이트·다크 검색 표면 marker를 확인한다. SQLite fixture는 공개 제목·본문 검색, 잠긴 본문과 초안 제외, LIKE wildcard escaping, HTML 본문 발췌 정규화를 검증한다.
- `.tools/bin/check-community-feed-cache-contract.php`는 #369 v1 feed cache 계약을 확인한다. everyone-discoverable 공개 게시판 baseline, viewer class 없는 context hash, fetch/display count 분리, `storage/cache/community-feed` 파일 영속 저장소, cache miss 시 사용자 홈 feed가 latest/popular/latest_comments snapshot 파일을 생성하는지, cache hit 시 snapshot으로 홈 카드 row와 최신댓글 row를 구성하는지 fixture로 확인한다. 최신댓글 excerpt가 CKEditor HTML 태그를 제거하고 블록 사이 공백을 보존하며, entity로 인코딩된 `>`가 태그 속성에 있어도 속성 조각을 노출하지 않는지 확인한다. `author_account_id`, 공개 댓글 수, 공개 excerpt, thumbnail source marker 기반 card snapshot과 author label/본문 원문/CSRF/계정별 권한/유료 접근권/thumbnail URL 금지 필드가 snapshot에 들어가지 않는지도 확인한다. `/admin/community/feed-cache` 점검 화면의 route/menu/permission/view marker와 운영 화면이 같은 context helper를 쓰는지도 확인한다.
- `.tools/bin/check-community-draft-runtime.php`는 #409 커뮤니티 게시글 임시저장 계약을 확인한다. SQLite fixture로 `sr_community_post_drafts` context hash upsert, 계정별 최대 개수 trim, 보존기간 cleanup, restore payload와 edit conflict용 content hash 경계를 확인하고, 정적 marker로 `/community/draft/autosave` route, JSON action의 로그인/CSRF guard, `sr_json_response` 사용, form의 draft config/payload, `sessionStorage` buffer, localStorage 미사용, privacy export/cleanup 연결을 대조한다.
- `.tools/bin/check-community-report-auto-actions.php`는 신고 임계치 자동 임시 조치의 모듈 소유 스키마, 기본 비활성 설정, 설치/업데이트 버전 marker, 활성 대상 unique index, 상태 helper, 신고 저장 후 평가 호출, 관리자 환경설정 저장/화면 marker, 신고 관리 확정/해제 action marker를 확인한다. SQLite fixture는 terminal 상태 전이가 `active_target_uid`를 비우고 해제/검토 metadata를 남기는지, terminal auto action cutoff 이후 신고만 재집계하는지, 임계치 도달 시 게시글과 첨부가 자동 숨김 처리되고 read-back hidden fingerprint가 저장되는지, 같은 대상의 중복 활성 조치가 막히는지, 해제 시 자동 숨김 대상과 첨부가 복원되고 활성 UID가 비워지는지 확인한다.
- `.tools/bin/check-community-account-guards.php`는 커뮤니티 계정 guard foundation의 모듈 소유 스키마, 기본 비활성 설정, 관리자 설정 저장/UI marker, 관리자 신고 화면의 활성 guard 목록/해제 marker, post-commit 평가 hook, public 게시글 작성 initial status marker, 설치/업데이트 버전 marker, active guard unique index, helper 로더, privacy export/cleanup marker를 확인한다. SQLite fixture는 만료된 guard가 현재 active guard 조회에서 제외되는지, 관리자 목록 helper가 활성 guard와 회원 표시 metadata를 반환하는지, terminal 전이가 `active_guard_uid`를 비우고 reviewer/snapshot metadata를 남기는지, 다중 active 게시글 자동조치가 overlap 기준에 따라 `publication_hold` 또는 `needs_review`로 갈라지는지, 반복 confirmed 자동조치가 `confirmed_hold`를 만드는지, active publication hold가 게시글만 pending으로 바꾸고 write cooldown이 작성을 차단하는지 확인한다.
- `.tools/bin/measure-community-home-feed.php`는 커뮤니티 홈 최신글/인기글 통합 쿼리를 SQLite fixture, `SR_COMMUNITY_FEED_MEASURE_DSN` 명시 DB, 또는 `SR_COMMUNITY_FEED_MEASURE_CONFIG=1` config DB에서 실행해 #361 형식의 EXPLAIN, cold/warm 응답 시간, 반환 row 수를 출력하는 측정 하니스다. 실제 홈 feed와 같은 query helper를 사용한다. `SR_COMMUNITY_FEED_MEASURE_FIXTURE=1`은 임시 MySQL/MariaDB DB에 fixture table을 drop/create해 문법과 실행계획을 smoke 확인한다. `SR_COMMUNITY_FEED_MEASURE_COMPARE=1`은 인덱스/쿼리 후보 비교를 출력하고, candidate index 생성/삭제는 로컬/staging disposable DB에서 `SR_COMMUNITY_FEED_MEASURE_COMPARE_MUTATION=1`을 명시할 때만 수행한다. SQLite/MariaDB fixture 결과는 target MySQL/MariaDB 측정을 대체하지 않는다.
...
- `.tools/bin/check-member-auth-policy.php`는 로그인 식별자, 이메일 검증 token, 비밀번호 재설정 token, 가입/재설정 비밀번호와 이메일이 허용 길이를 넘으면 truncate lookup이 아니라 거부 경로로 처리되는지 확인한다. 같은 점검은 `sr_get_string_without_truncation()`과 `sr_post_string_without_truncation()`의 길이 경계와 배열 입력 거부 fixture도 실행한다. 회원 선택 프로필 추가 항목 삭제가 저장값 정리 확인과 전체 회원 `field_key` 값 삭제 helper에 연결되는지도 확인한다. `.tools/bin/check-delivery-templates.php`는 알림 이메일 채널이 꺼진 fixture에서 회원 transactional email 상태와 발송이 비활성화되고, 회원 모듈이 `email-delivery.php` 계약을 통해 알림 transport 설정을 사용하는지 확인한다. `.tools/bin/check-notification-runtime.php`는 알림 읽음 redirect token도 truncate lookup을 쓰지 않는지 확인한다.
- `.tools/bin/check-member-oauth-runtime.php`는 OAuth state/nonce/PKCE 원문 DB 비저장, callback용 PKCE verifier 세션 임시 저장소 1회 회수, authorization URL PKCE parameter, state 1회 사용, link state account binding, provider subject HMAC과 hash prefix 표시값, provider 설정 저장/secret 마스킹 marker, 필수 scope 보존과 저장 scope 우선 적용, 선택 프로필 부분 동기화와 빈 값·선택지 밖 값·정의 밖 기존값 보존, 연결/해제 helper, 마지막 로그인 수단 해제 차단, 비밀번호 없는 계정의 비밀번호 설정 안내, generic provider callback과 completion의 policy document 동의 gate marker를 SQLite fixture와 정적 marker로 확인한다. 실제 외부 provider 계정과 브라우저 완료 흐름은 설치 DB가 있는 로컬 또는 staging smoke에서 별도 기록한다.
...
- `.tools/bin/check-runtime-helpers.php`는 rate limit fixture로 같은 window 안의 count 증가, 만료 row 제외, 만료 후 재증가 시 1로 재시작, HMAC key/subject hash 저장, 잘못된 bucket/subject 입력 무시를 SQLite에서 확인한다. 같은 점검은 `paths.php` wildcard route가 bare prefix가 아니라 하위 path만 매칭하고, route 충돌 판정도 같은 경계를 쓰는지 확인한다.
...
- `.tools/bin/check-antispam-runtime.php`는 자동등록방지 모듈의 강화 산술 challenge 변형, 정답 HMAC 세션 저장, 성공/실패/만료 후 challenge 폐기, honeypot, 최소 제출 시간, provider failure policy, `provider_unavailable`에만 허용되는 `fallback_math`, provider action/hostname 검증 토글, 활성 `antispam-providers.php` provider 계약과 `antispam-targets.php` 적용 대상 계약 로딩, provider endpoint/script URL의 HTTPS 공개 URL 제한과 검증 POST 직전 공개망 endpoint gate, `antispam_captcha_providers` 플러그인의 Turnstile/hCaptcha/reCAPTCHA 응답 fixture, reCAPTCHA 최소 점수, secret masking, 회원가입과 비회원 커뮤니티 글/댓글 적용 marker를 확인한다. 실제 provider key와 브라우저 widget 동작은 설치 DB가 있는 로컬 또는 staging smoke에서 별도 기록해야 하며, 운영 DB에서 mutation smoke를 실행하지 않는다.
- `.tools/bin/check-output-helpers.php`는 HTML view와 CKEditor config script의 JS 값 주입이 직접 `json_encode()`가 아니라 `sr_js_json_encode()`를 쓰는지 정적 스캔한다. helper fixture는 `</script>`, quote, ampersand, invalid UTF-8 입력이 script-safe JSON으로 encode되는지도 확인한다. 응답 헤더 fixture는 `sr_json_response()`와 `sr_send_file_headers()`의 추가 헤더 allowlist가 redirect, CRLF injection, malformed cache-control, 따옴표 없는 content-disposition, 비숫자 content-length, 비정규 content-type, 잘못된 ETag/Last-Modified, nosniff/pragma 외 값을 거부하는지도 확인한다.
- `.tools/bin/smoke-http.php`는 동적 진입점 응답에서 `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, 기본 `Content-Security-Policy`와 YouTube URL 임베드용 `frame-src`가 실제로 전송되는지 확인한다. `.tools/bin/check-identity-verification-runtime.php`는 본인확인 시작 응답의 provider POST용 `form-action` origin 허용도 함께 확인한다.
...
- `.tools/bin/check-privacy-contract-matrix.php`는 매트릭스의 상태와 `module.php` 계약 선언, 실제 `privacy-export.php`/`privacy-cleanup.php` 파일 존재, 계약 파일 반환 형태와 callable signature, `install.sql`의 계정 연결 컬럼과 식별자성 컬럼 문서화, ROPA 처리활동 씨앗의 32개 번들 모듈 row와 필수 컬럼 marker, `operational_retained` 모듈의 보존 사유/운영자 접근 범위/1.0 전 검토 항목, `export_retained`/field-level cleanup 모듈의 보존 사유/고위험 필드/1.0 전 검토 항목을 대조한다. bounded export가 반환 상한보다 한 행을 더 조회하고 `_limits` 초과 metadata를 제공하는지도 전체 모듈 계약에서 확인한다.
...
- `.tools/bin/check-privacy-export-runtime.php`는 SQLite fixture로 `message`, `quiz`, `survey`, `content`, `community` export 계약과 `asset_exchange`, `coupon`, `deposit`, `notification`, `payment_ledger`, `point`, `reward` 보존형 export 계약을 실행한다. 대상 계정의 상세 답변, snapshot, 접근권, 자산 로그, 다운로드 로그, 작가 신청, 시리즈, 쪽지, 댓글/게시글, post draft, 신고/스크랩/동의 증적이 포함되고 다른 계정 row가 제외되는지 확인한다. 보존형 fixture는 결제 record/item, 금액성 원장, 쿠폰 환불, 유료 쿠폰 발급본의 claim snapshot/asset reference, 환불/출금 계좌, 알림 delivery, 포인트/적립금 만료 소비 매핑 같은 운영 증빙 필드가 대상 계정 기준으로 포함되는지 확인한다.
- `.tools/bin/check-privacy-export-status.php`는 테스트용 모듈 실패와 section row 초과를 시뮬레이션해 `partial_export`, `module_export_status`, `overflow_sections`, `evidence_id`가 JSON에 남고 raw exception/secret이 노출되지 않는지 확인한다.
- `.tools/bin/check-privacy-cleanup-runtime.php`는 SQLite fixture로 `quiz`, `survey`, `content`, `community`, `notification`, `payment_ledger`, `policy_documents`, `reward` cleanup 계약을 실행한다. 대상 계정의 응시/응답, 보상 grant, 접근권, 다운로드 로그, 작가 신청, 닉네임, 레벨, 동의 증적, 시리즈 메타데이터, 댓글/게시글 작성자 snapshot이 익명화되고 post draft가 삭제되며 다른 계정 row가 유지되는지 확인한다. notification push endpoint ciphertext 제거, payment_ledger 결제 record 계정 연결 제거와 item account 참조 익명화, policy_documents 안내메일 delivery 계정 연결 제거, reward 출금 은행정보와 요청자/관리자 note 제거도 함께 확인한다.
...
- `.tools/bin/check-community-series-pagination.php`는 `/community/series`의 내 시리즈가 200건에서 끝나지 않고 전체 count 기반 페이지네이션으로 탐색되는지 확인한다. 목록 count와 각 페이지가 회원, 상태, 게시판 활성화, 게시판별 시리즈 사용 설정을 같은 기준으로 적용하는 45건 SQLite fixture도 함께 고정한다.
- `.tools/bin/check-public-comment-pagination.php`는 콘텐츠·퀴즈·설문 공개 댓글이 20개 단위 숫자 페이지로 전체 게시 댓글을 조회하는지, 40개 이후 row와 전체 수·마지막 페이지 보정·새 댓글 위치 계산을 SQLite fixture로 확인한다. 기본 테마와 기본 호환 스킨이 같은 페이지 helper를 사용하고, 커뮤니티 댓글과 같은 양끝·현재 주변 숫자 창과 UI kit 버튼 클래스를 적용하는지도 정적 marker로 확인한다.
- `.tools/bin/check-admin-comment-pagination.php`는 퀴즈·설문 관리자 댓글 목록이 고정 200건에서 끝나지 않고 필터 count 기반 공통 관리자 페이지네이션을 사용하는지 확인한다. 각 45건 SQLite fixture의 중간·마지막 페이지와 검색·상태 필터 count 일치도 함께 고정한다.
- `.tools/bin/check-quiz-consistency.php`는 퀴즈 제출 성공과 실패가 모두 Post/Redirect/Get으로 끝나는지, 실패 flash가 오류와 선택한 답을 한 번만 복원하는지, 기본 테마와 기본 호환 스킨이 같은 토스트·복원 계약을 쓰는지 확인한다.
...
- `.tools/bin/check-content-member-submission-pagination.php`는 회원 콘텐츠 제출 내역이 고정 200건에서 끝나지 않고 전체 목록을 페이지로 탐색하는지 확인한다. 45건 SQLite fixture의 중간·마지막 페이지와 편집 ID·목록 페이지 보존 marker도 함께 확인한다.
- `.tools/bin/check-content-admin-workflow-pagination.php`는 콘텐츠 등록자 신청·작성자 승인·회원 제출 검수 관리자 목록이 고정 200건에서 끝나지 않고 필터 count 기반 공통 관리자 페이지네이션을 사용하는지 확인한다. 각 45건 SQLite fixture의 마지막 부분 페이지와 상태·검수 조건·신청 ID 필터 count, 행 작업 복귀 경로 marker를 함께 고정한다.
...
- `.tools/bin/check-public-service-list-pagination.php`는 기존 `/quiz`, `/survey`, `/coupons` 경로가 고정 50건에서 끝나지 않고 전체 공개 목록을 페이지로 탐색하는지 확인한다. 각 45건 SQLite fixture의 count와 마지막 부분 페이지, 기본 테마 및 쿠폰존 페이지 링크 marker를 함께 고정한다.

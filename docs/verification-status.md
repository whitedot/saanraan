...
- `.tools/bin/check-module-foundation-lifecycle.php`는 SQLite fixture로 `asset_ledger`와 `payment_ledger` 기반 모듈 비활성화가 활성 의존 모듈이 있는 동안 차단되는지 확인한다. `payment_ledger`는 `content`, `community`, `coupon`의 활성 상태를 기준으로 검사한다.

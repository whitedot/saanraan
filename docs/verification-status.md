...
- 같은 검사는 입력 widget과 좌우 padding 없이 CKEditor 원본 font-size·line-height 및 첫·마지막 블록 spacing token을 유지하는 계약도 확인한다. `.tools/bin/check-ckeditor-assets.php`는 콘텐츠 상세와 커뮤니티 게시글·댓글이 같은 공식 `ckeditor5.css`, 프로젝트 토큰 stylesheet, `.sr-ckeditor[data-sr-editor-output] .ck-content` 구조를 사용하는지 고정한다.
...
- `.tools/bin/check-public-comment-pagination.php`는 콘텐츠·퀴즈·설문 공개 댓글이 20개 단위 숫자 페이지로 전체 게시 댓글을 조회하는지, 40개 이후 row와 전체 수·마지막 페이지 보정·새 댓글 위치 계산을 SQLite fixture로 확인한다. 기본 테마와 기본 호환 스킨이 같은 페이지 helper와 공통 공개 페이지네이션을 사용하는지도 정적 marker로 확인한다.

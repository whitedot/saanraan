...
이 문서는 Toycore 외부 모듈을 처음 만드는 개발자를 위한 시작 문서다. 처음에는 Git이나 GitHub Actions를 몰라도 된다. 핵심은 **모듈은 폴더 하나이고, 그 폴더를 zip으로 묶어 Toycore 관리자에서 업로드한다**는 점이다.
...
혼자 만들고 zip으로 업로드하는 모듈은 Git이나 CI 없이도 시작할 수 있다.

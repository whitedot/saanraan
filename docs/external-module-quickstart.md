...
이 명령은 `AGENTS.md`, `README.md`, `CHANGELOG.md`, `.tools/bin/package-module`, `module/module.php`, `module/install.sql`, `.github/workflows/check.yml`을 만든다. 기존 파일은 덮어쓰지 않으므로 빈 디렉터리를 대상으로 실행한다.

생성된 `AGENTS.md`는 구현 규칙과 AI 보조 작업의 기준 파일이다. 사람이 직접 구현할 때도, AI 코딩 도구에 작업을 맡길 때도 이 파일의 저장소 구조, 요청 흐름, 보안, 계약 파일, 점검 규칙을 기준으로 삼는다.

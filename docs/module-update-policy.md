...
- `/admin/modules`는 소유자 전용 화면이며 `모듈 파일 반영`, `설치 가능한 모듈`, `설치된 모듈` 순서로 한 화면에 표시한다. zip 업로드 버튼은 `ZipArchive`를 사용할 수 있고 모듈 파일 반영이 일시 허용된 동안만 표시한다. 파일 전용 업데이트 반영을 위한 일시 허용과 허용 닫기는 `ZipArchive`가 없어도 사용할 수 있다.

...
- zip 업로드는 owner 권한, 일시 허용 값, 재인증, ZipArchive 사용 가능 여부를 모두 확인하고 요청 처리 후 모듈 소스 반영 허용 값을 자동으로 끈다. 파일 전용 업데이트 반영은 ZipArchive 없이도 owner 권한, 일시 허용 값, 재인증을 확인한 뒤 처리한다.

...
`/admin/modules`의 zip 업로드 기능은 구현되어 있지만, 운영 UX와 보안 정책을 추가 검토하기 전까지 모듈 관리 화면의 진입 버튼은 숨긴다. zip 업로드 요청은 소유자 비밀번호 재확인을 통과한 경우에만 모듈 파일 반영을 일시적으로 허용하고, 업로드 처리가 끝나면 `admin.module_sources_enabled` 값을 bool false로 다시 저장한다.
...
- `/admin/modules`는 소유자 전용 화면이며 `설치 가능한 모듈`, `설치된 모듈` 순서로 한 화면에 표시한다. zip 업로드 진입 버튼은 기능 검토가 끝날 때까지 숨긴다.
...
- zip 업로드는 owner 권한, 재인증, ZipArchive 사용 가능 여부를 모두 확인하고 요청 처리 후 모듈 소스 반영 허용 값을 자동으로 끈다.

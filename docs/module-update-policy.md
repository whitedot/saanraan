...
`/admin/modules`의 zip 업로드는 모듈 관리 화면의 `설치 가능한 모듈` 섹션 제목 오른쪽 버튼에서 모달로 연다. zip 업로드 요청은 소유자 비밀번호 재확인을 통과한 경우에만 모듈 파일 반영을 일시적으로 허용하고, 업로드 처리가 끝나면 `admin.module_sources_enabled` 값을 bool false로 다시 저장한다.
...
- `/admin/modules`는 `설치 가능한 모듈`, `설치된 모듈` 순서로 한 화면에 표시하고, zip 업로드는 `설치 가능한 모듈` 섹션 제목 오른쪽 버튼에서 모달로 연다.

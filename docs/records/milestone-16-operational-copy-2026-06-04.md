...
- 배너는 본문, 링크, 이미지 URL, 스킨, 노출 기간, 정렬 순서, 대상 조건 전체를 복사한다. 운영자가 선택하면 집계 클릭 수만 함께 복사한다.
...
- 배너 클릭 수는 기본으로 복사하지 않으며, 클릭 dedupe hash 로그는 복사하지 않는다.

## 대량 게시판 복사

게시판 게시글/댓글/첨부파일 포함 복사는 동기 상한을 둔다. 상한 초과는 배치 복사 흐름으로 나누어 처리할 수 있다. S3 첨부 또는 원본 첨부파일 누락은 현재 복사할 수 없으므로 job 생성을 막고 저장소/파일 상태 정리를 먼저 안내한다.

게시판 복사 확인 화면은 첨부 총량 기준의 추가 여유 공간을 운영자가 이해하기 쉬운 문구로 미리 안내한다. 첨부가 없는 게시글/댓글 복사도 DB row를 새로 만들기 때문에 DB 용량 부족으로 실패할 수 있음을 함께 표시한다. 저가형 공유 호스팅에서는 디스크 용량뿐 아니라 DB 용량, inode 또는 파일 수 제한도 실행 전에 호스팅 관리 화면에서 확인하도록 안내한다.

게시판 복사 확인 화면은 게시글/댓글/첨부 수를 보여주므로 게시판 조회 권한과 게시글 조회 권한을 모두 요구한다. 전체 복사 실행과 배치 job 실행/정리는 게시판 편집 권한과 게시글 편집 권한을 모두 요구한다.

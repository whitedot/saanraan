...
0. #225 퀴즈 환경설정과 복사 기능을 구현해 새 퀴즈 기본값, 안전한 draft 복사, 보상/source/member group 재검증 기준을 정리했다.
...
- 관리자 설문 목록은 검색어, 상태, 응답 가능 여부 필터와 상태/QA/보상 배지, 복사/미리보기/삭제 동선을 제공한다.
...
- #227/#238: 동의 문구, 개인정보 안내, 민감정보, 재연락, 철회, 윤리 기준을 설문 메타데이터와 응답 스냅샷에 포함했다.
- #228/#232: 단일/복수 선택, 기타, 무응답, 텍스트, 숫자, 별점, 척도 문항과 필수/선택 수/숫자 범위 검증을 구현했다.
- #229/#239: 관리자 미리보기, 테스트 제출, QA 상태, 설문지 버전, 잠금 상태의 문항 변경 거부를 구현했다.
- #230/#231/#235: 품질 상태 관리, 테스트/제외 응답 배제, 안정 key 기반 통계, 원본/분석/코드북 CSV와 가중치·오차·분석 계획 메타데이터를 반영했다.
- #233/#234/#236/#237: 전문 조사 프로젝트 브리프, 표본·모집·실사·외부 패널/벤더·공표용 방법론 메타데이터를 저장하고 공개 안내에 노출할 수 있게 했다.
- #241/#242/#243/#244/#245: 공개 레이아웃, 초기화면 후보, UI Kit, 관리자 권한/감사 로그, SEO/sitemap, 공유호스팅형 CSV 제한과 테스트 문서를 보강했다.
- #247: 로그인/익명 정책, IP/user-agent 해시 중복 제한, 보상 설문 로그인 강제, 지급 전 provider 재검증, 관리자 테스트 보상 제외를 적용했다.

...
| `php .tools/bin/check-installed-gate-status.php` | 통과 | 설치 DB 게이트 상태표 도구의 출력 형식, `config-mode`/`config-owner-group` 출력, 필수 게이트 행, 관리자 화면 게이트의 `SR_SMOKE_ADMIN_IDENTIFIER`/`SR_SMOKE_ADMIN_PASSWORD` missing/incomplete/configured 분기, `--run-readonly`, `--run-browser-qa`, `--run-auth-smoke`, `--run-asset-smoke`, `--run-privacy-fixtures`, `--run-performance-fixtures`, 인증/자산 mutation smoke의 `SR_SMOKE_ALLOW_MUTATION=1` 차단/준비 분기, fixture 옵션 실행 시 개인정보/성능 게이트 `부분 확인` 출력, 성능 fixture별 exit summary, 문서 marker 확인 |

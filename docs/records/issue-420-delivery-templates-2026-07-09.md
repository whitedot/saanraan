...
- transactional email override는 `/admin/delivery-templates`에서 저장하고 복원한다. 회원 인증 메일(`member.email_verification`, `member.password_reset`, `member.login_mfa_email_code`)은 회원 알림/메일 관리 화면인 `/admin/member-notification-templates`에서도 같은 override 저장/복원 흐름으로 관리한다.
...
- transactional email은 override가 있으면 수정값을 사용하고, 기본값으로 되돌리려면 override를 삭제한다. 필수 placeholder가 없거나 렌더 결과 subject/body 필수 위치가 비거나 렌더 중 오류가 나는 깨진 override는 계약 default로 fallback한다.
- notification event와 transactional email의 inactive는 no-op 의미로 처리한다.
- 알림 모듈의 이메일 채널 사용 설정이 꺼져 있으면 알림 이벤트 채널에 `email`이 저장되어 있더라도 해당 채널의 발송 작업 row를 만들지 않는다. 회원 외부채널 수신 허용 또는 provider별 회원 수신 허용이 꺼져 있을 때도 저장된 회원 외부채널 선택값은 보존하되 해당 채널의 발송 작업 row를 만들지 않는다. 알림/메일 관리 목록은 저장된 채널값과 현재 채널 사용 가능 상태를 구분해 `이메일 중지`, `외부채널 중지` 상태를 표시한다.

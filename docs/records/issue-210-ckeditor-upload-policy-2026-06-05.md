...
- 저장 POST는 정화된 본문 HTML에서 허용된 업로드 URL을 파싱하고, 임시 경로의 파일을 콘텐츠별 경로로 복사한 뒤 HTML URL을 저장 경로용 프록시 URL로 바꾼다. 저장 transaction이 성공한 뒤 임시 경로 파일을 삭제한다.
...
- 콘텐츠 저장 transaction이 rollback되면 콘텐츠별 경로에 복사한 파일은 보상 삭제하고, 임시 경로 파일은 재시도할 수 있도록 남긴 뒤 TTL 정리 대상으로 둔다.

...
이 문서는 point/reward/deposit/coupon/asset_exchange와 콘텐츠/커뮤니티 연동에서 확인한 위험을 정리한다. 핵심 원장은 `FOR UPDATE` 잔액 잠금, 음수 잔액 차단, `balance_after` 기록, 정수 연산, dedupe key 기반 멱등성, 환전 실행의 그룹 트랜잭션을 갖추고 있다. 2026-05-31 기준 최초 과금 확인, 미완료 로그 마커, 환전 제출 토큰, 쿠폰 만료 전이, 알림 후처리 보강을 적용했다.
...
- 권장 조치: 운영 기본값은 `floor`로 고정하거나, 양방향 정책 저장 시 무수수료 가치 증가 경로를 검증한다. 그룹 단위 취소/정정 helper를 추가한다.
...
1. 환전 반올림 정책 검증과 그룹 단위 정정 helper
2. 관리자 조정 상한과 대액 승인
3. 데드락 재시도, 환전 로그 인덱스 정리

...
- 2026-05-31 보강: 포인트/적립금/예치금 관리자 조정은 서버에서 자산별 1회 1,000,000, 관리자별 일일 10,000,000 절대금액 상한을 검증한다. 초과 금액은 저장하지 않는다.
- 추가 고도화 후보: 실제 운영에서 상한 초과 조정이 필요하면 별도 승인 workflow나 권한 분리 정책을 추가한다.
...
- 2026-05-31 보강: 독립 환전 실행은 MySQL deadlock/lock wait timeout 계열 오류를 최대 3회 재시도한다. 상위 트랜잭션 안에서 호출될 때는 외부 작업 단위가 불명확하므로 내부 재시도를 하지 않고 예외를 전파한다.
- 추가 고도화 후보: 콘텐츠/커뮤니티 복합 자산 차감에도 상위 작업 단위 재시도 정책을 추가한다.

## 우선순위

1. 대액 조정 승인 workflow 또는 권한 분리
2. 콘텐츠/커뮤니티 복합 자산 작업의 상위 재시도 정책

위 항목은 현재 차단/롤백/정적 점검 방어를 넘어서는 운영 고도화 후보이며, 2026-05-31 보강 후 즉시 정합성 결함으로 남겨 둔 항목은 없다.

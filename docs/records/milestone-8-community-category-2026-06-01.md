...
- #98 댓글 멘션 알림은 커뮤니티 `@닉네임`, 콘텐츠 `@표시명` 기준으로 유지하고, 알림 모듈 비활성화 시 no-op 처리한다. 댓글 작성 감사 로그 metadata에는 작성자 알림 생성 여부를 남기고, 댓글 작성/수정 감사 로그에는 멘션 후보 수, 실제 멘션 알림 생성 수, 멘션 대상 공개 해시 목록을 남긴다.

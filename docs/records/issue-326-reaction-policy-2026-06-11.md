...
- 표시명과 설명은 plain text로 저장하고 출력 시 escape한다. 아이콘은 이모지, Material icon key, 또는 운영자가 직접 업로드한 JPG/PNG/WebP 이미지로 설정할 수 있다. 이미지 아이콘은 512px 이하, 1MB 이하로 제한하고 재인코딩 후 저장한다. 색상은 허용 swatch 또는 안전한 hex 값으로 제한한다.
- 이미지 아이콘은 reaction 모듈 저장소와 `/reaction/icon` 출력 action이 소유한다. 이미지 업로드 후 정의 저장 검증이 실패하면 해당 요청에서 새로 업로드한 파일은 삭제하고, 유효하지 않은 이미지 storage reference는 공개 아이콘으로 렌더링하지 않는다.
...
## preset 선택 위치와 적용 우선순위

- 콘텐츠는 콘텐츠 환경설정, 콘텐츠 그룹 설정, 개별 콘텐츠 설정에서 콘텐츠/댓글 리액션 preset을 선택할 수 있다. 실제 적용은 콘텐츠 환경설정 < 콘텐츠 그룹 설정 < 개별 콘텐츠 설정 순서로 우선한다.
- 커뮤니티는 커뮤니티 환경설정, 게시판 그룹 설정, 개별 게시판 설정에서 게시글/댓글 리액션 preset을 선택할 수 있다. 실제 적용은 커뮤니티 환경설정 < 게시판 그룹 설정 < 개별 게시판 설정 순서로 우선한다.
- 퀴즈는 퀴즈 환경설정과 개별 퀴즈 설정에서 퀴즈/댓글 리액션 preset을 선택할 수 있다. 그룹 계층이 없으므로 퀴즈 환경설정 < 개별 퀴즈 설정 순서로 우선한다.
- 설문은 설문 환경설정과 개별 설문 설정에서 설문/댓글 리액션 preset을 선택할 수 있다. 그룹 계층이 없으므로 설문 환경설정 < 개별 설문 설정 순서로 우선한다.
- 개별 설정값을 비워두면 상위 설정을 상속한다. 기존 레코드에 저장된 빈 preset key는 `emotions` 같은 고정 기본값을 의미하지 않고 상위 설정으로 해석한다.

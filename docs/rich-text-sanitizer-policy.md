...
- iframe, script, style, event handler, data URL 이미지를 허용하려는 변경은 보안 정책 변경으로 취급하고 [보안 제보와 처리 기준](security-response-policy.md)과 [프로젝트 리스크 레지스터](risk-register.md)를 함께 검토한다.

...
2026-05-14 기준으로 `/admin/design-tokens`는 다음 범위까지 구현했다.

- 현재 `assets/common.css`와 로컬 UI Kit 기준 CSS를 대조한다.
- 토큰은 `:root`, dark theme, 전체값, 원본값을 함께 표시한다.
- 클래스는 UI Kit 섹션 기준으로 그룹화하고 `원본 동일`, `saanraan 추가`, `원본 대비 누락`, `값 변경` 상태를 표시한다.
- 버튼, 배지, 폼, 유효성 피드백, 카드, 테이블, 페이지네이션, 탭, 드롭다운, 모달, 토스트 샘플을 실제 시맨틱 클래스 조합으로 렌더링한다.
- 원본 CSS 경로가 없는 환경에서는 현재 `assets/common.css` 기준 목록만 표시한다.

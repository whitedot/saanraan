...
DB에는 파일 경로를 저장하지 않고 `public_layout_key`, `layout_key`, `theme_key`, `skin_key` 같은 key만 저장합니다. 공개 레이아웃 key는 `common.basic`, `community.basic`처럼 provider namespace를 포함하며, 실제 파일 경로는 코드의 allowlist helper와 `layout-options.php` 계약이 결정합니다. 알 수 없는 key는 `common.basic`으로 fallback하고, 기존 `basic` 값은 `common.basic`으로 정규화합니다.

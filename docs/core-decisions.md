...
쿠폰·이용권은 포인트/적립금/예치금처럼 금액 잔액을 차감하는 자산이 아니라 특정 대상에 사용할 수 있는 권리성 회원 자산입니다. `coupon` 모듈이 쿠폰 종류, 회원별 지급, 사용 로그, 탈퇴 시 상태 전환을 소유하고, 콘텐츠와 커뮤니티는 쿠폰 테이블을 직접 알지 않고 좁은 helper로 열람권 사용 가능 여부와 사용 처리를 요청합니다. 콘텐츠 유료 열람과 커뮤니티 게시글/게시판 유료 열람은 사용할 수 있는 쿠폰을 먼저 확인하고, 없을 때 기존 금액성 자산 차감으로 내려갑니다. 쿠폰 사용 중복 방지는 `sr_coupon_redemptions.dedupe_key`에 둡니다.
...
포인트/적립금/예치금 원장 거래와 쿠폰·이용권 지급/사용/상태 변경은 알림 모듈이 활성화되어 있으면 `sr_notification_event_templates`의 DB 템플릿을 통해 회원 대상 사이트 알림을 만듭니다. 알림 모듈이 비활성화되어 있거나 설치되어 있지 않으면 no-op으로 처리하고, 자산 처리 자체를 실패시키지 않습니다. 외부 채널은 템플릿의 `channels_json`에 명시된 경우에만 delivery queue를 만든다는 정책을 유지합니다.

관리자 수동 자산 조정은 각 자산 모듈이 자기 설정으로 회원 그룹별 차등 정책을 소유합니다. 공통 helper는 회원의 현재 활성 그룹과 정책 배열을 받아 최종 금액과 스냅샷을 계산하는 좁은 역할만 맡고, 포인트/적립금/예치금 테이블이나 회원 테이블을 넓히지 않습니다. 여러 그룹이 매칭되면 우선순위가 가장 큰 활성 정책 1개만 적용하고, 동순위는 정책 ID와 그룹 ID 오름차순으로 결정합니다. 환불은 원거래 복구 성격이므로 수동 조정 그룹 정책을 적용하지 않습니다. 정책 적용 스냅샷은 관리자 감사 로그 metadata에 남기고, 최종 금액이 0이면 원장 거래를 만들지 않고 저장 오류로 안내합니다.

...
원본 `/ui-kit` 조회 화면은 초기/기본 공개 페이지와 public layout 런타임 기준 공통 원형을 확인하는 개발 보조 화면으로 보존합니다. 모듈 화면의 최종 디자인 정책은 원본 UI-KIT이 소유하지 않습니다. 콘텐츠/커뮤니티/퀴즈/설문 공개 화면은 공통 `kit` profile이나 루트 `assets/common.css`, `assets/public-ui.css`, `assets/public-ui-kit.css`를 직접 호출하지 않고, 각 원본 파일을 모듈 `assets/common.css`, `assets/public-ui.css`, `assets/ui-kit.css`로 복제해 호출합니다. 모듈별 UI-KIT 미리보기는 원본 `/ui-kit`의 `layouts/public/basic/ui-kit-samples/` 샘플 파일을 그대로 복제해 같은 샘플 구성을 유지하고, `/content/ui-kit`, `/community/ui-kit`, `/quiz/ui-kit`, `/survey/ui-kit` 사용자 화면에서 현재 공개 레이아웃 설정과 같은 stylesheet 조합으로 확인합니다. 기본 호출 순서는 각 모듈 기본 레이아웃 CSS, 모듈 `common.css`, 모듈 `public-ui.css`, 모듈 `ui-kit.css`, 모듈 공개 CSS입니다. 단, 운영자가 해당 모듈 환경설정에서 레이아웃을 변경한 경우에는 선택된 레이아웃의 호출 정책을 따릅니다.

모듈 theme나 skin에 전용 CSS가 필요하면 `sr_public_layout_begin()`의 layout context에 `stylesheets`를 전달합니다. 공개 화면은 layout option의 `style_profile` 또는 layout context의 `style_profile`로 `kit`, `minimal`, `install` 중 하나를 선택합니다. 공통/콘텐츠/커뮤니티 layout은 minimal profile을 기본값으로 삼아 공통 공개 UI primitive가 자동으로 섞이지 않게 합니다. 단, layout option의 profile은 fallback일 뿐 화면 성격보다 우선하지 않습니다. 홈/회원/계정/원본 공개 UI-KIT처럼 공통 `btn`, `form-*`, `table`, `public-ui-*` primitive에 의존하는 view는 사이트 기본 layout이 minimal profile이어도 layout context에 `style_profile => 'kit'`을 직접 명시합니다. 콘텐츠/커뮤니티/퀴즈/설문 공개 화면과 모듈 UI-KIT 조회 화면은 원본 공개 UI-KIT이 아니므로 공통 `kit` profile과 루트 `assets/*.css` UI-KIT 파일을 직접 호출하지 않고, 모듈 내부에 복제한 `assets/common.css`, `assets/public-ui.css`, `assets/ui-kit.css`를 호출합니다. 공통 공개 layout은 `assets/public-layout.css`와 `assets/public-layout.js`, 콘텐츠 layout은 `modules/content/assets/layout.css`와 `modules/content/assets/layout.js`, 커뮤니티 layout은 `modules/community/assets/layout.css`와 `modules/community/assets/layout.js`를 layout 파일이 직접 호출합니다. 콘텐츠/커뮤니티/퀴즈/설문 helper는 모듈 화면에 필요한 복제 UI-KIT CSS와 `assets/public.css`를 layout context에 추가합니다. public layout은 전달받은 stylesheet를 `<head>`에 출력하는 통로만 제공합니다. 출력 슬롯처럼 head 렌더링보다 뒤에서 HTML이 만들어지는 공개 모듈 출력도 해당 슬롯을 호출하는 view, skin, theme, public layout이 필요한 stylesheet를 layout context에 명시합니다. 공개 CSS는 활성화된 모든 모듈 기준으로 자동 호출하지 않습니다.

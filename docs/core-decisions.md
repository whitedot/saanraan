...
커뮤니티 모듈은 게시판 SEO/OG 기본값을 `sr_community_board_settings`의 `seo_title`, `seo_description`, `og_image_url` 설정 key로 소유한다. 제목과 설명은 검색 메타와 OG 메타에 함께 쓰며, 기존 `og_title`, `og_description` 설정 key는 과거 데이터 호환을 위해 읽기 fallback으로만 유지한다. 게시글별 값은 `sr_community_posts`의 SEO/OG 컬럼으로 소유하지만 공개 글쓰기 POST에서 임의 입력으로 생성하지 않는다. 공개 메타는 저장된 게시글 SEO 값, 게시글 제목, 공개 가능한 본문 요약 순으로 만들고, OG 제목/설명은 저장된 OG 값이 비어 있으면 SEO 결과로 fallback한다. 게시판 그룹은 SEO/OG 기본값을 제공하지 않는다. 공개 접근 가능한 게시판과 게시글만 `index, follow` 및 sitemap 후보가 되며, 검색 결과, 잘못된 카테고리, 권한 제한, 유료 열람 확인 전 화면은 noindex 기준을 사용한다. 게시글 OG 이미지는 지정된 활성 이미지 첨부, 첫 활성 이미지 첨부, 게시판 기본 OG 이미지, 사이트 기본 OG 이미지 순서로 이어지고, 작성자나 `/admin/community/posts` edit/delete 권한자는 지정 OG 이미지를 제거할 수 있다.
...
화면과 모듈 사이에서 회원을 가리키는 공개 해시는 내부 회원 ID를 그대로 노출하지 않는 app key 기반 불투명 식별자다. 공개 해시는 암호학적 권한 증명이 아니며, action은 공개 해시로 회원을 찾은 뒤에도 로그인, 권한, 대상 상태를 별도로 검증한다. 해시 payload는 회원 ID를 복원할 수 있는 keyed permutation과 위변조 확인용 MAC으로 구성해 단일 회원 ID 조회로 해석하며, 활성 회원 전체를 읽어 HMAC을 대조하지 않는다. app key 또는 공개 해시 알고리즘을 변경하면 이전에 복사한 공개 해시는 더 이상 조회 식별자로 사용할 수 없고 화면에서 현재 값을 다시 받아야 한다.
...
게시판별 일반 회원 운영권한은 커뮤니티 모듈의 `sr_community_board_managers`가 소유한다. 이 권한은 관리자 모드 접근 권한이 아니라 공개 사용자 모드에서 해당 게시판의 게시글·댓글을 운영하기 위한 권한이다. 권한 key는 `view_manage`, `write_notice`, `hide_post`, `delete_post`, `hide_comment`, `delete_comment`, `remove_post_og_image`를 허용한다. `view_manage`는 비밀글/비밀댓글 열람에만 쓰고, `write_notice`는 공개 글 작성/수정 화면과 게시글 상세 운영 action에서 게시글을 공지사항으로 저장하거나 해제하는 데 쓴다. 숨김/삭제 권한은 공개 게시글·댓글 화면의 운영 action 판정에 합산한다. 기존 `delete_post` 권한 보유자는 호환을 위해 게시글 숨김과 댓글 숨김/삭제 판정에도 인정한다. 게시판 운영권한은 게시글 본문 수정 권한으로 자동 확대하지 않으며, 전역 관리자 권한과 작성자 본인 권한도 별도 helper에서 합산해 판단한다.

커뮤니티 게시글과 댓글의 숨김 상태는 대상 row의 `status = hidden`으로 남기고, 숨김 메타데이터는 `sr_community_hidden_targets`가 target type/id 기준으로 소유한다. 게시글/댓글 본문 테이블은 숨김 사유, 만료일, 운영 메모, 처리자, 복원 전 상태 컬럼을 직접 보유하지 않는다.
...
회원가입 화면에 서비스 도메인별 추가 입력이 필요하면 회원 모듈이 해당 도메인을 직접 알지 않습니다. 소비 모듈은 `member-registration.php` 계약으로 입력 필드, 검증 함수, 저장 함수, 예외 메시지 매핑을 제공하고, 회원 모듈은 활성 모듈의 계약만 읽어 가입 트랜잭션 안에서 실행합니다. 확장 입력은 `registration_extensions[...]` POST namespace 아래에서만 받으며, 회원가입 기본 필드명이나 다른 모듈 확장 필드명과 충돌하는 key는 렌더링하지 않습니다. 입력 타입은 좁게 `text`와 `checkbox`를 허용하고, `checkbox`는 선택 `1`과 미선택 `0`으로 정규화합니다. 쪽지 수신 허용처럼 데이터 소유권은 서비스 모듈에 있지만 가입 시점에 기본 row가 필요한 설정은 이 계약으로 입력을 받거나 기본값을 저장합니다. 가입 확장 설정을 회원 계정 정보에서도 관리하려면 제공 모듈이 선택 `account_values_function`, `account_validate_function`, `account_save_function`을 명시하고 자기 테이블의 조회·검증·저장을 계속 소유합니다. 회원 모듈은 조회와 저장 함수가 모두 준비된 계약 필드만 계정 화면에 포함합니다. 닉네임처럼 여러 서비스 모듈이 공유하는 공개 회원 식별자는 이 계약으로 각 서비스가 소유하지 않고 member 공개 이름 계약으로 둡니다.

회원의 게시물, 댓글, 도메인 권한, 그룹, 자산 원장, 알림과 서비스 설정은 로그인 아이디가 아니라 `sr_member_accounts.id`를 가리키는 `account_id` 계열 값으로 연결합니다. 로그인 아이디는 인증 조회용 `login_id_hash`와 기존 설치 호환용 `account_identifier_hash`에만 반영하므로 회원이 로그인 아이디를 바꿔도 도메인 연결은 유지됩니다. 변경 시 두 인증 해시를 같은 계정 트랜잭션에서 함께 교체하고 다른 계정의 두 해시와 모두 중복되지 않는지 확인합니다. 원문 로그인 아이디는 저장하지 않으므로 계정 화면은 현재 값을 재표시하지 않고 새 값을 입력한 경우에만 교체합니다.
...
포인트/적립금/예치금 원장 거래, 쿠폰·이용권 지급/사용/상태 변경/사용 환불/발급 환불, 쿠폰 정의 사용 중지로 인한 미사용 지급건 회수 안내, 콘텐츠/커뮤니티 댓글 작성자 알림, 콘텐츠/커뮤니티/퀴즈/설문 댓글 멘션 알림은 알림 모듈이 활성화되어 있으면 `sr_notification_event_templates`를 통해 회원 대상 알림을 만듭니다. 이벤트 알림은 생성 시점에 렌더링한 제목과 본문을 `sr_notifications.title`/`body_text`에 snapshot으로 저장하고, `source_module_key`, `event_key`, `metadata_json`은 감사/호환 metadata로 함께 보관합니다. 운영자 자유 편집 UI 도입 이후 과거 알림의 제목/본문 의미가 갈라지는 것을 막기 위해 화면, delivery 발송, privacy export는 저장된 제목 snapshot을 사용하며 현재 템플릿으로 제목을 재렌더하지 않습니다. 포인트/적립금/예치금/쿠폰 모듈은 자기 알림/메일 관리 화면에서 케이스별 회원 알림 제목, 본문, 사용 여부와 채널을 저장하고, 비활성 케이스는 원래 자산 처리를 실패시키지 않은 채 알림만 만들지 않습니다. 환전 출금/입금/수수료 알림 케이스도 자산 모듈의 알림/메일 관리 화면에서 편집합니다. 포인트 회원 알림 케이스의 기본값은 사용 안 함이고, 적립금/예치금/쿠폰 회원 알림 케이스는 기존 동작 보존을 위해 기본값을 사용으로 둡니다. 알림 모듈이 비활성화되어 있거나 설치되어 있지 않으면 no-op으로 처리하고, 자산 처리나 댓글 저장 자체를 실패시키지 않습니다. 회원 외부 푸시는 알림 모듈 provider와 회원별 수신처가 준비되어 있고 알림 생성 요청이 외부 채널을 명시적으로 제한하지 않은 경우 회원 계정 알림에 자동으로 발송 후보를 추가합니다. 커뮤니티 신고, 콘텐츠 등록자 신청, 개인정보 처리 요청, 이메일 발송 실패 표시, 저장소 정리 재시도 실패처럼 운영자가 조치해야 하는 이벤트는 별도 `sr_admin_notifications` 흐름을 사용하고 공개 회원 알림 목록에 섞지 않습니다.
...
사이트 공통 레이아웃과 콘텐츠/커뮤니티/퀴즈/설문 공개 레이아웃은 같은 레이아웃 option registry에서 고릅니다. 콘텐츠·커뮤니티·퀴즈·설문 공개 화면은 각 모듈 환경설정의 레이아웃 key를 하위 공개 화면 전체에 적용합니다. 필수 target은 콘텐츠 `content.home`, `content.group`, `content.view`, `content.form`, 커뮤니티 `community.home`, `community.group`, `community.list`, `community.post`, `community.form`, `community.search`, 퀴즈 `quiz.home`, `quiz.list`, `quiz.view`, `quiz.result`, 설문 `survey.home`, `survey.list`, `survey.view`, `survey.complete`입니다. 교차 모듈 레이아웃은 이 target 전체를 명시적으로 지원할 때만 해당 모듈 환경설정에 표시됩니다. 퀴즈와 설문은 `/quiz`, `/survey` 메인과 `/quiz/list`, `/survey/list` 전체 목록을 분리해 메인 장식과 목록 화면 구성을 독립적으로 유지합니다. 과거 콘텐츠별/그룹별 `layout_key` 저장값은 입력 기본값과 revision 호환 기록으로 남길 수 있지만, 공개 렌더링의 header/footer 레이아웃 선택은 모듈 환경설정이 우선합니다. 모듈별 세부 목록/상세/작성 화면의 본문 구성은 계속 해당 모듈의 layout/skin 책임으로 둡니다.
...
1차 범위는 커뮤니티 게시글/댓글 숨김/삭제에 한정한다. 일반 적립금 직접 회수, 퀴즈 직접 회수, 커뮤니티 회원 보상 로그 신규 회수, 설문/콘텐츠 회수 신규 구현, 자산 교환 정정 비차단화, 가역 운영의 stale open row 자동 해소, 자동 재회수와 임계 알림은 후속 작업이다. 보상을 받은 회원이 보상을 소진한 뒤 콘텐츠가 숨김/삭제되면 운영 액션은 막지 않고 미회수로 남기는 악용 표면이 있으며, 운영자는 관리자 미회수 화면에서 조회, 수동 재회수, 수동 해소/취소를 처리한다.
...
## 23. 콘텐츠·퀴즈·설문 영구 삭제는 redaction 이후 단계다

#404 결정 gate 결과, `content`, `quiz`, `survey`의 일반 삭제는 원문 redaction, 공개 비노출, 복구 불가 운영 보존, 소유 저장소 파일 삭제 시도를 맡는다. 영구 물리 삭제는 이미 redaction된 본체 row와 삭제로 배정된 live 하위 row를 DB에서 제거하는 단계이며, 소유 파일을 처음 삭제하는 단계가 아니다.

영구 삭제는 결제, 다운로드, 자산, 보상, 감사, 개인정보 처리 이력 같은 운영 증빙 로그를 “모든 기록 삭제”로 제거하지 않는다. 본체 row가 없어도 보존 로그와 cleanup failure/pending row는 snapshot 또는 자체 storage key 기준으로 조회와 재시도가 가능해야 한다. 콘텐츠는 `status = deleted`, 퀴즈와 설문은 `deleted_at IS NOT NULL`을 삭제 판정으로 사용하고, 퀴즈의 `status = archived`는 삭제와 동의어가 아니다. 세부 테이블 매트릭스와 cleanup failure 계약은 [이슈 #404 삭제 상태와 영구 삭제 계약](records/issue-404-delete-state-permanent-delete-contract-2026-07-05.md)을 따른다.

...
- `member` 모듈의 로그인 정책은 최초 설치 때 `이메일 + 로그인 아이디` 또는 `로그인 아이디만` 중 하나로 정한다
- 로그인 정책은 운영 중 변경하면 기존 계정의 접근 가능성이 달라질 수 있으므로 관리자 설정 화면에서는 읽기 전용으로 노출한다
- 이메일 로그인이 허용된 경우 조회는 `email_hash`를 사용하고, 로그인 아이디 조회와 중복 검사는 `login_id_hash`를 우선 사용한다

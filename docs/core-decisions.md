...
퀴즈와 설문처럼 운영자가 개별 공개 대상을 관리하는 모듈은 환경설정의 `theme_key`/`skin_key`를 기본값으로 두고, 개별 퀴즈나 개별 설문 row의 값이 비어 있으면 그 기본값을 상속합니다. 개별 row에 값이 있으면 해당 모듈의 정적 허용 목록으로 다시 검증한 뒤 공개 렌더링에 사용합니다. 테마는 CSS class hook과 표현 보조값이고, 스킨은 허용된 view 파일 묶음을 선택하는 include 매핑 값입니다.

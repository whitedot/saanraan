...
## 9. 모듈 계약 버전은 외부 모듈 호환성 기준이다

`core/version.php`의 `TOY_MODULE_CONTRACT_VERSION`은 외부 모듈이 코어와 맞춰야 하는 모듈 계약 버전입니다. 이 값은 코어 버전과 별개로 관리합니다. 코어 자체 버그 수정이나 문서 변경은 코어 버전을 바꿀 수 있지만, 외부 모듈의 구조나 동작 계약이 그대로라면 모듈 계약 버전은 유지합니다.
...
- 배포 패키지 생성, 모듈 zip 업로드, registry, 관리자 설치 흐름에서 외부 모듈이 맞춰야 하는 검증 규칙이 바뀜
...
- 공식 배포 조합이나 README 설명만 바뀌고 외부 모듈 구조 요구사항은 그대로임
...
3. docs/module-repository-strategy.md의 외부 모듈 운영 설명
4. docs/module-ci-template.yml의 검증 대상 코어 ref 또는 기대 계약
5. 공식 모듈 저장소의 module.php toycore.module_contract
```

외부 모듈 저장소는 먼저 로컬에서 `.tools/bin/check-external-module.php`를 실행해 현재 코어의 계약 버전과 자신의 `toycore.module_contract`가 맞는지 확인합니다. CI를 사용하는 저장소는 같은 명령을 GitHub Actions에서 자동 실행합니다. 계약 버전이 올라가면 로컬 점검이나 CI가 실패해야 하고, 모듈 제작자는 새 계약에 맞게 수정한 뒤 `module.php`의 계약 버전을 갱신합니다.

...
커뮤니티 게시글/댓글 보상 회수 실패는 커뮤니티 모듈이 `sr_community_asset_recovery_failures`를 소유해 기록한다. `asset_ledger`는 공통 primitive이며 통합 recovery 도메인 정책을 소유하지 않는다. 같은 원 지급 로그와 canonical reversal event key는 하나의 미회수 row로 수렴하고, 운영 종류는 제한된 `operation_context_json` 필드로 남긴다.

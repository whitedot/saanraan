...
커뮤니티 게시글/댓글 적립, 글쓰기/댓글 차감, 게시글 유료 열람, 첨부 다운로드 차감 정책은 커뮤니티 모듈이 소유합니다. 커뮤니티 모듈은 포인트, 적립금, 예치금 모듈을 필수 의존으로 만들지 않고 활성 자산 모듈의 잔액 조회와 원장 생성 helper만 호출합니다. 커뮤니티 자산 처리 로그는 `sr_community_asset_logs`에 남기고, 자산 원장 참조는 `community.post`, `community.comment`, `community.attachment` 같은 커뮤니티 도메인 참조로 제한합니다.

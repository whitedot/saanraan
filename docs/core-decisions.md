...
페이지 유료 열람, 다운로드 과금, 완료 버튼 자산 처리 정책은 페이지 모듈이 소유합니다. 완료 버튼 자산 처리는 공개 페이지에서 회원이 버튼을 눌렀을 때만 실행되며, 단순 페이지 조회는 포함하지 않습니다. 페이지 모듈은 포인트, 적립금, 예치금 모듈을 필수 의존으로 만들지 않고 활성 모듈의 잔액/원장 helper를 좁게 호출합니다. 관리자 자산 선택 UI에는 설치되어 있고 활성화된 자산 모듈만 표시합니다. 차감 계열 설정은 여러 자산을 함께 고를 수 있습니다. 차감 우선순위: 포인트 > 적립금 > 예치금. 열람/다운로드 차감 기록은 `sr_page_asset_access_logs`, 완료 버튼 지급/차감 기록은 `sr_page_asset_action_logs`에 페이지 모듈 책임으로 남기며, `account_id` 기반 기록이므로 개인정보 사본 제공에도 포함합니다.

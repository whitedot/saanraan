...
- `sr_content_link_refs`, `sr_community_link_refs`는 1.0 호환을 위해 테이블은 남기되, 신규 저장과 운영 복사/삭제 차단에서는 참조 원장으로 쓰지 않는다. 업데이트는 기존 legacy refs 행을 비워 보이지 않는 참조가 운영 삭제를 막지 않게 한다. 기존 본문에 legacy 링크 카드 토큰이 남아 있는 콘텐츠/게시글은 새 복사본을 만들기 전에 본문에서 토큰을 제거해야 한다.
...
게시판별 일반 회원 위임 관리권한은 커뮤니티 모듈의 `sr_community_board_managers`가 소유한다. 권한 key는 `view_manage`, `delete_post`, `remove_post_og_image`만 허용하고, `delete_post`는 해당 게시판 게시글 삭제 판정에만 합산한다. 위임 관리권한은 게시글 본문 수정 권한으로 자동 확대하지 않으며, 전역 관리자 권한과 작성자 본인 권한도 별도 helper에서 합산해 판단한다.

...
- 결제 record item 중 접근권 부여 item의 `reference_id`는 접근 대상과 접근 종류만 식별하고 raw account id를 포함하지 않는다. 탈퇴/익명화 cleanup은 record `account_id`를 끊기 전에 과거 접근권 item reference의 `:account:{id}` 흔적도 익명 marker로 바꾼다.

...
원본 `/ui-kit` 조회 화면은 초기/기본 공개 페이지와 public layout 런타임 기준 공통 원형을 확인하는 개발 보조 화면으로 보존합니다. 모듈 화면의 최종 디자인 정책은 원본 UI-KIT이 소유하지 않습니다. 모듈별 화면 조합, minimal profile에서 직접 소유하는 버튼/본문/목록 표현, 모듈 공개 타이포그래피는 해당 모듈이 `assets/ui-kit.css` 또는 `{module_key}-ui-kit.css`와 `views/ui-kit-samples/` 같은 전용 프리뷰 파일로 검증할 수 있습니다. 예를 들어 콘텐츠 모듈은 `/content/ui-kit`, 커뮤니티 모듈은 `/community/ui-kit` 사용자 화면에서 현재 공개 레이아웃 설정과 함께 각 공개 타이포그래피가 역할 토큰을 어떻게 소비하는지 확인합니다. 이 미리보기는 관리자 UI-KIT에서 설치된 모듈 링크로 접근할 수 있으므로, 일반 공개 요청의 활성 모듈 기준은 유지하되 UI-KIT layout context에서만 설치 모듈의 layout option을 함께 해석할 수 있습니다.

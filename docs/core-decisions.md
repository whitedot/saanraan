...
외부 모듈 저장소는 CI에서 `.tools/bin/check-external-module.php`를 실행해 현재 코어의 계약 버전과 자신의 `toycore.module_contract`가 맞는지 확인합니다. 계약 버전이 올라가면 외부 모듈 CI가 실패해야 하고, 모듈 제작자는 새 계약에 맞게 수정한 뒤 `module.php`의 계약 버전을 갱신합니다.

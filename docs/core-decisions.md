...
`sr_admin_account_roles`는 `owner`만 저장한다. 관리자 권한 화면은 회원 검색 결과 안에서 소유자 여부와 메뉴별 조회/수정/삭제 권한을 편집한다. 사이드바 메뉴 노출은 `view` 권한 기준이며, 저장·상태 변경은 `edit`, 명시적 삭제/숨김 처리는 `delete` 권한을 요구한다. 관리자 메뉴 표시 순서와 숨김 설정은 전체 관리자 경험을 바꾸는 전역 설정이므로 회원별 권한으로 위임하지 않고 소유자만 관리한다.
...
콘텐츠 유료 열람, 다운로드 과금, 완료 버튼 자산 처리 정책은 콘텐츠 모듈이 소유합니다. 완료 버튼 자산 처리는 공개 콘텐츠에서 회원이 버튼을 눌렀을 때만 실행되며, 단순 콘텐츠 조회는 포함하지 않습니다. 콘텐츠 모듈은 포인트, 적립금, 예치금 모듈을 필수 의존으로 만들지 않고 활성 모듈의 잔액/원장 helper를 좁게 호출합니다. 관리자 자산 선택 UI에는 설치되어 있고 활성화된 자산 모듈만 표시합니다. 여러 자산을 함께 고르는 설정은 선택한 자산마다 금액을 따로 저장하되, 완료 버튼 지급 방향은 한 항목만 허용하고 차감 방향은 여러 항목을 각 자산 잔액 기준으로 개별 확인합니다. 회원 그룹별 적용은 선택한 자산과 맞는 정책 세트만 사용할 수 있어야 합니다. 기존 단일 금액 설정은 호환용 합계로 유지합니다. 열람/다운로드 차감 기록은 `sr_content_asset_access_logs`, 완료 버튼 지급/차감 기록은 `sr_content_asset_action_logs`에 콘텐츠 모듈 책임으로 남기며, `account_id` 기반 기록이므로 개인정보 사본 제공에도 포함합니다. 최초 1회 열람/다운로드 접근권은 거래 원장이 아니라 `sr_content_access_entitlements`에 별도 기록하고, 탈퇴/익명화 시 이 접근권의 `account_id`와 원천 참조를 끊어 개인정보 보유기간과 서비스 접근 상태를 분리합니다.
...
쿠폰 사용처 후보는 쿠폰 모듈이 고정 목록으로 소유하지 않고, 실제 소비 모듈이 `coupon-targets.php` 계약으로 제공합니다. 쿠폰 모듈은 활성 모듈의 계약만 읽어 관리자 선택지와 저장 검증에 사용하며, 해당 모듈이 비활성화되면 새 쿠폰 종류 생성 후보에서도 빠집니다.

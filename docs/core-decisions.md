...
`seo`, `site_menu`, `banner`, `popup_layer`, `point`, `deposit`, `reward`, `notification` 모듈은 배포 조합에 따라 포함할 수 있지만, 인증과 관리자 진입에 필요한 필수 모듈은 아닙니다. 설치 화면에서 선택한 모듈만 설치 SQL을 실행하고 `toy_modules`에 등록합니다. 선택하지 않은 코드 모듈은 `/admin/modules`에서 나중에 설치할 수 있습니다.

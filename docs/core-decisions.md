...
`seo` 모듈은 이 원칙 위에서 sitemap/robots/default meta 같은 운영 기능을 맡는다. 현재 SEO 모듈의 설정은 전용 도메인 테이블을 만들지 않고 `sr_module_settings`에 저장하며, 콘텐츠별 SEO 값은 콘텐츠/커뮤니티 같은 콘텐츠 소유 모듈이 자기 테이블에서 판단한다.

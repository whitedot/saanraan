...
| `community` | `export_cleanup` | 제공 | 제공 | 게시글, 댓글, 스크랩, 신고, 신고 자동 임시 조치 이력, 계정 guard 이력/현재 상태, 동의 증적, 접근권, 보상 로그 등 회원 활동 데이터와 IP/UA hash를 가진다. 보상 미회수 운영 기준은 `asset_ledger` 공통 큐가 소유하며, community export/cleanup은 호환 기간의 legacy `sr_community_asset_recovery_failures` row만 정리한다. |

...
| #159 특별범주/연령/고유식별자성 | `member_oauth`, `antispam_captcha_providers`, 본인확인 예정 플러그인, `quiz`, `survey` | 외부 provider profile, 연령/성인 인증, 설문/퀴즈 응답에서 민감정보가 생길 수 있는 경우 원문 저장 금지와 export 제외 기준을 둔다. |

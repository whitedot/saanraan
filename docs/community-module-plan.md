...
- `.tools/bin/smoke-community-auth.php` 인증 HTTP smoke(로그인 후 작성 결과/댓글/스크랩/쪽지 발송·수신·발신자 삭제 흐름, 쪽지 폼/답장 링크의 숫자 회원 id 비노출과 공개 해시 링크, 두 번째 계정으로 게시글 신고, 해당 신고 관리자 처리, 관리자 댓글 숨김과 게시글 숨김)

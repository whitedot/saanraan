...
| 릴리스 준비 | 진행 중 | `php .tools/bin/check.php`는 통과 중이며, HTTP smoke에 커뮤니티 공개/쪽지 진입점 점검이 포함됨 |

현재 남은 우선순위:

1. 모듈 버전/update SQL 정책을 릴리스 전 점검한다.
2. 인증이 필요한 커뮤니티 작성/쪽지/스크랩 흐름은 수동 HTTP smoke로 확인한다.
...
- `.tools/bin/smoke-http.php` 기본 HTTP smoke

...
1. 실제 로그인 계정과 게시판/수신자 데이터가 준비된 환경에서 `.tools/bin/smoke-community-auth.php`를 실행해 작성/댓글/쪽지/스크랩 성공 흐름을 확인한다.
...
- `.tools/bin/smoke-community-auth.php` 인증 HTTP smoke(로그인 후 작성, 댓글 작성, 스크랩, 쪽지 발송)
- 설치 전/후 시나리오 확인

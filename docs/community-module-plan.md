...
| 릴리스 준비 | 진행 중 | `php .tools/bin/check.php`는 커뮤니티 v1 릴리스 기준, 계약 파일 구조, 스타일 경계, 회원 전용 action guard, 계약 파일 선언/존재, 모듈 버전/update SQL 정책을 확인하고, HTTP smoke에 커뮤니티 public/admin 진입점, 비로그인 작성/쪽지/스크랩 guard, 내부 파일 보호 점검이 포함됨 |

현재 남은 우선순위:

1. 실제 로그인 세션이 필요한 커뮤니티 작성/쪽지/스크랩 성공 흐름은 수동 HTTP smoke로 확인한다.

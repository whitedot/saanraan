...
- 번들 `common.basic`, `content.basic`, `community.basic`, `quiz.basic`, `survey.basic` 푸터는 사이트 설정의 `site.business_info_items` 중 값이 있는 사업자 정보만 저장 순서대로 표시한다. 출력은 core 설정 helper를 사용하되 CSS class는 선택된 layout provider namespace를 유지한다.

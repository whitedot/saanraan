...
회원 탈퇴/익명화 시 member 모듈은 설치된 모듈의 `privacy-cleanup.php` 계약을 탈퇴 트랜잭션 안에서 실행한다. 관리자가 회원 상태를 탈퇴 또는 익명화로 직접 바꾸는 경우에도 같은 계약을 실행한다. 콘텐츠와 커뮤니티 모듈은 이 계약에서 접근권 테이블의 회원 연결을 제거한다. 커뮤니티 모듈은 커뮤니티 닉네임과 계정별 레벨 스냅샷/레벨 변경 로그도 삭제한다. 커뮤니티 게시글/댓글 작성자와 커뮤니티 시리즈 소유자는 해시가 아니라 내부 계정 ID이며 편집, 삭제, private 시리즈 열람 같은 권한 판정에 쓰이므로 null 처리하지 않고 계정 행 익명화로 처리한다. 콘텐츠/커뮤니티 시리즈의 nullable 작성자, 수정자, 검토자 메타데이터와 시리즈 항목 작성자 메타데이터는 cleanup에서 null 처리한다. 커뮤니티 닉네임은 멤버 간 중복을 허용하지 않으며, 가입과 커뮤니티 닉네임 설정 시 서버 검증과 DB 유니크 제약으로 함께 막는다. 탈퇴 또는 익명화 상태의 회원은 커뮤니티 공개 이름과 멤버 관리 목록에서 재식별 가능한 닉네임을 사용하지 않는다.

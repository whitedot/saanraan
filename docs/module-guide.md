...
- 관리자 화면을 가진 모듈은 자기 화면 조합을 확인하기 위해 모듈 전용 UI-KIT 조회 화면을 둘 수 있다. 이때 route와 view/action은 소유 모듈에 두고, 보조 스타일은 `modules/{module_key}/assets/reset.css`, `modules/{module_key}/assets/ui-kit.css`, `modules/{module_key}/assets/ui-kit-layout.css`, 샘플은 `modules/{module_key}/views/ui-kit-samples/`처럼 모듈 내부에 둔다. 콘텐츠/커뮤니티/퀴즈/설문처럼 공개 화면 전용 UI 원형을 가진 모듈은 모듈 내부에 복제한 `assets/reset.css`, `assets/ui-kit.css`, `assets/module.css`, `assets/module.js`를 공개 화면 context helper에서 호출하고 `consumer_domain`을 전달한다. 레이아웃 stylesheet와 script는 현재 화면 모듈이 아니라 선택된 `layout_key`의 제공자 namespace를 기준으로 core output helper가 주입한다. 예를 들어 커뮤니티 화면이 `content.*` 레이아웃을 선택하면 `/modules/content/assets/layout.css`, `/modules/content/assets/layout.js`, `/modules/community/assets/module.js` 조합을 사용한다. 공통 레이아웃 `common.*` 안에서 렌더링할 때는 공통 layout이 `/assets/layout.css`, header/footer, layout script를 소유하므로 모듈 레이아웃 stylesheet와 script를 중복 호출하지 않는다. 모듈 UI-KIT 화면은 원본 `/ui-kit`의 `layouts/public/basic/ui-kit-samples/` 파일을 그대로 복제한 전체 샘플 구성과 같은 호출 조합에 `assets/ui-kit-layout.css`를 더해 실제 공개 화면 기준을 확인한다. 기본 호출 순서는 모듈 `reset.css`, 모듈 `ui-kit.css`, 선택된 layout provider의 레이아웃 stylesheet, 모듈 `module.css`, UI-KIT 화면에서는 모듈 `ui-kit-layout.css` 추가다.
...
- 외부 공개 테마와 외부 스킨은 [스킨·테마 패키지 기준](skin-theme-packages.md)의 manifest 계약을 따른다. 외부 테마는 레이아웃 후보가 아니라 `source_type=external_theme`인 정규화 theme option으로 합쳐지고, 사이트 설정의 `public_theme_key` 또는 각 public 모듈 환경설정의 `theme_key`로 선택한다. 외부 스킨은 community, quiz, survey의 v1 view 계약에 맞는 경우에만 각 모듈 스킨 선택지에 합쳐진다.
- 번들 `content.basic`, `community.basic`, `quiz.basic`, `survey.basic`은 공통 레이아웃 파일을 공유하지 않는다. 콘텐츠는 `modules/content/layouts/basic/layout.php`, 커뮤니티는 `modules/community/layouts/basic/layout.php`, 퀴즈는 `modules/quiz/layouts/basic/layout.php`, 설문은 `modules/survey/layouts/basic/layout.php`를 사용한다. 헤더/푸터처럼 같은 시각 언어를 쓰더라도 모듈 전용 레이아웃 CSS를 사용해 모듈 경계를 유지한다.
...
모듈은 DB에 view 파일 경로를 저장하지 않는다. `public_layout_key`, `public_theme_key`, `layout_key`, `theme_key`, `skin_key`, `{module_key}_skin_key` 같은 key만 저장하고, 실제 파일 경로는 모듈 helper의 allowlist, `layout-options.php` 계약, 검증된 패키지 manifest에서 결정한다. 기존 공개 레이아웃 `basic` 값은 `common.basic`으로 정규화한다. 알 수 없는 사이트 공통 레이아웃 key 또는 현재 route 도메인을 지원하지 않는 레이아웃 key는 기본 공통 레이아웃으로 fallback한다. 사이트 설정의 공통 레이아웃 선택지는 기본 공통 레이아웃을 먼저 표시하고, 모듈 제공 레이아웃은 관리자 사이드바의 모듈 순서대로 정렬한다. 콘텐츠, 커뮤니티, 퀴즈, 설문처럼 공개 레이아웃과 테마를 설정하는 모듈은 모듈 환경설정의 레이아웃 key와 테마 key를 메인, 그룹/게시판, 상세/응시/응답 하위 화면 전체에 적용한다.
...
번들 콘텐츠 모듈은 `content.view` point와 `before_content`, `after_content` content slot을 제공한다. 배너/팝업레이어 관리 화면에서 콘텐츠 전체 또는 특정 콘텐츠 ID를 대상으로 출력 규칙을 저장할 수 있고, 콘텐츠 관리자 화면에서는 공용 배너/팝업레이어를 직접 선택할 수도 있다. 콘텐츠 그룹은 `sr_content_groups`가 소유하고 콘텐츠는 `sr_content_items.content_group_id`로 선택적으로 연결한다. 콘텐츠 환경설정의 콘텐츠 공개 레이아웃은 공개 콘텐츠 메인, 콘텐츠 그룹 목록, 콘텐츠 상세의 header/footer 레이아웃 선택에 전체 적용된다. 콘텐츠 공개 테마는 같은 환경설정의 `theme_key`로 별도 저장한다. 콘텐츠 생성/수정 화면의 상태, 대표/OG 이미지, 배너, 팝업레이어, 유료 열람, 완료 버튼 포인트/금액 처리는 현재 콘텐츠에 저장하며, 운영자가 선택한 `그룹`/`전체` 적용 옵션은 현재 편집값을 같은 그룹 또는 전체 콘텐츠에 한 번 복사한다.

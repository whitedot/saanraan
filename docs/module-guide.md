...
- 번들 `markdown_editor`의 기본 stylesheet는 `github-markdown-css` 5.9.0의 요소 구성을 `.markdown-editor-body` 범위와 산란 테마 토큰으로 재구성한 값으로 시작한다. 관리자 설정은 전체 본문, 문단/인라인 강조, 제목, 링크, 목록/작업 목록, 인용, 인라인 코드, 코드 블록, 표, 구분선 섹션에서 주요 속성을 조정한다.
- 관리자 화면은 실제 Markdown 원문 편집기와 서버 렌더링 결과를 나란히 보여주는 전체 폭 작업대로 구성한다. 별도 편집/분할/미리보기 버튼 묶음은 두지 않고 원문과 렌더링 결과 패널 헤더에 각각 펼치기 버튼을 둔다. 각 버튼은 해당 패널만 펼치며 단독 보기 상태에서는 같은 버튼이 분할 보기 복원으로 바뀐다. 운영자가 렌더링 결과의 문단, h1-h6, 링크, 목록, 인용, 인라인 코드, 코드 블록, 표, 구분선을 클릭하면 렌더링 결과 상단 컨텍스트 툴바가 해당 대상으로 바뀐다. 상단 툴바는 `Margin`, `Padding`, `Border`, `Text`, 요소 고유 속성을 에디터형 아이콘 버튼과 접근성 label/title로 제공한다. 아이콘을 누르면 값 입력 영역이 렌더링 캔버스 위에 absolute popover 서브메뉴로 열려 문서 흐름과 미리보기 높이를 바꾸지 않는다. 같은 아이콘 재클릭, 바깥 클릭, Escape로 닫고 Escape 닫기에서는 아이콘으로 focus를 복귀시킨다. 숫자·선택값 변경은 같은 `sr-control` CSS 선언을 갱신하고 짧은 debounce 뒤 서버 렌더링 결과에 반영한다. light/dark 색상 모드는 작업대 상단에 항상 표시한다. 별도 미리보기 배율은 제공하지 않는다. CSS 아이콘은 원본/변경값 모드와 CSS 원문만 담은 프로젝트 공통 fullscreen overlay 모달을 연다. 설정 저장과 참고 프로젝트 표기는 모달 안이 아니라 편집기 하단 작업 영역에 둔다. 표·체크 목록·코드 블록 문법은 운영 선택 없이 항상 사용하며 raw HTML 입력은 계속 허용하지 않는다. 실제 저장 control은 화면에 보이지 않는 form template으로 유지해 상단 툴바와 CSS 원문이 같은 값을 공유한다. 모든 선택 가능 요소는 공통 Box model 계약을 가지며 위·오른쪽·아래·왼쪽 margin, padding, border-width를 각각 설정하고 border style, border color token, radius를 설정할 수 있다. 텍스트가 있는 모든 요소는 공통 Text style 계약을 추가로 가지며 font family, size, weight, line-height, letter/word spacing, alignment, font style, decoration, text transform, color token을 같은 방식으로 설정한다.
- `style_source_mode`는 `default`와 `custom`을 저장한다. `default`는 저장된 사용자 stylesheet를 지우지 않고 번들 `github-markdown-css` 기반 원본을 공개 출력하며, `custom`은 운영자가 만든 전체 stylesheet를 출력한다. 원본 모드에서 상단 툴바 속성이나 CSS 원문을 편집하면 관리자 화면은 custom 모드로 전환한다. 요소별 컨트롤은 stylesheet의 `sr-control` 주석 뒤에 있는 실제 property 선언을 갱신한다. CSS를 직접 바꾸면 짧은 debounce 뒤 서버 검증을 거쳐 반환된 CSS가 라이브 미리보기와 인식 가능한 툴바 선언에 반영되며, CSS 모달은 반영 중·완료·오류 상태를 표시한다. 툴바의 재시작 아이콘은 현재 선택 요소만 초기화하고, CSS 모달의 전체 초기화는 stylesheet 전체에만 적용해 바깥의 색상 모드·보기 방식과 Markdown 원문은 유지한다. 저장 stylesheet의 모든 selector는 `.markdown-editor-body`로 시작해야 하며 at-rule, 외부 URL, 실행형 CSS 표현과 `</style` 종료 문자열은 거부한다.

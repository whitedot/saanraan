...
- 번들 `markdown_editor`의 기본 stylesheet는 `github-markdown-css` 5.9.0의 요소 구성을 `.markdown-editor-body` 범위와 마크다운 전용 `--md-*` 토큰으로 재구성한 값으로 시작한다. `--md-text`, `--md-info`, surface, border, 상태 토큰은 `.markdown-editor-body` 안에서 공통 `--color-*` 값에 연결되어 사이트 light/dark 모드를 따르며 마크다운 밖의 UI에는 영향을 주지 않는다. 이전 `var(--sr-*)` 마크다운 프로필과 stylesheet는 대응하는 `var(--md-*)`로 변환한다. 관리자 렌더링 미리보기는 공개 foundation과 같은 light/dark `--color-*` 값을 사용해 설정 화면과 실제 사용자 화면의 토큰 계산값을 맞춘다. 전체 본문 색 토큰을 바꾸면 아직 기본 본문색을 따르는 문단, H1-H5, 목록, 코드, 표의 요소별 색 선언도 함께 바꾸며, 별도 상태색을 지정한 H6, 링크, 인용과 운영자가 독립적으로 바꾼 요소 색은 유지한다. 인용의 Typography 색은 렌더링된 `blockquote` 내부 문단에도 직접 적용해 일반 문단색 선언에 덮이지 않게 한다. 일반 본문과 제목은 별도 `font-family`를 저장하거나 출력하지 않고 사이트 기본 글꼴을 상속하며, 코드 요소는 사이트 공통 reset의 monospace 규칙을 따른다. 관리자 설정은 전체 본문, 문단/인라인 강조, 제목, 링크, 목록/작업 목록, 인용, 인라인 코드, 코드 블록, 표, 구분선 섹션에서 주요 속성을 조정한다. 표는 헤더 행 다음에 열별 `---` 구분선 행이 있을 때 하나의 블록으로 인식하고, 구분선의 콜론에 따른 좌·중앙·우 정렬과 이후 본문 행을 `<thead>`/`<tbody>`로 출력한다.
...
- 관리자 화면은 실제 Markdown 원문 편집기와 서버 렌더링 결과, 오른쪽 속성 인스펙터를 함께 보여주는 작업대로 구성한다. 데스크톱에서는 원문·렌더링 작업 영역과 속성 인스펙터가 같은 grid 행과 높이를 사용하고 각 영역 내부만 스크롤하며, 좁은 화면에서는 인스펙터를 작업대 아래로 내려 자동 높이를 사용한다. 별도 편집/분할/미리보기 버튼 묶음은 두지 않고 원문과 렌더링 결과 패널 헤더에 각각 펼치기 버튼을 둔다. 각 버튼은 해당 패널만 펼치며 단독 보기 상태에서는 같은 버튼이 분할 보기 복원으로 바뀐다. 운영자가 렌더링 결과의 문단, h1-h6, 링크, 목록, 인용, 인라인 코드, 코드 블록, 표, 구분선을 클릭하면 오른쪽 인스펙터가 해당 요소의 설정으로 바뀐다. 인스펙터는 Figma 계열 속성 패널처럼 선택 요소 제목과 초기화 아이콘, `Margin`, `Padding`, `Stroke`, `Typography`, `Fill` 같은 접이식 섹션, 2열 입력 그리드를 제공한다. 숫자·선택값 변경은 같은 `sr-control` CSS 선언을 갱신하고 짧은 debounce 뒤 서버 렌더링 결과에 반영한다. 렌더링 결과의 초기 light/dark 상태는 설정 화면 진입 시 계산된 현재 관리자 `data-theme`을 따르고, 이후에는 렌더링 결과 헤더의 아이콘 버튼으로 독립 전환한다. 현재 상태에 따라 dark/light 아이콘과 접근성 안내를 바꾼다. 별도 미리보기 배율은 제공하지 않는다. 렌더링 결과 헤더의 CSS 아이콘은 전체 CSS textarea와 닫기 버튼만 있는 프로젝트 공통 fullscreen overlay 모달을 연다. 편집기 하단에는 GitHub 참고 링크, `기본`/`커스텀` 스타일 적용 라디오 토글, 저장 버튼을 스티키 제출 영역으로 제공한다. 표·체크 목록·코드 블록 문법은 운영 선택 없이 항상 사용하며 raw HTML 입력은 계속 허용하지 않는다. 모든 선택 가능 요소는 공통 Box model 계약을 가지며 위·오른쪽·아래·왼쪽 margin, padding, border-width를 각각 설정하고 border style, border color token, radius를 설정할 수 있다. 텍스트가 있는 모든 요소는 공통 Typography 계약을 추가로 가지며 size, weight, line-height, letter/word spacing, alignment, font style, decoration, text transform, color token을 같은 방식으로 설정한다.

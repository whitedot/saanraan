...
퀴즈와 설문은 각각 `sr_quiz_groups`, `sr_survey_groups`가 운영 묶음을 소유하고 개별 항목은 nullable 그룹 ID로 연결한다. `/admin/quiz/groups`와 `/admin/surveys/groups`에서 그룹을 관리하며 그룹 삭제 시 항목을 삭제하지 않고 그룹 연결만 해제한다. 퀴즈는 화면 표시, 공개 상태/기간, 채점, 응시 제한, 댓글, 리액션, 댓글 추가 입력, 보상 정책을 적용 범위로 제공한다. 설문은 화면/목록 표시, 공개 상태/기간, 참여 제한, 동의/개인정보 안내, 댓글, 리액션, 댓글 추가 입력, 보상 정책을 적용 범위로 제공한다. 제목, 설명, 퀴즈 문항/결과 규칙, 설문 문항/연구·QA 기록은 개별 콘텐츠이므로 범위 복사 대상에 포함하지 않는다. `sr_quiz_setting_sources`와 `sr_survey_setting_sources`는 마지막 선택 범위를 관리자 화면에 다시 표시하기 위한 기록이며 런타임 상속 계약이 아니다.

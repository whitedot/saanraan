...
- 평가 실행은 회원 모듈의 로그인 성공, 회원 목록의 단일 회원 재평가, 회원 그룹 규칙 화면의 그룹 기준 평가, 그리고 각 제공 모듈이 명시적으로 연결한 도메인 이벤트에서 일어난다. 그룹 기준 평가는 대상 그룹에 이미 속한 회원과 운영자가 뱃지로 추가한 제외 그룹에 속한 회원을 후보에서 제외한 뒤, 대상 그룹의 활성 규칙을 만족하는 회원을 자동 배정한다. 등록된 회원 그룹이 없으면 그룹 기준 재평가 버튼을 표시하지 않고, 등록된 회원 그룹이 1개이면 제외 그룹 선택을 제공하지 않으며, 2개 이상이면 대상 그룹 외 그룹만 제외 그룹으로 추가할 수 있다. 예를 들어 콘텐츠 모듈은 유료 열람, 유료 파일 다운로드, 완료 버튼 포인트/금액 처리 성공 직후 `source_module_key=content` 규칙만 재평가한다.

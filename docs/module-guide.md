...
회원 탈퇴/익명화 시 member 모듈은 설치된 모듈의 `privacy-cleanup.php` 계약을 탈퇴 트랜잭션 안에서 실행한다. 관리자가 회원 상태를 탈퇴 또는 익명화로 직접 바꾸는 경우에도 같은 계약을 실행한다. 콘텐츠와 커뮤니티 모듈은 이 계약에서 접근권 테이블의 회원 연결을 제거한다. 커뮤니티 모듈은 커뮤니티 닉네임과 계정별 레벨 스냅샷/레벨 변경 로그도 삭제한다. 커뮤니티 닉네임은 멤버 간 중복을 허용하지 않으며, 가입과 커뮤니티 닉네임 설정 시 서버 검증과 DB 유니크 제약으로 함께 막는다. 탈퇴 또는 익명화 상태의 회원은 커뮤니티 공개 표시명과 멤버 관리 목록에서 재식별 가능한 닉네임을 사용하지 않는다.

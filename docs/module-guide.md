...
- 회원 환경설정의 회원가입/탈퇴/계정보안 본인확인 항목은 본인확인 모듈이 설치되어 있고 활성화된 경우에만 편집 가능하다. 본인확인 모듈이 없으면 화면에서 사용할 수 없다는 안내를 보여주고, 저장 action은 본인확인 요구 설정을 거부한 뒤 disabled 상태로 정규화한다.

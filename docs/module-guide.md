...
유료 쿠폰 발급 캠페인은 `sr_coupon_claim_campaigns`에 무료/유료 발급 유형, 유료 가격/통화, 허용 포인트/금액 항목을 저장한다. 허용 포인트/금액 항목은 회원 선택의 강제 제약이며, 선택한 항목으로 부족하면 발급 전 실패한다. 무료/유료 claim helper는 transaction 안에서 campaign row를 `FOR UPDATE`로 잠근 뒤 발급 한도 검증, claim row 점유, 자산 차감, 발급본 생성을 진행해 서로 다른 nonce의 동시 발급도 같은 캠페인 한도 아래에서 직렬화한다.

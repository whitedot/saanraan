...
- public layout이 사이트 메뉴를 노출할 때는 하위 depth가 아니라 레이아웃 영역 구분으로 주 메뉴와 추가 메뉴 목록을 사용한다. 콘텐츠·커뮤니티·퀴즈·설문은 주 메뉴 key를 `sr_public_layout_begin()` layout context의 `site_menus.primary`로 전달하고, 필요한 만큼 추가한 메뉴 key 목록을 `site_extra_menus` 배열로 전달한다. 주 메뉴는 기본적으로 `header` 사이트 메뉴를 사용하며, 추가 메뉴는 기본 고정 슬롯을 만들지 않고 관리자 화면에서 항목을 추가한 순서대로 저장한다. 서비스 모듈의 공개 layout context는 화면 소유 모듈 표시용 `module_home_url`, `module_label`, `module_menu_label`도 함께 전달한다. layout shell의 namespace, layout asset, logo provider key, banner layout slot은 선택된 provider 기준을 유지하지만, header의 모듈명/모듈 홈/메뉴 label은 현재 화면 소유 모듈 기준으로 출력한다. 관리자 화면에서는 `주 메뉴`와 `추가 메뉴` 항목 목록으로 표시하고, 선택지는 `사용 안 함`, 모듈 내장 메뉴, 사이트 메뉴 순서로 둔다. 콘텐츠 내장 메뉴 key `sr_content_groups`는 콘텐츠 그룹을, 커뮤니티 내장 메뉴 key `sr_community_board_groups`는 게시판 그룹을 렌더링한다. 실제 위치는 레이아웃 구현이 결정한다. 번들 공통/콘텐츠/커뮤니티/퀴즈/설문 레이아웃은 주 메뉴를 header에, 추가 메뉴 목록을 footer 영역에 렌더링한다. 배너 모듈은 `layout-options.php` 제공자를 기준으로 `{module}.layout/before_layout`, `{module}.layout/before_footer` 위치를 관리자 후보에 추가하며, 번들 서비스 레이아웃은 각각 header 앞과 footer 앞에서 렌더링한다.
...
- 콘텐츠·커뮤니티·퀴즈·설문 환경설정에서는 번들 공개 레이아웃의 주 메뉴를 선택하고, 추가 메뉴는 항목 추가 방식으로 필요한 만큼 연결할 수 있다. 콘텐츠는 `사용 안 함` 다음에 내장 `콘텐츠 그룹` 메뉴를, 커뮤니티는 내장 `게시판 그룹` 메뉴를 표시하고, 그 뒤에 사이트 메뉴를 표시한다. 퀴즈와 설문은 사이트 메뉴를 표시한다.

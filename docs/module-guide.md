...
- 앱아이콘 기본 용도 `public.favicon`은 `사용자 화면 심볼로 사용` 옵션을 저장할 수 있다. 기본 공개/콘텐츠/커뮤니티/퀴즈/설문 레이아웃은 사용자화면 PC/모바일 로고가 둘 다 없을 때 이 심볼 후보를 브랜드 이미지 fallback으로 사용한다. 사용자화면 PC 또는 모바일 로고 중 한쪽만 있으면 다른 화면에서도 같은 로고를 fallback으로 사용한다. 별도 레이아웃에서는 `sr_logo_manager_public_symbol_logo()`, `sr_logo_manager_public_symbol_url()`, 또는 `sr_logo_manager_render_public_symbol_logo()`를 명시적으로 호출해야 반영된다.

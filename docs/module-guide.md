...
- CKEditor도 같은 본문 독립성 계약을 따른다. CKEditor plugin stylesheet는 `.sr-ckeditor .ck-content` 편집 영역에 scoped reset을 적용하고, CKEditor 실효 에디터로 출력되는 저장 HTML 본문은 공통 `/assets/editor-ck.css`가 `.content-body`와 `.community-post-body` 안에서 목록, 링크, 표, 코드, 미디어의 본문 기본값을 다시 선언한다. 직접 HTML 입력 본문은 저장 형식이 `html`이어도 이 CKEditor reset을 호출하지 않는다. 화면 모듈의 theme `assets/module.css`는 본문 컨테이너 배치와 색 토큰 같은 화면 스타일만 소유한다.
...
무레이아웃의 허용된 본문도 일반 콘텐츠 상세와 같은 `content-body` wrapper로 출력한다. layout shell은 호출하지 않지만 선택된 콘텐츠 테마의 `reset.css`를 먼저 로드해 편집 화면과 같은 reset 및 theme token baseline을 만든다. CKEditor 판정은 본문 렌더링과 같은 실효 포맷을 사용하므로 기존 row의 원본 `body_format`이 `plain`이어도 실효 에디터가 CKEditor이면 HTML 본문으로 처리한다. CKEditor HTML 본문은 `.sr-ckeditor .ck-content` 구조와 번들 `ckeditor5.css`, 프로젝트 `saanraan-ckeditor.css`를 사용해 제목, 목록, 표, 인용, 코드, 이미지 정렬·캡션 표현만 유지한다. `.ck-editor__main`, `.ck-editor__editable_inline`, `ck-read-only` 같은 입력 widget 구조는 공개 화면에 출력하지 않으므로 편집기 테두리, 안쪽 여백, 최소 높이가 생기지 않는다. Markdown 본문은 `/assets/editor-md.css`와 renderer stylesheet를 그대로 사용한다.

...
- 레이아웃 제공 모듈은 `layout-options.php` 계약으로 `common.basic`, `content.basic`, `community.basic`, `quiz.basic`, `survey.basic` 같은 namespace 포함 key와 allowlist view를 제공할 수 있다.
...
모듈은 DB에 view 파일 경로를 저장하지 않는다. `public_layout_key`, `public_theme_key`, `layout_key`, `theme_key`, `skin_key`, `{module_key}_skin_key` 같은 key만 저장하고, 실제 파일 경로는 모듈 helper의 allowlist, `layout-options.php` 계약, 검증된 패키지 manifest, 또는 모듈 내부 `theme/{theme_key}` 디렉터리에서 결정한다. 기존 공개 레이아웃 `basic` 값은 `common.basic`으로 정규화한다. 알 수 없는 사이트 공통 레이아웃 key 또는 현재 화면 target을 지원하지 않는 레이아웃 key는 기본 공통 레이아웃으로 fallback한다. 사이트 설정의 공통 레이아웃 선택지는 기본 공통 레이아웃을 먼저 표시하고, 모듈 제공 레이아웃은 관리자 사이드바의 모듈 순서대로 정렬한다. 콘텐츠, 커뮤니티, 퀴즈, 설문처럼 공개 레이아웃과 테마를 설정하는 모듈은 모듈 환경설정의 레이아웃 key와 테마 key를 메인, 그룹/게시판, 목록, 상세/응시/응답/완료 하위 공개 화면 전체에 적용한다. 이 환경설정의 레이아웃 선택지는 각 모듈의 필수 화면 target 전체를 지원하는 option만 표시하고 저장한다.

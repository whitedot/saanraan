...
- CKEditor도 같은 본문 독립성 계약을 따른다. CKEditor plugin stylesheet는 `.sr-ckeditor .ck-content`에 본문 표현을 적용한다. 콘텐츠 상세는 레이아웃 사용 여부와 무관하게 `sr_content_public_body_html()`이 이 구조를 만들고 번들 `ckeditor5.css`와 프로젝트 `saanraan-ckeditor.css`를 호출한다. 공개 출력 wrapper의 `data-sr-editor-output`은 `.ck-content` 배경만 투명하게 만들며 입력 편집기의 surface 배경은 유지한다. 커뮤니티의 CKEditor 저장 HTML 본문은 공통 `/assets/editor-ck.css`가 `.community-post-body` 안에서 목록, 링크, 표, 코드, 미디어의 본문 기본값을 다시 선언한다. 직접 HTML 입력 본문은 저장 형식이 `html`이어도 CKEditor stylesheet를 호출하지 않는다. 화면 모듈의 theme `assets/module.css`는 본문 컨테이너 배치와 색 토큰 같은 화면 스타일만 소유한다.

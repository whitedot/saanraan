...
- 회원 환경설정의 회원가입/탈퇴/계정보안 본인확인 항목과 커뮤니티, 콘텐츠, 퀴즈, 설문, 적립금 출금, 예치금 환불, 자산 환전 같은 소비 모듈의 본인확인 요구 항목은 본인확인 모듈이 설치되어 있고 활성화된 경우에만 편집 가능하다. 본인확인 모듈이 없으면 화면에서 사용할 수 없다는 안내를 보여주고, 저장 action은 본인확인 요구 설정을 거부한 뒤 disabled 상태로 정규화한다.
- 생년월일 사용은 본인확인 환경설정의 별도 opt-in이다. 켜진 경우 회원가입 본인확인 결과의 생년월일과 성인 여부를 회원 프로필 생년월일/성인 여부처럼 반영하고 잠근다.
- 성인확인 정책은 생년월일 사용이 켜져 있을 때만 저장할 수 있다. 현재 적용 대상은 커뮤니티 성인 게시판, 콘텐츠 작성자 성인 확인, 콘텐츠 열람 성인 확인, 퀴즈 참여 성인 확인, 설문 참여 성인 확인이다.

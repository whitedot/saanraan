...
회원 탈퇴/익명화 시 member 모듈은 설치된 모듈의 `privacy-cleanup.php` 계약을 탈퇴 트랜잭션 안에서 실행한다. 관리자가 회원 상태를 탈퇴 또는 익명화로 직접 바꾸는 경우에도 같은 계약을 실행한다. 커뮤니티 모듈은 이 계약에서 커뮤니티 닉네임과 계정별 레벨 스냅샷/레벨 변경 로그를 삭제한다. 커뮤니티 닉네임은 멤버 간 중복을 허용하지 않으며, 가입과 커뮤니티 닉네임 설정 시 서버 검증과 DB 유니크 제약으로 함께 막는다. 탈퇴 또는 익명화 상태의 회원은 커뮤니티 공개 표시명과 멤버 관리 목록에서 재식별 가능한 닉네임을 사용하지 않는다.
...
`privacy` 모듈은 개인정보 처리 요청과 개인정보 사본 제공 흐름을 조정한다. 회원 계정, 인증, 동의처럼 `member`가 소유한 데이터도 `modules/member/privacy-export.php`로 제공하고, 게시판, 커머스, 예약, 알림, 회원 자산 같은 확장 모듈의 개인정보도 각 모듈이 `privacy-export.php`로 제공한다. 포인트/적립금/예치금 원장, 쿠폰 발급/사용 로그, 콘텐츠/커뮤니티 계정별 자산 처리 로그처럼 `account_id`에 연결되는 운영 기록은 개인정보 사본 제공 대상인지 먼저 검토한다.

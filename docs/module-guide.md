...
게시판별 추가 입력 항목은 커뮤니티 모듈 안의 확장 정의/값 테이블로 분리한다. 현재 게시글 확장은 `sr_community_board_field_definitions`, `sr_community_post_field_values`를 사용하고, 댓글 확장이 필요할 때는 `sr_community_comment_field_values` 같은 별도 값 테이블을 둔다. core/member 테이블이나 `sr_community_posts`를 게시판별 필드로 넓히지 않는다. 게시판 그룹 추가 입력 항목을 저장하면 그룹 값을 상속하는 게시판의 정의 테이블도 동기화한다. `field_key`는 관리용 key이므로 소문자 영문, 숫자, `_`만 허용하고, 필수/조건부 필드는 관리자 UI, 공개 UI, 서버 POST 검증을 맞춘다. 필수 checkbox는 서버에서도 체크된 값만 유효하게 처리한다. 추가 입력 정의는 잘못된 key, 중복 key, 빈 라벨, 잘못된 유형/정책, 선택지 누락을 조용히 버리지 않고 저장 오류로 처리한다. 추가 입력값은 text/select 1000자, textarea 5000자를 넘거나 배열 payload이면 잘라 저장하지 않고 서버에서 거부한다. 개인정보 성격의 필드는 동의 문구, export/cleanup, 보관 기간 cleanup 정책과 함께 정의한다. 관리자 목록 표시와 추가 입력 검색은 `show_in_admin` snapshot이 켜진 값만 대상으로 한다. 개인정보 사본 제공은 값 테이블과 게시글 `extra_values_json` snapshot 모두에서 export 포함 정책인 값만 내보낸다. 탈퇴/익명화 cleanup은 값 테이블과 snapshot 모두에서 `retain`이 아닌 값을 비우며, 빈 정책이 남은 legacy 값도 익명화한다. 게시글 snapshot은 관리자 표시, export 정책, cleanup 정책을 보존한다. 세부 필드와 후속 범위는 GitHub 이슈에서 추적한다.

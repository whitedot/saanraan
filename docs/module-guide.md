...
- `purchase_power`는 콘텐츠/커뮤니티 같은 소비 모듈의 settlement 기준이며 환전 모듈 정책 row가 아니다. 소비 모듈의 가격 환산, 복합 차감, fallback 계산은 `sr_asset_exchange_policies`나 `sr_asset_exchange_logs`를 읽지 않는다.

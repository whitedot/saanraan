...
- settlement 통화는 사이트 기본 통화가 아니라 가격 row의 통화와 자산 `purchase_power.settlement_currency`가 일치해야 한다. `site.default_currency`는 새 가격 생성 기본값으로만 사용하고, 통화 min-unit registry와 settlement rounding/carry `policy_version`은 core/settings 소유 계약으로 snapshot에 저장한다.
...
- 확인 화면 이후 실행 전 잔액이 줄어든 경우와 구매력 snapshot, 통화 min-unit, rounding/carry `policy_version`이 바뀐 경우를 별도 무효화 사유로 기록하고 모두 재확인 대상으로 처리한다.
- 운영자가 통화 min-unit 또는 rounding/carry `policy_version`을 변경하면 기존 확인 화면의 in-flight 요청이 fail-closed 재확인으로 떨어질 수 있음을 변경 워크플로에 안내한다.

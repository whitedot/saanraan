...
- 퀴즈 공개 사이드바는 퀴즈 메인과 embed를 제외한 전체 목록, 풀이, 결과 화면에 렌더링한다. 퀴즈 그룹 메뉴는 `/quiz/list?group={group_key}`의 사용 상태 그룹 필터로 연결하고, 인기 퀴즈는 현재 공개 기간의 퀴즈만 조회하며 현재 퀴즈는 제외할 수 있다. 최신 댓글은 현재 로그인 회원이 제출 이력을 가진 퀴즈의 공개·비밀 아님 댓글만 plain text 요약으로 제공하고 전역 본인확인 보호가 켜진 환경에서는 목록 요약을 제공하지 않는다. `quiz.sidebar.summary` output point는 사이드 요약 다음 영역을 확장한다.
- 설문 공개 사이드바는 설문 메인을 제외한 전체 목록, 참여, 완료 화면에 렌더링한다. 설문 그룹 메뉴는 `/survey/list?group={group_key}`의 사용 상태 그룹 필터로 연결하고, 인기 설문은 공개 목록에 표시되는 현재 공개 기간 설문만 조회하며 현재 설문은 제외할 수 있다. 최신 댓글은 현재 로그인 회원이 제출 이력을 가진 공개 목록 설문의 공개·비밀 아님 댓글만 plain text 요약으로 제공하고 전역 본인확인 보호가 켜진 환경에서는 목록 요약을 제공하지 않는다. `survey.sidebar.summary` output point는 사이드 요약 다음 영역을 확장한다.
...
콘텐츠 그룹은 여러 콘텐츠를 묶어 공개 목록, 사이트 메뉴 연결 자산, sitemap 후보로 노출하는 운영 단위다. 콘텐츠 그룹은 개별 콘텐츠 설정 기본값을 제공하지 않으며, 콘텐츠 그룹 목록이나 콘텐츠 목록의 그룹 필터에서 새 콘텐츠를 만들면 그룹 ID만 선택된 상태로 시작한다. 콘텐츠별 `sr_content_setting_sources`는 기존 호환과 적용 상태 기록에 사용한다. 사용 상태의 콘텐츠 그룹은 `/content/group?key={group_key}` 공개 목록, 사이트 메뉴 연결 자산, sitemap 후보로 노출된다. 콘텐츠별 공개 레이아웃은 현재 콘텐츠에 저장하고, 편집 화면의 `그룹`/`전체` 적용은 선택값을 대상 콘텐츠에 한 번 복사한다. 빈 legacy `layout_key`는 콘텐츠 환경설정의 기본 레이아웃을 따르며 명시적인 `content.none`만 레이아웃 없음으로 해석한다. 콘텐츠나 커뮤니티 게시판처럼 공용 배너/팝업레이어를 직접 선택하는 관리자 화면은 선택 영역 근처에 배너/팝업레이어 관리 화면으로 이동하는 링크를 제공한다. 사이트 설정의 초기화면 후보는 콘텐츠 개별/그룹이 아니라 콘텐츠 서비스 메인으로 제한하며, 기본 홈페이지 자체의 본문 구성은 public layout/theme의 홈 템플릿 책임이다.

콘텐츠 공개 사이드바는 콘텐츠 메인을 제외한 그룹·검색 목록, 상세 읽기, 회원 콘텐츠 작성/수정 화면에만 렌더링한다. 메뉴는 사용 중인 콘텐츠 그룹 또는 선택한 사이트 메뉴 계약을 사용하고, 인기 콘텐츠는 공개 가능한 콘텐츠만 조회하며 현재 콘텐츠는 제외할 수 있다. 최신 댓글은 공개 콘텐츠의 공개 댓글만 후보로 삼고 비밀 댓글은 제외하며, 저장 형식과 관계없이 태그를 제거한 plain text 요약만 escape해 출력한다. 사이드바 출력은 `content.sidebar.summary` output point로 확장할 수 있고, 좁은 화면에서는 본문 아래 한 열로 이동한다.
...
- 공개 렌더링은 같은 요청 안에서 동일 `menu_key`의 enabled item tree를 재사용할 수 있지만, 현재 URL 활성 표시, 로그인 `next` 보정, slot class, 최종 HTML은 요청별 렌더 단계에서 계산한다. 초안 공개 반영 또는 seed 직후 같은 요청에서 다시 렌더링해야 하는 흐름은 `sr_site_menu_clear_runtime_cache()`를 호출한다.

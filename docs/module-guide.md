...
CKEditor와 Markdown Editor 같은 에디터 연동은 플러그인이다. 플러그인은 `type => plugin`으로 등록하고, 적용 대상 textarea와 저장/출력 정책은 화면을 소유한 모듈이 결정한다. 콘텐츠는 각 콘텐츠 row에 본문 에디터 선택을 명시 저장하며 기본값은 `textarea`다. 콘텐츠 환경설정의 본문 툴바 구성은 CKEditor 같은 플러그인 에디터가 선택됐을 때의 화면 구성을 제공한다. 커뮤니티는 환경설정에서 게시글 툴바 구성을 저장하고, 게시글 에디터 선택은 커뮤니티 환경설정 또는 게시판 자체 설정만 따른다. 커뮤니티 게시판 그룹에는 게시글 에디터 설정이 없으며, 커뮤니티 게시글 row에는 에디터 선택이나 `body_format`을 저장하지 않는다. 팝업레이어는 팝업레이어 설정에서 본문 에디터를 저장하고, 약관/방침 문서는 새 문서의 첫 버전과 새 version 작성 화면에서 본문 입력 모드를 선택한다. 코어 내장 `textarea`와 직접 `HTML`은 플러그인이 아니라 저장 형식과 출력 helper가 제공하는 기본 입력 방식이다. Markdown 입력 후보, 새 Markdown 저장 허용, Markdown 출력 스타일은 `markdown_editor` 플러그인의 `editor-options.php`와 `markdown-renderer.php` 계약이 활성화되어 있을 때만 제공한다. HTML은 저장/출력 경계에서 공통 rich text sanitizer를 통과하고, Markdown은 renderer 계약이 raw HTML을 허용하지 않는 단일 parser/style profile로 escape 기반 HTML, plain text, stylesheet 결과를 반환한다. CKEditor 플러그인은 에셋 로딩, 초기화 스크립트, 전역 기본 툴바 preset, 화면 소유 모듈이 넘긴 upload endpoint에 대한 adapter 연결만 소유하고, 화면 소유 모듈은 저장 형식 결정, HTML sanitizer, plain text URL 자동 링크 여부, Markdown 출력, 파일 권한과 보존 정책을 책임진다. CKEditor 본문 reset stylesheet는 저장 형식이 `html`인 모든 본문이 아니라 실효 에디터가 CKEditor일 때만 호출한다. 기본 `standard` preset은 제목, 제한된 글자 크기·색상, 기본 강조, 정렬, 링크, 이미지, 표, 구분선, 인용, 목록, 들여쓰기, 서식 제거를 제공하고 전체 선택과 찾기/바꾸기는 제외한다. 이미지 삽입 버튼은 항상 표시해 URL 삽입을 제공하고, upload endpoint가 있는 화면에서는 같은 메뉴에 로컬 파일 업로드도 제공한다. 툴바가 화면 폭을 넘으면 더보기 버튼 없이 여러 줄로 접히며 에디터 컨테이너는 부모 너비를 넘지 않는다. 이전 preset key는 설정 조회 시 `standard`로 정규화한다. CKEditor 툴바 preset은 콘텐츠/커뮤니티 환경설정의 명시 override, CKEditor 전역 기본 `toolbar_preset`, 코드 fallback 순서로 적용한다. 관리자 공통 에디터 설정은 업로드 endpoint를 자동으로 제공하지 않으며, 알림/팝업레이어/설정 화면처럼 rich textarea를 가진 소유 모듈이 필요할 때만 자기 subject id 또는 setting key 기준 upload endpoint를 textarea에 명시한다. CKEditor와 Markdown Editor 설정은 관리자 사이드바의 `플러그인` 분류에서 접근할 수 있지만, 이 메뉴는 도메인 관리 화면이 아니라 플러그인 런타임 자산 설정이다. 플러그인이 비활성화되거나 에셋 로딩에 실패하면 일반 textarea 제출이 유지되어야 한다.

커뮤니티 댓글 에디터도 커뮤니티 환경설정의 기본값과 게시판 자체 설정으로 결정하며, 게시글과 같은 CKEditor 툴바 구성을 사용한다. 게시판 그룹에는 댓글 에디터 설정이 없고 댓글 row에도 에디터 선택이나 `body_format`을 저장하지 않으므로, 실효 댓글 에디터를 바꾸면 기존 댓글의 출력 방식도 함께 바뀐다. 회원 댓글·답글·수정 폼에는 실효 에디터를 적용하고 비회원 폼은 기본 textarea를 유지한다. 댓글 HTML은 저장과 출력에서 정제하며 Markdown은 활성 renderer 계약으로 출력한다.

콘텐츠·퀴즈·설문 댓글 에디터는 각 항목의 `comment_editor_key`가 `inherit`이 아니면 그 값을 우선하고, `inherit`이면 각 모듈 환경설정의 `comment_editor`를 사용한다. 이 값은 각각 `sr_content_items`, `sr_quiz_sets`, `sr_survey_forms`가 소유하며 관리 화면에서 단독·그룹·전체 적용 범위를 선택할 수 있다. 댓글 row에는 에디터나 `body_format` snapshot을 저장하지 않으므로 항목별 또는 환경설정의 실효 에디터 변경은 해당 항목의 기존 댓글 공개 해석에도 적용된다. 세 모듈의 회원 댓글·답글·수정 폼은 같은 실효 에디터를 사용하고, HTML은 저장과 출력에서 공통 rich text sanitizer를 통과하며 Markdown은 활성 renderer 계약으로 출력한다. 콘텐츠 댓글의 CKEditor는 콘텐츠 본문 툴바 구성을 함께 사용하고, 퀴즈·설문 댓글은 `standard` 툴바 구성을 사용한다. 에디터 플러그인이 비활성화되면 설정 조회 시 기본 textarea로 안전하게 돌아간다. 숨겨진 답글·수정 모달의 CKEditor는 초기 페이지 로드에서 모두 만들지 않고 해당 overlay가 열릴 때 초기화해 댓글 페이지당 에디터 인스턴스 수를 제한한다.

커뮤니티 게시글 본문 포맷은 예외적으로 row snapshot이 아니라 현재 게시판 또는 커뮤니티 에디터 설정에서 유도한다. 운영자가 게시판 에디터나 상속되는 전역 기본값을 바꾸면 기존 게시글의 공개 출력 방식도 바뀔 수 있으므로, 관리자 화면은 이 운영 영향을 안내해야 한다. 이 정책을 바꾸려면 `sr_community_posts.body_format` 같은 저장 컬럼 복원과 기존 데이터 백필/해석 기준을 별도 DDL 결정으로 다룬다.
...
모듈 간 본문 연결은 URL 기반 임베딩을 사용한다. 콘텐츠와 커뮤니티의 본문에는 사용자가 작성한 plain text 또는 허용된 HTML 링크만 저장해야 하며, 완성 카드 HTML, 전용 marker, hidden 참조 목록을 저장 진실원으로 삼지 않는다. 사용자는 YouTube, X, Instagram, 내부 콘텐츠 URL을 본문에 단독으로 붙여 넣고, 공개 렌더링은 공통 helper와 활성 모듈의 `url-embed-targets.php` 계약으로 standalone URL 또는 bare URL 링크를 해석한다.
사용자가 브라우저 주소창에서 복사해 붙여넣은 같은 사이트의 절대 URL은 내부 URL로 정규화해 모듈 계약에 전달한다. 캐시의 canonical URL은 사이트 도메인 변경에 흔들리지 않도록 모듈 계약이 돌려준 내부 canonical을 기준으로 저장한다.
저장 action은 본문 URL을 scan해 URL 임베드 파생 캐시/index를 갱신할 수 있지만, 캐시 row가 없어도 본문 URL 자체는 출력 가능해야 한다. URL 임베딩은 전역 내부/외부 허용값, owner 모듈의 `internal_embed_enabled`·`external_embed_enabled`, scope 설정이 먼저 gate하며, 해당 종류가 꺼져 있으면 resolver와 renderer를 호출하지 않는다. 공개 표시 HTML은 공통 관리 모듈 카드가 아니라 외부 provider renderer 또는 대상 모듈의 `render_embed`가 제목, 요약, 대표 이미지, 공개 권한, 삭제/비공개 fallback을 결정한다. 내부 URL 임베드 renderer는 `aside` 같은 호출처 의미 태그가 아니라 대상별 custom tag를 출력하고, 제목 링크와 별도로 사이트 base URL을 붙인 전체 canonical URL 텍스트 링크를 함께 표시한다. fragment sanitizer는 계약의 `target_module`에 맞는 `sr-{module}-embed` custom tag와 `data-{module}-embed` 속성을 허용하므로 renderer와 stylesheet는 같은 모듈 네임스페이스를 사용해야 한다. 계약 제공 모듈은 자기 모듈 key와 같은 `target_module`만 선언할 수 있고, 공통 helper는 다른 모듈 target을 선언한 항목을 등록하지 않는다. 스타일은 호출처의 `module.css`를 재사용하지 않고 대상 모듈이 `embed_stylesheet` 계약으로 선언한 전용 `assets/embed.css`가 소유한다. 설문 URL 임베드는 목록 노출 여부가 아니라 직접 공개 화면과 같은 활성/기간 조건으로 렌더링하며, 로그인·회원 그룹 참여 제한은 링크 대상 설문 화면에서 계속 판정한다. YouTube, X, Instagram 같은 외부 URL은 대상 모듈이 없으므로 공통 `sr-url-embed` custom tag와 `/assets/url-embed.css`만 사용한다. 공개 fragment cache를 켠 renderer의 마크업이나 sanitizer allowlist를 변경하면 기존 HTML 조각을 재사용하지 않도록 `fragment_cache_schema`를 새 값으로 올린다. fragment cache 파일은 대상 모듈이 자기 관리자 화면의 임베드 캐시 메뉴에서 조회하고, 확인 문구와 `delete` 권한을 요구하는 조건부 정리 action으로 관리한다.
콘텐츠와 커뮤니티는 본문 owner이면서 내부 target provider이므로 `external_embed_enabled`와 `internal_embed_enabled`를 각각 저장한다. 외부 설정은 해당 모듈 본문의 YouTube, X, Instagram 렌더링만 제어한다. 내부 설정은 해당 모듈 본문의 내부 URL 렌더링과 다른 본문에서 이 모듈 URL을 임베드 대상으로 제공하는 계약을 함께 제어한다. 퀴즈와 설문은 일반 URL 본문 owner가 아니므로 `internal_embed_enabled`만 제공한다. 이전 `embed_enabled`는 업데이트 시 역할에 맞는 새 설정값으로 복사한 뒤 제거한다.

공통 반응/추천 기능은 커뮤니티 전용 테이블을 먼저 만들지 않고 공식 선택 모듈 `reaction`으로 둔다. 대상 모듈은 `reaction-targets.php` 계약으로 단건 resolve와 batch resolve, target 상태, viewer 기준 공개 열람 가능 여부, 반응 가능 여부, 공개/관리자 URL, label snapshot, 알림 가능 여부, 적용 가능한 preset/key를 제공한다. reaction 모듈은 도메인 테이블을 직접 조회하지 않고 계약 결과 위에서 key 적용, 취소, 변경, 중복, 작성자 본인 제한, throttle, CSRF, 개인정보 export/cleanup을 처리한다. 1차 target은 `content/content`, `content/comment`, `community/post`, `community/comment`, `quiz/quiz_set`, `quiz/comment`, `survey/survey_form`, `survey/comment`이며, 커뮤니티 시리즈와 추가 target은 후속 확장으로 둔다. 커뮤니티 게시글은 게시글별 reaction preset override 컬럼을 두지 않고 커뮤니티 환경설정과 게시판 설정의 preset을 사용한다. 운영자는 리액션 정의의 표시 아이콘을 이모지, Material icon key, 직접 업로드한 JPG/PNG/WebP 이미지 중 선택할 수 있고, 업로드 이미지는 reaction 모듈 저장소와 `/reaction/icon` 출력 경로가 소유한다. 익명 write는 허용하지 않고 회원 탈퇴/익명화 시 계정의 reaction record를 삭제한다. reaction은 스크랩, 콘텐츠 완료, 퀴즈 시도, 설문 응답, 보상 grant, 랭킹/SEO 정책을 대신하지 않는다.
...
- site-setting-references.php (optional)
- logo-positions.php (optional)
- notification-events.php (optional)
- email-delivery.php (optional)
- admin-notification-events.php (optional)
...
큰 관리자 action 파일은 숨은 dispatcher로 바꾸지 않고, 요청 흐름을 보존하는 helper 문단으로 나눈다. action 파일에는 로그인/권한/CSRF 확인, `intent` 분기, 성공/실패 redirect, 최종 view include가 계속 보여야 한다. 다만 입력 수집, 정규화, 서버 검증, 저장할 row/settings 배열 구성, 감사 로그 payload 구성, 화면 표시용 row 보강처럼 한 문단이 길어지는 작업은 모듈 helper 함수로 이름을 붙인다. helper 이름은 `sr_{module}_admin_{domain}_{verb}`처럼 소유 모듈과 관리자 맥락을 드러내고, 코어 helper나 범용 dispatcher로 올리지 않는다.

분리 우선순위는 다음 기준으로 잡는다.

- action 파일이 1,500줄에 가까워지거나 하나의 `intent` 처리 문단이 200줄을 넘으면 먼저 입력/검증/저장/audit 문단을 찾는다.
- 같은 POST 입력 조립이나 검증이 등록/수정/복사 action 사이에서 반복되면 모듈 helper로 합친다.
- action이 여러 테이블 저장을 조정하더라도 transaction과 redirect 위치는 action에서 추적 가능하게 둔다.
- helper는 도메인 정책을 숨기지 않고, 이름과 인자에서 어떤 관리자 작업의 일부인지 드러낸다.
- 여러 모듈이 비슷한 모양을 갖더라도 게시글, 게시판, 설문, 자산처럼 도메인 단어가 들어가면 각 모듈 helper에 둔다.
...
회원 제출, 신청, 승인, 검수처럼 계속 누적되는 운영 목록은 최근 N건 고정 조회를 전체 목록 계약으로 사용하지 않는다. 필터와 같은 조건의 total count를 구하고 공통 관리자 페이지네이션으로 bounded slice를 조회하며, 행 작업의 `return_to`에는 현재 필터와 페이지를 보존한다. 콘텐츠 모듈의 등록자 신청, 작성자 승인, 회원 제출 검수 목록은 이 기준을 따른다.

쿠폰 발급 캠페인과 발급 로그도 같은 전체 목록 계약을 사용한다. 발급 캠페인 수정 폼처럼 후보 선택을 일정 수로 제한하는 경우에는 현재 저장된 대상을 별도로 조회해 후보에 합쳐, 오래된 연결값이 선택기에서 사라지지 않게 한다.

설문 응답·통계·보상 로그의 설문 선택기와 퀴즈·설문 보상 쿠폰 선택기도 같은 현재값 보존 기준을 따른다. 후보 수 제한은 대형 select의 렌더링 비용을 막는 경계일 뿐, 이미 저장되었거나 현재 필터로 요청된 유효 대상을 숨기는 기준으로 사용하지 않는다.

퀴즈·설문의 `public_list_limit`은 전체 공개 항목 수 상한이 아니라 `/quiz`, `/survey` 목록의 페이지당 표시 수다. 공개 쿠폰존 `/coupons`는 50건씩 페이지를 나누며, 세 경로 모두 기존 첫 화면을 전체 목록의 첫 페이지로 사용하고 별도 전체보기 경로를 만들지 않는다. 단건 쿠폰 캠페인 `/coupons?campaign={campaign_key}`는 페이지 목록과 분리된 기존 상세 동작을 유지한다.
...
- theme/basic/layout.php
- theme/basic/assets/layout.css
...
공개 화면 디자인 책임은 public layout, 모듈 layout, 모듈 skin을 구분한다.

- public layout은 사이트 전체 껍데기만 담당한다. `<html>`, `<head>`, 공통 header/footer, 사이트 메뉴, output slot, 전체 폭과 기본 여백이 여기에 속한다.
- public layout이 사이트 메뉴를 노출할 때는 하위 depth가 아니라 레이아웃 영역 구분으로 주 메뉴와 추가 메뉴 항목을 사용한다. 콘텐츠·커뮤니티·퀴즈·설문은 주 메뉴 key를 `sr_public_layout_begin()` layout context의 `site_menus.primary`로 전달하고, 추가 메뉴는 `area_key`, `label`, `menu_key`를 가진 항목 목록을 `site_extra_menus` 배열로 전달한다. `area_key`는 새 항목에 자동 생성되는 중복 없는 6바이트 hex key다. 주 메뉴는 기본적으로 `header` 사이트 메뉴를 사용하며, 추가 메뉴는 기본 고정 슬롯을 만들지 않고 관리자 화면에서 추가한 순서대로 저장한다. 번들 공통/콘텐츠/커뮤니티/퀴즈/설문 레이아웃은 주 메뉴만 기본 렌더링하고, 추가 메뉴 렌더링은 제작자가 만든 레이아웃이 필요할 때 직접 처리한다. 서비스 모듈의 공개 layout context는 화면 소유 모듈 표시용 `module_home_url`, `module_label`, `module_menu_label`도 함께 전달한다. layout shell의 namespace, layout asset, logo provider key, banner layout slot은 선택된 provider 기준을 유지하지만, header의 모듈명/모듈 홈/메뉴 label은 현재 화면 소유 모듈 기준으로 출력한다. 관리자 화면에서는 `주 메뉴`와 `추가 메뉴` 항목 목록으로 표시하고, 선택지는 `사용 안 함`, 모듈 내장 메뉴, 사이트 메뉴 순서로 둔다. 콘텐츠 내장 메뉴 key `sr_content_groups`는 콘텐츠 그룹을, 커뮤니티 내장 메뉴 key `sr_community_board_groups`는 게시판 그룹을 렌더링한다. 실제 위치는 레이아웃 구현이 결정한다. 배너 모듈은 `layout-options.php` 제공자를 기준으로 `{module}.layout/before_layout`, `{module}.layout/before_footer` 위치를 관리자 후보에 추가하며, 번들 서비스 레이아웃은 각각 header 앞과 footer 앞에서 렌더링한다.
...
- 관리자 화면을 가진 모듈은 자기 화면 조합을 확인하기 위해 모듈 전용 UI-KIT 조회 화면을 둘 수 있다. 이때 route와 action은 소유 모듈에 두고, 콘텐츠/커뮤니티/퀴즈/설문처럼 공개 화면 전용 UI 원형을 가진 모듈의 UI-KIT page view는 선택된 `modules/{module_key}/theme/{theme_key}/ui-kit.php`에서 include한다. 반복 샘플 partial은 `modules/{module_key}/views/ui-kit-samples/`처럼 모듈 내부 공유 디렉터리에 둘 수 있다. 콘텐츠/커뮤니티/퀴즈/설문 공개 화면은 선택된 view theme의 `modules/{module_key}/theme/{theme_key}/assets/reset.css`, `common.css`, `module.css`를 공개 화면 context helper에서 호출하고, 화면 view는 `consumer_target`으로 `content.home`, `community.post`, `quiz.result`, `survey.complete` 같은 화면 단위 target을 전달한다. 레이아웃 shell과 stylesheet는 현재 화면 모듈이 아니라 선택된 `layout_key`의 제공자 namespace와 현재 theme key를 기준으로 core output helper가 선택한다. 예를 들어 커뮤니티 환경설정에서 `content.*` 레이아웃을 선택하려면 해당 layout option이 커뮤니티 필수 target 전체를 명시적으로 지원해야 하며, 그렇게 선택된 경우에만 `/modules/content/theme/{theme_key}/layout.php`, `/modules/content/theme/{theme_key}/assets/layout.css`, `/modules/content/assets/layout.js`, `/modules/community/assets/module.js` 조합을 사용한다. 공통 레이아웃 `common.*` 안에서 렌더링할 때는 공통 layout이 `/assets/layout.css`, header/footer, layout script를 소유하므로 모듈 레이아웃 stylesheet와 script를 중복 호출하지 않는다. 모듈 UI-KIT 화면은 선택 theme의 `ui-kit.php`, `assets/reset.css`, `assets/common.css`, `assets/ui-kit-layout.css`, 선택된 layout provider theme의 `layout.php`/`assets/layout.css`, 모듈 theme `module.css`/`theme.css` 조합으로 실제 공개 화면 기준을 확인한다. 기본 호출 순서는 모듈 theme `reset.css`, 모듈 theme `common.css`, 선택된 layout provider theme의 `layout.php`/`layout.css`, 모듈 theme `module.css`, 모듈 theme `theme.css`, UI-KIT 화면에서는 모듈 theme `ui-kit-layout.css` 추가다.
- 퀴즈와 설문은 서비스 메인과 전체 목록을 각각 `/quiz`·`/quiz/list`, `/survey`·`/survey/list`로 분리한다. 목록 view는 `quiz.list`, `survey.list` consumer target을 전달하고 선택 theme은 `list.php`를 제공해야 한다.
- 퀴즈 공개 사이드바는 퀴즈 메인과 embed를 제외한 전체 목록, 풀이, 결과 화면에 렌더링한다. 퀴즈 그룹 메뉴는 `/quiz/list?group={group_key}`의 사용 상태 그룹 필터로 연결하고, 공개 그룹 메뉴 원본은 `public-side-menu` 파일 캐시에서 재사용하며 그룹 저장·삭제 직후 무효화한다. 인기 퀴즈는 현재 공개 기간의 퀴즈만 조회하며 현재 퀴즈는 제외할 수 있다. 최신 댓글은 현재 로그인 회원이 제출 이력을 가진 퀴즈의 공개·비밀 아님 댓글만 plain text 요약으로 제공하고 전역 본인확인 보호가 켜진 환경에서는 목록 요약을 제공하지 않는다. `quiz.sidebar.summary` output point는 사이드 요약 다음 영역을 확장한다.
- 설문 공개 사이드바는 설문 메인을 제외한 전체 목록, 참여, 완료 화면에 렌더링한다. 설문 그룹 메뉴는 `/survey/list?group={group_key}`의 사용 상태 그룹 필터로 연결하고, 공개 그룹 메뉴 원본은 `public-side-menu` 파일 캐시에서 재사용하며 그룹 저장·삭제 직후 무효화한다. 인기 설문은 공개 목록에 표시되는 현재 공개 기간 설문만 조회하며 현재 설문은 제외할 수 있다. 최신 댓글은 현재 로그인 회원이 제출 이력을 가진 공개 목록 설문의 공개·비밀 아님 댓글만 plain text 요약으로 제공하고 전역 본인확인 보호가 켜진 환경에서는 목록 요약을 제공하지 않는다. `survey.sidebar.summary` output point는 사이드 요약 다음 영역을 확장한다.
- 레이아웃 제공 모듈은 `layout-options.php` 계약으로 `common.basic`, `content.basic`, `community.basic`, `quiz.basic`, `survey.basic` 같은 namespace 포함 key와 allowlist view를 제공할 수 있다. 번들 `content.basic`, `community.basic`, `quiz.basic`, `survey.basic`은 콘텐츠/커뮤니티/퀴즈/설문 공개 화면 target 전체를 `supports`에 선언하므로 각 모듈 환경설정에서 다른 모듈 레이아웃도 선택 후보로 표시될 수 있다.
- 공개 화면 DOM을 바꾸는 테마는 [공개 모듈 테마 기준](public-module-themes.md)에 따라 `core/views/theme/{theme_key}` 또는 `modules/{module_key}/theme/{theme_key}`처럼 화면 소유자 내부 theme 디렉터리에 둔다. 해당 테마 CSS는 루트 초기화면은 `assets/theme/{theme_key}.css`, 모듈 화면은 `modules/{module_key}/theme/{theme_key}/assets/theme.css`에 둔다. 스킨과 테마는 별도 레이어이며, 테마 view가 없을 때만 기존 스킨 view가 fallback으로 사용된다.
- 번들 `content.basic`, `community.basic`, `quiz.basic`, `survey.basic`은 공통 레이아웃 파일을 공유하지 않는다. 콘텐츠는 `modules/content/theme/{theme_key}/layout.php`, 커뮤니티는 `modules/community/theme/{theme_key}/layout.php`, 퀴즈는 `modules/quiz/theme/{theme_key}/layout.php`, 설문은 `modules/survey/theme/{theme_key}/layout.php`를 사용한다. 헤더/푸터처럼 같은 시각 언어를 쓰더라도 선택된 layout provider theme의 `assets/layout.css`를 사용해 모듈 경계를 유지한다.
- 번들 `common.basic` 푸터는 사이트 설정의 `site.business_info_items` 중 값이 있는 사업자 정보만 저장 순서대로 표시한다. 번들 `content.basic`, `community.basic`, `quiz.basic`, `survey.basic` 푸터는 각 화면 소유 모듈 환경설정의 `business_info_visible`이 켜져 있을 때 같은 사업자 정보를 표시한다. 출력은 core 설정 helper를 사용하되 CSS class는 선택된 layout provider namespace를 유지한다.
...
모듈은 DB에 view 파일 경로를 저장하지 않는다. `public_layout_key`, `public_theme_key`, `layout_key`, `theme_key`, `skin_key`, `{module_key}_skin_key` 같은 key만 저장하고, 실제 파일 경로는 모듈 helper의 allowlist, `layout-options.php` 계약, 모듈 내부 `theme/{theme_key}` 디렉터리에서 결정한다. 기존 공개 레이아웃 `basic` 값은 `common.basic`으로 정규화한다. 알 수 없는 사이트 공통 레이아웃 key 또는 현재 화면 target을 지원하지 않는 레이아웃 key는 기본 공통 레이아웃으로 fallback한다. 사이트 설정의 공통 레이아웃 선택지는 기본 공통 레이아웃을 먼저 표시하고, 모듈 제공 레이아웃은 관리자 사이드바의 모듈 순서대로 정렬한다. 콘텐츠, 커뮤니티, 퀴즈, 설문처럼 공개 레이아웃과 테마를 설정하는 모듈은 모듈 환경설정의 테마 key와 레이아웃 key를 메인, 그룹/게시판, 목록, 상세/응시/응답/완료 하위 공개 화면에 적용한다. 콘텐츠 상세는 예외적으로 콘텐츠별 `layout_key`를 우선하며 명시적인 `content.none`은 public layout을 사용하지 않는다. 관리자 폼에서는 테마를 먼저 고르고 그 아래에서 레이아웃을 고른다. 이 환경설정의 레이아웃 선택지는 각 모듈의 필수 화면 target 전체를 지원하는 option만 표시하고 저장하며, 번들 서비스 레이아웃은 네 공개 모듈 target 전체를 지원하므로 다른 모듈 레이아웃도 후보에 포함된다.

CSS class는 범위를 드러내는 이름을 사용한다. 모듈 전용 class는 `{module_key}-*` 또는 `sr-{module_key}-*`, 특정 스킨 전용 class는 `{module_key}-skin-{skin_key}-*` 형식을 우선한다. 모듈 skin은 전역 `body`, `a`, `.container`, `.btn`처럼 넓은 선택자를 직접 재정의하지 않고, 필요한 경우 자기 wrapper 아래에서만 스타일을 제한한다. `btn`, `card-*`, `form-*`, `filtering-*`, `modal-content`처럼 UI-KIT이 소유한 class는 모듈 CSS에서 재정의하지 않고, 필요한 배치는 모듈 전용 wrapper나 도메인 class에 둔다.

공개 화면의 저장, 수정, 삭제, 댓글 처리처럼 짧은 결과 안내는 본문 흐름의 `<p>`나 별도 notice box로 남기지 않고 `sr_public_feedback_toasts('{module_key}', $notice, $errors)`로 출력한다. 이 helper는 UI-KIT의 removable alert 샘플과 같은 `alert-removable alert alert-success|alert-danger` 구조를 사용하므로 색상, 표면, border, 닫기 버튼 배치는 UI-KIT이 맡고, 모듈 CSS는 `{module_key}-toast-stack` 위치와 `{module_key}-toast` 줄바꿈 보정만 담당한다. 닫기와 자동 숨김 동작도 `data-{module_key}-toast-*` 속성을 기준으로 모듈 `assets/module.js`에 둔다.

모듈 CSS에서 제목, 본문, 도움말, 메타, 캡션, 코드 텍스트를 새로 정의해야 할 때는 먼저 공통 역할 class인 `.type-page-title`, `.type-section-title`, `.type-card-title`, `.type-body`, `.type-small`, `.type-meta`, `.type-caption`, `.type-code` 또는 현재 런타임 토큰의 `--type-*-size`, `--type-*-line-height`, `--text-strong`, `--text-muted`, `--text-subtle` 값을 사용한다. 공개 런타임은 `assets/reset.css`, 관리자 런타임은 `modules/admin/assets/tokens.css`와 `modules/admin/assets/common.css`를 기준으로 한다. 공개 foundation은 `--sr-*` 의미 토큰을 실제 선언하지만, 관리자 모듈에서 같은 의미 토큰이 필요하면 관리자 foundation 전역을 바꾸지 않고 해당 모듈 wrapper 안에만 관리자 `--color-*` 매핑을 선언한다. 모듈 고유 CSS에 `.8125rem` 같은 임의 크기나 새 회색값을 반복하기 전에 `/admin/ui-kit`와 `/ui-kit`의 Typography 섹션에서 기존 역할이 맞는지 확인하고, 모듈 전용 UI-KIT가 있으면 그 Typography 섹션에서 실제 화면 조합도 확인한다.

모듈 skin처럼 특정 view 조합에만 전용 CSS가 필요하면 view에서 `sr_public_layout_begin()`의 네 번째 인자로 stylesheet를 요청한다. 공개 layout option이나 layout context의 `style_profile`은 `kit`, `minimal`, `install`, `module` 중 하나이며, 공통 layout은 `kit`을 기본으로 사용해 공통 공개 UI kit primitive를 제공한다. 회원, 알림, 포인트/자산, 개인정보처럼 자기 public layout을 갖지 않는 account/utility 화면은 `style_profile`을 직접 지정하지 않고 사이트 설정의 공개 layout 속성을 그대로 상속하며, 스킨 stylesheet가 필요한 경우에만 추가한다. 콘텐츠/커뮤니티/퀴즈/설문 layout은 `minimal`을 기본으로 사용한다. 원본 공개 UI-KIT처럼 공통 공개 UI kit primitive 확인이 목적이면 `style_profile => 'kit'`을 명시한다. 루트 `assets/ui-kit-layout.css`는 원본 `/ui-kit` 조회 화면의 샘플 보조 CSS로만 쓰고, 모듈 UI-KIT 화면은 모듈 theme 내부 `assets/ui-kit-layout.css`에 의존한다. 콘텐츠/커뮤니티/퀴즈/설문 helper는 `style_profile => 'module'`로 루트 reset 자동 호출을 끄고, 모듈 화면에 필요한 theme `assets/reset.css`, `assets/common.css`, `assets/module.css`, `assets/theme.css`, `assets/module.js`를 layout context에 추가한다. 선택된 레이아웃 shell은 provider의 `theme/{theme_key}/layout.php`에서, stylesheet는 provider의 `theme/{theme_key}/assets/layout.css`에서 core output helper가 찾고, 같은 provider의 `theme/basic`으로만 fallback한다. layout shell script는 provider `assets/layout.js`, 화면 모듈 JavaScript는 context의 `assets/module.js`로 분리한다. 내부 view theme이 선택되면 해당 public action은 같은 파일명의 body view를 `modules/{module_key}/theme/{theme_key}`에서 include하고, 같은 key의 `modules/{module_key}/theme/{theme_key}/assets/theme.css`가 있으면 모듈 layout context에 추가한다. UI-KIT 화면에서는 선택 theme의 `assets/ui-kit-layout.css`를 추가한다. 스킨을 사용하는 페이지와 요소는 내장 스킨의 `skins/{skin_key}/skin.css`에 둔다. 자체 public layout이나 서비스 화면 스타일을 소유한 공개 모듈은 공개 화면용 CSS 파일명으로 theme `assets/module.css`를 사용할 수 있지만, 레이아웃, 테마, 스킨 책임은 별도 파일로 분리한다. public layout은 `<head>` 출력만 담당한다. 출력 슬롯처럼 head 렌더링보다 뒤에서 HTML이 만들어지는 공개 모듈 출력도 해당 슬롯을 호출하는 view, skin, public layout이 필요한 stylesheet와 script를 layout context에 명시한다. 공개 CSS와 script는 활성화된 모든 모듈 기준으로 자동 호출하지 않는다.

화면 기능 전용 script도 같은 원칙을 따른다. `mention-input.js`, `member-recipient-picker.js`처럼 특정 `data-*` marker가 있어야 동작하는 자산은 layout shell의 공통 목록에 두지 않고, 실제 marker를 출력하는 view가 layout context의 `scripts`로 요청한다. 로그인 상태처럼 marker 출력 조건이 달라지면 script 요청 조건도 동일하게 맞춘다.

```php
sr_public_layout_begin($pdo ?? null, $site ?? null, $seo, [
    'stylesheets' => [
        '/modules/community/skins/qna/skin.css',
    ],
]);
```
...
커뮤니티 게시판 운영권한은 `sr_community_board_managers`가 소유하는 게시판별 사용자 모드 권한이다. 이 권한은 관리자 모드 메뉴 접근 권한이 아니며, 공개 게시판 화면에서 해당 게시판의 공지사항 작성, 게시글·댓글 숨김/삭제, 비밀글/비밀댓글 열람, 게시글 OG 이미지 제거 같은 운영 action에만 사용한다. 게시판 운영권한을 부여해도 게시글 본문 수정 권한이나 관리자 게시글 목록 접근 권한으로 확대하지 않는다.
...
- `/content/*`처럼 path 끝에 `/*`를 붙이면 해당 prefix 아래의 한 모듈 action으로 요청을 보낼 수 있다. wildcard는 `/content/item` 같은 하위 경로만 매칭하고 `/content/` bare prefix 자체는 매칭하지 않는다. wildcard는 끝에만 둘 수 있고, 루트 catch-all 용도로 사용하지 않는다.
...
`sr_request_contract_mark()`와 `sr_request_contract_guard_blocked()`는 action 파일에서 직접 호출하지 않는다. action은 `sr_require_csrf()`, `sr_member_require_login()`, `sr_admin_require_permission()`, `sr_admin_require_owner()` 같은 공개 helper를 호출해 contract mark가 자연스럽게 기록되게 둔다. 외부 provider가 POST로 되돌아오는 본인확인 return/callback처럼 사이트 CSRF 토큰을 쓸 수 없는 경로는 모듈의 공개 guard helper인 `sr_identity_verification_require_provider_response()`를 통과한 뒤 state와 provider 응답 검증을 수행한다.
...
- 다운로드 응답은 `sr_send_download_headers()`로 content type, disposition, filename, optional length, cache-control을 정규화한 뒤 본문 출력 후 `sr_finish_response()`로 종료한다. S3 signed URL 같은 외부 저장소 response disposition도 `sr_download_content_disposition()`으로 만든다.
- 이미지나 본문 이미지 proxy처럼 브라우저에서 표시되는 파일 streaming 응답은 `sr_send_file_headers()`로 content type, optional length, cache-control, `nosniff`, 필요한 추가 CSP를 보낸 뒤 본문 출력 후 `sr_finish_response()`로 종료한다.
...
- `module_key`: 캐시 경로에서 어느 모듈이 만든 썸네일인지 구분한다.
- `storage_driver`, `storage_key`: 원본 저장소 위치를 나타낸다.
- `public` 또는 `public_url`: 공개 원본인지와 실패 시 fallback URL을 나타낸다.
- `source_version`, `checksum_sha256`, `checksum`: 원본 교체 시점에 새 캐시 URL을 만들기 위한 값이다.
...
S3 원본은 cache miss 또는 metadata 부족으로 선조회가 불가능할 때 helper가 `HeadObject`와 임시 파일 다운로드를 거쳐 처리한다. S3 `VersionId`가 있으면 같은 version을 다운로드하도록 presigned URL에 반영하고, 실제 MIME은 내려받은 파일 기준으로 다시 판단한다. 같은 S3 저장소 안의 파일 복사는 공유호스팅 호환성을 위해 server-side copy API 대신 presigned GET 임시 파일과 재업로드를 사용하므로, 큰 파일 복사는 각 모듈의 업로드 상한 안에서만 호출한다. 썸네일 캐시 저장소 자체는 현재 local `storage/cache/thumbnails`이며, S3 캐시 저장소 선택은 별도 adapter 계획 범위다.
...
허용 심볼 이름은 다음과 같다. `settings`, `admin-mode`, `users`, `user`, `content`, `stats`, `home`, `folder`, `image`, `layers`, `search`, `menu-list`, `bell`, `shield`, `coins`, `point`, `database`, `savings`, `payments`, `wallet`, `gift`, `ticket`, `message-circle`, `service`, `quiz`, `poll`, `emoji_emotions`, `integration_instructions`.

프로젝트 기본 아이콘셋은 번들 폰트로 직접 제공하는 Google Material Symbols Outlined다. 공용 helper `sr_icon()`으로 출력하면 공개 런타임의 `assets/reset.css`, 관리자 런타임의 `modules/admin/assets/common.css`, 그리고 `sr_icon_bootstrap_script()`가 폰트 준비 전 ligature 텍스트 노출을 막는다. 기존 `sr_material_icon_html()`은 호환 래퍼로 유지된다. 운영자는 `/admin/settings`에서 아이콘 키별로 Material Symbols 이름을 바꾸거나 이미지 파일을 업로드할 수 있고, 기본 키 외 사용자 키를 추가할 수 있다. 기본 키는 초기화만 가능하며 제거할 수 없고, 추가한 키만 제거할 수 있다. 운영자가 업로드한 공용 아이콘 이미지는 admin 모듈 설정 저장소와 권한 확인이 있는 `/admin/icon-image`가 소유하며, 개별 모듈의 계약 파일이나 자산 소유권으로 편입하지 않는다. 모듈은 렌더링 구현을 직접 고정하지 말고 안정적인 심볼 키만 선언해야 한다.
...
- 현재 `asset_exchange` 모듈은 실행/로그/정정 기준으로 포인트, 적립금, 예치금의 고정 3자산만 환전 후보로 사용한다. 다른 모듈의 `asset-exchange.php` 계약은 향후 확장 후보일 수 있지만 현재 환전표와 회원 환전 실행에서는 무시한다.
...
- settlement 기반 복합 차감을 지원하는 자산은 `purchase_power => ['asset_units' => 양의 정수, 'settlement_units' => 양의 정수, 'settlement_currency' => 통화 코드]`를 snapshot 가능한 구조로 제공한다. 내부 계산은 `settlement_units / asset_units` rational로 수행하고, 방향이 모호한 스칼라 rate만 저장하지 않는다. 통화 최소단위 미만 rational 값은 row별로 올림하지 않고 다음 allocation으로 carry할 수 있으며, carry snapshot에는 `settlement_numerator`, `settlement_denominator`, `cumulative_settlement_numerator`, `fractional_carry_numerator`, `fractional_carry_denominator`를 남긴다. `asset_units`와 `settlement_units`는 양의 정수, `settlement_currency`는 core/settings min-unit registry에 존재하는 통화인지 자산 설정 저장 또는 관리자 config 로드 시점에 검증해 setup 오류로 노출한다.
- `purchase_power`는 콘텐츠/커뮤니티 같은 소비 모듈의 settlement 기준이며 환전 모듈 정책 row가 아니다. 소비 모듈의 가격 환산과 복합 차감 배분은 `sr_asset_exchange_policies`나 `sr_asset_exchange_logs`로 계산하지 않는다. 다만 확인 UI가 있는 유료 열람/다운로드에서 현재 선택된 포인트/적립금/예치금 조합이 정확 충당에 실패하고 활성 환전 정책으로 같은 조합 안의 잔액을 맞출 수 있으면, 사용자가 환전 후 결제를 명시 확인한 POST에서만 `asset_exchange` 실행 후 settlement 계획을 다시 계산할 수 있다.
...
- settlement 통화는 사이트 기본 통화가 아니라 가격 row의 통화와 자산 `purchase_power.settlement_currency`가 일치해야 한다. `site.default_currency`는 새 가격 생성 기본값과 통화가 빠진 자산 구매력 계약의 fallback 기준으로만 사용하고, 통화 min-unit registry와 settlement `snapshot_schema_version`, rounding/carry `rounding_policy_version`, 0원/legacy 분류 `settlement_kind`는 core/settings 소유 계약을 참조해 도메인 로그에 저장한다. 사이트 기본 통화가 위험 작업으로 변경되어도 기존 가격 row, 거래 로그, 구매력 snapshot은 변환하거나 재해석하지 않는다. `settlement_kind`는 `paid`, `free`, `paid_settled_zero`, `preview_test_zero`, `legacy_unknown` 중 하나로 시작한다. `free`는 무료 접근뿐 아니라 지급/적립처럼 기준가격 settlement가 발생하지 않는 non-use row를 포함하고, 자산 증감량은 `direction`과 `asset_amount`로 별도 해석한다.
- 마지막 자산의 잔여 settlement 흡수는 정확 충당이 가능한 범위까지만 허용한다. 1 자산 단위가 통화 최소단위보다 큰 경우 ceil overpay를 허용하지 않으며, 예를 들어 1P = 10 KRW로 1,005 KRW를 포인트만으로 충당할 수 없으면 잔액 부족/결제 불가로 거부하고 재계획 없이 재확인을 요구한다. 반대로 각각 0.5 KRW 가치인 두 allocation처럼 통화 최소단위 미만 값들이 합쳐 정확히 1단위를 만들 수 있으면 같은 계획 안에서 충당할 수 있다.
...
- `cash_like`는 예치금처럼 회원이 환전으로 받은 뒤 다시 출금 항목으로 사용할 수 있는 금액성 자산을 표시하는 좁은 힌트다. 현재 환전 환경설정 화면은 수수료 적용 조건으로 `사용 안 함`과 `항상 적용`만 제공하며, cash-like 재환전 전용 수수료 조건은 지원하지 않는다.
...
`identity_verification` 소비 정책:

- 본인확인 모듈은 provider 설정, 시도, 결과, purpose별 계정 연결을 소유하고, 각 소비 모듈은 자기 환경설정에서 본인확인 요구 여부를 저장한다.
...
- 자산 모듈 폴더 기준 `helpers`, `balance_function`, `transaction_function`, `balance_table`, `transaction_table`, 표시용 `label` 또는 `label_function`, `unit_label` 또는 `unit_function`을 제공한다.
...
- `refund_split_function`은 선택 callable이며, 한 환불 요청이 여러 원장 transaction으로 쪼개질 수 있는 자산이 제공한다. callable 형식은 `function (PDO $pdo, array $data): array`이고 transaction ID 목록을 반환한다. 소비 도메인은 특정 자산 key를 분기하지 않고 이 capability를 통해 환불 transaction 목록을 받는다.
- `balance_table`과 `transaction_table`은 `asset_ledger` reconciliation 대상 발견에 사용한다. 금액성 원장 자산이 이 필드를 빠뜨리면 reconciliation은 해당 자산을 조용히 생략하지 않고 계약 오류로 표시한다.
...
- `format_value`는 해당 에디터가 저장/출력에 요구하는 body format이며 `plain`, `html`, `markdown` 중 하나다. core 내장 `textarea`는 `plain`, 내장 `HTML`은 `html`로 처리하고, 플러그인 에디터는 활성 `editor-options.php`의 `format_value`를 읽는다. 계약 미발견 또는 비활성 에디터는 `textarea`와 `plain`으로 강등한다.
- 화면 소유 모듈은 저장한 editor key를 core helper에 넘기고, core helper는 활성 플러그인의 계약만 읽어 textarea 속성, format 값, 에셋 HTML을 만든다. 저장/출력 분기는 `ckeditor`, `markdown` 같은 플러그인 key를 직접 비교하지 않고 core format helper 결과를 사용한다.

`asset-recovery-targets.php`:

- 배열을 반환한다.
- `asset_ledger` 공통 미회수 큐가 source 모듈별 회수 재시도, source label, subject link를 구성할 때 사용한다.
- `helpers`는 자산 모듈 폴더 기준 helper 파일이며 `helpers.php` 또는 `helpers/{name}.php` 형식만 허용한다.
- `label`은 관리자 큐에서 source 모듈 표시명으로 사용한다.
- `retry_function` callable 형식은 `function (PDO $pdo, int $accountId, string $grantEventKey, string $subjectType, int $subjectId, string $reversalEventKey, string $canonicalReversalEventKey, string $reason, array $operationContext): array`이다. 반환 shape는 기존 회수 실행 결과와 같은 `operation_allowed`, `recovery_status`, `message` 등을 포함한다.
- `subject_links`는 `subject_type`별 `label`, `url_template` 맵이다. `url_template`에는 `{subject_id}` placeholder를 사용할 수 있고, `asset_ledger`는 relative URL만 허용한다.
- 계약 미제공, 비활성 source 모듈, callback 미제공은 운영자에게 보이는 실패 메시지와 함께 fail-closed 한다.
...
- 번들 `markdown_editor`의 기본 stylesheet는 `github-markdown-css` 5.9.0의 요소 구성을 `.markdown-editor-body` 범위와 마크다운 전용 `--md-*` 토큰으로 재구성한 값으로 시작한다. `--md-text`, `--md-info`, surface, border, 상태 토큰은 `.markdown-editor-body` 안에서 공통 `--color-*` 값에 연결되어 사이트 light/dark 모드를 따르며 마크다운 밖의 UI에는 영향을 주지 않는다. 이전 `var(--sr-*)` 마크다운 프로필과 stylesheet는 대응하는 `var(--md-*)`로 변환한다. 관리자 렌더링 미리보기는 공개 foundation과 같은 light/dark `--color-*` 값을 사용해 설정 화면과 실제 사용자 화면의 토큰 계산값을 맞춘다. 전체 본문 색 토큰을 바꾸면 아직 기본 본문색을 따르는 문단, H1-H5, 목록, 코드, 표의 요소별 색 선언도 함께 바꾸며, 별도 상태색을 지정한 H6, 링크, 인용과 운영자가 독립적으로 바꾼 요소 색은 유지한다. 인용의 Typography 색은 렌더링된 `blockquote` 내부 문단에도 직접 적용해 일반 문단색 선언에 덮이지 않게 한다. H1-H6 기본 크기는 관리자 UI kit의 타이포그래피 단계를 두 단계씩 높여 참고한 고정값 `32px`, `26px`, `24px`, `20px`, `18px`, `16px`를 사용하며 관리자 stylesheet나 token에는 의존하지 않는다. 저장 stylesheet의 H1-H6 공통·요소별 크기가 모두 이전 기본값일 때만 이 새 단계로 함께 변환하고, 하나라도 별도 조정한 stylesheet는 자동 변경하지 않는다. 일반 본문과 제목은 별도 `font-family`를 저장하거나 출력하지 않고 사이트 기본 글꼴을 상속하며, 코드 요소는 사이트 공통 reset의 monospace 규칙을 따른다. 관리자 설정은 전체 본문, 문단/인라인 강조, 제목, 링크, 목록/작업 목록, 인용, 인라인 코드, 코드 블록, 표, 구분선 섹션에서 주요 속성을 조정한다. 표는 헤더 행 다음에 열별 `---` 구분선 행이 있을 때 하나의 블록으로 인식하고, 구분선의 콜론에 따른 좌·중앙·우 정렬과 이후 본문 행을 `<thead>`/`<tbody>`로 출력한다.
- Markdown 본문 URL이 임베드로 바뀌면 공통 URL 임베드 helper는 임베드 custom element를 `.markdown-editor-body` 밖으로 이동하고 앞뒤 Markdown wrapper를 같은 profile 속성으로 나눈다. Markdown stylesheet와 운영자가 저장한 scoped custom selector는 임베드 내부에 적용되지 않는다.
- 관리자 화면은 실제 Markdown 원문 편집기와 서버 렌더링 결과, 오른쪽 속성 인스펙터를 함께 보여주는 작업대로 구성한다. 데스크톱에서는 원문·렌더링 작업 영역과 속성 인스펙터가 같은 grid 행과 높이를 사용하고 각 영역 내부만 스크롤하며, 좁은 화면에서는 인스펙터를 작업대 아래로 내려 자동 높이를 사용한다. 별도 편집/분할/미리보기 버튼 묶음은 두지 않고 원문과 렌더링 결과 패널 헤더에 각각 펼치기 버튼을 둔다. 각 버튼은 해당 패널만 펼치며 단독 보기 상태에서는 같은 버튼이 분할 보기 복원으로 바뀐다. 운영자가 렌더링 결과의 문단, h1-h6, 링크, 목록, 인용, 인라인 코드, 코드 블록, 표, 구분선을 클릭하면 오른쪽 인스펙터가 해당 요소의 설정으로 바뀐다. 인스펙터는 Figma 계열 속성 패널처럼 선택 요소 제목과 초기화 아이콘, `Margin`, `Padding`, `Stroke`, `Typography`, `Fill` 같은 접이식 섹션, 2열 입력 그리드를 제공한다. 숫자·선택값 변경은 같은 `sr-control` CSS 선언을 갱신하고 짧은 debounce 뒤 서버 렌더링 결과에 반영한다. 미리보기 갱신·오류 메시지는 하단 영역에 두지 않고 렌더링 결과 전체를 덮는 theme-aware mask로 표시하며, 메시지가 비면 mask를 숨긴다. 렌더링 결과의 초기 light/dark 상태는 설정 화면 진입 시 계산된 현재 관리자 `data-theme`을 따르고, 이후에는 렌더링 결과 헤더의 아이콘 버튼으로 독립 전환한다. 현재 상태에 따라 dark/light 아이콘과 접근성 안내를 바꾼다. 별도 미리보기 배율은 제공하지 않는다. 렌더링 결과 헤더의 CSS 아이콘은 전체 CSS textarea와 닫기 버튼만 있는 프로젝트 공통 fullscreen overlay 모달을 연다. 편집기 하단에는 GitHub 참고 링크 없이 `기본`/`커스텀` 스타일 적용 라디오 토글과 저장 버튼만 스티키 제출 영역으로 제공한다. 표·체크 목록·코드 블록 문법은 운영 선택 없이 항상 사용하며 raw HTML 입력은 계속 허용하지 않는다. 모든 선택 가능 요소는 공통 Box model 계약을 가지며 위·오른쪽·아래·왼쪽 margin, padding, border-width를 각각 설정하고 border style, border color token, radius를 설정할 수 있다. 텍스트가 있는 모든 요소는 공통 Typography 계약을 추가로 가지며 size, weight, line-height, letter/word spacing, alignment, font style, decoration, text transform, color token을 같은 방식으로 설정한다.
- `style_source_mode`는 하단 스티키 제출 영역의 `기본`과 `커스텀` 라디오 토글로 `default`와 `custom`을 저장한다. `default`는 저장된 사용자 stylesheet를 지우지 않고 번들 `github-markdown-css` 기반 원본을 공개 출력하며, `custom`은 운영자가 만든 전체 stylesheet를 출력한다. 기본 모드에서 오른쪽 속성 패널이나 CSS 원문을 편집하면 관리자 화면은 커스텀 모드로 전환한다. 요소별 컨트롤은 stylesheet의 `sr-control` 주석 뒤에 있는 실제 property 선언을 갱신한다. CSS를 직접 바꾸면 짧은 debounce 뒤 서버 검증을 거쳐 반환된 CSS가 라이브 미리보기와 속성 패널에 반영된다. 속성 패널의 초기화 아이콘은 현재 선택 요소만 초기화하며 CSS 모달에는 전체 초기화 기능을 두지 않는다. 저장 stylesheet의 모든 selector는 `.markdown-editor-body`로 시작해야 하며 at-rule, 외부 URL, 실행형 CSS 표현과 `</style` 종료 문자열은 거부한다.
- Markdown 본문 stylesheet는 관리자 미리보기와 public content/community 화면에서 공통 `/assets/editor-md.css`의 `.markdown-editor-body` scoped reset을 먼저 적용한 뒤 style profile stylesheet를 적용해야 한다. 이 reset은 관리자 `common.css`나 public UI kit의 전역 reset이 제목, 목록, 링크, 표, 코드, checkbox 같은 Markdown 본문 요소를 오염시키지 않도록 하는 계약이다.
- CKEditor도 같은 본문 독립성 계약을 따른다. 콘텐츠 상세와 커뮤니티 게시글·댓글의 CKEditor 저장 HTML은 모두 `.sr-ckeditor[data-sr-editor-output] .ck-content` 구조를 만들고 번들 공식 `ckeditor5.css`와 프로젝트 토큰 연결용 `saanraan-ckeditor.css`를 호출한다. 공개 출력 wrapper의 `data-sr-editor-output`은 입력 widget을 복제하지 않고 `.ck-content` 배경만 투명하게 만들며, 제목, 목록, 링크, 표, 코드, 정렬, 들여쓰기, 글자 크기·색상, 이미지 정렬·캡션은 편집기와 같은 공식 content style 규칙을 사용한다. 직접 HTML 입력 본문은 저장 형식이 `html`이어도 CKEditor stylesheet를 호출하지 않는다. 화면 모듈의 theme `assets/module.css`는 본문 컨테이너 배치와 색 토큰 같은 화면 스타일만 소유한다.
- 콘텐츠 공개 출력은 입력 widget 클래스나 좌우 padding 없이 `--ck-content-font-size`, `--ck-content-line-height`, `--ck-spacing-large` 첫·마지막 블록 margin을 재사용해 편집 영역의 글자와 세로 간격을 맞춘다.
- 에디터가 있는 콘텐츠/커뮤니티 테마는 읽기 본문과 편집 본문이 같은 배경색·글자색 의미를 갖도록 `--sr-editor-body-surface`, `--sr-editor-body-text`, `--sr-editor-body-muted`, `--sr-editor-body-border` 계약을 함께 고려해야 한다. CKEditor textarea에는 화면 소유 모듈이 `data-sr-editor-body-theme="content.{theme_key}"` 또는 `data-sr-editor-body-theme="community.{theme_key}"`를 붙이고, CKEditor loader가 이 값을 `.sr-ckeditor` wrapper로 전달한다. 테마가 공개 본문의 `--content-*`, `--community-*`, `--sr-*` 토큰과 다른 강한 색 조합을 쓰면 CKEditor stylesheet에 해당 `data-sr-editor-body-theme` selector의 light/dark/system 색 매핑을 추가해야 한다. 특히 배경색과 글자색을 바꾸는 테마는 읽기 화면, 관리자 편집 화면, 공개 작성 화면에서 모두 대비가 유지되는지 확인해야 한다.
...
- 쿠폰 지급/사용/환불/상태 변경과 쿠폰 정의 사용 중지처럼 업무 저장은 성공해야 하지만 회원 안내가 필요한 이벤트는 소비 모듈 환경설정에서 케이스별 발송 여부와 회원 대상 채널을 보관하고, 알림 모듈 비활성화나 템플릿 누락은 원 업무 실패로 전파하지 않는다.
...
- 배열을 반환한다.
- 각 항목은 `target_type`, `label`, `helpers`, `resolver_function`을 가진다.
- `target_type`은 전역 신고 대상 key이며, `message`처럼 소유 모듈이 안정적으로 책임지는 literal key를 사용한다.
- `resolver_function` callable 형식은 `function (PDO $pdo, int $targetId, int $actorAccountId): ?array`이다.
- 반환 row는 신고 화면과 관리자 처리에 필요한 공개 가능 제목, URL, 소유자 계정, 신고 가능 상태만 포함하고, 본문 원문이나 제3자 식별자는 불필요하게 노출하지 않는다.
...
- transactional email override는 `/admin/delivery-templates`에서 관리한다. 회원 모듈의 이메일 인증, 비밀번호 재설정, 로그인 2차 인증 이메일 같은 인증 메일은 운영 동선을 줄이기 위해 `/admin/member-notification-templates`의 회원 알림/메일 관리 화면에도 함께 표시한다. 회원 알림 이벤트 템플릿은 중앙 발송 템플릿 화면에 섞지 않고 각 소유 모듈의 알림/메일 관리 화면에서 관리한다. 발송 수단 기본값이 이메일이 아닌 알림 이벤트는 이메일을 추가 선택할 수 있지만, transactional email 기본 템플릿은 이메일 발송 전용이므로 발송 수단을 선택하지 않고 사용 여부만 선택한다. 알림 모듈의 이메일 채널이나 회원 외부채널 수신 허용이 꺼져 있어도 알림 이벤트 화면은 저장된 선택값을 보존하되 목록과 편집 선택지에서 채널 중지 상태를 표시하고, 런타임은 해당 채널의 발송 작업 row를 만들지 않는다. 수정값이 있으면 별도 적용 옵션 없이 수정값을 사용하고, 기본값으로 되돌리려면 복원 action으로 override를 삭제한다. 알림 모듈이 설치되어 활성화된 경우에만 각 모듈의 알림/메일 관리 메뉴를 노출하며, 알림 모듈이 없거나 비활성화된 환경에서 해당 URL을 직접 열면 관리 화면을 렌더링하지 않는다.
...
- notification event와 transactional email의 `disable_policy`는 `no_op`이며, inactive는 이벤트 생성 또는 메일 발송 no-op으로 해석한다.
...
- 알림 모듈이 제공하며 `helpers`, `status_function`, `send_function`을 반환한다.
- `status_function`은 알림 환경설정의 이메일 채널 사용 여부를 반환하고, `send_function`은 같은 환경설정의 `php_mail`/SMTP/HTTP API 설정을 현재 발송에 적용한 뒤 기존 런타임 설정을 복원한다.
- 회원 모듈의 이메일 인증, 비밀번호 재설정, 로그인 이메일 2차 인증은 이 계약을 통해 발송한다. 알림 모듈이 없거나 비활성화된 설치에서는 코어 메일 설정을 사용하는 기존 직접 발송을 유지한다.
- 알림 모듈이 활성화되어 있으나 이메일 채널이 꺼져 있으면 회원 모듈은 메일 토큰을 만들거나 발송을 시도하지 않는다. 비밀번호 재설정과 인증 메일 재요청은 안내와 함께 비활성화하고, 이메일 인증이 필수인 회원가입과 이메일 주소 변경도 진행하지 않으며, 이메일 로그인 2차 인증은 사용 가능한 provider 후보에서 제외한다.
...
| `delivery-templates.php` | core delivery template helper | 발송 전 렌더링 | 모듈 소유 transactional email 기본 문구와 변수 계약 수집. 회원 알림 이벤트 문구는 소유 모듈의 알림/메일 관리 화면에서 `sr_notification_event_templates`와 모듈 설정을 함께 저장. 알림톡처럼 provider가 승인 템플릿을 소유하는 채널은 본문을 사이트에서 편집하지 않고 `sr_notification_channel_template_bindings`에 provider 템플릿 코드만 연결 |
| `email-delivery.php` | `member` 모듈 | 이메일 인증, 비밀번호 재설정, 로그인 이메일 2차 인증 발송 전 | 알림 이메일 채널 상태와 관리자 설정 transport를 적용한 transactional email 발송 함수 |
...
| `report-targets.php` | `community` 모듈 | 신고 대상 resolve와 신고 가능 여부 검증 | 모듈별 신고 대상 후보와 target 공개/소유자/URL 상태 |
...
| `notification-events.php` | `point`, `reward`, `deposit`, `coupon`, `content`, `community`, `message`, `quiz`, `survey` 모듈 | 거래/쿠폰 상태 변경 성공 뒤, 쿠폰 사용 중지 회수 안내, 댓글 작성자/멘션/쪽지 알림 생성 시 | 선택 알림 모듈의 계정 알림 생성 함수 |
...
- 현재 context에서 실제 출력이 있을 때만 자산이 필요하면 정적 metadata 대신 `assets_function`에서 출력 조건을 동일하게 판정해 자산 배열을 반환한다. 빈 slot 때문에 provider CSS/JS를 로드하지 않는다.
...
배너 관리자 화면은 운영자에게 `서비스 -> 상세` 2단계 노출 위치 선택으로 보여주되, 저장값은 기존 `module_key + point_key + slot_key` 조합으로 유지한다. 기본적으로 선택한 조합 전체에 노출한다. 특정 대상 노출이 필요하면 노출 범위 항목에서 특정 대상 노출을 선택하고, 쿠폰 대상 조회와 같은 검색 모달을 사용해 노출 대상 번호를 채운다. 노출 범위와 대상 번호 입력은 특정 대상 선택을 지원하는 노출 위치에서만 표시한다. 현재 번들 기준으로 `content.view`는 콘텐츠, `community.board.list`와 `community.post.form`은 게시판, `community.post.view`는 게시글 검색으로 연결한다. 검색 가능한 대상 계약이 없는 노출 위치는 전체 노출로만 저장한다. 팝업레이어 관리자 화면은 같은 extension point를 읽지만 overlay 표시 특성상 slot별 삽입 위치를 운영자에게 노출하지 않고 `서비스 -> 노출위치`로 축약해 보여주며, 저장값에는 해당 화면의 대표 slot 키를 유지한다. 팝업레이어의 선택 매칭 대상 검색은 콘텐츠, 커뮤니티 게시판/게시글, 퀴즈, 설문 검색 계약을 사용한다.
...
- 팝업레이어는 선택적으로 공개 무료 쿠폰 발급 캠페인을 CTA로 연결할 수 있다. 관리자 저장 시 `popup_layer` 노출 위치가 포함된 공개 무료 캠페인만 허용하고, 공개 렌더링 시에도 쿠폰 캠페인 상태, 로그인 상태, 재고, 회원당 제한을 쿠폰 helper로 다시 판정한다. 팝업 닫기/오늘 하루 보지 않기 쿠키는 팝업 노출 상태만 제어하며 쿠폰 발급 여부나 claim row를 만들지 않는다.

번들 콘텐츠 모듈은 `content.view` point와 `before_content`, `after_content` content slot을 제공한다. 배너/팝업레이어 관리 화면에서 콘텐츠 전체 또는 특정 콘텐츠 ID를 대상으로 출력 규칙을 저장할 수 있고, 콘텐츠 관리자 화면에서는 공용 배너/팝업레이어를 직접 선택할 수도 있다. 콘텐츠 그룹은 `sr_content_groups`가 소유하고 콘텐츠는 `sr_content_items.content_group_id`로 선택적으로 연결한다. 콘텐츠 환경설정의 콘텐츠 공개 레이아웃은 공개 콘텐츠 메인과 콘텐츠 그룹 목록의 기본 header/footer 레이아웃을 정하고, 콘텐츠 상세는 `sr_content_items.layout_key`에 저장된 콘텐츠별 선택을 우선 적용한다. 콘텐츠 상세에서 `content.none`을 선택하면 공통 layout shell, 요약·메타데이터·대표 이미지·배너·리액션·시리즈·파일·댓글 같은 부가 영역을 출력하지 않고 제목과 본문만 독립 문서로 렌더링한다. `sr_content_items.show_title = 0`이면 이 무레이아웃 문서의 화면 제목도 숨기지만 관리자 목록과 문서 `<title>`에는 제목을 유지한다. 콘텐츠 공개 테마는 환경설정의 `theme_key`로 별도 저장하며 `modules/content/theme/{theme_key}`의 `home.php`, `group.php`, `content.php`가 모두 있을 때 선택지에 표시된다. 콘텐츠 생성/수정 화면의 상태, 본문 에디터 선택, 대표/OG 이미지, 배너, 팝업레이어, 유료 열람, 완료 버튼 포인트/금액 처리는 현재 콘텐츠에 저장하며, 운영자가 선택한 `그룹`/`전체` 적용 옵션은 현재 편집값을 같은 그룹 또는 전체 콘텐츠에 한 번 복사한다.
대표/OG 이미지는 `sr_content_items.cover_image_url`에 저장하고 URL 입력, 파일 업로드, 현재 이미지 삭제를 지원한다. 파일 업로드는 콘텐츠 모듈 저장소 `content/cover-images/{Y}/{m}`에 저장하며 공개 출력은 `/content/cover-image` 프록시를 사용할 수 있다. 이미지가 비어 있으면 공유 미리보기 metadata는 사이트 기본 OG 이미지 fallback을 사용한다. 업로드 이미지가 교체/삭제되거나 콘텐츠 삭제로 더 이상 참조되지 않으면 저장소 파일을 정리하고, 실패한 정리는 `sr_content_storage_cleanup_failures`에 기록한다. 콘텐츠 그룹 삭제는 콘텐츠를 삭제하지 않고 그룹 연결만 해제하므로 이미지 저장소 파일을 정리하지 않는다. 다운로드 파일은 `sr_content_files` 저장소와 `/admin/content/files` 관리 화면에서 먼저 등록하고, 콘텐츠 생성/수정 화면은 `sr_content_file_links`로 사용 상태 파일을 연결만 한다. 파일 제목, 숨김 상태, 다운로드 과금 정책은 다운로드 파일 관리 화면이 소유한다.
...
콘텐츠 유료 열람, 다운로드 과금, 완료 버튼 포인트/금액 처리는 콘텐츠 모듈이 접근/액션 정책과 로그를 소유하고, 포인트/적립금/예치금 모듈의 잔액 조회와 원장 생성 helper만 호출한다. 완료 버튼 처리는 공개 콘텐츠에서 회원이 버튼을 눌렀을 때만 실행되며, 단순 콘텐츠 조회는 포함하지 않는다. 관리자 화면에는 설치되어 있고 활성화된 자산 모듈만 표시하되, 운영자 라벨은 `자산`보다 `처리 항목`, `포인트/금액`, `잔액` 표현을 우선 사용한다. 완료 버튼 처리 항목은 항목 선택과 항목별 금액을 같은 입력 묶음으로 보여주되, 지급 방향은 한 항목만 선택할 수 있고 차감 방향은 여러 항목을 선택할 수 있다. 차감 방향은 입력 금액 합계를 기준 통화 가격으로 보고 각 항목의 `purchase_power`에 따라 정확히 충당 가능한 자산 단위만 배분한다. 회원 그룹별 적용은 항목 선택과 다른 의미의 정책 세트 선택이므로 별도 행으로 표시하되, 실제 설정 화면에서는 현재 선택한 항목과 맞는 정책 세트만 선택할 수 있고 뱃지 요약도 같은 항목 기준으로 표시한다. 콘텐츠와 커뮤니티의 실제 자산 설정 화면에서는 미리 만든 정책을 `회원 그룹별 적용`으로 표시하고 셀렉트로 여러 개 선택할 수 있으며, 그룹별 계산 방식과 조정값 편집은 전용 관리 화면에서 수행한다. 실제 자산 설정 화면은 선택한 회원 그룹별 적용을 하단 뱃지로 추가하고 그룹별 포인트/금액 실제 적용값 요약을 함께 보여준다. 기존 단일 금액은 호환용 합계로 유지한다. 차감 로그의 `amount`는 실제 자산 단위이고 `settlement_amount`/`settlement_currency`는 기준가격 충당 단위, `purchase_power_snapshot_json`은 실행 당시 구매력/통화/정책 snapshot이다. 콘텐츠 차감 로그에는 `settlement_kind`, `snapshot_schema_version`, `rounding_policy_version`도 함께 저장해 통계/export와 환불 정책이 같은 정규 필드를 참조한다. 결제 자산 모듈은 콘텐츠 도메인을 알 필요가 없으며, 거래 참조는 열람 `reference_type=content.view`, 다운로드 `reference_type=content.download`, 완료 버튼 처리 `reference_type=content.action`으로 남긴다. 파일 다운로드 성공 이력은 무료/유료를 함께 `sr_content_file_download_logs`에 남기고, 유료 행은 `sr_content_asset_access_logs`, 접근권, 쿠폰 redemption 링크, `refund_policy_version`을 대조한다. 유료 다운로드 수동 환불은 환불 사유와 포인트 환불 유효기간 기준을 받아 연결된 차감/접근 로그가 있는 이력만 처리하고, 차감 로그별 `refund` 원장 거래를 새로 만들며, 다운로드 이력 행의 `refund_status`로 중복 환불을 막는다. 포인트 환불 유효기간 기본값은 참조 원거래 유효기간이고, 포인트가 아닌 항목에는 적용하지 않는다. 최초 1회 접근권이나 최종 금액 0으로 생긴 접근권은 환불/회수 처리 시 grant의 `source_kind`와 `source_reference`가 해당 결제 단위 row와 일치할 때만 삭제한다. 최초 1회 정책에서 기존 접근권으로 내려받은 반복 다운로드처럼 이번 요청에 연결된 차감/접근 로그가 없는 이력은 금액을 0으로 남기고 환불/회수 대상에서 제외한다. 파일이 여러 콘텐츠에 연결된 경우 공개 다운로드 URL의 `content_id`를 우선 맥락으로 기록하고, 누락되면 활성/공개 연결 중 하나로 fallback한다. 계정별 열람/다운로드/완료 버튼 처리 로그와 접근권 기록은 콘텐츠 모듈의 `privacy-export.php`에 포함한다. 최초 1회 열람/다운로드 판정은 거래 원장이나 쿠폰 사용 로그를 직접 읽지 않고 `sr_content_access_entitlements`를 우선 사용한다. 거래 원장과 쿠폰 사용 로그는 회계/운영 증빙 보관 대상으로 남기고, 회원 탈퇴/익명화 시 콘텐츠 접근권의 `account_id`와 원천 참조, 파일 다운로드 이력의 `account_id`를 비워 개인정보 보유기간과 접근권 상태를 분리한다. 콘텐츠 모듈은 이 세 로그를 기준으로 전체/특정 콘텐츠/특정 콘텐츠 그룹 회원 그룹 규칙을 제공하고, 성공 로그가 생긴 직후 콘텐츠 출처 규칙을 즉시 재평가한다.

#391 기준으로 콘텐츠 유료 열람 성공은 `sr_content_view_payment_logs`에 결제 단위 row를 같은 트랜잭션에서 함께 남긴다. 이 row는 `payment_type`, `settlement_kind`, 연결 자산 차감 로그 ID JSON, 쿠폰 redemption 링크, `payment_dedupe_key`, `refund_policy_version`을 보존한다. 전액 쿠폰, 일부 쿠폰+자산, 자산 결제, 최종 0원 settlement는 모두 영속 payment-unit row를 만들고, 기존 접근권 재사용은 새 결제 단위로 보지 않는다. `sr_content_refund_view_payment()`는 열람 payment-unit row를 잠그고 연결 자산 로그 환불, 쿠폰 state-only 환불, source-scope 접근권 회수, payment ledger item reversal, 환불 스탬프를 하나의 도메인 트랜잭션으로 처리한다. 결제 단위와 일치하는 접근권 source를 회수할 수 없으면 쿠폰/자산 환불 상태도 함께 rollback한다. 회원 탈퇴/익명화 시 열람 payment-unit의 `account_id`도 파일 다운로드 이력처럼 비워 개인정보 보유기간과 결제/환불 증빙을 분리한다.

#391 기준으로 커뮤니티 유료 게시글 열람 성공은 `sr_community_post_read_payment_logs`에 결제 단위 row를 같은 트랜잭션에서 함께 남긴다. 이 row는 게시판/게시글 snapshot, `payment_type`, `settlement_kind`, 연결 `sr_community_asset_logs` ID JSON, 쿠폰 redemption 링크, `payment_dedupe_key`, `refund_policy_version`을 보존한다. 전액 쿠폰, 일부 쿠폰+자산, 자산 결제, 최종 0원 settlement는 모두 영속 payment-unit row를 만들고, 기존 접근권 재사용은 새 결제 단위로 보지 않는다. `sr_community_refund_post_read_payment()`는 게시글 열람 payment-unit row를 잠그고 연결 자산 로그 환불, 쿠폰 state-only 환불, source-scope 접근권 회수, payment ledger item reversal, 환불 스탬프를 하나의 도메인 트랜잭션으로 처리한다. 결제 단위와 일치하는 접근권 source를 회수할 수 없으면 쿠폰/자산 환불 상태도 함께 rollback한다. 회원 탈퇴/익명화 시 게시글 열람 payment-unit의 `account_id`도 첨부 다운로드 이력처럼 비워 개인정보 보유기간과 결제/환불 증빙을 분리한다.

#391 기준으로 콘텐츠와 커뮤니티의 복합 자산 결제 허용 설정은 기본값이 켜짐이다. 끈 상태에서는 유료 열람/다운로드처럼 결제 단위를 남기는 화면의 관리자 저장 검증이 여러 포인트/금액 항목 선택을 거부해야 하며, public 결제 실행도 settlement allocation이 두 항목 이상이면 실패해야 한다. 일부 할인 쿠폰을 먼저 사용한 뒤 남은 금액이 복합 자산으로 배분되는 경우에는 쿠폰 사용 row, 쿠폰 사용 횟수, 자산 거래, payment-unit row가 같은 작업 트랜잭션에서 함께 rollback되어 접근권 없이 쿠폰만 소모된 상태를 남기지 않아야 한다.

커뮤니티 모듈도 같은 원칙을 따른다. 게시글/댓글 적립, 글쓰기/댓글 차감, 게시글 열람 차감, 첨부 다운로드 차감은 커뮤니티 설정과 게시판 설정에서 결정하고, 실제 포인트/적립금/예치금 증감은 활성 자산 모듈 helper를 호출한다. 관리자 자산 선택 UI에는 설치되어 있고 활성화된 자산 모듈만 표시한다. 글쓰기/댓글 차감, 게시글 열람 차감, 첨부 다운로드 차감은 여러 자산을 함께 선택할 수 있으며, 자산 선택과 자산별 금액을 같은 입력 묶음으로 보여준다. 차감 실행은 입력 금액 합계를 기준 통화 가격으로 보고 각 자산의 `purchase_power`에 따라 정확히 충당 가능한 자산 단위만 배분한다. 회원 그룹별 적용 선택은 자산 선택과 다른 의미의 정책 세트 선택이므로 별도 행으로 표시하되, 실제 설정 화면에서는 현재 선택한 자산과 맞는 정책 세트만 선택할 수 있고 뱃지 요약도 같은 자산 기준으로 표시한다. 기존 단일 금액은 호환용 합계로 유지한다. 커뮤니티 자산 로그의 `amount`는 실제 자산 단위이고 `settlement_amount`/`settlement_currency`는 기준가격 충당 단위, `purchase_power_snapshot_json`은 실행 당시 구매력/통화/정책 snapshot이다. 커뮤니티 자산 로그에는 `settlement_kind`, `snapshot_schema_version`, `rounding_policy_version`도 함께 저장해 통계/export와 환불 정책이 같은 정규 필드를 참조한다. 게시글/댓글 적립은 단일 자산 지급으로 유지한다. 게시판의 상태, 스킨, 접근, 레벨 활동 점수, 첨부, 배너, 팝업레이어, 게시글 적립, 댓글 적립, 글쓰기 차감, 댓글 차감, 유료 열람, 첨부 다운로드 차감은 현재 게시판에 저장하며, 운영자가 선택한 `그룹`/`전체` 적용 옵션은 현재 편집값을 같은 그룹 또는 전체 게시판에 한 번 복사한다. 게시판 그룹은 개별 게시판 설정 기본값을 제공하지 않고, 게시판 그룹 목록이나 게시판 목록의 그룹 필터에서 새 게시판을 만들면 그룹 ID만 선택된 상태로 시작한다. 게시판 생성 화면은 저장 시점의 게시판 값을 자체 설정으로 저장한다. 회원 자산 항목의 사용 여부, 자산, 금액, 과금 방식은 같은 항목 설정 묶음으로 저장한다. 커뮤니티 자동 회원 그룹 규칙은 전체 활동, 특정 게시판, 특정 게시판 그룹 기준을 제공하고, 게시판과 게시판 그룹 대상은 관리자 화면에서 선택 셀렉트로 고른다. 별도 적용값 미리보기 문구는 표시하지 않는다. 첨부 직접 접근도 게시글 유료 열람 정책을 확인하며, `once` 정책은 같은 세션의 중복 차감을 피하고 `every_view` 정책은 첨부 접근도 별도 열람으로 처리한다. `/admin/community/attachments` 조회 권한이 있는 관리자의 첨부 확인/다운로드는 운영 확인 동작으로 보고 게시글 유료 열람 차감과 첨부 다운로드 차감을 실행하지 않는다. 중복 방지는 원장 기록에서는 `sr_community_asset_logs.dedupe_key`로 처리하되, 최초 1회 접근권 판정은 `sr_community_access_entitlements`를 우선 사용한다. 커뮤니티 첨부파일은 게시글 첨부 행인 `sr_community_attachments`를 정본으로 삼고, `/admin/community/attachments`에서 상태와 다운로드 횟수를 관리한다. 실제 첨부 다운로드 성공 이력은 무료/유료를 함께 `sr_community_attachment_download_logs`에 남기며, `/admin/community/attachment-downloads`에서 게시판, 게시글, 첨부, 회원, 기간 기준으로 조회한다. 유료 첨부 다운로드 이력은 연결 자산 차감 로그 ID JSON, 쿠폰 redemption 링크, 쿠폰 dedupe key, `refund_policy_version`, 환불 스탬프를 보존한다. 유료 첨부 다운로드 수동 환불은 연결된 `sr_community_asset_logs`의 원 자산 거래를 환불 거래로 되돌리고, 최초 1회 첨부 다운로드 접근권을 회수하며, 같은 결제 record의 자산 거래/자산 로그/접근권 item을 `reversed`로 닫는다. 환불 사유와 포인트 환불 유효기간 기준은 관리자 POST에서 필수로 받으며, 다운로드 이력의 `refund_status`로 중복 처리를 막는다. 계정별 자산 로그, 접근권 기록, 첨부 다운로드 이력은 커뮤니티 모듈의 `privacy-export.php`에 포함하고, 회원 탈퇴/익명화 시 접근권의 `account_id`와 원천 참조, 첨부 다운로드 이력의 `account_id`를 비운다. 게시글 스크랩과 시리즈 스크랩도 계정별 개인 보관 데이터이므로 커뮤니티 모듈의 `privacy-export.php`에 포함한다. 커뮤니티 게시글/댓글 보상 회수 실패는 기존 커뮤니티 전용 화면이 아니라 `asset_ledger`의 `/admin/assets/recovery-failures` 포인트/금액 미회수 관리 화면에 기록한다.

`/community/scraps`의 게시글 스크랩과 시리즈 스크랩은 각각 20건씩 독립 페이지를 사용하고 전체 개인 보관 목록을 끝까지 탐색할 수 있어야 한다. 한 스크랩 종류의 페이지를 이동해도 다른 종류의 현재 페이지를 보존하며, 목록에서 해제한 뒤에는 게시글 상세로 이동하지 않고 가능한 현재 스크랩 페이지로 돌아간다.

보상 회수 실패 큐는 `asset_ledger`가 공통으로 소유한다. 정상 지급 성공 row를 `sr_asset_recovery_failures`에 저장하지 않고, 실패한 회수 작업만 `dedupe_key = source:{source_module}:{source_log_id}:rev:{reversal_event_key}` 형식으로 기록한다. 공통 관리자 화면은 `/admin/assets/recovery-failures` 포인트/금액 미회수 관리이며, 지급 로그는 각 자산/도메인 모듈의 읽기 전용 로그 화면으로 유지한다. 회수 실패 row의 상태는 `open`, `recovered`, `manually_resolved`, `cancelled`이고, 부분 회수는 `open` 상태에서 `recovered_amount > 0`으로 표현한다. 수동 해소와 취소는 남은 `unrecovered_amount`를 보존하며, 실제 reversal 원장 거래는 `sr_asset_recovery_reversal_links`에 연결한다. source 모듈은 `source_module`, `source_log_id`, `subject_type`, `subject_id`, `grant_event_key`, `reversal_event_key`, 운영 context를 넘기고, event key는 `{source_module}.{subject_domain}.{purpose}` 네임스페이스를 사용한다. `operation_context_json`은 allowlist 키만 저장하고, `failure_reason`은 자유 텍스트가 아니라 코드로 기록한다. 공통 회수 큐의 개인정보 export/cleanup도 `asset_ledger` 계약이 소유하며, 대상 회원 row와 관리자 actor row를 모두 사본/정리 대상으로 본다. source label, subject link, 회수 재시도 callback은 source 모듈의 `asset-recovery-targets.php` 계약에서 발견하며, 계약 미제공 또는 비활성 source 모듈은 운영자 메시지와 함께 fail-closed 한다.

게시글/댓글 비회원 작성은 표면별 운영 옵션으로만 연다. 작성자 표시명은 필수 snapshot으로 저장하고, 수정/삭제 검증에는 원문 비밀번호가 아니라 password hash 또는 1회성 관리 토큰 hash를 사용한다. IP/User-Agent는 원문 저장 없이 hash만 남기며, 게시판 동의가 켜져 있으면 계정 ID 없이도 제출 동의 snapshot을 남길 수 있어야 한다. 회원 그룹, 레벨, 보상, 멘션, 알림, 회원 자산 지급처럼 계정이 필요한 기능은 비회원 작성에서 제외하거나 별도 제한을 명시한다.

게시판별 추가 입력 항목은 게시글용과 댓글용을 분리해 설정한다. 게시글 확장은 `sr_community_board_field_definitions`, `sr_community_post_field_values`와 게시글 `extra_values_json` snapshot을 사용한다. 댓글용 정의는 게시판 설정의 `comment_extra_fields_json`에 두고, 제출 시점의 라벨·유형·수집 목적·목적 표시 여부·사본 포함·계정 정리 정책과 값을 `sr_community_comments.extra_values_json`에 함께 snapshot으로 저장한다. 커뮤니티 모듈 환경설정의 `extra_fields_json`과 `comment_extra_fields_json`은 새 게시판 등록 폼의 게시글·댓글 추가 입력 초기값만 제공한다. 콘텐츠·퀴즈·설문 모듈 환경설정의 `comment_extra_fields_json`도 신규 등록 폼의 초기값만 제공한다. 등록 전 별도 DB row에 복제하지 않고 기존 항목도 갱신하지 않으며, 저장 시에는 게시판 설정, `sr_content_items.comment_extra_fields_json`, `sr_quiz_sets.comment_extra_fields_json`, `sr_survey_forms.comment_extra_fields_json`에 최종 정의를 독립 저장한다. 콘텐츠·퀴즈·설문 관리 화면에서 `그룹` 또는 `전체`를 선택해 저장하면 현재 편집값을 같은 운영 그룹 또는 삭제되지 않은 전체 항목에 한 번 복사하고, 이후에는 각 항목이 다시 독립 값을 가진다. 공개 댓글 작성과 검증은 모듈 환경설정이 아니라 해당 항목에 저장된 정의를 사용하고, 댓글 값 snapshot은 각각 `sr_community_comments`, `sr_content_comments`, `sr_quiz_comments`, `sr_survey_comments`의 `extra_values_json`에 저장한다. 댓글과 답글은 같은 정의를 적용하며, 비밀 댓글의 추가 값은 본문 열람 권한이 있는 사용자에게만 보인다. core/member 테이블을 추가 입력값으로 넓히지 않는다.

퀴즈와 설문은 각각 `sr_quiz_groups`, `sr_survey_groups`가 운영 묶음을 소유하고 개별 항목은 nullable 그룹 ID로 연결한다. `/admin/quiz/groups`와 `/admin/surveys/groups`에서 그룹을 관리하며 그룹 삭제 시 항목을 삭제하지 않고 그룹 연결만 해제한다. 게시판·콘텐츠·퀴즈·설문 그룹을 삭제할 때 해당 그룹을 가리키던 설정 적용 범위 기록은 각 항목의 `단독` 값으로 되돌리고, `전체` 적용 기록은 유지한다. 퀴즈는 화면 표시, 공개 상태/기간, 채점, 응시 제한, 댓글, 리액션, 댓글 추가 입력, 보상 정책을 적용 범위로 제공한다. 설문은 화면/목록 표시, 공개 상태/기간, 참여 제한, 동의/개인정보 안내, 댓글, 리액션, 댓글 추가 입력, 보상 정책을 적용 범위로 제공한다. 제목, 설명, 퀴즈 문항/결과 규칙, 설문 문항/연구·QA 기록은 개별 콘텐츠이므로 범위 복사 대상에 포함하지 않는다. `sr_quiz_setting_sources`와 `sr_survey_setting_sources`는 마지막 선택 범위를 관리자 화면에 다시 표시하기 위한 기록이며 런타임 상속 계약이 아니다.

추가 입력 항목의 내부 key는 운영자에게 노출하지 않고 소문자 영문, 숫자, `_`로 자동 생성한다. 필수/조건부 필드는 관리자 UI, 공개 UI, 서버 POST 검증을 맞추고 필수 checkbox는 서버에서도 체크된 값만 유효하게 처리한다. 잘못된 key, 중복 key, 빈 라벨, 잘못된 유형/정책, 선택지 누락은 저장 오류로 처리한다. 추가 입력값은 text/select 1000자, textarea 5000자를 넘거나 배열 payload이면 잘라 저장하지 않고 서버에서 거부한다. 수집·이용 목적은 `show_privacy_purpose`가 켜진 항목만 공개 작성 화면의 해당 입력란 아래에 표시하며, 기존 정의처럼 이 값이 없으면 표시로 해석한다. 개인정보 사본 제공은 snapshot의 `export_policy=include` 값만 내보내고, 탈퇴/익명화 cleanup은 `cleanup_policy=retain`이 아닌 값을 비운다. 댓글 삭제 상태 전환 시 추가 입력 snapshot도 본문과 함께 비운다. 게시글 관리자 목록 표시와 검색은 기존 `show_in_admin` snapshot 정책을 유지한다.

쿠폰은 포인트/적립금/예치금과 다른 지급형 회원 자산이므로 `coupon` 모듈이 쿠폰 종류, 회원별 지급, 사용 이력을 독립 소유한다. `sr_coupon_definitions.coupon_type`은 1차 사용 모델 분류이며, 현재 `access`, `fixed_discount`, `percent_discount`를 관리자 폼에서 생성할 수 있다. `access` 쿠폰은 단발 또는 제한 횟수 접근권 사용을 `sr_coupon_redemptions`에 기록하고 쿠폰별 잔액을 보존하지 않으므로, 부분 사용 가능한 쿠폰 잔액 원장과 `stored_value` 모델은 아직 구현된 상태로 보지 않는다. `fixed_discount`와 `percent_discount`는 콘텐츠 유료 열람, 콘텐츠 파일 다운로드, 커뮤니티 유료 게시글 열람, 커뮤니티 첨부 다운로드에서 사용처 pricing resolver가 반환한 결제 기준 금액에 적용된다. 할인액이 결제 금액 전체를 덮으면 쿠폰 접근권만 남기고, 일부만 덮으면 같은 작업 트랜잭션에서 쿠폰 사용 row와 남은 금액의 자산 차감을 함께 확정한다. 다만 할인 쿠폰 사용과 남은 자산 차감을 하나의 도메인 취소로 되돌리는 계약은 아직 없으므로, `fixed_discount`와 `percent_discount` 정의는 환급 가능 정책으로 저장할 수 없다. 관리자 쿠폰 정의 폼의 정액 할인 금액은 현재 원화 기준 단일 값으로 입력받으며, 다중 통화별 할인 금액, 환율 snapshot, 적용 통화 선택은 소비 도메인의 할인 계산 계약에서 별도로 정의해야 한다. 쿠폰 정의는 생성 시점에만 사용기간 정책을 저장하며 `none`, `fixed_range`, `fixed_expiry`, `relative_days`를 지원한다. `fixed_range`는 시작/만료 시각을 모두 요구하고, `fixed_expiry`는 만료 시각을 요구하며, `relative_days`는 발급 후 1일부터 3650일 사이의 기간을 요구한다. 만료 시각이 현재 이하이거나 만료가 시작 이하이면 저장 또는 지급을 거부한다. 수동 지급은 정의 정책을 그대로 발급본 `starts_at`/`expires_at`에 반영하고, 발급 캠페인은 정의의 미래 `valid_from`이 있어도 발급본 시작을 `issued_at`으로 당겨 즉시 사용 가능하게 하며 캠페인별 고정 만료 또는 상대 만료 override를 둘 수 있다. 회원 보유 쿠폰 목록과 탈퇴 자산 산정은 미래 시작 활성 쿠폰을 포함하지만, 사용 후보 조회, 쿠폰 사용 SQL, 회원 요약 배지의 usable count는 `starts_at <= now`인 지급건만 포함한다. 쿠폰 사용처 후보는 소비 모듈이 `coupon-targets.php` 계약으로 제공하고, 쿠폰 모듈은 활성 모듈의 계약만 읽어 관리자 선택지와 저장 검증에 사용한다. `coupon-targets.php`의 `target_type`은 쿠폰, 배너, 팝업레이어가 함께 쓰는 전역 식별자이므로 모듈 간 중복 선언을 허용하지 않고, 런타임 소비자는 이미 등록된 `target_type`을 덮어쓰지 않는다. 특정 target에 연결한 환급 가능 쿠폰은 저장 시점에 `access` 혜택 모델과 해당 target 계약의 `revoke_access` capability를 요구한다. 사용처 검색과 수동 환불 시 접근권 회수도 `search_function`, `revoke_access_function` 같은 소비 모듈 콜백으로 위임하되, 환불하는 redemption의 `target_type`이 가리키는 계약만 호출한다. 환급 가능 사용 내역에 해당 target의 `revoke_access_function`이 없거나 회수 대상 접근권이 0건이면 환불 전이를 rollback한다. 금액성/차감형 사용처는 `capabilities`와 `pricing_function`을 통해 현재 가격, 통화, 이미 접근권이 있는지 여부를 조회할 수 있고, search-only target은 금액성 사용처 후보에서 제외한다. 관리자 대상 검색 결과는 target capability와 현재 가격 또는 가격 조회 불가 사유를 함께 표시해 운영자가 저장 전에 사용처 계약 상태를 확인할 수 있게 한다. 쿠폰 사용 helper는 같은 `dedupe_key`의 기존 사용을 먼저 반환하고, 그 다음 pricing resolver가 `already_entitled`를 반환하면 쿠폰 사용 row와 사용 횟수를 남기지 않는 no-op으로 끝낸다. 사용처가 pricing resolver를 제공하면 쿠폰 사용 시점의 금액, 통화 또는 자산 단위, 정책 요약, 가격 산정 시각, target type/id, 적용 쿠폰 유형, 할인액, 남은 결제액을 화이트리스트 스냅샷으로 `sr_coupon_redemptions`에 저장한다. 쿠폰 사용은 `sr_coupon_redemptions.dedupe_key`로 중복 사용을 막고, 회원 탈퇴 시 활성 쿠폰은 환급 정책에 따라 `withdrawn_expired` 또는 `refund_requested` 상태로 전환한다. 쿠폰 지급/사용 내역과 수동 환불 이력은 쿠폰 모듈의 `privacy-export.php`에 포함한다. 관리자 화면은 쿠폰 관리(`/admin/coupons`), 지급 내역(`/admin/coupons/issues`), 사용 내역(`/admin/coupons/redemptions`), 발급 캠페인(`/admin/coupons/campaigns`)으로 나뉘며, 쿠폰 추가와 개별 쿠폰 지급은 쿠폰 관리 화면의 모달에서 처리한다. 발급 캠페인은 `sr_coupon_claim_campaigns`가 공개 노출, 발급 기간, 재고, 회원당 제한을 소유하고, `sr_coupon_claim_logs`가 발급 의도 dedupe, 점유 상태, 발급 결과와 claim log 단위 자산 reference audit 사본을 기록한다. `sr_coupon_issues`는 claim 유형, campaign/log 연결, 명목 가격, 자산 reference, claim snapshot을 발급본 기준으로 보존하며, 이 필드는 환불/감사/개인정보 사본 제공의 기준 자료다. 자산 차감형 유료 발급은 `payment_reference_*`를 사용하지 않고 `asset_reference_*`와 claim snapshot으로 복원 근거를 남기며, 사용자가 선택한 포인트/금액 항목 목록 전체가 캠페인 허용 목록 안에 없으면 다른 항목으로 fallback하지 않고 발급 전 실패로 끝낸다. `/admin/coupons/campaigns`는 캠페인 생성/수정과 검색 가능한 캠페인 목록을 보여주고, `/admin/coupons/campaigns/logs`는 회원/상태/발급 표면/캠페인·쿠폰 검색어로 필터링할 수 있는 최근 발급 로그를 별도 읽기 전용 화면으로 보여주며, 발급 로그가 생긴 캠페인의 key와 연결 쿠폰 변경, 이미 점유된 수보다 낮은 발급 한도 변경을 서버에서 차단한다. lazy 재고 계산에서 이미 빠졌지만 아직 sweep 또는 self-expiry로 물질화되지 않은 예약 행은 `만료(미정리)`로 구분한다. 공개 `/coupons` 쿠폰존은 활성 공개 무료 캠페인 목록을 표시하고, `/coupons?campaign={campaign_key}`는 같은 발급 helper를 쓰는 단건 캠페인 화면으로 동작한다. 쿠폰 모듈은 `url-embed-targets.php` 계약으로 `/coupons?campaign=...` 내부 URL 임베드를 제공하며, 본문 임베드 카드는 캠페인 상태와 남은 수량을 서버에서 계산해 보여주고 실제 발급은 같은 쿠폰존 POST로 이어진다. 팝업레이어는 `coupon_claim_campaign_key`로 `popup_layer` 노출 위치가 켜진 공개 무료 캠페인을 CTA로 연결할 수 있고, 렌더링 시 같은 쿠폰 공개 helper로 발급 가능 상태를 다시 계산해 로그인 CTA, 받기 CTA, 이미 발급/소진 상태 메시지를 표시한다. 같은 발급 의도 토큰의 재시도는 기존 claim row와 발급본으로 수렴한다. 지급 내역은 상태, 사용처, 회원 검색, 쿠폰 검색어로 필터링하고 회원, 쿠폰, 사용처, 상태, 사용 횟수, 지급일을 정렬할 수 있다. 사용 내역은 상태, 환급 정책, 사용처, 회원 검색, 쿠폰 검색어로 필터링하고 회원, 쿠폰, 사용 대상, 가격 스냅샷, 상태, 사용일, 환불일을 확인할 수 있다. 지급하기 모달은 회원 검색 스택 모달로 개별 회원을 선택하거나 활성 회원 전체, 활성 회원 그룹을 대상으로 지급할 수 있다. 쿠폰 추가 모달의 대상 번호는 사용처별 검색 스택 모달에서 소비 모듈이 제공한 검색 콜백으로 대상을 찾아 채울 수 있다. `/admin/coupons/issues`는 최근 지급 내역과 지급 취소를 제공하고, `/admin/coupons/redemptions`는 최근 사용 내역과 환급 가능 정책의 사용 완료 내역 수동 환불을 제공한다. 수동 환불은 환불 사유를 필수로 받으며, 쿠폰 사용으로 생긴 소비 모듈 접근권을 사용 로그의 `target_type`과 `dedupe_key` 기준으로 먼저 회수한다. 접근권 회수가 실패하거나 회수 대상이 없으면 같은 트랜잭션 rollback으로 사용 로그와 지급 건 상태를 유지하고, 회수가 성공할 때만 사용 로그를 `refunded`로 바꾸며 지급 건의 사용 횟수를 1회 되돌린다. 지급 건이 `used` 상태였으면 `active`로 되돌린다.

회원 `/account/coupons`는 사용 가능한 보유 쿠폰·이용권을 20건씩 나누고 100건 이후를 포함한 전체 보유 목록을 끝까지 탐색할 수 있어야 한다. 특정 결제 대상의 사용 후보 조회는 최근 보유 쿠폰 일부를 PHP에서 다시 거르는 방식이 아니라 계정·사용기간·대상 조건을 SQL에서 직접 적용해, 더 최신인 무관 쿠폰이 많아도 대상 쿠폰을 놓치지 않는다.

#391 기준으로 쿠폰 환불은 `sr_coupon_refund_redemption_state_only()`와 단독 관리자 환불 wrapper를 분리한다. state-only primitive는 redemption 상태, 환불 dedupe key, issue `used_count`/상태만 바꾸고 접근권 회수, payment ledger item reversal, 알림 발송은 하지 않는다. 기존 `sr_coupon_refund_redemption()`은 같은 primitive를 호출한 뒤 쿠폰 source 접근권 회수와 payment ledger reversal을 같은 트랜잭션에서 처리하고, commit 이후에만 `redemption.refunded` 알림을 발화한다. 콘텐츠/커뮤니티 열람 복합결제 환불은 도메인 payment-unit 환불 계약에서 state-only primitive와 자산 환불, source-scope 접근권 회수를 하나의 도메인 트랜잭션으로 오케스트레이션한다. `coupon_partial_discount_asset`로 연결된 redemption은 쿠폰 단독 환불을 fail-closed하고 도메인 결제 내역 환불로 처리한다.

쿠폰 정의 상태는 `active`, `issue_stopped`, `disabled`로 나뉜다. `active`는 신규 지급과 기존 지급분 사용을 모두 허용하고, `issue_stopped`는 신규 지급과 공개 발급 캠페인을 막되 이미 지급된 활성 쿠폰 사용은 허용하며, `disabled`는 신규 지급과 기존 지급분 사용을 모두 막는다. 발급/사용 이력 또는 운영 참조는 상태 변경을 차단하지 않고 참조 현황과 감사 로그로 남긴다.

정액 할인, 정률 할인, 특정 주문/상품 금액 차감권처럼 금액 계산이 필요한 쿠폰의 정의와 지급 상태는 쿠폰 모듈이 소유한다. 실제 주문, 결제, 콘텐츠 구매, 커뮤니티 유료 기능처럼 적용 도메인이 생길 때 해당 모듈은 할인 계산/적용 계약을 제공해야 한다. 적용 도메인이 최소 금액, 과세/비과세, 배송비, 부분 취소, 환불, 중복 할인 같은 정책을 소유하고, 쿠폰 모듈은 쿠폰 종류/지급/사용 이력과 중복 사용 방지처럼 쿠폰 자체의 생명주기를 맡는다. 금액형 쿠폰도 포인트/적립금/예치금 원장에 합쳐서 잔액처럼 관리하지 않고, 사용 결과는 `sr_coupon_redemptions` 또는 쿠폰 모듈 소유의 확장 사용 이력 테이블에 기록한다. 소비 도메인은 쿠폰 helper를 통해 계산/사용을 요청하고, 자기 도메인 로그에는 적용된 쿠폰 사용 ID와 할인 결과만 참조한다.

회원 탈퇴는 member 모듈이 조정하되 잔액 처리는 각 자산 모듈의 원장 helper를 호출한다. 탈퇴 화면은 포인트/적립금/예치금 잔액이 남아 있을 때만 남은 자산 처리 섹션을 표시한다. 포인트와 적립금은 `member.withdrawal` 참조의 `expire` 거래로 0 처리하고, 예치금은 환불 은행/예금주/계좌번호를 입력받은 뒤 `member.withdrawal` 참조의 `withdraw` 거래로 0 처리한다. 예치금 탈퇴 거래 사유에는 환불 계좌가 제공됐다는 사실만 남기고 은행명, 예금주, 계좌번호 원문은 남기지 않는다. 이 자산 원장 처리는 프로필 삭제, 세션 폐기, 동의 철회, 계정 익명화와 같은 탈퇴 트랜잭션 안에서 수행한다.

회원이 로그인 상태에서 보유 자산을 정산 요청하는 흐름은 각 자산 모듈이 소유한다. 적립금은 `/account/rewards`에서 출금 신청을 받고 `sr_reward_withdrawal_requests`에 계좌 정보, 금액, 상태, 처리 메모를 남긴다. `/admin/rewards/settings`는 출금 신청 사용 여부와 신청 대상을 저장한다. 출금 신청을 사용하지 않으면 회원 화면 폼과 직접 POST를 막고, 사용할 때는 허용 회원 그룹 중 하나에 속한 회원만 신청할 수 있다. 허용 회원 그룹을 선택하지 않으면 전체 로그인 회원이 신청할 수 있다. 예치금은 `/account/deposits`에서 환불 신청을 받고 `sr_deposit_refund_requests`에 같은 형태로 남긴다. `/admin/deposits/settings`는 환불 신청 사용 여부와 신청 대상을 저장한다. 환불 신청을 사용하지 않으면 회원 화면 폼과 직접 POST를 막고, 사용할 때는 허용 회원 그룹 중 하나에 속한 회원만 신청할 수 있다. 허용 회원 그룹을 선택하지 않으면 전체 로그인 회원이 신청할 수 있다. 대기 중 신청 금액은 신청 가능액에서 제외하며, 회원은 대기 상태 신청만 직접 취소할 수 있다. `/account/points`의 거래 내역과 `/account/rewards`, `/account/deposits`의 신청·거래 내역은 20건씩 페이지를 나누고 전체 이력을 끝까지 탐색할 수 있어야 한다. 신청과 거래가 한 화면에 함께 있는 경우 각각 독립된 페이지 번호를 사용하고 한 목록을 이동해도 다른 목록의 현재 페이지를 보존한다. 관리자는 `/admin/rewards/withdrawal-requests`, `/admin/deposits/refund-requests`에서 대기 신청을 완료 또는 거부한다. 개별 거부는 별도 모달에서 거부 사유를 입력해 처리한다. 일괄처리는 현재 필터와 검색 조건에 맞는 대기 신청을 최대 100건까지 서버에서 다시 조회해 처리한다. 완료 처리만 각 원장에 `transaction_type=withdraw` 음수 거래를 만들고, 거부/취소는 신청 상태만 바꾼다. 신청 계좌 정보와 처리 이력은 해당 자산 모듈의 `privacy-export.php`에 포함한다. 적립금 출금 신청과 예치금 환불 신청은 탈퇴/익명화 cleanup에서 금액, 상태, 거래 연결, 처리자 증빙은 유지하되 은행명, 계좌번호, 예금주, 요청자/관리자 note를 빈 값으로 정리한다. 탈퇴 또는 익명화 상태의 회원에 연결된 포인트/적립금/예치금 원장, 쿠폰 지급/사용/발급 로그, 환전 로그는 법정 보관기간 만료 후 관리자 보관 정책 정리에서 `account_id`와 처리자 계정 연결을 0 또는 null로 낮추고 금액, 상태, 거래번호, 거래 시각은 증빙으로 유지한다. 전자상거래성 금액 기록은 5년, 분쟁성 메모와 실패 사유는 3년 cut-off를 사용한다.

포인트/적립금/예치금 원장 helper와 쿠폰·이용권 지급/사용/상태 변경/사용 환불/발급 환불 helper는 거래 또는 권리 상태 변경 성공 후 알림 모듈이 활성화되어 있고 `notification-events.php` 계약을 제공하면 계약의 계정 이벤트 생성 함수를 통해 회원 대상 알림을 만든다. 포인트, 적립금, 예치금, 쿠폰 모듈은 각 알림/메일 관리 화면에서 거래/지급/사용/환불/상태 변경 같은 회원 알림 케이스별 제목, 본문, 사용 여부와 채널을 저장한다. 알림/메일 관리의 상태가 중지이면 계정 이벤트 생성 함수는 `sr_notifications` row를 만들지 않고 no-op으로 끝낸다. 알림톡 채널을 연동하는 경우 본문은 카카오 비즈니스에서 승인된 템플릿을 사용하므로 사이트는 알림톡 본문을 편집하지 않고 이벤트별 알림톡 템플릿 코드만 `sr_notification_channel_template_bindings`에 저장한다. 쪽지 모듈도 같은 알림/메일 관리 화면 패턴으로 새 쪽지 알림의 제목, 본문, 사용 여부와 채널을 저장한다. 이 알림/메일 관리 메뉴와 화면은 알림 모듈이 활성화된 경우에만 사용할 수 있다. 회원 모듈은 이메일 인증 완료, 비밀번호 변경/재설정 완료, 2차 인증 설정/복구코드 재발급/해제, OAuth 연결/해제 같은 계정 보안 완료 이벤트를 `member.security.*` 계정 이벤트로 만들고 운영자는 `/admin/member-notification-templates`에서 문구와 채널을 관리한다. 같은 화면은 `member.email_verification`, `member.password_reset`, `member.login_mfa_email_code` 발송 템플릿도 함께 보여주며, 메일 문구 수정은 `sr_delivery_template_overrides`에 저장하고 기본값 복원은 override 삭제로 처리한다. 환전 출금/입금/수수료 알림도 포인트/적립금/예치금 모듈의 알림/메일 관리 화면에서 편집한다. 환전 환경설정 화면은 전역 환전 사용 여부도 저장하고, 이를 끄면 회원 환전 후보, 예상 금액 계산, 실행 helper가 모두 거부되지만 기존 로그 조회와 완료 묶음 정정은 유지된다. 포인트 모듈의 회원 알림 케이스 기본값은 사용 안 함이고, 적립금/예치금/쿠폰 모듈은 기존 동작 보존을 위해 케이스 기본값을 사용으로 둔다. 쪽지 모듈은 기존 새 쪽지 사이트 알림 동작 보존을 위해 새 쪽지 알림 케이스 기본값을 사용으로 둔다. 콘텐츠/커뮤니티 댓글 작성자 알림과 콘텐츠/커뮤니티/퀴즈/설문 댓글 멘션도 같은 계정 이벤트 생성 함수와 DB 템플릿을 사용한다. 댓글 멘션 대상은 member 공개 이름/멘션 helper로 해석하며, 후보 자동완성은 `@공개이름#prefix`를 삽입해 동명이인을 구분한다. 새 쪽지 알림의 기본 링크는 `/message?id={message_id}`이며, 알림 링크 읽음 처리는 같은 링크의 안읽은 알림을 함께 읽음 처리한다. 닉네임 강제 초기화처럼 회원에게 보여줄 비템플릿 알림은 같은 계약의 일반 알림 생성 함수만 호출하고 알림 저장 테이블을 직접 쓰지 않는다. 커뮤니티 신고, 콘텐츠 등록자 신청, 개인정보 요청 대응 기록 생성/상태 변경, 이메일 발송 실패 표시, 저장소 정리 재시도 실패처럼 운영자가 조치해야 하는 이벤트는 `admin-notification-events.php` 계약으로 관리자 운영 알림을 만들며, 알림 모듈이 비활성화되었거나 운영 알림 테이블이 없으면 기존 업무 처리를 실패시키지 않는다. 포인트와 적립금 모듈은 각 환경설정의 유효기간 설정이 1 이상이면 새 `grant` 양수 거래에 유효기간을 붙이고, 이후 사용/차감 거래는 만료 예정 지급분부터 `expires_remaining`을 줄이며 각각 `sr_point_expiration_consumptions`, `sr_reward_expiration_consumptions`에 소비 거래와 원 지급 거래를 연결한다. 포인트 환불 거래는 환불 건마다 `refund_expiration_policy`를 받아 기본값 `original`이면 환불 참조 원거래의 `expires_at`을 우선 사용하고, 참조가 사용/차감 거래이면 소비 매핑에 남은 원 지급 거래의 유효기간을 사용한다. 이전에 같은 원거래를 일부 환불한 경우 그 수량만큼 소비 매핑을 건너뛰고, 여러 유효기간 지급분이 복원되면 유효기간별 `refund` 거래를 나누어 만든다. 포인트/적립금/예치금 원장 환불은 차감된 원거래를 선택한 경우에만 허용하며, 참조 없는 수동 환불과 양수 원거래 환불은 서버에서 거부한다. 원거래의 남은 환불 가능 수량/금액을 넘는 환불도 트랜잭션 안에서 다시 검증해 거부한다. 포인트 환불에서 `reset`이면 환불 시점부터 유효기간을 계산한다. `original`을 선택했지만 참조 원거래나 소비 매핑에 유효기간이 없으면 무기한 복원을 피하기 위해 유효기간이 켜진 경우 환불 시점 기준으로 계산한다. 포인트의 기한이 지난 지급분은 다음 포인트 거래 전 또는 운영 스크립트 `.tools/bin/expire-points.php` 실행 시 `transaction_type=expire`, `reference_type=point_expiration`, `reference_id=point_transaction:{id}` 거래로 차감한다. 적립금의 기한이 지난 지급분은 다음 적립금 거래 전 또는 회원 적립금 화면 조회 시 `transaction_type=expire`, `reference_type=reward_expiration`, `reference_id=reward_transaction:{id}` 거래로 차감한다. 적립금 지급 내역을 되돌릴 때는 환불이 아니라 `transaction_type=reclaim` 음수 거래를 사용한다. 예치금을 회원에게 내보내는 처리는 원장 환불이 아니라 `withdraw` 음수 거래 또는 예치금 환불 신청 완료 흐름으로 처리한다. 적립금 운영자 직접 회수는 별도 회수 알림 템플릿을 사용한다. 회수 거래는 양수 원거래를 `reference_type=reclaim`, `reference_id=reward_transaction:{id}`로 참조해야 하며, 같은 원거래의 누적 회수액은 원거래 금액을 넘을 수 없다. 회수는 거래 내역 화면에서 대상 내역을 선택해 처리하고, 회수 거래는 다시 환불하지 않는다. 알림 모듈이 비활성화되어 있거나 설치되어 있지 않으면 no-op으로 처리하고 자산 처리나 댓글 저장을 실패시키지 않는다. 이벤트 템플릿은 `sr_notification_event_templates`가 소유하며 기본 채널은 `site`다. 회원 외부 푸시는 알림 모듈 provider와 회원별 수신처가 준비되어 있고 알림 생성 요청이 외부 채널을 명시적으로 제한하지 않은 경우 회원 계정 알림에 자동으로 발송 후보를 추가한다. 회원에게 노출되는 링크는 각 자산의 `/account/points`, `/account/rewards`, `/account/deposits`, `/account/coupons`, `/mypage/security` 화면 또는 댓글/쪽지 대상 URL을 사용한다. 회원이 비어 있지 않은 `link_url`을 가진 알림을 읽음 처리하면 같은 계정의 동일 `link_url` 안읽은 알림과 전체 대상 동일 `link_url` 알림도 해당 회원 기준으로 함께 읽음 처리한다.

회원의 `/account/notifications` 읽음·안읽음 목록과 `/messages` 받은·보낸 쪽지함은 각각 20건씩 페이지를 나누고 전체 이력을 끝까지 탐색할 수 있어야 한다. 알림 페이지 이동은 읽음 상태 필터를, 쪽지 페이지 이동은 받은·보낸 편지함 구분을 보존하며 행 단위 읽음·삭제 작업 뒤에도 가능한 현재 목록 위치로 돌아간다.

알림 모듈의 이메일 채널 사용 설정이 꺼져 있으면 알림 이벤트 채널에 `email`이 저장되어 있더라도 `sr_notification_deliveries`에는 해당 채널의 발송 작업을 등록하지 않는다. 회원의 transactional email도 `email-delivery.php` 계약에서 같은 설정을 확인하므로 이메일 인증, 비밀번호 재설정, 로그인 이메일 2차 인증 발송을 시도하지 않는다. 회원 화면은 이메일이 필요한 요청을 안내와 함께 비활성화하고 서버도 직접 POST를 거부한다. 회원 외부채널 수신 허용 또는 provider별 회원 수신 허용이 꺼져 있을 때도 저장된 `slack_webhook`, `discord_webhook`, `telegram_bot` 선택값은 보존하되 발송 작업은 등록하지 않는다. 이 상태는 알림/메일 관리 목록의 상태 열에서 `사용(부분중지)`로 요약하고, 발송 수단 열과 상태 설명에서 `이메일 중지` 또는 `외부채널 중지`로 구체 원인을 노출한다. `외부채널 중지` 상태 설명은 `/admin/notifications/settings#notification-settings-section-external-push`를 새 탭으로 여는 설정 링크를 제공한다.

공통 리액션은 `reaction` 모듈이 정의와 원장을 소유하고, 콘텐츠·커뮤니티·퀴즈·설문은 `reaction-targets.php` 계약으로 대상 검증과 공개 가능 여부만 제공한다. 1차 target은 `content/content`, `content/comment`, `community/post`, `community/comment`, `quiz/quiz_set`, `survey/survey_form`, `survey/comment`이며 계약은 단건과 batch resolve, target 상태, viewer별 열람 가능 여부, 반응 가능 여부, target owner/author, 알림 생성 가능 여부, 적용 preset key 또는 허용 reaction key 목록을 반환해야 한다. 계약 제공 모듈은 자기 모듈 key와 같은 `target_module`만 선언할 수 있고, `reaction`은 다른 모듈 target을 선언한 계약 항목을 무시한다. 1차 리액션은 target/account 기준 단일 선택이고 unique 기준은 `(account_id, target_module, target_type, target_id)`다. 같은 key 재클릭은 취소, 다른 key 클릭은 같은 row의 `reaction_key` 교체로 처리한다. 작성자 본인은 자기 글/댓글/콘텐츠/퀴즈/설문 target에 신규 적용/변경 write를 할 수 없고, 알림 생성에서도 actor와 recipient가 같으면 no-op 처리한다. 쓰기 자체가 불가능하거나 target 계약에서 알림 제외로 표시한 target은 알림 생성에서도 제외한다. 알림 모듈이 비활성화되어 있거나 템플릿이 없으면 reaction write는 실패하지 않는다. 운영자는 관리자 화면에서 리액션 종류와 preset을 추가, 수정, 정렬, 사용 중지할 수 있으며, 리액션 아이콘은 이모지, Material icon key, 직접 업로드한 JPG/PNG/WebP 이미지 중 선택한다. 콘텐츠와 커뮤니티는 환경설정 preset을 기본값으로 쓰고 개별 콘텐츠/게시판 설정이 있으면 그 값을 우선한다. 퀴즈/설문은 환경설정 < 개별 설정 순으로 preset을 상속한다. 콘텐츠·커뮤니티·퀴즈·설문의 리액션 사용 여부와 기본 preset 환경설정은 `reaction` 모듈이 설치되어 있고 활성화된 경우에만 편집 가능하며, 모듈이 없으면 화면에서 disabled 안내를 보여주고 저장 action은 직접 전달된 리액션 설정 값을 거부하거나 disabled 상태로 정규화한다. 전체 definition 수는 제한하지 않되 preset별 공개 노출 key 수는 기본 6개, hard safety cap 12개로 제한한다. 사용 중지 key의 기존 레코드는 보관 후 공개 숨김, 관리자/통계 유지, 삭제, 병합 중 운영자가 선택하고, 삭제/병합은 영향 수 계산과 확인 문구, CSRF, 별도 권한, 감사 로그 metadata를 요구한다. reaction은 스크랩, 콘텐츠 완료, 퀴즈 시도, 설문 응답, 보상 grant, 추천/랭킹/SEO 정책을 대신하지 않는다.
...
커뮤니티 피신고자 계정 guard는 커뮤니티 모듈 소유의 가역적 guard 표면으로 둔다. `sr_community_account_guard_events`는 판단 이벤트와 trigger fingerprint/snapshot을, `sr_community_account_guards`는 현재 guard 상태를, `sr_community_account_guard_locks`는 작성자 단위 평가 직렬화를 담당한다. 기본 설정은 publication hold와 confirmed hold 모두 비활성이며, `/admin/community/settings`의 신고 자동조치 섹션에서 운영자가 publication hold 대상 수/기간/overlap 검토 기준과 confirmed hold 집계 기간/확정 건수/기간을 명시적으로 저장한다. #388 자동조치가 active로 생성되거나 관리자가 active 자동조치를 confirmed/released로 전이한 뒤에는 post-commit 평가 helper가 작성자 lock row 아래에서 현재 active 게시글 자동조치 또는 confirmed 자동조치 집합을 재집계한다. 다중 active 게시글 자동조치의 신고자 overlap이 설정 기준 이상이면 active hold 대신 `needs_review` 이벤트만 남기고, 조건을 통과하면 `publication_hold`를 만든다. confirmed 반복 조건을 통과하면 별도 `confirmed_hold`를 만든다. current guard는 `active_guard_uid = account_id:guard_type` unique 기준으로 같은 계정/guard type당 활성 row 하나만 허용하고, released/expired/cancelled 같은 terminal 전이에서 helper가 active UID를 비운다. `/admin/community/reports`는 현재 활성 계정 guard 목록을 함께 보여주고, 운영자 해제는 필수 해제 메모를 받은 뒤 `released`로 전이하며 메모를 포함한 `community.account_guard.released` 감사 로그를 남긴다. public 회원 게시글 작성 경로는 active `write_cooldown`을 차단으로 해석하고, active `confirmed_hold` 또는 `publication_hold`를 신규 게시글 initial status `pending`으로 해석한다. pending 저장 후 작성자는 `/community/my?type=posts`에서 검토 대기 상태를 확인한다. 댓글 pending surface는 아직 없으므로 active hold가 댓글 작성 initial status를 바꾸지 않는다. import, board-copy, admin 상태 변경 같은 비대화형/운영자 경로는 이 write decision helper를 호출하지 않는다.
...
회원 콘텐츠 임시저장·제출과 적립금 출금·예치금 환불 신청 POST는 성공과 검증 실패 모두 같은 회원 GET 화면으로 redirect한다. 실패 입력값과 오류는 한 번 소비하는 짧은 세션 flash로 복원하고 UI kit alert 기반 토스트로 표시해, 새로고침이 같은 작업을 다시 제출하지 않게 한다.

회원 `/account/content`의 제출 내역은 20건씩 페이지를 나누고 전체 제출본을 끝까지 탐색할 수 있어야 한다. 제출본 편집 `id`와 목록 `page`는 독립 query로 유지하며, 목록에서 편집을 열거나 임시저장·제출·검증 실패로 돌아와도 가능한 현재 제출 내역 페이지를 보존한다.
...
`privacy` 모듈은 관리자 전용 개인정보 요청 대응 기록과 사본 제공 보조 흐름을 조정한다. #377 결정에 따라 회원이 직접 요청 티켓을 등록하는 워크플로우는 기본 기능으로 열지 않고, 회원 화면은 사본 직접 내려받기와 운영자 문의 안내에 머문다. 운영자가 외부 문의로 접수한 건을 기록해야 할 때만 관리자 개인정보 요청 대응 기록을 사용한다. 정정, 삭제, 처리 제한, 처리 거부, 동의 철회 request type은 접수 분류와 이력을 남기는 값이며, `privacy` 모듈이 데이터 소유 모듈의 action을 자동 실행하지 않는다. 회원 계정, 인증, 동의처럼 `member`가 소유한 데이터도 `modules/member/privacy-export.php`로 제공하고, 게시판, 커머스, 예약, 알림, 회원 자산 같은 확장 모듈의 개인정보도 각 모듈이 `privacy-export.php`로 제공한다. 포인트/적립금/예치금 원장, 쿠폰 지급/사용 로그, 콘텐츠/커뮤니티 계정별 자산 처리 로그와 접근권 테이블처럼 `account_id`에 연결되는 운영 기록은 개인정보 사본 제공 대상인지 먼저 검토한다. 접근권 테이블은 서비스 이용 상태이고, 거래 원장과 쿠폰 사용 로그는 증빙 기록이므로 개인정보처리방침의 보유기간을 서로 다르게 둘 수 있다.

새 모듈이나 새 테이블이 `account_id`, `created_by_account_id`, `author_account_id`, `processed_by_account_id` 같은 계정 연결 컬럼이나 `email`, `recipient`, `phone`, `birth_date`, IP/User-Agent hash, provider subject hash, 동의/답변 snapshot 같은 식별자성 컬럼을 추가하면 `privacy-export.php`, `privacy-cleanup.php`, 운영 보존 중 어느 책임을 갖는지 함께 정한다. `export_retained` 또는 `operational_retained`로 둔 데이터도 사본 제공 제외가 자동 확정되는 것은 아니며, 보존 기간과 운영자 접근 범위를 별도 정책이나 GitHub 이슈로 설명해야 한다.
...
쿠폰 대상 검색 계약을 배너/팝업레이어의 대상 선택에도 재사용하는 모듈은 기존 단순 배열 응답을 유지할 수 있다. 대량 데이터가 될 수 있는 대상은 검색 함수가 선택적으로 `items`, `next_cursor`, `has_more`, `limit`, `notice` 형태를 반환할 수 있으며, 호출자는 기존 `items`만 있는 응답과 cursor 응답을 모두 처리해야 한다. 커뮤니티 게시글처럼 증가량이 큰 대상은 ID 정확 조회, 빈 검색 최신순, 보드/상태 필터, `id < cursor` 방식의 더 보기를 우선하고, 기본 검색 흐름에서 `COUNT(*)`를 요구하지 않는다.

배너/팝업레이어가 `match_type=exact`로 저장하는 대상은 검색 UI 결과만 신뢰하지 않는다. 대상 contract의 `health_function` 또는 같은 의미의 서버 검증을 저장 action에서 다시 호출해 대상 존재 여부와 삭제 상태를 확인한다. 숨김/비공개처럼 운영자가 의도적으로 지정할 수 있는 상태는 화면 요약에 드러내고, 삭제된 대상은 저장을 거부한다.
...
3. 운영자가 `/admin/modules`에서 zip을 업로드하거나 압축 해제 후 modules/{module_key}/에 배치
...
유료 쿠폰 발급 캠페인은 `sr_coupon_claim_campaigns`에 무료/유료 발급 유형, 유료 가격/통화, 허용 포인트/금액 항목을 저장한다. 허용 포인트/금액 항목은 회원 선택의 강제 제약이며, 선택한 항목으로 부족하면 발급 전 실패한다. active 유료 캠페인으로 저장하거나 전환할 때는 선택한 허용 포인트/금액 항목마다 캠페인 가격 통화와 같은 `purchase_power.settlement_currency`가 있어야 하며, 통화가 맞지 않으면 저장을 거부한다. 공개 claim form의 발급 의도 token은 현재 계정 ID와 campaign ID를 함께 사용한 세션 key에 저장하고, POST에서 같은 계정 세션의 현재 token인지 검증한다. 성공한 claim만 token을 회전시키며, 잔액 부족이나 validation 실패는 같은 token을 유지해 같은 발급 의도 재시도를 같은 dedupe key로 수렴시킨다. 무료/유료 claim helper는 transaction 안에서 campaign row를 `FOR UPDATE`로 잠근 뒤 발급 한도 검증, claim row 점유, 자산 차감, 발급본 생성을 진행해 서로 다른 nonce의 동시 발급도 같은 캠페인 한도 아래에서 직렬화한다.
...
콘텐츠 상세의 `layout_key`가 `content.none`이면 공통 layout shell과 부가 영역을 호출하지 않고 독립 문서를 렌더링한다. 유료 열람 확인이 필요하면 일반 콘텐츠 화면과 같은 `asset-confirmation-modal.php`를 사용하고, 선택된 콘텐츠 테마의 `reset.css`, `common.css`, `module.css`와 공통 overlay script를 그 문서에 직접 로드한다. 따라서 공통 header/footer는 호출하지 않으면서도 열람 확인 모달의 마크업, 스타일, 차감·환전·쿠폰 POST 계약은 일반 레이아웃과 동일하게 유지한다.

콘텐츠 상세의 수정 링크는 현재 계정이 `/admin/content` 수정 권한을 가지면 관리자 수정 화면으로 연결한다. 관리자 권한이 없지만 `sr_content_items.created_by`와 현재 계정이 같고 연결된 본인 회원 제출본이 있으면 `/account/content?id={submission_id}`로 연결한다. 승인된 제출본은 작성자가 새 수정안으로 다시 저장하거나 제출할 수 있지만, 검수 전에는 기존 공개 콘텐츠를 변경하지 않는다. 검수 중인 제출본에는 동시 수정 링크를 노출하지 않는다.

무레이아웃의 허용된 본문도 일반 콘텐츠 상세와 같은 `content-body` wrapper 및 `sr_content_public_body_html()`을 사용한다. layout shell은 호출하지 않지만 선택된 콘텐츠 테마의 `reset.css`를 먼저 로드해 편집 화면과 같은 reset 및 theme token baseline을 만든다. CKEditor 판정은 본문 렌더링과 같은 실효 포맷을 사용하므로 기존 row의 원본 `body_format`이 `plain`이어도 실효 에디터가 CKEditor이면 HTML 본문으로 처리한다. 모든 콘텐츠 상세의 CKEditor HTML 본문은 `.sr-ckeditor[data-sr-editor-output] .ck-content` 구조와 번들 `ckeditor5.css`, 프로젝트 `saanraan-ckeditor.css`를 사용해 제목, 목록, 표, 인용, 코드, 이미지 정렬·캡션 표현만 유지하고 배경은 투명하게 출력한다. `.ck-editor__main`, `.ck-editor__editable_inline`, `ck-read-only` 같은 입력 widget 구조는 공개 화면에 출력하지 않는다. Markdown 본문은 `/assets/editor-md.css`와 renderer stylesheet를 그대로 사용한다.

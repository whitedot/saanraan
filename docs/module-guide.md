...
알림 모듈의 이메일 채널 사용 설정이 꺼져 있으면 알림 이벤트 채널에 `email`이 저장되어 있더라도 `sr_notification_deliveries`에는 해당 채널의 발송 작업을 등록하지 않는다. 회원 외부채널 수신 허용 또는 provider별 회원 수신 허용이 꺼져 있을 때도 저장된 `slack_webhook`, `discord_webhook`, `telegram_bot` 선택값은 보존하되 발송 작업은 등록하지 않는다. 이 상태는 알림/메일 관리 목록에서 `이메일 중지` 또는 `외부채널 중지` 뱃지와 상태 설명으로 노출한다.

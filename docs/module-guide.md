...
- 관리자 화면을 가진 모듈은 자기 화면 조합을 확인하기 위해 모듈 전용 UI-KIT 조회 화면을 둘 수 있다. 이때 route와 view/action은 소유 모듈에 두고, 보조 스타일은 `modules/{module_key}/assets/common.css`, `modules/{module_key}/assets/public-ui.css`, `modules/{module_key}/assets/ui-kit.css`, 샘플은 `modules/{module_key}/views/ui-kit-samples/`처럼 모듈 내부에 둔다. 콘텐츠/커뮤니티/퀴즈/설문처럼 공개 화면 전용 UI 원형을 가진 모듈은 공통 `kit` profile이나 루트 `assets/common.css`, `assets/public-ui.css`, `assets/public-ui-kit.css`를 직접 호출하지 않고, 원본 파일을 모듈 내부로 복제한 CSS를 공개 화면 context helper에서 `assets/public.css`와 함께 호출한다. 모듈 UI-KIT 화면은 원본 `/ui-kit`의 `layouts/public/basic/ui-kit-samples/` 파일을 그대로 복제한 전체 샘플 구성과 같은 호출 조합으로 실제 공개 화면 기준을 확인한다. 기본 호출 순서는 모듈 기본 레이아웃 CSS, 모듈 `common.css`, 모듈 `public-ui.css`, 모듈 `ui-kit.css`, 모듈 공개 CSS다. 단, 모듈 환경설정에서 레이아웃을 변경한 경우에는 선택된 레이아웃의 호출 정책을 따른다.
...
모듈 theme나 skin처럼 특정 view 조합에만 전용 CSS가 필요하면 view에서 `sr_public_layout_begin()`의 네 번째 인자로 stylesheet를 요청한다. 공개 layout option이나 layout context의 `style_profile`은 `kit`, `minimal`, `install` 중 하나이며, 공통/콘텐츠/커뮤니티 layout은 `minimal`을 기본으로 사용한다. minimal profile은 루트 `assets/common.css`와 `assets/public-ui.css`를 호출하지 않으므로, 콘텐츠/커뮤니티/퀴즈/설문처럼 버튼/입력/목록 표현이 필요한 모듈은 복제한 자기 `assets/common.css`, `assets/public-ui.css`, `assets/ui-kit.css`를 공개 화면 context helper에서 명시 호출한다. 홈/회원/계정/원본 공개 UI-KIT처럼 공통 공개 UI kit primitive가 필요한 화면은 `style_profile => 'kit'`을 명시한다. 루트 `assets/public-ui-kit.css`는 원본 `/ui-kit` 조회 화면의 샘플 보조 CSS로만 쓰고, 모듈 UI-KIT와 모듈의 실제 사용자 화면은 모듈 내부 복제 파일에 의존한다. 공통 공개 layout은 `assets/public-layout.css`, 콘텐츠 layout은 `modules/content/assets/layout.css`, 커뮤니티 layout은 `modules/community/assets/layout.css`를 layout 파일이 직접 호출한다. 콘텐츠/커뮤니티/퀴즈/설문 helper는 모듈 화면에 필요한 복제 UI-KIT CSS와 `assets/public.css`를 layout context에 추가한다. public layout은 `<head>` 출력만 담당한다. 출력 슬롯처럼 head 렌더링보다 뒤에서 HTML이 만들어지는 공개 모듈 출력도 해당 슬롯을 호출하는 view, skin, theme, public layout이 필요한 stylesheet를 layout context에 명시한다. 공개 CSS는 활성화된 모든 모듈 기준으로 자동 호출하지 않는다.

...
#391 기준으로 쿠폰 환불은 `sr_coupon_refund_redemption_state_only()`와 단독 관리자 환불 wrapper를 분리한다. state-only primitive는 redemption 상태, 환불 dedupe key, issue `used_count`/상태만 바꾸고 접근권 회수, payment ledger item reversal, 알림 발송은 하지 않는다. 기존 `sr_coupon_refund_redemption()`은 같은 primitive를 호출한 뒤 쿠폰 source 접근권 회수와 payment ledger reversal을 같은 트랜잭션에서 처리하고, commit 이후에만 `redemption.refunded` 알림을 발화한다. 콘텐츠/커뮤니티 복합결제 환불은 이후 도메인 payment-unit 환불 계약에서 state-only primitive와 자산 환불, source-scope 접근권 회수를 하나의 도메인 트랜잭션으로 오케스트레이션한다.

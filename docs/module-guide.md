...
- 관리자 화면을 가진 모듈은 자기 화면 조합을 확인하기 위해 모듈 전용 UI-KIT 조회 화면을 둘 수 있다. 이때 route와 view/action은 소유 모듈에 두고, 보조 스타일은 `modules/{module_key}/assets/reset.css`, `modules/{module_key}/assets/ui-kit.css`, `modules/{module_key}/assets/ui-kit-layout.css`, 샘플은 `modules/{module_key}/views/ui-kit-samples/`처럼 모듈 내부에 둔다. 콘텐츠/커뮤니티/퀴즈/설문처럼 공개 화면 전용 UI 원형을 가진 모듈은 모듈 내부에 복제한 `assets/reset.css`, `assets/ui-kit.css`를 공개 화면 context helper에서 `assets/module.css`와 함께 호출한다. 모듈 UI-KIT 화면은 원본 `/ui-kit`의 `layouts/public/basic/ui-kit-samples/` 파일을 그대로 복제한 전체 샘플 구성과 같은 호출 조합에 `assets/ui-kit-layout.css`를 더해 실제 공개 화면 기준을 확인한다. 기본 호출 순서는 모듈 `reset.css`, 모듈 `ui-kit.css`, 모듈 `module.css`, UI-KIT 화면에서는 모듈 `ui-kit-layout.css` 추가다. 단, 운영자가 해당 모듈 환경설정에서 레이아웃을 변경한 경우에는 선택된 레이아웃의 호출 정책을 따른다.

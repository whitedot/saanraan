...
게시판별 추가 입력 항목은 커뮤니티 모듈 안의 확장 정의/값 테이블로 분리한다. 후보 구조는 `sr_community_board_field_definitions`, `sr_community_post_field_values`, 댓글 확장이 필요할 때의 `sr_community_comment_field_values`이며, core/member 테이블이나 `sr_community_posts`를 게시판별 필드로 넓히지 않는다. `field_key`는 관리용 key이므로 소문자 영문, 숫자, `_`만 허용하고, 필수/조건부 필드는 관리자 UI, 공개 UI, 서버 POST 검증을 맞춘다. 개인정보 성격의 필드는 동의 문구, export/cleanup, 보관 기간 cleanup 정책과 함께 정의한다. 현재 조사와 세부 후보 필드는 [이슈 #328 설계 기록](records/issue-328-author-submission-data-design-2026-06-13.md)을 따른다.

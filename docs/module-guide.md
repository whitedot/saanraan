...
커뮤니티 피신고자 계정 guard는 커뮤니티 모듈 소유의 가역적 guard 표면으로 둔다. `sr_community_account_guard_events`는 판단 이벤트와 trigger fingerprint/snapshot을, `sr_community_account_guards`는 현재 guard 상태를, `sr_community_account_guard_locks`는 향후 작성자 단위 평가 직렬화를 담당한다. 기본 설정은 publication hold와 confirmed hold 모두 비활성이며, `/admin/community/settings`의 신고 자동조치 섹션에서 운영자가 publication hold 대상 수/기간과 confirmed hold 집계 기간/확정 건수/기간을 명시적으로 저장한다. current guard는 `active_guard_uid = account_id:guard_type` unique 기준으로 같은 계정/guard type당 활성 row 하나만 허용하고, released/expired/cancelled 같은 terminal 전이에서 helper가 active UID를 비운다. 이 foundation 단계는 자동으로 회원 상태를 정지하거나 신규 글을 pending으로 바꾸지 않는다.

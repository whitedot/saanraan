...
CKEditor 같은 에디터 연동은 플러그인이다. 플러그인은 `type => plugin`으로 등록하고, 적용 대상 textarea와 저장/출력 정책은 화면을 소유한 모듈이 결정한다. 예를 들어 커뮤니티 게시글 본문은 커뮤니티 모듈이 `data-sr-editor="ckeditor"` 적용 여부, `body_format`, HTML sanitizer, 출력 helper를 책임지고, CKEditor 플러그인은 에셋 로딩과 초기화 스크립트만 소유한다. 플러그인이 비활성화되거나 에셋 로딩에 실패하면 일반 textarea 제출이 유지되어야 한다.

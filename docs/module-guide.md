...
허용 심볼 이름은 다음과 같다. `settings`, `admin-mode`, `users`, `user`, `content`, `stats`, `home`, `folder`, `image`, `layers`, `search`, `menu-list`, `bell`, `shield`, `coins`, `wallet`, `gift`, `message-circle`, `service`, `quiz`, `poll`, `emoji_emotions`, `integration_instructions`.

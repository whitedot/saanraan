...
`privacy` 모듈은 관리자 전용 개인정보 요청 대응 기록과 사본 제공 보조 흐름을 조정한다. 회원이 직접 요청 티켓을 등록하는 워크플로우는 기본 기능으로 열지 않고, 회원 화면은 사본 직접 내려받기와 운영자 문의 안내에 머문다. 운영자가 외부 문의로 접수한 건을 기록해야 할 때만 관리자 개인정보 요청 대응 기록을 사용한다. 정정, 삭제, 처리 제한, 처리 거부, 동의 철회 request type은 접수 분류와 이력을 남기는 값이며, `privacy` 모듈이 데이터 소유 모듈의 action을 자동 실행하지 않는다. 회원 계정, 인증, 동의처럼 `member`가 소유한 데이터도 `modules/member/privacy-export.php`로 제공하고, 게시판, 커머스, 예약, 알림, 회원 자산 같은 확장 모듈의 개인정보도 각 모듈이 `privacy-export.php`로 제공한다. 포인트/적립금/예치금 원장, 쿠폰 지급/사용 로그, 콘텐츠/커뮤니티 계정별 자산 처리 로그와 접근권 테이블처럼 `account_id`에 연결되는 운영 기록은 개인정보 사본 제공 대상인지 먼저 검토한다. 접근권 테이블은 서비스 이용 상태이고, 거래 원장과 쿠폰 사용 로그는 증빙 기록이므로 개인정보처리방침의 보유기간을 서로 다르게 둘 수 있다.

...
- 외부 공개 테마와 외부 스킨은 [스킨·테마 패키지 기준](skin-theme-packages.md)의 manifest 계약을 따른다. 외부 테마는 레이아웃 후보가 아니라 `source_type=external_theme`인 정규화 theme option으로 합쳐지고, 사이트 설정의 `public_theme_key`에서 선택할 수 있는 asset/초기화면 보조 레이어다. 사용자 공개 화면 DOM을 바꾸는 테마는 외부 패키지가 아니라 `core/views/theme/{theme_key}` 또는 `modules/{module_key}/theme/{theme_key}`처럼 화면 소유자 내부 theme 디렉터리로 둔다. 해당 테마 CSS는 루트 초기화면은 `assets/theme/{theme_key}.css`, 모듈 화면은 `modules/{module_key}/theme/{theme_key}/assets/theme.css`에 둔다. 외부 스킨은 community, quiz, survey의 v1 view 계약에 맞는 경우에만 각 모듈 스킨 선택지에 합쳐진다. 스킨과 테마는 별도 레이어이며, 테마 view가 없을 때만 기존 스킨 view가 fallback으로 사용된다.
...
모듈은 DB에 view 파일 경로를 저장하지 않는다. `public_layout_key`, `public_theme_key`, `layout_key`, `theme_key`, `skin_key`, `{module_key}_skin_key` 같은 key만 저장하고, 실제 파일 경로는 모듈 helper의 allowlist, `layout-options.php` 계약, 검증된 패키지 manifest, 또는 모듈 내부 `theme/{theme_key}` 디렉터리에서 결정한다. 기존 공개 레이아웃 `basic` 값은 `common.basic`으로 정규화한다. 알 수 없는 사이트 공통 레이아웃 key 또는 현재 화면 target을 지원하지 않는 레이아웃 key는 기본 공통 레이아웃으로 fallback한다. 사이트 설정의 공통 레이아웃 선택지는 기본 공통 레이아웃을 먼저 표시하고, 모듈 제공 레이아웃은 관리자 사이드바의 모듈 순서대로 정렬한다. 콘텐츠, 커뮤니티, 퀴즈, 설문처럼 공개 레이아웃과 테마를 설정하는 모듈은 모듈 환경설정의 테마 key와 레이아웃 key를 메인, 그룹/게시판, 목록, 상세/응시/응답/완료 하위 공개 화면 전체에 적용한다. 관리자 폼에서는 테마를 먼저 고르고 그 아래에서 레이아웃을 고른다. 이 환경설정의 레이아웃 선택지는 각 모듈의 필수 화면 target 전체를 지원하는 option만 표시하고 저장하며, 번들 서비스 레이아웃은 네 공개 모듈 target 전체를 지원하므로 다른 모듈 레이아웃도 후보에 포함된다.

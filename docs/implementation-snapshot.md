...
커뮤니티 게시글 묶음 feed cache의 v1 사전 helper는 everyone-discoverable 공개 게시판 baseline만 다룬다. `context_hash` 입력에는 viewer tier/class 차원을 넣지 않고, 회원 전용/그룹별 discoverable 후보는 후속 viewer-class cache 모델이 필요할 때 별도 구현한다. 홈 최신글/인기글 조회와 측정 하니스는 같은 feed query helper를 사용해 fixture/EXPLAIN 증거와 운영 쿼리가 어긋나지 않게 한다. `/admin/community/feed-cache` 점검 화면은 영속 저장소 상태, 공개 baseline count, 유료 열람 제한 count, context hash를 read-only로 보여 주며, `storage/cache/community-feed`는 향후 파일 캐시 후보 감지용 경로일 뿐 현재 화면이 생성하거나 삭제하지 않는다. 카드 snapshot helper는 `author_account_id`를 저장하고 작성자 label은 렌더 시 resolve하며, `author_label_snapshot`, `published_comment_count`, 렌더된 thumbnail URL, 본문 전체, CSRF token, 계정별 권한 결과, 유료 접근권 상태를 snapshot JSON에 넣지 않는다. 영속 cache table, DB row lock/refresh, write path generation bump, 홈 feed 이전, 캐시 삭제/재생성 작업은 #361 기준 fixture/EXPLAIN 또는 별도 측정 증거 뒤에 진행한다.

...
모듈 간 본문 연결은 전용 관리 모듈이나 marker가 아니라 사용자가 본문에 단독으로 붙여 넣은 URL을 공개 렌더링 시점에 해석하는 방식으로 처리한다. 공통 helper `core/helpers/url-embed.php`는 standalone URL 또는 URL 텍스트 링크를 감지하고, YouTube, X, Instagram 같은 외부 URL과 활성 모듈의 `url-embed-targets.php` 계약이 해석하는 내부 URL을 안전한 공개 HTML로 바꾼다. 저장 HTML에는 완성 카드 HTML, 전용 marker, hidden 참조 목록을 남기지 않으며 legacy `{{sr_link_card ...}}` 토큰은 신규 저장에서 계속 거부한다. 콘텐츠 CKEditor 본문 이미지는 다운로드 파일 모델과 DB 참조 테이블 없이 콘텐츠 모듈의 로컬 저장 경로 `storage/content/body/{content_id}`가 소유하며, 저장된 HTML에서 살아남은 콘텐츠 프록시 URL만 서버가 보존 대상으로 인정한다.

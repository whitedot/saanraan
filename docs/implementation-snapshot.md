...
모듈 간 본문 연결의 1.0 정책은 본문 저장값을 참조 원장으로 쓰지 않는 것이다. 콘텐츠와 커뮤니티의 본문에는 사용자가 작성한 plain text 또는 허용된 HTML만 저장하며, 링크 카드 토큰은 신규 저장에서 거부한다. 콘텐츠 관리 폼은 `/admin/content/link-card-targets`로 공개 커뮤니티 게시글을 검색해 허용 HTML 또는 텍스트 링크를 삽입하고, 커뮤니티 작성/수정 폼은 `/community/link-card-targets`로 공개 콘텐츠를 검색해 같은 방식으로 본문에 삽입한다. 기존 `sr_content_link_refs`, `sr_community_link_refs` 원장은 업데이트에서 비워지고, 저장/복사/삭제 차단 판단에 쓰지 않으며 관리자 링크 카드 점검 화면도 노출하지 않는다. 기존 본문에 legacy 링크 카드 토큰이 남아 있는 콘텐츠/게시글은 새 복사본을 만들기 전에 본문에서 토큰을 제거해야 한다. 새 `embed_manager` 공식 선택 모듈은 이 결정을 바꾸는 후보로 `sr_embed_manager_refs`와 제한된 본문 marker를 제공한다. refs는 임베드 렌더링과 관리자 점검용 명시적 참조이며, 대상 삭제나 비활성화를 기본 차단하지 않고 broken/private/deleted 상태 표시와 점검으로 처리한다. 대상 모듈은 `embed-manager-targets.php` 계약으로 검색, 검증, 공개 URL, snapshot, 상태와 variant 정보를 제공한다. 콘텐츠 CKEditor 본문 이미지는 다운로드 파일 모델과 DB 참조 테이블 없이 콘텐츠 모듈의 로컬 저장 경로 `storage/content/body/{content_id}`가 소유하며, 저장된 HTML에서 살아남은 콘텐츠 프록시 URL만 서버가 보존 대상으로 인정한다.

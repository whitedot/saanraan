...
콘텐츠 관리자는 `/admin/content/link-refs`, 커뮤니티 관리자는 `/admin/community/link-refs`에서 링크 카드 참조와 깨진 대상을 확인한다. 정리 화면은 참조 행을 직접 삭제하지 않고 원본 본문 편집 화면으로 연결한다. 운영자가 토큰을 제거하거나 교체하면 다음 저장 reconciliation에서 참조 행이 갱신된다.

...
커뮤니티 게시글 묶음 feed cache helper는 `요약 영역 노출 허용`이 켜진 everyone-discoverable 공개 게시판 baseline만 다룬다. 게시판에서 이 설정을 끄면 해당 게시판의 글, 댓글, 시리즈는 커뮤니티 메인, 사이드바, 최근글/인기글 같은 게시판 목록 밖 요약 영역 후보와 feed cache 입력 board set에서 제외되지만 게시판 목록과 게시글 직접 접근 정책은 바뀌지 않는다. 게시글 테이블은 `summary_feed_candidate`를 가진다. 신규 글은 작성 시점의 요약 영역 노출 설정을 복사하고, 관리자 게시판 설정에서 요약 영역 노출 값을 바꾸면 해당 게시판의 기존 글 후보값을 일괄 동기화한다. 홈 최신글/인기글 live query는 게시판 설정 테이블 join 대신 이 게시글 후보값을 우선 사용한다. `context_hash` 입력에는 viewer tier/class 차원을 넣지 않고, 회원 전용/그룹별 discoverable 후보는 후속 viewer-class cache 모델이 필요할 때 별도 구현한다. 요약 영역 노출 설정 기준이 반영된 cache policy version을 사용해 이전 기준으로 만들어진 snapshot을 재사용하지 않는다. 홈 최신글/인기글 조회와 측정 하니스는 같은 feed query helper를 사용해 fixture/EXPLAIN 증거와 운영 쿼리가 어긋나지 않게 한다. `/admin/community/feed-cache` 점검 화면은 영속 저장소 상태, 공개 baseline count, 유료 열람 제한 count, context hash를 read-only로 보여 주며, `storage/cache/community-feed`는 향후 파일 캐시 후보 감지용 경로일 뿐 현재 화면이 생성하거나 삭제하지 않는다. 카드 snapshot helper는 `author_account_id`, 공개 댓글 수, 공개 홈 excerpt, 썸네일 source marker를 저장하고 작성자 label은 렌더 시 resolve한다. `author_label_snapshot`, `author_public_name_snapshot`, 렌더된 thumbnail URL, 본문 전체, CSRF token, 계정별 권한 결과, 유료 접근권 상태를 snapshot JSON에 넣지 않는다. 공개 홈 cache hit는 후보 재선정과 게시글 row hydrate를 건너뛰고 snapshot으로 카드 row를 구성한다.

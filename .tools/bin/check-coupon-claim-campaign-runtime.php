...
function sr_coupon_claim_runtime_static_contract(): void
{
    ...
    sr_coupon_claim_runtime_assert(str_contains($helpers, 'c.starts_at <= :starts_now') && str_contains($helpers, 'c.ends_at >= :ends_now'), 'public coupon zone campaign listing must use distinct date placeholders for MySQL PDO compatibility.');
    sr_coupon_claim_runtime_assert(!str_contains($helpers, 'c.starts_at <= :now_value') && !str_contains($helpers, 'c.ends_at >= :now_value'), 'public coupon zone campaign listing must not reuse a named date placeholder.');
    ...
    sr_coupon_claim_runtime_assert(str_contains($helpers, '$lockedAllowedAssetModules = sr_coupon_asset_module_keys_from_value($pdo, $campaign[\'allowed_asset_modules_json\'] ?? \'\');'), 'paid claim helper must revalidate selected asset modules after locking the campaign row.');
...
function sr_coupon_claim_runtime_fixture(): void
{
    ...
    $contract = require SR_ROOT . '/modules/coupon/url-embed-targets.php';
    ...
    $rendered = is_array($resolved) && is_callable($target['render_embed'] ?? null)
        ...
        : [];
    $sanitized = sr_url_embed_sanitize_rendered_fragment((string) ($rendered['html'] ?? ''));

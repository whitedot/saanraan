...
sr_delivery_template_check_assert(str_contains($notificationEventTemplateView, '사용(부분중지)') && str_contains($notificationEventTemplateView, '$statusClass = \'is-warning\';'), 'notification/mail admin status cell must summarize partially disabled delivery channels.');
...
sr_delivery_template_check_assert(str_contains($notificationEventTemplateView, '상태 설명') && str_contains($notificationEventTemplateView, 'card-description-block') && str_contains($notificationEventTemplateView, '알림 모듈의 외부채널 수신 허용'), 'notification/mail admin list must include status descriptions for item status and disabled delivery channels.');
sr_delivery_template_check_assert(str_contains($notificationEventTemplateView, '/admin/notifications/settings#notification-settings-section-external-push') && str_contains($notificationEventTemplateView, 'target="_blank"') && str_contains($notificationEventTemplateView, 'rel="noopener noreferrer"'), 'notification/mail admin external channel status description must link to notification external channel settings in a new tab.');
...
sr_delivery_template_check_assert(str_contains($notificationEventTemplateView, '자동 메일 템플릿의 상태에는 적용되지 않습니다.'), 'notification/mail admin bulk status help must explain that delivery template rows are excluded.');

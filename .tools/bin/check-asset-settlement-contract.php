...
sr_asset_settlement_check_contains('docs/core-decisions.md', [
    ...
    '확인-실행 사이 구매력 snapshot, 통화 min-unit, rounding/carry `policy_version`이 달라진 경우를 분리해 기록합니다',
...
sr_asset_settlement_check_contains('docs/module-guide.md', [
    ...
    '확인 화면 이후 실행 전 잔액이 줄어든 경우와 구매력 snapshot, 통화 min-unit, rounding/carry `policy_version`이 바뀐 경우를 별도 무효화 사유로 기록하고',
...
sr_asset_settlement_check_contains('docs/smoke-test.md', [
    ...
    '구매력/통화 min-unit/policy_version이 바뀌면 snapshot drift 사유로 별도 기록하며',
    ...
    '통화 min-unit 또는 rounding/carry `policy_version` 변경 직후 기존 확인 화면의 in-flight 요청은 fail-closed 재확인으로 떨어질 수 있음을 운영 워크플로에서 확인한다',

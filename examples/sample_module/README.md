...
이 샘플은 Toycore 최소 `0.1.1`, 모듈 계약 `1.0` 기준으로 점검합니다.
...
- 응답 종료는 `toy_redirect()`, `toy_render_error()`, `toy_finish_response()`를 통과하고 action에서 `exit`, `die`, `header('Location: ...')`를 직접 호출하지 않습니다.

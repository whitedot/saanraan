...
작은 CMS를 목표로 시작했지만, 지금은 회원, 관리자, 콘텐츠, 커뮤니티, 퀴즈, 설문, 리액션, 자산, 운영 보조 기능이 실제로 어떻게 맞물리는지 검증하는 회원 중심 모듈형 베이스에 가깝습니다.
...
- 콘텐츠, 커뮤니티, 퀴즈, 설문, 리액션, 자산, 쿠폰, 알림, 개인정보 요청을 선택 모듈로 확장.
- 현재 상태: 설치 가능한 회원 중심 모듈형 베이스로 개발 중이며, 구현해 보고 싶었던 커뮤니티/콘텐츠/리액션/포인트·금액/운영 보조 모듈이 함께 자라고 있습니다.

## 개발 방향

saanraan은 절차형 PHP 개발을 내 방식대로 끝까지 밀어붙여 보는 프로젝트입니다. 핵심 목표는 각 모듈의 독립성을 유지하면서도 회원, 콘텐츠, 커뮤니티, 퀴즈, 설문, 자산, 알림 같은 기능이 실제 서비스 안에서 최대한 촘촘하게 연계될 수 있는 구조를 검증하는 것입니다.

또 다른 목표는 기능이 늘어나도 코어 수정을 최소화하는 것입니다. 도메인별 정책과 화면, DB 테이블은 가능한 한 모듈 안에 두고, 코어는 요청 진입, 설정, 모듈 조회, 보안 helper, 출력 슬롯, 설치/업데이트 같은 실행 기반에 머물게 합니다. 이렇게 하면 커스터마이징이 들어간 운영 환경에서도 코어 패치와 배포판 업데이트를 비교적 쉽게 따라갈 수 있습니다.

그래서 배포판만 놓고 보면 특정 사이트에 바로 맞춘 완성형 솔루션보다 활용성이 낮아 보일 수 있습니다. 대신 실제 사용은 커스터마이징을 전제로 하며, 필요한 모듈을 고르고 모듈 경계를 유지한 채 연동을 확장할 때 활용성이 커지는 것을 목표로 합니다. 이 관점에서 사이트별 확장을 어디에 둘지 판단하는 기준은 [커스터마이징과 업데이트 충돌 가이드](docs/customization-guide.md)에 따로 정리했습니다.

이 방향의 구체적인 기준은 [모듈 작성 가이드](docs/module-guide.md)와 [핵심 설계 결정](docs/core-decisions.md)에 정리되어 있습니다. 모듈 간 참조를 직접 수정 없이 계약으로 확인하는 방식은 [읽기 참조 계약 계획](docs/plans/read-reference-contract-plan.md)에서 더 자세히 볼 수 있습니다.

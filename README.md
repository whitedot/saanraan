...
| 선택 번들 | 자동등록방지, OAuth 로그인, 본인확인, 쪽지, 커뮤니티, 콘텐츠, 퀴즈, 설문, 리액션, 메뉴, 로고, SEO, 배너, 팝업레이어, 알림, 포인트, 적립금, 예치금, 포인트/금액 환전, 쿠폰·이용권, CKEditor, Markdown Editor, OAuth/본인확인/captcha provider 플러그인 |
...
[산란 특장점 소개](docs/operator-feature-list.md)는 “운영자가 무엇을 할 수 있고 어떤 점이 편해지는가”를 기준으로 현재 기능을 설명합니다. 회원과 본인확인부터 콘텐츠 검색·발행, 커뮤니티·퀴즈·설문, 자산·쿠폰·유료 이용, 알림·발송 문구, 약관·개인정보, 메뉴·로고·테마·SEO, CKEditor·Markdown 작성 경험까지 실제 구현 범위를 자세히 살펴볼 수 있습니다.
...
- DB 기반 회원 계정, 로그인 세션, 이메일 인증, OAuth, 이메일 코드/TOTP/백업 코드 2차 인증, 비밀번호 재설정, 마이페이지, 프로필, 팔로우, 쪽지, 회원 CSV, 마케팅 수신거부 반영, 탈퇴·익명화 처리.
- 관리자 대시보드, 관리자 메뉴, 권한, 감사 로그, 설정, 업데이트, 지연 작업 점검, 보존 데이터 즉시·자동 정리 화면.
...
- 콘텐츠 전체검색과 콘텐츠·커뮤니티 공개 화면, 관리자 화면, sitemap 후보, 회원 그룹 규칙 연동.
- 콘텐츠/커뮤니티 맥락에서 시작하는 퀴즈 응시, 채점, 결과, 보상 지급 흐름.
- 설문 공개 목록/응답, 관리자 설문/응답/통계/CSV/매뉴얼, 보상 지급 흐름.
- 사이트 메뉴, 로고 매니저, 공개 레이아웃·테마, 사업자 정보 푸터, SEO, 배너, 팝업레이어, output slot asset 계약 같은 사이트 운영 모듈.
- 알림 모듈, 사이트 알림, 이메일·Slack·Discord·Telegram delivery queue, 중앙 발송 문구와 모듈별 알림 템플릿.
...
- 기본 textarea/Markdown/직접 HTML 입력 모드, CKEditor 5 선택 플러그인, Markdown 원문·렌더링 스타일 작업대, `body_format=html` 저장, 서버 측 HTML sanitizer.
- 공개 댓글, 공개 서비스 목록, 회원 활동·자산·알림·쪽지, 주요 관리자 업무 목록의 페이지네이션과 현재 선택값 보존.
...
| 회원 | `member`, `member_oauth`, `identity_verification`, `message` | 회원 계정, 인증, OAuth 로그인, 본인확인, 마이페이지, 프로필, 회원 그룹, 쪽지, 탈퇴 |
...
| 서비스 | `community` | 게시판, 댓글, 신고, 스크랩, 시리즈, 레벨/회원 그룹 규칙 |

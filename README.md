...
Toycore의 특장점은 코어를 작게 유지하면서도 운영에 필요한 기능을 모듈 단위로 바로 조합할 수 있다는 점입니다. 각 모듈은 자기 테이블, 관리자 화면, 권한 정책, 계약 파일을 소유하므로 기능이 늘어도 코어의 요청 흐름이 복잡해지지 않습니다.

### 코어 기준선

- 설치, 설정 파일 생성, DB 연결, 모듈 상태 확인, 업데이트 실행을 코어가 맡아 새 사이트의 반복 작업을 줄입니다.
- CSRF, 안전한 redirect, 보안 응답 헤더, 요청 계약 검사, 감사 로그, 실패 marker를 기본 제공해 작은 프로젝트도 운영 안전장치를 갖추고 시작할 수 있습니다.
- 게시글, 포인트, 배너, 메뉴 같은 도메인을 코어에 넣지 않아 필요한 기능만 붙이고, 사이트 성격에 맞지 않는 기능은 설치하지 않을 수 있습니다.
- Git 기반 배포와 릴리스 zip 배포를 모두 고려하고, DB 변경은 `/admin/updates`에서 명시적으로 실행해 운영 반영 절차를 추적하기 쉽습니다.

### 필수 모듈

- `member`: 회원 인증을 코어가 아닌 모듈로 분리해 로그인, 가입, 비밀번호 재설정, 이메일 인증, 탈퇴/익명화, DB 세션을 한곳에서 관리합니다. 로그인 실패 제한, 재설정/인증 요청 제한, 공개 회원 식별자, 프로필 항목 설정까지 포함해 회원 기반 사이트를 바로 시작할 수 있습니다.
- `admin`: 운영자가 매일 쓰는 관리 기준선을 제공합니다. 관리자 권한, 회원 관리, 사이트 설정, 모듈 관리, 개인정보 요청, 감사 로그, 업데이트 실행을 한 화면 흐름으로 묶고, 커뮤니티·배너 같은 도메인 관리는 해당 모듈 관리자 화면으로 자연스럽게 연결합니다.

### 선택 운영 모듈

- `seo`: 기본 meta, robots.txt, sitemap.xml을 모듈로 분리해 검색 노출 기본기를 가볍게 붙일 수 있습니다. sitemap은 각 모듈의 `sitemap.php` 계약을 합산하므로 새 도메인 모듈을 추가해도 SEO 모듈을 크게 고치지 않아도 됩니다.
- `site_menu`: 사이트 공통 메뉴를 모듈별 링크 후보와 연결합니다. 각 모듈이 `menu-links.php`를 제공하면 관리자는 노출할 링크를 선택하고, public layout의 정해진 출력 지점에 일관된 메뉴를 렌더링할 수 있습니다.
- `banner`: 공용 배너와 모듈별 화면 배너를 함께 관리합니다. `extension-points.php`가 선언한 위치에 배너를 붙이고 클릭 집계까지 남길 수 있어, 게시판 목록·읽기·쓰기처럼 전환이 필요한 화면에 맞춰 운영할 수 있습니다.
- `popup_layer`: 공용 팝업과 모듈별 화면 팝업을 같은 extension point 방식으로 연결합니다. 노출 기간과 닫기 쿠키 기간을 설정할 수 있어, 전체 공지부터 커뮤니티 주요 화면 안내까지 과도한 코어 수정 없이 운영할 수 있습니다.
- `notification`: 사이트 알림과 외부 발송 대기열의 기반을 제공합니다. 커뮤니티 댓글, 신고, 관리자 알림 같은 이벤트에서 필요한 경우에만 호출할 수 있어 알림 기능을 붙여도 모듈 간 의존이 과하게 커지지 않습니다.
- `community`: 게시판 운영에 필요한 글, 댓글, 신고, 쪽지, 스크랩, 첨부파일을 한 모듈 안에서 제공합니다. 게시판 그룹, 회원 그룹, 10단계 커뮤니티 레벨을 조합해 읽기/쓰기/댓글/쪽지 권한을 세밀하게 나눌 수 있고, 배너·팝업레이어는 목록, 글보기, 글쓰기 폼 같은 주요 화면별로 선택 적용할 수 있습니다.

### 회원 연계 모듈

- `point`: 회원별 포인트 잔액과 거래 원장을 모듈 안에서 관리합니다. 포인트 지급·차감 정책을 코어에 묶지 않기 때문에 커뮤니티 활동 보상, 이벤트 지급, 운영자 조정 같은 정책을 사이트 성격에 맞춰 확장하기 좋습니다.
- `deposit`: 회원별 예치금 잔액과 거래 원장을 분리해 관리합니다. 결제, 환불, 정산 같은 민감한 업무 정책을 코어와 분리할 수 있어 금액성 기능을 붙일 때 변경 범위가 명확합니다.
- `reward`: 회원별 적립금 잔액과 거래 원장을 별도 모듈로 둡니다. 포인트, 예치금과 유사하지만 용도가 다른 금액성 도메인을 독립적으로 운영할 수 있어 정책 충돌을 줄이고 사이트별 혜택 구조를 나누기 쉽습니다.

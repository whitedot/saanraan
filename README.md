...
- 절차형 PHP 흐름을 유지해 파일을 열면 요청 처리 과정을 따라가기 쉽습니다.
- 코어는 설치, 설정, DB 연결, 보안 helper, 모듈 로딩 같은 실행 기반에 집중하고 도메인 기능은 모듈이 소유합니다.
- `paths.php`, `admin-menu.php`, `extension-points.php` 같은 명시적 계약 파일로 모듈 경계를 드러냅니다.
- CSRF, 로그인, 관리자 권한, 감사 로그, 개인정보 export, 업데이트 lock 같은 운영·보안 기준선을 기본 제공합니다.
- Git 기반 배포와 릴리스 zip 배포를 모두 고려하며, `/admin/updates`에서 DB 업데이트를 명시적으로 실행합니다.
- PHP, 바닐라 JavaScript, plain CSS 중심으로 저가형 공유호스팅에서도 이해하고 운영하기 쉬운 구조를 지향합니다.
- 회원, 관리자, SEO, 메뉴, 배너, 팝업레이어, 알림, 커뮤니티 등 선택 모듈을 조합해 작게 시작하고 확장할 수 있습니다.

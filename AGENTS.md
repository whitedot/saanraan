...
- 다른 모듈에서도 사용하는 기능은 화면별 조립 관례가 아니라 기능 소유 모듈의 공개 계약으로 제공합니다. 데이터 조회, 권한/공개 정책, 상태 계산, fallback, 표시 모델, 공통 마크업, 필요한 CSS/JavaScript와 계약 테스트를 가능한 한 같은 소유 경계에 둡니다.
- 소비 모듈은 기능 소유 모듈이 허용한 배치, 크기, variant 같은 표현 옵션만 선택합니다. 다른 모듈 소유 테이블을 직접 조회하거나 공개 정책, fallback, 공통 마크업, 공통 JavaScript 동작을 다시 구현하지 않습니다.
- 계약 파일은 모듈 디렉터리 바로 아래에 두고 `module.php`의 `contracts.provides`에 선언합니다. 소비 모듈은 `contracts.consumes`와 필요한 경우 `requires.contracts`에 소비 관계를 선언합니다.
- `module.php`의 계약 및 의존성 선언은 설치/업데이트/정적 검사에 사용하는 검증 메타데이터입니다. 이 선언을 런타임 서비스 컨테이너, 자동 컴포넌트 탐색, 숨은 dispatch 또는 자동 wiring에 사용하지 않습니다.
- 실제 요청 처리 파일에는 외부 모듈 계약의 명시적 `require_once`, 입력 및 대상 ID 수집, 단건 또는 일괄 context 조회, 필요한 asset 배열 병합, 최종 view/theme `include` 순서가 드러나야 합니다. 개발자가 진입점과 `paths.php`에서 시작해 파일을 순서대로 열어 실제 요청 흐름을 추적할 수 있어야 합니다.
- 계약을 사용하는 renderer가 전역 layout context에 asset을 암묵적으로 등록하거나 요청 상태를 변경하지 않게 합니다. 소비 요청은 계약이 제공한 stylesheet/script 목록을 현재 layout context에 명시적으로 병합합니다.
- 목록 화면에서 교차 모듈 기능을 반복 출력할 때는 소유 모듈이 제공하는 일괄 context 조회를 우선합니다. item loop 안에서 다른 모듈의 단건 조회나 정책 조회를 반복하지 않습니다.
- 자동 검사는 올바른 흐름을 대신 생성하지 않고 잘못된 경계를 거부하는 데 사용합니다. 최소한 미선언 계약 소비, 다른 모듈 내부 파일 직접 접근, 소비 화면의 소유 정책/fallback/공통 asset 경로 재구현, 필수 asset 누락, 계약 우회 호출을 검사합니다.
- 공개 계약 변경은 제공자 계약 테스트, 소비자 경계 테스트, 대표 요청 흐름 통합 테스트를 함께 갱신합니다. 문서나 체크리스트만으로 계약 준수를 가정하지 않습니다.

## 기능 변경 완결성

- 기능을 변경하기 전에 그 기능을 소유한 모듈을 먼저 확정합니다. 여러 모듈에 비슷한 화면이나 코드가 있다는 사실만으로 새 공통 모듈, 코어 기능 또는 소유 계약을 만들지 않습니다. 자연스러운 기능 소유자가 이미 있고 실제로 같은 기능을 소비할 때만 기존 교차 모듈 계약 원칙을 적용합니다.
- 사용자가 여러 모듈을 함께 지목하면 각 모듈의 도메인 소유권을 유지한 상태로 모듈별 요청 흐름을 각각 확인합니다. 한 모듈의 구현을 다른 모듈에 기계적으로 복제하는 것을 전체 검토로 간주하지 않습니다.
- 변경 대상 요청은 public/admin 진입점과 `paths.php`에서 시작해 action, 상태·권한 계산 helper, 데이터 조회, 기본 view/theme, fallback skin/view, layout context와 asset, POST action과 redirect까지 파일 순서대로 추적합니다. 실제 인간 개발자가 같은 순서로 열어 동작을 재구성할 수 있어야 합니다.
- 화면 기능은 최소한 비회원/회원, 권한 허용/거부, 기능 활성/비활성, 빈 데이터/데이터 있음, 성공/validation failure, 기본 view/fallback view, 일반/미리보기·embedded·fragment 같은 해당 기능의 실제 분기를 목록화합니다. 존재하지 않는 분기를 억지로 추가하지 말고, 소스에 있는 분기를 누락하지 않습니다.
- 수정과 검증은 정상 출력만 확인하지 않습니다. 없어야 하는 버튼, 링크, 문구, fallback, 단건 조회, 중복 asset 같은 부정 조건도 가장 가까운 전용 검사에 포함합니다. 정적 marker 하나나 route 200 응답만으로 상태별 렌더링이 같다고 결론 내리지 않습니다.
- 비슷한 UI를 정렬하는 작업에서도 도메인별 저장·권한·정책은 각 소유 모듈에 남깁니다. 공통 UI kit 또는 기존 기능 계약이 이미 소유한 요소만 재사용하고, 시각적 유사성만을 이유로 도메인 소유권을 이동하지 않습니다.
- 완료 보고 전에는 사용자가 지적한 실제 조건과 같은 상태의 렌더링 증거를 우선합니다. 필요한 브라우저·계정·fixture가 없어 실제 상태를 확인하지 못했다면 자동 검사 통과와 구분해 미검증 범위를 밝히고, 정확한 동일성이나 완전한 완료를 주장하지 않습니다.
...
- 증상을 숨기는 fallback, override, 호환 shim, 예외 무시, 중복 보정 같은 우회 처리보다 문제의 실제 원인과 정본을 찾아 직접 수정하는 해결책을 우선합니다.
- 사용자가 명시적으로 우회 처리를 요청하지 않은 경우에는 증상 우회 처리를 구현하지 않습니다.
- 직접 수정이 현재 범위나 권한 안에서 불가능하면 임의의 우회 처리를 추가하지 말고, 확인한 원인과 직접 수정이 막힌 이유를 보고한 뒤 사용자 지시를 기다립니다.
- 사용자가 임시 우회 처리를 명시적으로 요청한 경우에도 적용 범위, 부작용, 제거 조건을 분명히 하고 직접 해결책을 대체하는 영구 계약으로 확장하지 않습니다.
